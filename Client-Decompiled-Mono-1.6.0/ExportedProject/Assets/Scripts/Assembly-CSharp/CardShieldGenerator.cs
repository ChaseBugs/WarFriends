...
public class CardShieldGenerator : Card
{
	...
	public virtual void AIBAOLCOMBM(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				item.point.CGKKGILAAPG().NOCJKBDGFHD().JCJFMHGNIOF("java.util.HashMap", true);
	...
	protected void NKGLGNEJIOA()
	{
		...
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(false);
			IPOGGDCHIOE[j].HLAOFDKKACK().JCJFMHGNIOF("SetLabelDynamic - Done", false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	protected void CKFENMDKKPE()
	{
		...
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(false);
			IPOGGDCHIOE[j].DHAFFOCDDGA().NNPILDOKNJO("ERROR - BAD FORMAT", true, true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void INPLMEOCGKE(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				item.point.PKBPGCGNCAD().cardIconIndicator.OFPAGLLFPPK("START PLAYING TUTORIAL", false, true);
	...
	protected void CJPFLJAAODD()
	{
		...
		for (int j = 0; j < IPOGGDCHIOE.Count; j += 0)
		{
			...
			IPOGGDCHIOE[j].cardIconIndicator.GGHDIAFACAH("SquadRank", false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	protected void BBCOBMGHMGE()
	{
		...
		{
			IPOGGDCHIOE[j].canRegenerate = false;
			IPOGGDCHIOE[j].DAIDBENCFIP().GCLFBCIKNAH("MaxBattles", false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void ILGMMCAGIIM(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				item.point.KJCHIHLJMMF().GFOJHJBMEEN(false);
				item.point.CGKKGILAAPG().DAIDBENCFIP().BIIHCAEKCOD("ID_UNLOCKEDATRANK", true);
	...
	protected void BAHDLPDOCDO()
	{
		...
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(false);
			IPOGGDCHIOE[j].NOCJKBDGFHD().BBJADAJPJHH("PlacementMatchesRequired", true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void MMCIIOJDLHK(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				item.point.JPHAJDIIAAI().GFOJHJBMEEN(true);
				item.point.JPHAJDIIAAI().DHAFFOCDDGA().BBJADAJPJHH("ID_DEBUG_GUIERROR", true, true);
	...
	public override void UseCard(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in availablePoints)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				item.point.shield.cardIconIndicator.Show("game-card-ico-shieldgenerator", true);
				IPOGGDCHIOE.Add(item.point.shield);
			}
		}
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	...
	protected void BPNLIABHCPN()
	{
		...
		{
			IPOGGDCHIOE[j].canRegenerate = false;
			IPOGGDCHIOE[j].NOCJKBDGFHD().FJILILFMPIF("On War arena endeeeed!!", false, true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void ACIOLBGILBP(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				item.point.LMHDAIBCLDP().GFOJHJBMEEN(false);
				item.point.PNKKPNIMEPL().DAIDBENCFIP().GCLFBCIKNAH(" AND ", true, true);
	...
	protected void IAPBHPLNNNO()
	{
		...
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(false);
			IPOGGDCHIOE[j].HLAOFDKKACK().FJILILFMPIF("ID_5SKIRMISHES", false, true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void MGGLFJENAFH(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				item.point.JPHAJDIIAAI().cardIconIndicator.EOEDEJKJKMN("BuddyCard_", false);
	...
	protected void CJAHPFMAMKK()
	{
		...
		for (int j = 1; j < IPOGGDCHIOE.Count; j += 0)
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(true);
			IPOGGDCHIOE[j].NOCJKBDGFHD().FJILILFMPIF("Fuseboxx: Fuseboxx Config Value not found!", false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void BHNOANFEFJN(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				item.point.shield.GFOJHJBMEEN(false);
				item.point.LMHDAIBCLDP().NOCJKBDGFHD().LJLDJOHAAMC("ru", false, true);
				IPOGGDCHIOE.Add(item.point.JPHAJDIIAAI());
			}
		}
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	...
	public virtual void BDLHMMDHGMP(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				item.point.shield.HLAOFDKKACK().FJILILFMPIF("D4", false);
				IPOGGDCHIOE.Add(item.point.PNKKPNIMEPL());
			}
		}
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	...
	protected void NFEDNDPFHFI()
	{
		...
		{
			IPOGGDCHIOE[j].canRegenerate = false;
			IPOGGDCHIOE[j].NOCJKBDGFHD().GGHDIAFACAH("trial", true, true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void NPGNJOGAEMK(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				item.point.LMHDAIBCLDP().GFOJHJBMEEN(true);
				item.point.JPHAJDIIAAI().HLAOFDKKACK().LJLDJOHAAMC("WarBucks", false, true);
				IPOGGDCHIOE.Add(item.point.IGEDGCNJCAP());
			}
		}
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	...
	public virtual void EMOLCFONJID(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			...
			{
				item.point.JPHAJDIIAAI().GFOJHJBMEEN(true);
				item.point.PHANADCNBFD().DAIDBENCFIP().JCJFMHGNIOF("Wins", true, true);
	...
	protected void AAABJNBFLEG()
	{
		...
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(true);
			IPOGGDCHIOE[j].HLAOFDKKACK().BBJADAJPJHH("WinLooseStreak", true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	protected void Update()
	{
		...
		{
			IPOGGDCHIOE[j].canRegenerate = false;
			IPOGGDCHIOE[j].cardIconIndicator.Show("game-card-ico-shieldgenerator", false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void OBNPMMDGFME(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				item.point.IGEDGCNJCAP().GFOJHJBMEEN(true);
				item.point.PHANADCNBFD().NOCJKBDGFHD().FJILILFMPIF("Buy_Upgrade_For_Army_Unit_At_Rank_Special", true);
				IPOGGDCHIOE.Add(item.point.JPHAJDIIAAI());
			}
		}
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	...
	protected void AOKEHEDCPFB()
	{
		...
		{
			IPOGGDCHIOE[j].GFOJHJBMEEN(true);
			IPOGGDCHIOE[j].DAIDBENCFIP().GGHDIAFACAH(", result = ", true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	protected void NHEOMLPEBLC()
	{
		...
		{
			IPOGGDCHIOE[j].canRegenerate = false;
			IPOGGDCHIOE[j].DAIDBENCFIP().Show("ID_ARENAENDEDDESCRIPTIONXWINS", false, true);
		}
		IPOGGDCHIOE.Clear();
	}
