...
public class ArmyUnitStatistics : Core_BaseScript
{
	...
	{
		IMFAHHGINPP.gameObject.SetActive(true);
		IMFAHHGINPP.color = Colours.grayMax;
		NIKKFEOFHFP.gameObject.SetActive(false);
		KJKIMOFGPAC.gameObject.SetActive(true);
	...
	{
		IMFAHHGINPP.gameObject.SetActive(false);
		NIKKFEOFHFP.gameObject.SetActive(true);
		NIKKFEOFHFP.text = AMFBOIJIGKL;
		KJKIMOFGPAC.gameObject.SetActive(false);
	}
	...
	public void Initialize(Tuple<string, float[]> ODFAFEEGGJK, bool KNBNNLOEBIH, bool JHFKBODINGN, bool LPFLPCOLHPJ = false)
	{
		...
		if (KNBNNLOEBIH)
		{
			...
			{
				NAECNOIDJBM((!flag) ? string.Empty : string.Format("+{0}", MEJMLNDFDBP.MAGANIIGHON(num4)));
			}
			if (JHFKBODINGN && flag)
			{
				HALLAKMKEEN();
