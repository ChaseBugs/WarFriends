...
public class EliteUnitPreviewDialog : GuiElementSingle<EliteUnitPreviewDialog>, PAIIOKBBHBC
{
	...
	private void EJADLBLFPGM()
	{
		if (isShowed)
		{
			Singleton<LootBoxCameraArena>.instance.JFICPJELINH(IFFDIHCPKFE, false);
		}
	}
	...
	private void GAHAMMDHAOF()
	{
		if (NPFFMLLLDAF())
		{
			Singleton<LootBoxCameraArena>.instance.GPIHLDEBOLF(IFFDIHCPKFE, true);
		}
	}
	...
	public override void InitGUIValues()
	{
		...
		JGPKGKEIPEG.fillAmount = progress;
		DIAJOBOCEBH.text = ((!flag) ? string.Format("{0}{1}[-] {2}/[-] {3}", Colours.stringGreenArena, MEJMLNDFDBP.GMIPFLIEOHD(currentParts), Colours.stringGray, MEJMLNDFDBP.GMIPFLIEOHD(upgradePriceParts)) : string.Format("{0}{1}[-]", Colours.stringGreenArena, MEJMLNDFDBP.GMIPFLIEOHD(currentParts)));
