...
public class GameShootableEntity : Core_BaseScript, KMHCMCBGFFC
{
	...
	public class ShotTarget
	{
		...
		public GameShootableEntity shootableEntity
		{
			get
			{
	...
	public MainGameEntity mainGameEntity
	{
		get
		{
	...
	private void OnDisable()
	{
		if (owner != null)
		{
			List<GameShootableEntity> value;
			if (CHPGDNCPNCF.TryGetValue(owner.GetType(), out value))
			{
				value.Remove(this);
			}
			else
	...
	public static List<GameShootableEntity> GOHKFHIOEHA(GHPGNELIDBM CIOPAKLHFIL, Type[] BIKDCLNLAAP)
	{
		...
		{
			List<GameShootableEntity> value;
			if (!CHPGDNCPNCF.TryGetValue(key, out value))
			{
				continue;
			}
	...
	public static List<GameShootableEntity> GetAll<T>() where T : class
	{
		DLGJCLOKFCH = new List<GameShootableEntity>();
		List<GameShootableEntity> value;
		if (CHPGDNCPNCF.TryGetValue(typeof(T), out value))
		{
			foreach (GameShootableEntity item in value)
			{
				NKMKCCDCHCL nKMKCCDCHCL = item.owner as NKMKCCDCHCL;
				if (nKMKCCDCHCL != null && nKMKCCDCHCL.isAlive)
				{
					DLGJCLOKFCH.Add(item);
				}
			}
	...
	private void LCODLCGHEHH()
	{
		...
		{
			List<GameShootableEntity> value;
			if (CHPGDNCPNCF.TryGetValue(GEAMOMEEFKM().GetType(), out value))
			{
				value.Remove(this);
			}
			else
	...
	public static void Init()
	{
		...
		{
			HashSet<Type> value;
			if (LPCFGOMKAJI.TryGetValue(item.behaviour.unitType, out value))
			{
	...
	private void JCNCGJPJDAC()
	{
		...
		{
			Debug.LogError("ID_TUTORIAL_DELIVER");
		}
	}
	...
	private void GPABBGFMLMB()
	{
		if (GEAMOMEEFKM() != null)
		{
			...
			{
				Debug.LogError("Player_Had_To_Select_Grenade");
			}
		}
		else
		{
			Debug.LogError("ID_WARNING_MAINTENANCE");
		}
	}
	...
	private void OPPIIAAJLGI()
	{
		if (owner != null)
		{
			...
			{
				Debug.LogError("#AccoutCheck# GamecenterChangeDuringTutorial - NOT connect -> ignore for this moment");
			}
		}
		else
		{
			Debug.LogError("com.tune.TuneEvent");
		}
	}
