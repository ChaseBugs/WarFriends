...
public class SessionManager : Singleton<SessionManager>
{
			...
			case 1u:
				if (!BJGCPDNMHDH.gcAuthenticated && BJGCPDNMHDH.IMKILHFKDEP != MFGMKAJDCAI.None)
				{
					...
					else
					{
						...
						BJGCPDNMHDH.FPOPAHHGNCE = Time.time;
						Singleton<GameCenterProvider>.instance.NJLDEEGPGCA(false);
					}
				}
				goto IL_00f4;
			default:
			...
			case 1u:
				if (!BJGCPDNMHDH.BPDLJAFEFHI() && BJGCPDNMHDH.IMKILHFKDEP != MFGMKAJDCAI.None)
				{
					...
					else
					{
						...
						BJGCPDNMHDH.FPOPAHHGNCE = Time.time;
						Singleton<GameCenterProvider>.instance.BLCNBIGMDIJ(false);
					}
				}
				goto IL_00f4;
			default:
			...
			case 1u:
				if (!BJGCPDNMHDH.gcAuthenticated && BJGCPDNMHDH.IMKILHFKDEP != MFGMKAJDCAI.None)
				{
					...
					else
					{
						...
						BJGCPDNMHDH.FPOPAHHGNCE = Time.time;
						Singleton<GameCenterProvider>.instance.Authenticate(false);
					}
				}
				goto IL_00f4;
			default:
	...
	public bool schedulingOrInProgressAnyUpdate
	{
		get
		{
			return IMKILHFKDEP != MFGMKAJDCAI.None || !gcAuthenticated || MHMDPINDGBI || NBEMOMKJDFL;
		}
	}
	...
	private bool AKFIJOIODPB
	{
		get
		{
			return !GameLoginManager.instance.data.isDeviceRegistered || Singleton<GameController>.instance.isTutorialInProgressOrPlaned || GameLoginManager.instance.acountDataDownloadingInProgress;
		}
	}

	private bool MGIKLEDLHBP
	{
		get
		{
	...
	protected void JOCHCGHAIGH()
	{
		...
		if (BGHMGBMOAAO && GameLoginManager.instance.acountDataDownloadingInProgress && Singleton<GameController>.instance.isInMenuOrWait && !Singleton<BeanstalkServerManager>.instance.GFFNKLKCHBA() && GuiElementSingle<UserExistsDialog>.instance.isShowed)
		{
			...
			flag = true;
			NBEMOMKJDFL = true;
			LoadingDialog.FDBHFJFKALH(Localization.Localize("ID_STARTERASSIGNMENT"), false, false, true);
		}
	...
	protected void OnApplicationPause(bool NMCKKMEKOHM)
	{
		if (NMCKKMEKOHM)
		{
			...
			UnityEngine.Debug.Log(string.Format("Device PAUSED at {0} RealTime: {1}, Doing CG.Collect() = {2}", DateTime.UtcNow, DAPNFEHBKDJ, Singleton<GameController>.instance.gameState == GameController.AEMPHGEIJNH.Menu));
			StopAllCoroutines();
		}
		else
		{
			UnityEngine.Debug.Log(string.Format("Device UNPAUSED at {0}", DateTime.UtcNow));
		}
	}
	...
	private void OnApplicationResumed()
	{
		...
		else
		{
			...
			UnityEngine.Debug.Log(string.Format("Device slept at {0}, RealTime: {1} waked up at {2} RealTime: {3} and slept for {4}", KHPOCJCCICL, DAPNFEHBKDJ, DateTime.UtcNow, Time.realtimeSinceStartup, num));
			MCPIKBHONJF(num);
	...
	private void EDLAMGECFDJ()
	{
		...
		{
			UnityEngine.Debug.Log(string.Format("Check Device sleep at {0}, waked up at {1} Dif: {2} Session update: {3}, ScheduleSessionUpdate: {4}", AJCMJOOEEFB, DateTime.UtcNow, num, IMKILHFKDEP, MHMDPINDGBI));
			MCPIKBHONJF(num);
		}
		AJCMJOOEEFB = DateTime.UtcNow;
	}
	...
	protected void BAHDLPDOCDO()
	{
		...
		if (BGHMGBMOAAO && GameLoginManager.instance.acountDataDownloadingInProgress && Singleton<GameController>.instance.isInMenuOrWait && !Singleton<BeanstalkServerManager>.instance.GFFNKLKCHBA() && GuiElementSingle<UserExistsDialog>.instance.isShowed)
		{
			...
			flag = false;
			NBEMOMKJDFL = false;
			LoadingDialog.ShowLoading(Localization.Localize("http://"), true, false, true);
		}
	...
	private void MCPIKBHONJF(float BMJLBEKLGFL)
	{
		if (IMKILHFKDEP != MFGMKAJDCAI.Full && !MHMDPINDGBI)
		{
			...
			{
				LoadingDialog.ShowLoading(Localization.Localize("ID_LOADING"), false, false, true);
	...
	private void JGHKIBFHDMK(float BMJLBEKLGFL)
	{
		if (IMKILHFKDEP != MFGMKAJDCAI.None && !MHMDPINDGBI)
		{
			...
			{
				LoadingDialog.FDBHFJFKALH(Localization.Localize("New Assignments!"), true, false, true);
