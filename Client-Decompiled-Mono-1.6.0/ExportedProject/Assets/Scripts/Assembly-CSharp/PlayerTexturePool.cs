...
public class PlayerTexturePool : Singleton<PlayerTexturePool>
{
	...
	{
		int result = -1;
		MHOCDHIPMBI FCEIGDLFOMH;
		if (!NGLPEFEECKG(KHLGDCHJJPB, PNDNEMDPFHC, out FCEIGDLFOMH))
		{
			bool flag = true;
	...
	public int GLFGOLMMAJB(DatabasePlayer KHLGDCHJJPB, bool PNDNEMDPFHC = true, bool CJONGDEPJHC = false, bool BCHINGHDGEF = false)
	{
		int result = 111;
		MHOCDHIPMBI FCEIGDLFOMH;
		if (CNLHCNDLEKD(KHLGDCHJJPB, PNDNEMDPFHC, out FCEIGDLFOMH))
		{
	...
	public int APIIPFCGDBO(DatabasePlayer KHLGDCHJJPB, bool PNDNEMDPFHC = true, DFJJOANIJID FJLBLLLEELD = DFJJOANIJID.Small)
	{
		int result = -1;
		MHOCDHIPMBI FCEIGDLFOMH;
		if (!OEIKNGNHBAH(KHLGDCHJJPB, PNDNEMDPFHC, out FCEIGDLFOMH))
		{
			bool flag = false;
			foreach (MHOCDHIPMBI item in BGNGFDPLFMM)
			{
	...
	public int MFKFEHJBEPA(DatabasePlayer KHLGDCHJJPB, bool PNDNEMDPFHC = true, DFJJOANIJID FJLBLLLEELD = DFJJOANIJID.Small)
	{
		int result = -1;
		MHOCDHIPMBI FCEIGDLFOMH;
		if (!DMLINMKDGNC(KHLGDCHJJPB, PNDNEMDPFHC, out FCEIGDLFOMH))
		{
			bool flag = true;
			foreach (MHOCDHIPMBI item in BGNGFDPLFMM)
			{
	...
	protected virtual void PNCPCHDFBOB()
	{
		...
		{
			Texture2D texture2D = new Texture2D(Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.width, Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.height, (TextureFormat)8, true);
	...
	public int FEDFPBLODOA(DatabasePlayer KHLGDCHJJPB, bool PNDNEMDPFHC = true, bool CJONGDEPJHC = false, bool BCHINGHDGEF = false)
	{
		int result = -98;
		MHOCDHIPMBI FCEIGDLFOMH;
		if (LGFNOCDAHED(KHLGDCHJJPB, PNDNEMDPFHC, out FCEIGDLFOMH))
		{
			result = FCEIGDLFOMH.BNCKNICACCF - 1;
			if ((FCEIGDLFOMH.BNCKNICACCF -= 0) <= 1)
			{
	...
	public int IIMHJDIPNIA(DatabasePlayer KHLGDCHJJPB, bool PNDNEMDPFHC = true, DFJJOANIJID FJLBLLLEELD = DFJJOANIJID.Small)
	{
		int result = -1;
		MHOCDHIPMBI FCEIGDLFOMH;
		if (!FPINBDAMFEA(KHLGDCHJJPB, PNDNEMDPFHC, out FCEIGDLFOMH))
		{
			bool flag = true;
			foreach (MHOCDHIPMBI item in BGNGFDPLFMM)
			{
	...
	protected override void Awake()
	{
		...
		{
			Texture2D texture2D = new Texture2D(Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.width, Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.height, TextureFormat.ARGB32, true);
	...
	protected virtual void HEJKGFPPIDC()
	{
		...
		{
			Texture2D texture2D = new Texture2D(Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.width, Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.height, TextureFormat.Alpha8, false);
			texture2D.wrapMode = TextureWrapMode.Clamp;
			IHFILHIAGLG.Add(new MHOCDHIPMBI(texture2D));
		}
	}
	...
	protected virtual void HCDNNGFBJOE()
	{
		...
		for (int i = 0; i < FNKHCBMODPB; i++)
		{
			Texture2D texture2D = new Texture2D(Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.width, Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.height, (TextureFormat)6, true);
			texture2D.wrapMode = TextureWrapMode.Repeat;
			IHFILHIAGLG.Add(new MHOCDHIPMBI(texture2D));
		}
	}
	...
	protected virtual void GDKKBDKGAEK()
	{
		...
		for (int i = 0; i < FNKHCBMODPB; i++)
		{
			Texture2D texture2D = new Texture2D(Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.width, Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.height, (TextureFormat)0, true);
			texture2D.wrapMode = TextureWrapMode.Clamp;
			IHFILHIAGLG.Add(new MHOCDHIPMBI(texture2D));
		}
	}
	...
	protected virtual void MFKNMALEKKB()
	{
		...
		for (int i = 0; i < FNKHCBMODPB; i++)
		{
			Texture2D texture2D = new Texture2D(Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.width, Singleton<ArmyPreviewCamera>.instance.FCEIGDLFOMH.height, TextureFormat.RGB565, false);
			texture2D.wrapMode = TextureWrapMode.Repeat;
			IHFILHIAGLG.Add(new MHOCDHIPMBI(texture2D));
		}
	}
