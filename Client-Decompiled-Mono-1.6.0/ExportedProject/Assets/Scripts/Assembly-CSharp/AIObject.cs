...
public class AIObject : MainGameEntity, NKMKCCDCHCL, KMHCMCBGFFC
{
	...
	protected void SpawnByCardRPC(float PAENPHMEMGC, string ICDKHLOBKIE)
	{
		...
		cardId = ICDKHLOBKIE;
		spawnedByCard = true;
		UnityEngine.Debug.Log(string.Format("Spawned by card:  progress:{0}, obj: {1},uu: {2} ", cardProgress, base.gameObject.name, PFGLIAHPMCK));
	}
	...
	public virtual void ReSyncRPC(byte KFBOCCACLLK, Vector3 FLJHPFJFJIN, Quaternion OOMCGGAFJNH, UpgradeSlots.UnitUpgrades KBBDGCLPOCJ, bool LGKBHPBDNFE, float PAENPHMEMGC, string ICDKHLOBKIE)
	{
		...
		{
			UnityEngine.Debug.LogError(string.Format("Error in Resync OwnerId: {0}, My Id: {1}, isOwnerActive: {2}, isMasterClient:{3}", LAADDLGBFIG.ownerId, PhotonNetwork.player.ID, LAADDLGBFIG.isOwnerActive, PhotonNetwork.isMasterClient));
		}
	...
	protected virtual void ODDDGIALOPM(GHPGNELIDBM CIOPAKLHFIL)
	{
		if (destroyableObj != null)
		{
			destroyableObj.ChangeLayer(CIOPAKLHFIL, false);
		}
	}
