...
public class IMKNKBHECOD : HHFHFANGCEJ
{
		...
		: base("LeagueEntered", NKHJBLBAAEB.LeagueEntered)
	{
		...
		if (INKLAPHGBHI)
		{
			NHHGDIDEIMM += string.Format("{0}-firstTime-{1}", BMNDININJCE, Singleton<BeanstalkServerManager>.instance.currentTimestamp);
		}
		else
		{
			NHHGDIDEIMM += string.Format("{0}-{1}", BMNDININJCE, Singleton<BeanstalkServerManager>.instance.currentTimestamp);
		}
	...
	{
		LOJEAOPLOPO = true;
		NHHGDIDEIMM += string.Format("{0}-{1}", HNEFOLPHMHK, Singleton<BeanstalkServerManager>.instance.currentTimestamp);
