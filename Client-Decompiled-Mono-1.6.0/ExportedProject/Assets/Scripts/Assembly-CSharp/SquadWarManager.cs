...
public class SquadWarManager : Core_BaseScript
{
	...
	public static SquadWarManager instance
	{
		get
		{
			BJHPKLAEFCI = BJHPKLAEFCI ?? ((SquadWarManager)UnityEngine.Object.FindObjectsOfType(typeof(SquadWarManager))[0]);
			return BJHPKLAEFCI;
		}
	}
	...
	private List<string> PBKGFCMJJCG(int PDMGPGEHLNM)
	{
		...
		for (int i = 1; i <= PDMGPGEHLNM; i++)
		{
			string item = string.Format("FakeDoNotClick{0:2}", i);
			list.Add(item);
		}
		return list;
	}
	...
	private void FKMGJKEMANA(DatabaseAction IFGAGNBDKBE)
	{
		...
		{
			List<DatabasePlayer> list = Singleton<ServerResultsCache>.instance.OOCMLOAOOIB(GameLoginManager.currentPlayer.squadName, true);
			if (list == null)
			{
	...
	private void PMMMGCGNEBM(DatabaseAction IFGAGNBDKBE)
	{
		...
		case DatabaseAction.GetSquadWarsDivision:
		{
			...
			AANECPGDMGM aANECPGDMGM = Singleton<ServerResultsCache>.instance.MKKMNBIBCFM(fCOHPKBKGJA.CGIFEHPDBAL, true);
	...
	private AANECPGDMGM LMIGAJNJCGJ(string MHPNDNJDPGE, AANECPGDMGM AOEGHPABLAO, int KPEKCHDBFIG)
	{
		...
		aANECPGDMGM.DAANKCOLJGJ = string.Format("menu-squad-{0}", UnityEngine.Random.Range(1, 37));
