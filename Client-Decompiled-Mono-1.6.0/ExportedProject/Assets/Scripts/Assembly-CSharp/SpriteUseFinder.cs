...
public class SpriteUseFinder : Core_BaseScript
{
	...
	{
		UISprite[] componentsInChildren = base.gameObject.GetComponentsInChildren<UISprite>(false);
	...
	private string LOBJLHFGILO(GameObject PLIEAOICGJC, GameObject EJDBAIHFDCH)
	{
		...
		return string.Format("{0} -> {1}", LOBJLHFGILO(PLIEAOICGJC.transform.parent.gameObject, EJDBAIHFDCH), PLIEAOICGJC.name);
	}
	...
	private void DIBBFJIEKOH()
	{
		...
		Debug.Log(string.Format("Prefabs to go through: {0} + {1}", files.Length, files2.Length));
		string[] array = files;
	...
	private void DCHGEFCMILM()
	{
		...
		Debug.Log(string.Format("Scripts to go through: {0}", files.Length));
		string[] array = files;
		foreach (string text in array)
		{
			string[] array2 = File.ReadAllLines(text);
	...
	private void LGPKOIHAHGG()
	{
		...
		if (num > 0)
		{
			Debug.LogWarning(string.Format("Not used sprites ({0}/{1}) in atlas \"{2}\" :\n{3}", num, PJDAIENPLBE.Count, ANBAFEJBEPD.name, text));
		}
		else
		{
			Debug.Log(string.Format("All \"sprites\" ({0}) from atlas \"{1}\" are used", PJDAIENPLBE.Count, ANBAFEJBEPD.name));
		}
		if (!MOOPDDGOPBL)
		{
			return;
		}
		string text2 = string.Format("Sprite \"{0}\" was used in files:", KCHMDALPMBN);
	...
	private void HKPENELPMIC()
	{
		...
		LLEBDFOLOPF();
		Debug.Log(string.Format("Dictionary filled and contains {0} ids", PJDAIENPLBE.Count));
	}
	...
	private void IBMKBIDKBMF()
	{
		...
		Debug.Log(string.Format("Scenes to go through: {0}", files.Length));
		string[] array = files;
		foreach (string text in array)
		{
	...
	private void NPEEDLGFOHL()
	{
		...
		Debug.Log(string.Format("Selected Atlas \"{0}\"", ANBAFEJBEPD.name));
		HKPENELPMIC();
		LJAEMGBJAFK();
		if (LECEBJHCPAL)
		{
