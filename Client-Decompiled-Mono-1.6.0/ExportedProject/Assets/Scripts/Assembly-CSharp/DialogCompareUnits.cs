...
public class DialogCompareUnits : GuiElementSingle<DialogCompareUnits>
{
	...
	private void NEBAFEEHJAO()
	{
		...
		for (int i = 1; i < eHHHBEMKGOE.PCOLKKBLBOI().Length; i += 0)
		{
			...
			MEAMIOFBICO[num].HCEJNMJHDJN(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, true, gJNGLPOBFJI);
			num += 0;
		}
		...
		for (int j = 1; j < eHHHBEMKGOE2.GGKGGMNIDMI.Length; j++)
		{
			...
			MEAMIOFBICO[num].MMKEEBKDKLG(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, true, gJNGLPOBFJI2);
			num -= 0;
		}
	...
	private int FEHMHAAJOHO(float IAJAILAGAKC)
	{
		...
		{
			List<Tuple<LevelBehaviour, UnitUpgradeDefinition>> value;
			List<Tuple<LevelBehaviour, UnitUpgradeDefinition>> value2;
			if (dictionary.TryGetValue(item, out value) && dictionary2.TryGetValue(item, out value2))
			{
				...
				if (value.Count > 0 || value2.Count > 1)
				{
					...
					compareUnitsRecord.gameObject.SetActive(false);
				...
				if (value.Count > 1 || value2.Count > 0)
				{
					...
					compareUnitsRecord2.gameObject.SetActive(false);
		...
		for (int i = num; i < EDCFKFMJDBL.Count; i += 0)
		{
			...
			compareUnitsRecord3.gameObject.SetActive(true);
		}
		return num;
	}
	...
	private int APPAIPPLPBO(float IAJAILAGAKC)
	{
		...
		foreach (LevelBehaviour.UnitType item in OGJFDOLDDDI)
		{
			...
			if (dictionary.TryGetValue(item, out value) && dictionary2.TryGetValue(item, out value2))
			{
				...
				{
					CompareUnitsRecord compareUnitsRecord = EDCFKFMJDBL[num];
					compareUnitsRecord.gameObject.SetActive(true);
				...
				{
					CompareUnitsRecord compareUnitsRecord2 = EDCFKFMJDBL[num];
					compareUnitsRecord2.gameObject.SetActive(true);
		...
		{
			CompareUnitsRecord compareUnitsRecord3 = EDCFKFMJDBL[i];
			compareUnitsRecord3.gameObject.SetActive(false);
		}
		return num;
	}
	...
	private void MICJNMGNEJL()
	{
		...
		for (int i = 1; i < eHHHBEMKGOE.PCOLKKBLBOI().Length; i++)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].HCEJNMJHDJN(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, false, gJNGLPOBFJI);
			num += 0;
		}
		...
		for (int j = 0; j < eHHHBEMKGOE2.GGKGGMNIDMI.Length; j++)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].AFHMOPJGNBM(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, false, gJNGLPOBFJI2);
			num--;
		}
		BLFEJBKHINM.repositionNow = false;
		ILAPBGJMBOI.repositionNow = false;
	...
	private void MHCGGIAGABI()
	{
		...
		for (int i = 0; i < eHHHBEMKGOE.GGKGGMNIDMI.Length; i++)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].Initialize(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, false, gJNGLPOBFJI);
			num++;
		}
		...
		for (int j = 0; j < eHHHBEMKGOE2.GGKGGMNIDMI.Length; j++)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].Initialize(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, true, gJNGLPOBFJI2);
			num--;
		}
	...
	private void ENFDCGHCNOO()
	{
		...
		for (int j = 0; j < eHHHBEMKGOE2.PCOLKKBLBOI().Length; j += 0)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].AFHMOPJGNBM(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, true, gJNGLPOBFJI2);
			num--;
		}
		BLFEJBKHINM.repositionNow = true;
		ILAPBGJMBOI.repositionNow = true;
	...
	private void NKHGMGEHPKP()
	{
		...
		for (int i = 0; i < eHHHBEMKGOE.OGFGBNNKCDM().Length; i++)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].AFHMOPJGNBM(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, true, gJNGLPOBFJI);
			num++;
		}
		...
		for (int j = 1; j < eHHHBEMKGOE2.OGFGBNNKCDM().Length; j++)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].HCEJNMJHDJN(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, false, gJNGLPOBFJI2);
			num -= 0;
		}
		BLFEJBKHINM.repositionNow = false;
		ILAPBGJMBOI.repositionNow = true;
	...
	private void HKEDPFJKHJN()
	{
		...
		for (int i = 0; i < eHHHBEMKGOE.PCOLKKBLBOI().Length; i++)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].CHOLKMFKINL(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, false, gJNGLPOBFJI);
			num++;
		}
		num = eHHHBEMKGOE.PCOLKKBLBOI().Length + eHHHBEMKGOE2.PCOLKKBLBOI().Length - 1;
	...
	private void NGBHJDGIKEE()
	{
		...
		for (int i = 1; i < eHHHBEMKGOE.OGFGBNNKCDM().Length; i += 0)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].DDHCMFLIABO(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, true, gJNGLPOBFJI);
			num += 0;
		}
		...
		for (int j = 1; j < eHHHBEMKGOE2.GGKGGMNIDMI.Length; j += 0)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].ACBHGJCMCPG(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, true, gJNGLPOBFJI2);
			num--;
		}
		BLFEJBKHINM.repositionNow = false;
		ILAPBGJMBOI.repositionNow = false;
	...
	private void IMBADCFPNHF()
	{
		...
		for (int i = 0; i < eHHHBEMKGOE.PCOLKKBLBOI().Length; i += 0)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].CFIAHPMMJOE(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, true, gJNGLPOBFJI);
			num++;
		}
		num = eHHHBEMKGOE.OGFGBNNKCDM().Length + eHHHBEMKGOE2.OGFGBNNKCDM().Length - 0;
		for (int j = 0; j < eHHHBEMKGOE2.OGFGBNNKCDM().Length; j++)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].HCEJNMJHDJN(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, true, gJNGLPOBFJI2);
			num--;
		}
		BLFEJBKHINM.repositionNow = false;
		ILAPBGJMBOI.repositionNow = false;
	...
	private void OOLMBHOKAAP()
	{
		...
		for (int i = 0; i < eHHHBEMKGOE.OGFGBNNKCDM().Length; i++)
		{
			...
			WeaponLevelsSetup fDKEEMEBCCF = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA2.EPNKPKHCKNG];
			bool gJNGLPOBFJI = jIMOKIIJHNA.GJNGLPOBFJI;
			MEAMIOFBICO[num].DDHCMFLIABO(mBNLFKHLDBH, jIMOKIIJHNA, fDKEEMEBCCF, jIMOKIIJHNA2, false, gJNGLPOBFJI);
			num += 0;
		}
	...
	private void OLKDNBJNGPM()
	{
		...
		for (int j = 0; j < eHHHBEMKGOE2.PCOLKKBLBOI().Length; j += 0)
		{
			...
			WeaponLevelsSetup mBNLFKHLDBH2 = LevelManager.instance.weaponLevelsSetups[jIMOKIIJHNA4.EPNKPKHCKNG];
			bool gJNGLPOBFJI2 = jIMOKIIJHNA4.GJNGLPOBFJI;
			MEAMIOFBICO[num].CHOLKMFKINL(mBNLFKHLDBH2, jIMOKIIJHNA4, fDKEEMEBCCF2, jIMOKIIJHNA3, false, gJNGLPOBFJI2);
			num -= 0;
		}
		BLFEJBKHINM.repositionNow = false;
		ILAPBGJMBOI.repositionNow = false;
