...
public class SquadScreen : GuiScreenSingle<SquadScreen>
{
	...
	{
		EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
	}
	...
	private void DMLPBGJGCPH(bool PDPDGCNHCFN)
	{
		...
		EIGENHGGBEB[1].KIBNCDGOLJD(true);
		EIGENHGGBEB[1].LAHIJFMLODM(true);
		if (EIGENHGGBEB[4].gameObject.activeSelf)
		{
			EIGENHGGBEB[6].CGNAGNAOEJA(false);
		}
		EIGENHGGBEB[0].HEBAICIOOOJ(false);
		EIGENHGGBEB[8].BOFICLOIGDJ(false);
		EIGENHGGBEB[1].IPNKGKNJOLO(true).onFinished = delegate
		{
			EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
		};
	}
	...
	private void BBNKOAKBGGE(DatabaseAction IFGAGNBDKBE)
	{
		...
		case DatabaseAction.CreateAccount:
		{
			...
			if (!string.IsNullOrEmpty(currentPlayer.squadName))
			{
				...
				if (aANECPGDMGM == null)
				{
					aANECPGDMGM = Singleton<ServerResultsCache>.instance.MKKMNBIBCFM(currentPlayer.squadName, true);
				}
				if (aANECPGDMGM != null)
				{
	...
	private void ELEOEEAHJPH(bool PDPDGCNHCFN)
	{
		...
		EIGENHGGBEB[0].HODJNFBKFHC(true);
		EIGENHGGBEB[0].CGNAGNAOEJA(false);
		if (EIGENHGGBEB[1].gameObject.activeSelf)
		{
			EIGENHGGBEB[7].LAHIJFMLODM(true);
		}
		EIGENHGGBEB[6].DANJCHNBGJF(false);
		EIGENHGGBEB[2].IPNKGKNJOLO(true);
		EIGENHGGBEB[2].ANEFFNMMALL(false).onFinished = delegate
		{
			EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
		};
	}
	...
	private void KNNADMFOKFH(GameObject KHAHPAKDIKE)
	{
		...
		lMLBAEALJOF.DGJCAIJPEIM = KHAHPAKDIKE.GetComponentsInChildren<SquadButton>(true)[0];
	...
	private void ADAOHOIBJBO(bool PDPDGCNHCFN)
	{
		...
		EIGENHGGBEB[1].IIHMBOEGCOH(true);
		EIGENHGGBEB[1].MEMHLENAOMB(false);
		if (EIGENHGGBEB[3].gameObject.activeSelf)
		{
			EIGENHGGBEB[4].ANEFFNMMALL(false);
		}
		EIGENHGGBEB[1].KIBNCDGOLJD(true);
		EIGENHGGBEB[1].ANEFFNMMALL(false);
		EIGENHGGBEB[2].IPNKGKNJOLO(true).onFinished = delegate
		{
			EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
		};
	}
	...
	private void OKKHKLPNHHK(bool PDPDGCNHCFN)
	{
		...
		if (PDPDGCNHCFN)
		{
			BBNLILBDOEC.Animate(flag, true);
			FCIOFJOCLPJ.Animate(flag2, true);
			NILIEMMOFNN.Animate(flag3, true);
			PEBBFADKJFC.Animate(flag4, true);
			BAJAOALGHMK.Animate(flag5, true);
			GHINFPBNJFC.Animate(flag6, true);
			return;
		}
		if (BBNLILBDOEC.gameObject.activeSelf && !flag)
		{
			BBNLILBDOEC.Animate(flag, false);
		}
		if (FCIOFJOCLPJ.gameObject.activeSelf && !flag2)
		{
			FCIOFJOCLPJ.Animate(flag2, false);
		}
		if (NILIEMMOFNN.gameObject.activeSelf && !flag3)
		{
			NILIEMMOFNN.Animate(flag3, false);
		}
		if (PEBBFADKJFC.gameObject.activeSelf && !flag4)
		{
			PEBBFADKJFC.Animate(flag4, false);
		}
		if (BAJAOALGHMK.gameObject.activeSelf && !flag5)
		{
			BAJAOALGHMK.Animate(flag5, false);
		}
		if (GHINFPBNJFC.gameObject.activeSelf && !flag6)
		{
			GHINFPBNJFC.Animate(flag6, false);
		}
		if (!BBNLILBDOEC.gameObject.activeSelf && flag)
		{
			BBNLILBDOEC.Animate(flag, false);
	...
	private void EIELNPKGKIH(bool PDPDGCNHCFN)
	{
		...
		if (PDPDGCNHCFN)
		{
			BBNLILBDOEC.Animate(flag, true);
			FCIOFJOCLPJ.Animate(flag2, false);
			NILIEMMOFNN.HKDCHMLHKEB(flag3, false);
			PEBBFADKJFC.OGHIOCHEGGK(flag4, true);
			BAJAOALGHMK.NNPOEMEJGGP(flag5, true);
			GHINFPBNJFC.EAIHHHMDLBM(flag6, true);
			return;
		}
		if (BBNLILBDOEC.gameObject.activeSelf && !flag)
		{
		...
		if (NILIEMMOFNN.gameObject.activeSelf && !flag3)
		{
			NILIEMMOFNN.HKDCHMLHKEB(flag3, true);
		}
		if (PEBBFADKJFC.gameObject.activeSelf && !flag4)
		{
			PEBBFADKJFC.OGHIOCHEGGK(flag4, true);
		...
		if (BAJAOALGHMK.gameObject.activeSelf && !flag5)
		{
			BAJAOALGHMK.PDBGNFKGFJO(flag5, true);
		}
		if (GHINFPBNJFC.gameObject.activeSelf && !flag6)
		{
			GHINFPBNJFC.Animate(flag6, false);
		...
		if (!FCIOFJOCLPJ.gameObject.activeSelf && flag2)
		{
			FCIOFJOCLPJ.HKDCHMLHKEB(flag2, false);
		}
		if (!NILIEMMOFNN.gameObject.activeSelf && flag3)
		{
			NILIEMMOFNN.Animate(flag3, false);
		...
		if (!PEBBFADKJFC.gameObject.activeSelf && flag4)
		{
			PEBBFADKJFC.CMLKAPGBLGO(flag4, true);
		}
		if (!BAJAOALGHMK.gameObject.activeSelf && flag5)
		{
			BAJAOALGHMK.LFLGJGOKPND(flag5, false);
		}
		if (!GHINFPBNJFC.gameObject.activeSelf && flag6)
		{
			GHINFPBNJFC.EAIHHHMDLBM(flag6, true);
		...
		if (PEBBFADKJFC.gameObject.activeSelf && flag4)
		{
			PEBBFADKJFC.CMLKAPGBLGO(flag4, false);
		}
		if (BAJAOALGHMK.gameObject.activeSelf && flag5)
		{
			BAJAOALGHMK.LFLGJGOKPND(flag5, false);
		...
		if (GHINFPBNJFC.gameObject.activeSelf && flag6)
		{
			GHINFPBNJFC.DOMDAJKLILC(flag6, true);
		}
	}
	...
	protected virtual void HJJOAJHBKCH()
	{
		...
		EIGENHGGBEB[1].JIMBEDOMAFD(Localization.Localize("ID_REMINDER_UPGRADEAVAILABLE"), HAMKIKMJHDP, true);
		EIGENHGGBEB[6].gameObject.SetActive(false);
		EIGENHGGBEB[8].Initialize(Localization.Localize("globalPlayerLeaderboard"), HAMKIKMJHDP, true);
		...
		HIOONEIFKMF.gameObject.SetActive(false);
		IMKKLAKPIIN();
	...
	public void SetSquadIcon(string PCDMKLAJINM)
	{
		if (LCLNBHPNCIK != PCDMKLAJINM)
		{
			Debug.Log(string.Format("Setting squad icon: {0}", PCDMKLAJINM));
			GEAGOKJMJFD.mainTexture = Resources.Load<Texture>("SquadIcons/" + PCDMKLAJINM);
			LCLNBHPNCIK = PCDMKLAJINM;
		}
		else
		{
			Debug.Log(string.Format("Squad icon: {0} already displayed", PCDMKLAJINM));
		}
	}
	...
	private void CNKMNOPMPNJ(bool PDPDGCNHCFN)
	{
		...
		EIGENHGGBEB[0].Animate(false);
		EIGENHGGBEB[1].Animate(false);
		if (EIGENHGGBEB[2].gameObject.activeSelf)
		{
			EIGENHGGBEB[2].Animate(false);
		}
		EIGENHGGBEB[3].Animate(false);
		EIGENHGGBEB[4].Animate(false);
		EIGENHGGBEB[5].Animate(false).onFinished = delegate
		{
			EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
		};
	}
	...
	private void MIJMPEHBFMP(bool PDPDGCNHCFN)
	{
		...
		EIGENHGGBEB[1].CGNAGNAOEJA(false);
		EIGENHGGBEB[1].IPNKGKNJOLO(true);
		if (EIGENHGGBEB[6].gameObject.activeSelf)
		{
			EIGENHGGBEB[2].LAHIJFMLODM(true);
		}
		EIGENHGGBEB[4].ANEFFNMMALL(true);
		EIGENHGGBEB[8].BOFICLOIGDJ(true);
		EIGENHGGBEB[6].JLBALIEPGGC(true).onFinished = delegate
		{
			EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
		};
	}
	...
	private void IGOAEDMOHJF(bool PDPDGCNHCFN)
	{
		...
		EIGENHGGBEB[1].CEACEHMMBEI(true);
		EIGENHGGBEB[0].HODJNFBKFHC(false);
		if (EIGENHGGBEB[6].gameObject.activeSelf)
		{
			EIGENHGGBEB[4].DANJCHNBGJF(true);
		}
		EIGENHGGBEB[5].KIBNCDGOLJD(true);
		EIGENHGGBEB[2].CGNAGNAOEJA(false);
		EIGENHGGBEB[1].HNGLCCDCHGG(false).onFinished = delegate
		{
			EIGENHGGBEB[(int)FIDMHHGBLOE].Animate(true);
		};
	}
	...
	public virtual void MJGOCAKKDBK()
	{
		...
		if (FIDMHHGBLOE == MPJAFCCIOEO.Member)
		{
			BBNLILBDOEC.Animate(false, true);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Member)
		{
			FCIOFJOCLPJ.Animate(false, true);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Cardpool)
		{
			NILIEMMOFNN.Animate(true, false);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Stats)
		{
			PEBBFADKJFC.Animate(true, false);
		}
		if (FIDMHHGBLOE == (MPJAFCCIOEO)8)
		{
			BAJAOALGHMK.HBIKDPNHHEG(true, true);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Event)
		{
			GHINFPBNJFC.BBAJLLLEAID(false, false);
		}
	...
	private void HIHOANALHPH()
	{
		...
		if (flag2)
		{
			...
			if (aANECPGDMGM == null)
			{
				aANECPGDMGM = Singleton<ServerResultsCache>.instance.MKKMNBIBCFM(squadName, true);
			}
	...
	protected virtual void KNOKOJKOCEB()
	{
		...
		EIGENHGGBEB[1].CIPKPHAGJMP(Localization.Localize("126 MENU BATTLE SCREEN SHOWN"), HAMKIKMJHDP, true);
		EIGENHGGBEB[1].HHFELCAFANH(Localization.Localize("game-card-ico-shieldgenerator"), HAMKIKMJHDP);
		EIGENHGGBEB[1].gameObject.SetActive(false);
		EIGENHGGBEB[5].Initialize(Localization.Localize("Name"), HAMKIKMJHDP, true);
		EIGENHGGBEB[3].MKENDIKJPLJ(Localization.Localize("IDFV"), HAMKIKMJHDP, true);
		EIGENHGGBEB[7].OMCNLOHKCBJ(Localization.Localize("Medals/"), HAMKIKMJHDP, true);
	...
	public void SetMembersButton(int GKBPHAGAKAE, int BHGMAEOMLHG, int CIFCDKNDABO)
	{
		...
		if (squadButtonSpecial != null)
		{
			squadButtonSpecial.InitializeSpecial(string.Format("{0} / {1}", GKBPHAGAKAE, BHGMAEOMLHG), FIDMHHGBLOE == MPJAFCCIOEO.Member, IBCIOGABEEA);
	...
	protected override void AGIKPOLCGNF()
	{
		...
		EIGENHGGBEB[2].gameObject.SetActive(false);
	...
	private void MGECKFLAOIO(DatabasePlayer KKHKAJPNDEC, bool NCMHGPNPEJM)
	{
		...
		if (aANECPGDMGM != null)
		{
			...
			if (!aANECPGDMGM.IOLNJBGOOLJ && flag)
			{
				WarningDialog.ShowError(Localization.Localize("NULL WARCARD"), Localization.Localize("Buy_Unit_Upgrade"), 891f, null, string.Empty, false, true);
			}
	...
	public override void DoBeforeHide()
	{
		...
		if (FIDMHHGBLOE == MPJAFCCIOEO.Member)
		{
			BBNLILBDOEC.Animate(false, false);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Cardpool)
		{
			FCIOFJOCLPJ.Animate(false, false);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Event)
		{
			NILIEMMOFNN.Animate(false, false);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Stats)
		{
			PEBBFADKJFC.Animate(false, false);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Emblem)
		{
			BAJAOALGHMK.Animate(false, false);
		}
		if (FIDMHHGBLOE == MPJAFCCIOEO.Info)
		{
			GHINFPBNJFC.Animate(false, false);
		}
		Singleton<AwaitingSquadMembersManager>.instance.RemoveErrorPlayers();
	}
	...
	public void EIMIICGAJNI(GameObject KHAHPAKDIKE)
	{
		...
		if (!aANECPGDMGM.IOLNJBGOOLJ && flag)
		{
			WarningDialog.ShowError(Localization.Localize("menu-assignments-type-score"), Localization.Localize("set unit not borrowed"), 1135f, null, string.Empty, true);
		}
