...
public class SelectFriendsDialog : GuiElementSingle<SelectFriendsDialog>, PAIIOKBBHBC
{
	...
	public NKHJBLBAAEB currentType
	{
		get
		{
			return GPABKIOEGKM;
	...
	public bool isSquadInviteDialog
	{
		get
		{
			return GPABKIOEGKM == NKHJBLBAAEB.INVITE;
		}
	}
	...
	private void LIAKPKPKGPN(string EFHMCBKOAEO)
	{
		if (EFHMCBKOAEO.Length < 4)
		{
			...
			{
				LLEBFAKNBPM();
				CNCFOPONEFK.gameObject.SetActive(false);
				HEKJIEEOJIJ(Localization.Localize("ID_SELECTFRIENDERRORSHORTNAME"));
				PFDFPFMOEDF.UpdateScrollbars(true);
				PFDFPFMOEDF.AlignToPos(true);
			}
		}
		else
		{
	...
	private void EKOFHHFAPCI(GameObject KHAHPAKDIKE)
	{
		...
		gEOEAKHBELJ.DGJCAIJPEIM = KHAHPAKDIKE.GetComponentsInChildren<UIButton>(true)[0];
	...
	private void COGFCKDOLAF()
	{
		...
		{
			MGBAIEMOCDN.Add(new Tuple<bool, DatabasePlayer>(false, item));
		}
		if (MGBAIEMOCDN.Count > 0 && MGBAIEMOCDN.Count <= GameVariables.lowFacebookFriends)
		{
			MGBAIEMOCDN.Add(new Tuple<bool, DatabasePlayer>(true, null));
		}
	...
	private void KHEEMLNHPGA()
	{
		...
		ADOICKINGIM.SetActive(false);
		FOPDLAOBLKI.SetActive(false);
		GDKAHDBHDPF.SetActive(false);
		GNBBKBJHIKD.SetActive(false);
		FIAPALMLLKD.gameObject.SetActive(false);
		JCDGKKHPJKP();
		LLEBFAKNBPM();
		if (IOFOBBGDJLL != MPJAFCCIOEO.Search)
		{
	...
	private void FODOPIPGHOA()
	{
		...
		if (IOFOBBGDJLL == MPJAFCCIOEO.Search)
		{
			FIAPALMLLKD.gameObject.SetActive(true);
			if (CDCFCEIMAFE)
			{
	...
	private void KBGGIBBHOHL()
	{
		...
		PFDFPFMOEDF.UpdateScrollbars(true);
		PFDFPFMOEDF.AlignToPos(false, UIDraggablePanel.Alingment.BottomRight, true);
	}
	...
	public override void DoBeforeShowUp()
	{
		...
		JGNJCGCAIDC.gameObject.SetActive(true);
		ACHPJBMGCOG.gameObject.SetActive(true);
	}
	...
	private void GIFODEKMPME()
	{
		...
		CDCFCEIMAFE = false;
		CNCFOPONEFK.gameObject.SetActive(true);
	}
