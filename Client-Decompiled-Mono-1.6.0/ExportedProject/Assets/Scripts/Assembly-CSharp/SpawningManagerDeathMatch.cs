...
public class SpawningManagerDeathMatch : Singleton<SpawningManagerDeathMatch>
{
			...
			default:
				...
				if (JMBKPALDGAL < GIFJOLADLEB.numberOfEnemies)
				{
					...
					if (NEMLKKBGIKJ != null && NEMLKKBGIKJ.prefab != null)
					{
						...
						{
							SpawningManager.OKBAIGKCPKA().CDHOPCCMOIH(NEMLKKBGIKJ, MCCLPJGMIDG, EEAMBOACAGF, Vector3.one * 960f, false);
						}
						else
						{
			...
			default:
				...
				if (JMBKPALDGAL < GIFJOLADLEB.numberOfEnemies)
				{
					...
					if (NEMLKKBGIKJ != null && NEMLKKBGIKJ.LELHDMOANJG() != null)
					{
						...
						{
							SpawningManager.OKBAIGKCPKA().Spawn(NEMLKKBGIKJ, MCCLPJGMIDG, EEAMBOACAGF, Vector3.one * 888f, false);
						}
						else
						{
	...
	public bool powerForSemiDeploy
	{
		get
		{
			return (int)FJBPNHHCJBE > (int)(float)Singleton<GameVariables>.instance.constants.GetRow(Constants.rowIds.SemiAutoDeployEnergy).FLOATVALUE;
		}
	}
	...
	private int CGNBHIJMIGN
	{
		get
		{
			return (int)(float)Singleton<GameVariables>.instance.constants.GetRow(Constants.rowIds.MaxEnergy).FLOATVALUE;
		}
	}
	...
	public void OKGNKIHCMJP()
	{
		if (!IONCCKCOEKI)
		{
			if (PreGenerateRandomUnit(false) && NBHLHPBODAH != null)
			{
	...
	public List<ArmyUnitDefinition> GetThreeRandomUnits(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		{
			List<ArmyUnitDefinition> value;
			if (EBKKGEIPJFO.TryGetValue(item4.armyUnit.behaviour.unitType, out value))
			{
				value.Add(item4);
				continue;
			}
	...
	public void EOPEDKLKKCD()
	{
		if (!IONCCKCOEKI)
		{
			...
			{
				NBHLHPBODAH(PHFLIBDGLCP);
				KNFDNEMOOOC(PHFLIBDGLCP, PlayerController.OGMBJPKOPCB.fraction, false);
			}
		}
		else if (NBHLHPBODAH != null)
		{
		...
		foreach (ArmyUnit item in HCEDGKDKKIG)
		{
			foreach (ArmyUnitDefinition unitsCount in item.unitsCounts)
			{
				if (unitsCount.index == DFHAAIFFLOE)
				{
					...
					List<ArmyUnitDefinition> value;
					if (AJFCOJMHFPJ.TryGetValue((GHPGNELIDBM)CIOPAKLHFIL, out value))
					{
	...
	public void DEENJAFLNND()
	{
		if (!IONCCKCOEKI)
		{
			...
			{
				NBHLHPBODAH(PHFLIBDGLCP);
				KNFDNEMOOOC(PHFLIBDGLCP, PlayerController.OGMBJPKOPCB.fraction, false, true);
			}
		}
		else if (NBHLHPBODAH != null)
		{
