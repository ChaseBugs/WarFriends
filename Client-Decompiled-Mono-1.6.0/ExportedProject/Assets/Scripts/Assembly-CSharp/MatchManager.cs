...
public class MatchManager : Singleton<MatchManager>
{
			...
			case 0u:
				...
				try
				{
					while (KFCAECGLKKO.MoveNext())
					{
						...
						current.point.PNKKPNIMEPL().GCAKAFMDFJA(true);
		...
		public bool BJOHOCFKMJO()
		{
			...
			case 0u:
				...
				try
				{
					...
					{
						MapDefinition.DefendPosition current = KFCAECGLKKO.Current;
						current.point.PHANADCNBFD().GCAKAFMDFJA(false);
		...
		public bool LLOCENNFFFJ()
		{
			...
			case 0u:
				...
				try
				{
					...
					{
						MapDefinition.DefendPosition current = KFCAECGLKKO.Current;
						current.point.LHHMNLJKOAO().ENKDOKELOED(false);
		...
		public bool IJEDIDEJIII()
		{
			...
			case 0u:
				...
				try
				{
					...
					{
						MapDefinition.DefendPosition current = KFCAECGLKKO.Current;
						current.point.JPHAJDIIAAI().ENKDOKELOED(true);
		...
		public bool PMBOHHPPJAC()
		{
			...
			case 0u:
				...
				try
				{
					...
					{
						MapDefinition.DefendPosition current = KFCAECGLKKO.Current;
						current.point.IGEDGCNJCAP().ENKDOKELOED(true);
	...
	public static string matchStatusPlayer
	{
		get
		{
			return (!string.IsNullOrEmpty(BNLIAKFBAKE[matchState].Value1)) ? Localization.Localize(BNLIAKFBAKE[matchState].Value1) : string.Empty;
		}
	}
	...
	public static string matchStatusAfterMatch
	{
		get
		{
			...
			{
				string text = EANNFJAPKIK(Singleton<GameController>.instance.KKHKAJPNDEC, true);
				if (text != null)
				{
					return text;
				}
	...
	public static float invitationTimeRemaining
	{
		get
		{
	...
	public static bool isReconnect
	{
		get
		{
			return reconnectState != KAKELCLPDDK.None && Singleton<GameController>.instance.isLoadingOrPlaying;
		}
	}
	...
	public static bool afterMatch
	{
		get
		{
			return ILHNFHOFLCM >= GLFPILGCAPM.GameFinished;
		}
	}
		...
		try
		{
			...
			else if (aIObject != null)
			{
				if (aIObject.spawnedByCard)
				{
					...
					{
						IJNHABOLCNN(cardInstanceAll.id, 0, cardInstanceAll.iconName, true);
					}
				}
	...
	public void PHKIPDLJFFH(DestroyableObject.CJLNCMMNPBM KPJJFGIHFAC)
	{
		try
		{
			...
			else if (aIObject != null)
			{
				if (aIObject.spawnedByCard)
				{
					Card cardInstanceAll = CardManager.instance.GetCardInstanceAll(aIObject.NFJMLOKBEON(), false);
					if (cardInstanceAll == null)
					{
						IJNHABOLCNN(aIObject.IAJJBAJOGDM.unitDictionaryId, aIObject.spawnedCardLevel, aIObject.IAJJBAJOGDM.upgradeSlots.iconName, true);
					}
					else
					{
						IJNHABOLCNN(cardInstanceAll.id, 1, cardInstanceAll.iconName, true);
					}
				}
				else if (aIObject.IAJJBAJOGDM != null && aIObject.IAJJBAJOGDM.upgradeSlots != null)
				{
