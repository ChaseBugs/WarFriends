...
public class RateAppDialog : GuiElementSingle<RateAppDialog>, PAIIOKBBHBC
{
	...
	private string PKGGBAIEIJL
	{
		get
		{
			return string.Format("{0}{1}", (!LPMPDHFLCKL) ? string.Empty : "U", GMKAALLKIGH);
		}
	}

	private string MJPBKHPHEIE
	{
		get
		{
			return (!LPMPDHFLCKL) ? "_New_User" : "_Existing_User";
		}
	}
	...
	private void OOMGIAKJKLM(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			...
			{
				DialogManager.instance.NoMoreRateAppInThisVersion();
				Singleton<EventTrackingManager>.instance.LCPLAKMLCGH(string.Format("Ignore_Text_{0}{1}_Dont_Ask_Again", OFLEKCEFPPK, MJPBKHPHEIE), IMBPEAIHHEG);
			}
			else
			{
				Singleton<EventTrackingManager>.instance.LCPLAKMLCGH(string.Format("Ignore_Text_{0}{1}", OFLEKCEFPPK, MJPBKHPHEIE), IMBPEAIHHEG);
			}
			HideDialog();
		}
	}
	...
	private void FBGOLGKAJOA(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.LCPLAKMLCGH(string.Format("Rate_Text_{0}{1}", OFLEKCEFPPK, MJPBKHPHEIE), IMBPEAIHHEG);
			Application.OpenURL(GameVariables.GooglePlayMarketURL);
			DialogManager.instance.GameWasRated();
			HideDialog();
		}
