...
{
	private sealed class BCCEDLCAPFL : IEnumerator, IDisposable, IEnumerator<object>
	{
		internal float OGGDPMCDCLJ;
		...
		public bool IGOKEIDOCKB()
		{
			...
			case 0u:
				...
				PHDOCKCBJOF = new WaitForSeconds(OGGDPMCDCLJ);
				if (!FDKOKMKIIMG)
				{
					CLPPIAGLCJJ = 1;
				}
			...
			case 1u:
				if (!__AdWillCloseCalled)
				{
					AdWillClose -= __AdWillCloseMoneybackGuaranteeR;
					OnAdWillClose();
	...
	public static event Action<GFLKKPFAFGC> SessionLoginError;

	public static event Action GameConfigurationReceived;

	public static event Action<AccountType, string> AccountLoginComplete;

	public static event Action<string, GFLKKPFAFGC> AccountLoginError;

	public static event Action<string> NotificationAction;

	public static event Action NotificationWillClose;

	public static event Action<string, GFLKKPFAFGC> FriendAdded;

	public static event Action<string, GFLKKPFAFGC> FriendRemoved;

	public static event Action<string, GFLKKPFAFGC> FriendAccepted;

	public static event Action<string, GFLKKPFAFGC> FriendRejected;

	public static event Action<string, GFLKKPFAFGC> FriendsMigrated;

	public static event Action<List<EBDBJAKJNLD>> FriendsListUpdated;

	public static event Action<GFLKKPFAFGC> FriendsListError;

	public static event Action<int, string, string> PurchaseVerification;

	public static event Action<bool, GFLKKPFAFGC> AdAvailabilityResponse;
	...
	public static event Action<CKIENDMMJOG> RewardedAdCompletedWithObject;

	public static event Action<MFJJLPLILJF> IAPOfferAcceptedWithObject;

	public static event Action<KIDFEANMAKJ> VirtualGoodsOfferAcceptedWithObject;
	...
	public static event Action<int, GFLKKPFAFGC> GameDataError;
	...
	private void Awake()
	{
		...
		{
			base.gameObject.AddComponent<FuseSDK_Prime31_IAB>().PLEPNKNABMM = logging;
		}
	...
	public static void StartSession()
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK instance not initialized. Awake may not have been called.");
		}
	}
	...
	private static void _StartSession(string gameId, bool handleAdURLs)
	{
		if (_sessionStarted)
		{
			UnityEngine.Debug.LogWarning("FuseSDK: Duplicate StartSession call. Ignoring.");
			return;
		}
		if (string.IsNullOrEmpty(gameId))
		{
			UnityEngine.Debug.LogError("FuseSDK: Null or empty App ID. Make sure your App ID is entered in the FuseSDK prefab");
			return;
		}
	...
	public static bool RegisterEvent(string name, string paramName, string paramValue, Hashtable variables)
	{
		...
		try
		{
			...
			double[] array2 = variables.Values.Cast<object>().Select(Convert.ToDouble).ToArray();
			return _fuseUnityPlugin.CallStatic<bool>("registerEvent", new object[5] { name, paramName, paramValue, array, array2 });
		}
		catch (Exception exception)
		{
			UnityEngine.Debug.LogError("FuseSDK: Error parsing hashtable in RegisterEvent. Operation failed.");
	...
	public static void RegisterAndroidInAppPurchase(OFJOIMIHPKF purchaseState, string purchaseToken, string productId, string orderId, DateTime purchaseTime, string developerPayload, double price, string currency)
	{
		RegisterAndroidInAppPurchase(purchaseState, purchaseToken, productId, orderId, purchaseTime.FJOPPAABFPB(), developerPayload, price, currency);
	}

	public static void RegisterAndroidInAppPurchase(OFJOIMIHPKF purchaseState, string purchaseToken, string productId, string orderId, long purchaseTime, string developerPayload, double price, string currency)
	{
	...
	public static void RegisterIOSInAppPurchaseList(BFBLJPDGMHH[] products)
	{
	}

	public static void RegisterIOSInAppPurchase(string productId, string transactionId, byte[] transactionReceipt, OFJOIMIHPKF transactionState)
	{
	}
	...
	public static CKIENDMMJOG GetRewardedInfoForZone(string zoneId)
	{
		FuseLog("GetRewardedInfoForZoneID");
		string hHLLCFBCONC = _fuseUnityPlugin.CallStatic<string>("getRewardedInfoForZoneID", new object[1] { zoneId });
		return new CKIENDMMJOG(hHLLCFBCONC);
	}

	public static KIDFEANMAKJ GetVGOfferInfoForZone(string zoneId)
	{
		FuseLog("GetVGOfferInfoForZone");
		string hHLLCFBCONC = _fuseUnityPlugin.CallStatic<string>("getVirtualGoodsOfferInfoForZoneID", new object[1] { zoneId });
		return new KIDFEANMAKJ(hHLLCFBCONC);
	}

	public static MFJJLPLILJF GetIAPOfferInfoForZone(string zoneId)
	{
		FuseLog("GetIAPOfferInfoForZone");
		string hHLLCFBCONC = _fuseUnityPlugin.CallStatic<string>("getIAPOfferInfoForZoneID", new object[1] { zoneId });
		return new MFJJLPLILJF(hHLLCFBCONC);
	}
	...
	public static void RegisterGender(GDEKGPOEECL gender)
	{
	...
	public static void FuseLog(string str)
	{
		...
		{
			UnityEngine.Debug.Log("FuseSDK: " + str);
		}
	}
	...
	public static List<EBDBJAKJNLD> GetFriendsList()
	{
	...
	private void _SessionLoginError(string error)
	{
		...
		int result;
		if (int.TryParse(error, out result))
		{
	...
	private void _PurchaseVerification(string param)
	{
		...
		int result;
		if (array.Length == 3 && int.TryParse(array[0], out result))
		{
			OnPurchaseVerification(result, array[1], array[2]);
		}
		else
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _PurchaseVerification");
		}
	}
	...
	private void _AdAvailabilityResponse(string param)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _AdAvailabilityResponse");
		}
	}
	...
	private void _AdDidShow(string param)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _AdDidShow");
		}
	}
	...
	private void _AccountLoginComplete(string param)
	{
		...
		string[] array = param.Split(',');
		int result;
		if (array.Length == 2 && int.TryParse(array[0], out result))
		{
			OnAccountLoginComplete(result, array[1]);
		}
		else
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _AccountLoginComplete");
		}
	}
	...
	private void _AccountLoginError(string param)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _AccountLoginError");
		}
	}
	...
	private void _TimeUpdated(string timestamp)
	{
		...
		{
			OnTimeUpdated(result.JHHJCIOHPAD());
		}
		else
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _TimeUpdated");
		}
	}
	...
	private void _FriendAdded(string param)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _FriendAdded");
		}
	}
	...
	private void _FriendsMigrated(string param)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _FriendsMigrated");
		}
	}

	private static List<EBDBJAKJNLD> DeserializeFriendsList(string[] friendList)
	{
		List<EBDBJAKJNLD> list = new List<EBDBJAKJNLD>();
		if (friendList == null)
		{
			UnityEngine.Debug.LogWarning("FuseSDK: NULL FriendsList.");
			return list;
		}
		foreach (string text in friendList)
		{
			...
			if (array.Length == 4)
			{
				list.Add(new EBDBJAKJNLD
				{
					OAKIALCHAIG = array[0],
					DPDEIHFAPJP = array[1],
					IEKIGEMEECE = array[2],
					HELOLIKHOCB = (array[3] != "0")
				});
			}
			else
			{
				UnityEngine.Debug.LogError("FuseSDK: Error reading FriendsList data. Invalid line: " + text);
			}
		}
		return list;
	}
	...
	private void _FriendsListError(string error)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _FriendsListError");
		}
	}
	...
	private void _GameDataSetAcknowledged(string requestId)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _GameDataSetAcknowledged");
		}
	}
	...
	private void _GameDataError(string param)
	{
		...
		{
			UnityEngine.Debug.LogError("FuseSDK: Parsing error in _GameDataError");
		}
	}
	...
	private void _GameDataReceived(string param)
	{
		...
		if (array.Length == 2)
		{
			...
			{
				UnityEngine.Debug.LogError("FuseSDK: Parsing error in _GameDataReceived");
			}
		}
		Dictionary<string, string> dictionary = new Dictionary<string, string>();
		int num = 100;
	...
	{
		BCCEDLCAPFL bCCEDLCAPFL = new BCCEDLCAPFL();
		bCCEDLCAPFL.OGGDPMCDCLJ = timeout;
		return bCCEDLCAPFL;
	}
	...
	private static void OnSessionLoginError(int error)
	{
		...
		{
			FuseSDK.SessionLoginError((error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnAdAvailabilityResponse(int available, int error)
	{
		...
		{
			FuseSDK.AdAvailabilityResponse(available != 0, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnRewardedAdCompleted(CKIENDMMJOG rewardInfo)
	{
	...
	private static void OnIAPOfferAccepted(MFJJLPLILJF offerInfo)
	{
	...
	private static void OnVirtualGoodsOfferAccepted(KIDFEANMAKJ offerInfo)
	{
	...
	private static void OnAccountLoginError(string accountId, int error)
	{
		...
		{
			FuseSDK.AccountLoginError(accountId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnFriendAdded(string fuseId, int error)
	{
		...
		{
			FuseSDK.FriendAdded(fuseId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnFriendRemoved(string fuseId, int error)
	{
		...
		{
			FuseSDK.FriendRemoved(fuseId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnFriendAccepted(string fuseId, int error)
	{
		...
		{
			FuseSDK.FriendAccepted(fuseId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnFriendRejected(string fuseId, int error)
	{
		...
		{
			FuseSDK.FriendRejected(fuseId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnFriendsMigrated(string fuseId, int error)
	{
		...
		{
			FuseSDK.FriendsMigrated(fuseId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}

	private static void OnFriendsListUpdated(List<EBDBJAKJNLD> friends)
	{
	...
	private static void OnFriendsListError(int error)
	{
		...
		{
			FuseSDK.FriendsListError((error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
	...
	private static void OnGameDataError(int error, int requestId)
	{
		...
		{
			FuseSDK.GameDataError(requestId, (error >= 7) ? GFLKKPFAFGC.UNDEFINED : ((GFLKKPFAFGC)error));
		}
	}
