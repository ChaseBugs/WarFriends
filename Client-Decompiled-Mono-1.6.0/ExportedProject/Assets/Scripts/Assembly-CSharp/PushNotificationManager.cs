...
public class PushNotificationManager : InGameSerializedObjectGeneric<PushNotificationManager.PushNotificationData>
{
	...
	private void InitNotifications()
	{
		if (!notificationsInitialized)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				string text2 = Localization.Localize(string.Format("ID_{0}_NAME", text));
	...
	public void ScheduleDailyRewardNotification(int onTime, string text)
	{
		if (onTime > 1)
		{
			ScheduleNotification("NextDailyRewardLocalnotification", text, onTime, BAIMJGGJNHF.GAME_INFO, 1, true);
			Save();
		}
		else
		{
	...
	public void SheduleLocalSkirmishNotification(int timeStamp)
	{
		if (timeStamp > Singleton<BeanstalkServerManager>.instance.currentTimestamp)
		{
			...
			ScheduleNotification("instantBattleAvailable", message, timeStamp, BAIMJGGJNHF.GAME_INFO, 1, true);
			Save();
		}
	}
	...
	public void ScheduleLocalWarArenaReminder(int arenaStartTime)
	{
		if (!LevelManager.instance.isWarArenaLocked)
		{
			...
			{
				string keyFormat = string.Format("ID_NOTIFICATION_WARARENAREMINDER{0}", UnityEngine.Random.Range(1, 5));
				string message = Localization.LocalizeFormat(keyFormat, num);
				ScheduleNotification("warArenaReminder", message, num3, BAIMJGGJNHF.EVENT_INFO, 1, true);
				Save();
			}
		}
	}
	...
	public void ScheduleLocalWarArenaStart(int phaseStartTime, bool isArenaStartAlso = false)
	{
		if (!LevelManager.instance.isWarArenaLocked)
		{
			...
			{
				string key = ((!isArenaStartAlso) ? string.Format("ID_NOTIFICATION_WARARENAPHASE{0}", UnityEngine.Random.Range(1, 3)) : string.Format("ID_NOTIFICATION_WARARENASTART{0}", UnityEngine.Random.Range(1, 5)));
				string message = Localization.Localize(key);
				ScheduleNotification("warArenaStart", message, phaseStartTime, BAIMJGGJNHF.EVENT_INFO, 1, true);
				Save();
			}
		}
	}
	...
	{
		LocalNotificationData value;
		if (data.localNotifications.TryGetValue(key, out value))
		{
	...
	{
		CancelLocalNotification(notificationKey, false);
	...
	private void ReactToNotification(DatabaseAction databaseAction)
	{
		...
		case DatabaseAction.JoinSquadRequest:
		{
			...
			{
				Singleton<BeanstalkServerManager>.instance.IIFBHBOKGKA(currentPlayer2.squadName, true);
			}
			break;
		}
	...
	public void OnMessageReceived(object sender, MessageReceivedEventArgs e)
	{
		...
		if (e.Message.Data != null && e.Message.Data.ContainsKey("id"))
		{
			...
			int result;
			if (int.TryParse(e.Message.Data["id"], out result))
			{
