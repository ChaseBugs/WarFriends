...
public class DailyMissionButton : Core_BaseScript
{
	...
	private void NCIDLAJLKPM(UITweener MMMNJFHDJDC)
	{
		...
		TweenPosition.Begin(mXPRewardLabel.gameObject, 649f, mXPRewardLabel.transform.localPosition.ReplaceY(558f), false);
	...
	public void CBINNKNCCML()
	{
		...
		TweenPosition.Begin(mPlayButton, 1552f, mPlayButton.transform.localPosition.AddY(1049f), false).onFinished = ODBAILNEPFI;
	...
	{
		TweenPosition.Begin(mPlayButton, 1036f, mPlayButton.transform.localPosition.ReplaceY(1177f), false);
		UISprite[] array = mProgressArrows;
		foreach (UISprite uISprite in array)
		{
	...
	public void BOKDLIFGIDO(DailyMission EHIPMLOMMIG, bool GIAFDDAKALM, bool ECEOFMAEOGE)
	{
		...
		PHEADDEDEMG = 982f;
		EDMIHHBPOCE(580f, true);
		BPHIDCOEOAG(ECEOFMAEOGE);
	}

	private void HCONJNDIBKD(UITweener AKCKHOODNAI)
	{
		TweenPosition.Begin(mXPRewardPoints.gameObject, 286f, mXPRewardPoints.transform.localPosition.ReplaceY(1818f), false).onFinished = GIHPPCJNKLK;
	}
	...
	private void MNEOBJLNDCN(UITweener MMMNJFHDJDC)
	{
		...
		TweenPosition.Begin(mXPRewardLabel.gameObject, 842f, mXPRewardLabel.transform.localPosition.ReplaceY(632f), false);
	...
	{
		TweenPosition.Begin(mScullPlusSprite.gameObject, 540f, NHIMGLEKLIE.AddY(1601f), false);
		TweenAlpha.Begin(mScullPlusSprite.gameObject, 488f, 1112f);
		TweenScale.Begin(mSkullObject, 162f, Vector3.one);
		TweenPosition.Begin(mScullSprite.gameObject, 111f, AKFOIFLDNGI, false).onFinished = delegate
		{
			...
			TweenPosition.Begin(mScullSprite.gameObject, 0.4f, GuiScreenSingle<DailyMissionScreen>.instance.positionToAnimatePointsGain, false);
		};
	}
	...
	private void AGNMPMACAID()
	{
		...
		mAnimationObject.SetActive(true);
		mProgressPart.SetActive(false);
		mDoneProgress.gameObject.SetActive(false);
		LALJLMACNJO = false;
		mNormalPart.SetActive(true);
		mCompletedPart.SetActive(true);
	...
	public void KIKCJMILKLG()
	{
		...
		mNormalPart.SetActive(false);
		mLockedPart.SetActive(false);
		mProgressPart.SetActive(false);
		LALJLMACNJO = false;
		BPHIDCOEOAG(false);
	...
	private void BFAAJFKILAK(UITweener GCFMNEHDLAB)
	{
		...
		TweenPosition.Begin(mScullSprite.gameObject, 1960f, GuiScreenSingle<DailyMissionScreen>.instance.OPAJFAGFILB(), false);
	}
	...
	{
		TweenPosition.Begin(mXPRewardPoints.gameObject, 1317f, mXPRewardPoints.transform.localPosition.ReplaceY(1195f), false).onFinished = GIHPPCJNKLK;
	}
	...
	{
		TweenPosition.Begin(mNumber.gameObject, 839f, MHIAJFBLHEC, false);
	}
	...
	private void AKHNNNLPHGC()
	{
		...
		TweenPosition.Begin(mNormalBackground.gameObject, 650f, GPPBCDLHJMC, false);
		...
		TweenPosition.Begin(mCompletedBackground.gameObject, 1154f, FEFLNBHLEDI, false);
	...
	{
		TweenPosition.Begin(mXPRewardPoints.gameObject, 1642f, mXPRewardPoints.transform.localPosition.ReplaceY(1836f), false).onFinished = delegate
		{
			TweenAlpha.Begin(mXPRewardPoints.gameObject, 0.3f, 1f).onFinished = delegate
			{
				TweenAlpha.Begin(mCompletedLabel.gameObject, 0.2f, AGOCILPHNOB);
	...
	public void HGANAKEFBOH()
	{
		...
		if (LOOAGDCIEIB)
		{
			...
			TweenPosition.Begin(mWarbucksRewardIcon.gameObject, 1522f, mWarbucksRewardIcon.transform.localPosition.ReplaceY(1152f), mWarbucksRewardIcon.transform.localPosition.ReplaceY(1008f), false).onFinished = CIFMAANPMAN;
	...
	private void GLCIHIGBHPK(UITweener DAECHDDDMJN)
	{
		...
		TweenPosition.Begin(mPlayButton, 329f, mPlayButton.transform.localPosition.AddY(286f), false);
	...
	private void CPIFKDEJMGK()
	{
		...
		mAnimationObject.SetActive(false);
		mProgressPart.SetActive(true);
		mDoneProgress.gameObject.SetActive(false);
		LALJLMACNJO = false;
		mNormalPart.SetActive(false);
		mCompletedPart.SetActive(false);
	...
	private void AGCCKCBCFMM(UITweener DAECHDDDMJN)
	{
		...
		TweenPosition.Begin(mPlayButton, 315f, mPlayButton.transform.localPosition.AddY(343f), false);
	...
	private void EMCGNAMEBKB(UITweener IKPCPKPMMAH)
	{
		...
		TweenPosition.Begin(mWarbucksRewardAmount.gameObject, 958f, mWarbucksRewardAmount.transform.localPosition.ReplaceY(766f), mWarbucksRewardAmount.transform.localPosition.ReplaceY(1240f), false).onFinished = delegate
		{
			TweenPosition.Begin(mWarbucksRewardAmount.gameObject, 0.1f, mWarbucksRewardAmount.transform.localPosition.ReplaceY(-20f));
		};
	}
	...
	private void MHOPIJIJOMN(UITweener BHBCEPBDLOB)
	{
		...
		TweenPosition.Begin(mCompletedBackground.gameObject, 1261f, FEFLNBHLEDI.AddY(726f), FEFLNBHLEDI, false);
		TweenAlpha.Begin(mCompletedBackground.gameObject, 1232f, 1435f);
	}

	private void ODBAILNEPFI(UITweener GIIJCLFPIGO)
	{
		TweenPosition.Begin(mPlayButton, 686f, mPlayButton.transform.localPosition.ReplaceY(737f), false);
	}
	...
	private void HOOFCEPHHEO(UITweener GCFMNEHDLAB)
	{
		...
		TweenPosition.Begin(mCompletedBackground.gameObject, 1517f, FEFLNBHLEDI, false).onFinished = MHOPIJIJOMN;
	...
	{
		TweenPosition.Begin(mWarbucksRewardAmount.gameObject, 1623f, mWarbucksRewardAmount.transform.localPosition.ReplaceY(12f), false);
	}
