...
public class EventAssignmentManager : DatabaseSerializedObjectGeneric<EventAssignmentManager.EventAssignmentData>
{
	...
	public static EventAssignmentManager instance
	{
		get
		{
			BJHPKLAEFCI = BJHPKLAEFCI ?? ((EventAssignmentManager)UnityEngine.Object.FindObjectsOfType(typeof(EventAssignmentManager))[0]);
			return BJHPKLAEFCI;
		}
	}

	public EventAssignmentsConfig config { get; private set; }

	public List<EventAssignment> assignments { get; private set; }

	public List<Tuple<int, GameReward>> milestones { get; private set; }
	...
	public bool isEventActive
	{
		get
		{
			return PHEADDEDEMG < Singleton<BeanstalkServerManager>.instance.currentTimestamp && remainigTimeTillEnd > 0;
		}
	}
	...
	private int PHEADDEDEMG
	{
		get
		{
			return (config != null) ? config.startTime : 0;
		}
	}
	...
	private int CGAKMJBHKKG
	{
		get
		{
			return (config != null) ? config.endTime : 0;
		}
	}

	public int remainigTimeTillEnd
	{
		get
		{
	...
	public int maxValue
	{
		get
		{
			return (config != null) ? config.eventTargetValue : 0;
		}
	}
	...
	public float progress
	{
		get
		{
			return (maxValue != 0) ? Mathf.Clamp01((float)value / (float)maxValue) : 0f;
		}
	}
	...
	private void DNHHFEGOLPI()
	{
		...
		{
			Type type;
			if (PDEBGEAACNM.TryGetValue(assignment.type, out type))
			{
	...
	public void IBLJCNMAJON()
	{
		...
		{
			Singleton<MessageManager>.instance.AddMessage(new CBCIEFJHKEL(), true);
		}
	}
