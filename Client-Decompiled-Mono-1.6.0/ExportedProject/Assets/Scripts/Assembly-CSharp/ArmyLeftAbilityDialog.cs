...
public class ArmyLeftAbilityDialog : Core_BaseScript
{
	...
	public void OAAABGFLAFG()
	{
		...
		FPPPLPHDBMO();
		PKMBOCKEPIE(false);
		UIPanel[] componentsInChildren = CHNKLHLJHDP.GetComponentsInChildren<UIPanel>(true);
	...
	{
		GuiScreenSingle<ArmyScreen>.instance.KPEENKKMNDD.EDHCOMHOKDC.MEEKMLJEOBB.UpdateLeftContent(false);
	...
	private void LBBOFMMKHEI(int LFNBJLJPEGP)
	{
		if (IFFDIHCPKFE.upgradeSlots.equipped)
		{
			...
			Debug.Log(string.Format("Army Power Ability Animation from {0} to {1}", LFNBJLJPEGP, armyPowerX));
	...
	private void ONFHPJLFLLM(GameObject KHAHPAKDIKE)
	{
		...
		if (levelBehaviour != null)
		{
			...
			else
			{
				...
				{
					GuiElementSingle<OtherUpgradeDialog>.instance.ShowDialogUnits(levelBehaviour, IFFDIHCPKFE, true);
					return;
				}
		...
		GuiScreenSingle<ArmyScreen>.instance.EFGAIHKNKGG(IFFDIHCPKFE, num, true);
	...
	public void DGFIGGCNJNI(float EJEDANLAMBI, float PAENPHMEMGC)
	{
		...
		LANKMOCLBML.text = MEJMLNDFDBP.CJCFPDLDMEK(EJEDANLAMBI, "{0} {1}", string.Empty, false);
	...
	public void HideDialog()
	{
		...
		UIPanel[] componentsInChildren = CHNKLHLJHDP.GetComponentsInChildren<UIPanel>(false);
		foreach (UIPanel uIPanel in componentsInChildren)
		{
	...
	public void JDNEBEKOHPG()
	{
		...
		TweenPosition.Begin(CHNKLHLJHDP, JCAFFELIFAO / 1541f, new Vector3(1771f, 944f, 843f), new Vector3(1104f, 839f, 1914f), false).onFinished = delegate
		{
			if (CNBJMDECEJG)
			{
				TweenPosition.Begin(CHNKLHLJHDP, JCAFFELIFAO / 2f, new Vector3(0f, 0f, 0f)).onFinished = null;
	...
	private void AMBIFCOIHKA()
	{
		...
		LCAFKBDICLL.text = ((!flag) ? string.Format("{0} {1} {2}/ {3}[-]", IFFDIHCPKFE.unitAbilityName, boughtIndex, Colours.stringGray, maxRowForActualTier) : IFFDIHCPKFE.unitAbilityName);
