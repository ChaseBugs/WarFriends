...
public class CBBKFKCOLPP
{
	...
	public float PKIFGJGBBCP { get; private set; }

	public float MMNGHBHJPDL { get; private set; }

	public float PKDAIEJDIFF { get; private set; }

	public bool HMGOHGOLMJB { get; private set; }

	public bool HOHCFBAPGFN { get; private set; }

	public List<Card> EEMPEECADGP
	{
		get
		{
			return GCGJFOJENDL;
	...
	public int HAFMGCFMCPC
	{
		get
		{
			return (int)((float)PGOFCHMMCDL * PKDAIEJDIFF * LBLGIKMIELO);
		}
	}
	...
	public int GGGEBJOEKPE
	{
		get
		{
			return (int)((float)EECGPHJJLHN * MMNGHBHJPDL);
		}
	}

	public int MOLBBCELGCF
	{
		get
		{
			return (int)((float)KOENBBNGCGA * MMNGHBHJPDL);
		}
	}

	private float FEMLFIMJCKB
	{
		get
		{
			return (!HMGOHGOLMJB) ? 1f : Singleton<VipManager>.instance.vipWarbucksMultiplier;
		}
	}

	private float LBLGIKMIELO
	{
		get
		{
			return (!HMGOHGOLMJB) ? 1f : Singleton<VipManager>.instance.vipGoldMultiplier;
		}
	}

	public bool GOFNDLHIFLM
	{
		get
		{
			return CCGELMHFCFG.Count > 0;
		}
	}

	public int GAJIJKKEMGJ
	{
		get
		{
			return (int)((float)BEBCNKHDJDC * PKIFGJGBBCP);
		}
	}

	public int DLHGGEHHCDI
	{
		get
		{
			return (int)((float)BEBCNKHDJDC * PKIFGJGBBCP * Singleton<VipManager>.instance.vipWarbucksMultiplier);
		}
	}

	public int PJCCEDPHNML
	{
		get
		{
			return (int)((float)PGOFCHMMCDL * PKDAIEJDIFF);
		}
	}

	public int MCBAJIAHCJB
	{
		get
		{
	...
	{
		Debug.Log("#Mission Rewards# Mission Game Rewards Came");
		BEBCNKHDJDC = 0;
		EECGPHJJLHN = 0;
		KOENBBNGCGA = 0;
		PGOFCHMMCDL = 0;
	...
	public void NHBDJABCIHC()
	{
		...
		{
			Debug.Log("#Mission Rewards# New missions data are loaded to client");
	...
	private void INMAHALJMEM(Dictionary<string, object> EENKJBCCPBG)
	{
		...
		if (HOHCFBAPGFN)
		{
			...
			Debug.Log("#PLAYERID# Saving Player Account with PlayerId = ");
	...
	private void JNNGFLGOPIJ(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("Wrong_Category"))
		{
			...
			Debug.Log("Camosmanager.prefab" + num);
		}
	}
	...
	public void GIPICGLPFPC()
	{
		...
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC(IHFDGLMHHBE.CardPack, DailyMissionsManager.instance.IFMAHECILCC(), CCGELMHFCFG.ToArray()), true);
		}
		if (CFCLPHCHEPP > 1)
		{
		...
		if (HDGCLIOMMMM > 0)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)(-24), HDGCLIOMMMM), true);
		}
	}
	...
	private void FCDFOPIPHIO(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("DailyMissionsCompletionRewardGold"))
		{
			...
			Debug.LogFormat("#Mission Rewards# 3 Solo Daily Missions completed. Gained {0} gold", EHKKGDJGCAK);
		}
		if (EENKJBCCPBG.ContainsKey("DailyMissionsCompletionRewardTickets"))
		{
			...
			Debug.LogFormat("#Mission Rewards# 3 Solo Daily Missions completed. Gained {0} arena tickets", EHKKGDJGCAK);
		...
		if (EENKJBCCPBG.ContainsKey("DailyMissionsCompletionRewardScraps"))
		{
			...
			Debug.LogFormat("#Mission Rewards# 3 Solo Daily Missions completed. Gained {0} scraps", EHKKGDJGCAK);
		}
		...
		Debug.LogFormat("#Mission Rewards# 3 Solo Daily Missions completed. Gained {0} warcards", EHKKGDJGCAK);
		JToken jToken = null;
	...
	private void HBLBNEMJDBL(Dictionary<string, object> EENKJBCCPBG)
	{
		...
		if (jToken != null)
		{
			Debug.Log("#Mission Rewards# Game Rewards:\n" + jToken);
		}
	...
	private void FBDPONJPBNI(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("506 MENU ARMY SCREEN UPGRADE ACTIVATED"))
		{
			...
			BKPEMBPBFLF = PNFCNJHKBBN.Gold;
			Singleton<Wallet>.instance.COLJGNLBEII(EHKKGDJGCAK);
			Debug.LogFormat("VipRewardForDay", EHKKGDJGCAK);
		}
		if (EENKJBCCPBG.ContainsKey("Shots_Fired"))
		{
			...
			array[0] = EHKKGDJGCAK;
			Debug.LogFormat("country-egypt", array);
		...
		if (EENKJBCCPBG.ContainsKey("Freeze game:"))
		{
			...
			Debug.LogFormat(" Golds for squad event - completed tier ", array2);
		}
		...
		Debug.LogFormat("Show2RPC", EHKKGDJGCAK);
		JToken jToken = null;
		foreach (JToken item2 in jArray)
		{
			jToken = item2;
	...
	public void FOMBLGNKDKL()
	{
		...
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)(-68), EHKKGDJGCAK), true);
		}
		...
		if (BKPEMBPBFLF == PNFCNJHKBBN.EliteParts)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC(IHFDGLMHHBE.Gold, EHKKGDJGCAK), true);
		}
	}
	...
	private void CFIPPGGJKDE(Dictionary<string, object> EENKJBCCPBG)
	{
		...
		{
			Debug.Log("#Mission Rewards# New missions data (midnight or heroic completed)");
	...
	private void OOEBPECLBAJ(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("Player entered normal league: {0} and beginners league: {1}"))
		{
			...
			Singleton<Wallet>.instance.BKFOPCDLPCJ(BCDPLPIDJGM, false);
			object[] array = new object[0];
			array[1] = BCDPLPIDJGM;
			Debug.LogFormat("menu-weaponstats-rateoffire", array);
		}
		if (EENKJBCCPBG.ContainsKey("{0}{1} / {2}"))
		{
			...
			Debug.LogFormat("RewardAmount", array2);
		}
		if (EENKJBCCPBG.ContainsKey("com/google/android/gms/games/Games"))
		{
			...
			Debug.LogFormat("#VOJTA# ON GET NEW DAILY MISSIONS", array3);
		...
		if (EENKJBCCPBG.ContainsKey("TryRecconnect TryRecconnectToPhotonCoroutine going to reconnect"))
		{
			...
			if (IINOIBMKAPI == null)
			{
				...
				array4[0] = text;
				Debug.LogErrorFormat("03", array4);
			}
			else
			{
				...
				array5[0] = text;
				Debug.LogFormat("Network pool: InstantiatePooledObjects '", array5);
			}
		}
		...
		Debug.LogFormat("DOWNLOADBTN", jArray.Count);
		JToken jToken = null;
		foreach (JToken item2 in jArray)
		{
			jToken = item2;
	...
	public void LFEIKGBPFOP()
	{
		if (IINOIBMKAPI != null && HMMPONAIBOC > 0)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)74, IINOIBMKAPI, HMMPONAIBOC), true);
		}
		if (BCDPLPIDJGM > 1)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC(IHFDGLMHHBE.Gold, BCDPLPIDJGM), true);
		}
		...
		if (CFCLPHCHEPP > 1)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)83, CFCLPHCHEPP), true);
		}
		if (HDGCLIOMMMM > 0)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)(-16), HDGCLIOMMMM), true);
		}
	}
	...
	private void KNJBBJFBPKO(Dictionary<string, object> EENKJBCCPBG)
	{
		...
		{
			DailyMissionsManager.instance.data.isHeroicOpened = true;
			Debug.Log("#Mission Rewards# Heroic was opened");
	...
	private void KKBFLNLJBOH(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("SyncShield"))
		{
			...
			Singleton<Wallet>.instance.BKFOPCDLPCJ(BCDPLPIDJGM, false);
			Debug.LogFormat("Card ", BCDPLPIDJGM);
		}
		...
		if (EENKJBCCPBG.ContainsKey("BoughtIndex"))
		{
			...
			Debug.LogFormat("AssignmentsUpdate", array2);
		...
		if (EENKJBCCPBG.ContainsKey("WinCount"))
		{
			...
			if (IINOIBMKAPI == null)
			{
				object[] array3 = new object[0];
				array3[0] = text;
				Debug.LogErrorFormat("DailyMissionsCompletionRewardGold", array3);
			}
			else
			{
				UpgradeSlotElite upgradeSlotElite = IINOIBMKAPI.upgradeSlots.upgradeSlotElite;
		...
		Debug.LogFormat("gold", jArray.Count);
		JToken jToken = null;
		foreach (JToken item2 in jArray)
		{
			jToken = item2;
	...
	private void OOPBOFBLOJD(Dictionary<string, object> EENKJBCCPBG)
	{
		...
		{
			Debug.Log("ID_SKILLSHOTHINT_DOUBLEKILL");
	...
	private void LLHGOFBJOHB(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("HeroicMissionsCompletionRewardGold"))
		{
			...
			Debug.LogFormat("#Mission Rewards# 5 Heroic Missions completed. Gained {0} gold", BCDPLPIDJGM);
		}
		if (EENKJBCCPBG.ContainsKey("HeroicMissionsCompletionRewardTickets"))
		{
			...
			Debug.LogFormat("#Mission Rewards# 5 Heroic Missions completed. Gained {0} arena tickets", CFCLPHCHEPP);
		}
		if (EENKJBCCPBG.ContainsKey("HeroicMissionsCompletionRewardScraps"))
		{
			...
			Debug.LogFormat("#Mission Rewards# 5 Heroic Missions completed. Gained {0} scraps", HDGCLIOMMMM);
		...
		if (EENKJBCCPBG.ContainsKey("HeroicMissionsCompletionRewardArmyUnitId"))
		{
			...
			if (IINOIBMKAPI == null)
			{
				Debug.LogErrorFormat("#Mission Rewards# 5 Heroic Missions completed. Unit {0} does not exists!", text);
			}
			else
			{
				IINOIBMKAPI.upgradeSlots.upgradeSlotElite.currentParts += HMMPONAIBOC;
				Debug.LogFormat("#Mission Rewards# 5 Heroic Missions completed. Gained {0} {1} elite parts", HMMPONAIBOC, text);
			}
		}
		...
		Debug.LogFormat("#Mission Rewards# 5 Heroic Missions completed. Gained {0} warcards", jArray.Count);
		JToken jToken = null;
		foreach (JToken item2 in jArray)
		{
			jToken = item2;
	...
	private void OOCIPGNFBLD(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("ERROR WRONG DAILY REWARDS: "))
		{
			...
			Singleton<Wallet>.instance.BKFOPCDLPCJ(EHKKGDJGCAK, false);
		...
		if (EENKJBCCPBG.ContainsKey("animationEnd"))
		{
			...
			Debug.LogFormat("LevelName", array2);
		...
		if (EENKJBCCPBG.ContainsKey("ID_WARNING_OPPONENTDISCONNECTED"))
		{
			...
			Debug.LogFormat("ID_READYTIME", EHKKGDJGCAK);
		}
	...
	public void HMKIILAPCDD()
	{
		if (!string.IsNullOrEmpty(EBBNLNJLOFL))
		{
			Debug.Log("testingidsquadrankup");
			DailyMissionsManager.instance.LoadFromJsonString(EBBNLNJLOFL);
			EBBNLNJLOFL = null;
		}
	}
	...
	public void NGJLLGJMOKF()
	{
		if (!string.IsNullOrEmpty(EBBNLNJLOFL))
		{
			Debug.Log("null facebook friends");
			DailyMissionsManager.instance.LoadFromJsonString(EBBNLNJLOFL);
			EBBNLNJLOFL = null;
		}
	}
	...
	private void DJGHDJFKCDD(Dictionary<string, object> EENKJBCCPBG)
	{
		if (EENKJBCCPBG.ContainsKey("HeroicPoints"))
		{
			...
			DailyMissionsManager.instance.data.heroicPoints = num;
			Debug.Log("#Mission Rewards# Current Heroic Points: " + num);
		}
	}
	...
	public void OFOPLLLIPNJ()
	{
		...
		if (BKPEMBPBFLF == PNFCNJHKBBN.Gold)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC(IHFDGLMHHBE.Dogtag, EHKKGDJGCAK), true);
		}
	}
	...
	public void GDGIDPPEPIB()
	{
		...
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)(-17), IINOIBMKAPI, HMMPONAIBOC), true);
		}
		if (BCDPLPIDJGM > 1)
		{
		...
		if (HDGCLIOMMMM > 1)
		{
			Singleton<MessageManager>.instance.AddMessage(new AGPANDBMDLC((IHFDGLMHHBE)58, HDGCLIOMMMM), true);
		}
	}
}
