...
public class UISpriteRelativeWidth : MonoBehaviour
{
	...
	private void GLNNBICCGPG()
	{
		if (CJKENEBMHFJ)
		{
			...
			{
				rect = NGUIMath.ConvertToPixels(rect, component.atlas.texture.width, component.atlas.texture.height, true);
			}
	...
	private void IFAGEMEKBOL()
	{
		if (CJKENEBMHFJ)
		{
			...
			if (component.atlas.coordinates != UIAtlas.Coordinates.Pixels)
			{
				rect = NGUIMath.ConvertToPixels(rect, component.atlas.texture.width, component.atlas.texture.height, false);
			}
			object[] array = new object[2];
