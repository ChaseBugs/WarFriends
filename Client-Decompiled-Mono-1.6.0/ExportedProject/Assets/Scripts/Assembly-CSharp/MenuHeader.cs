...
public class MenuHeader : GuiElementSingle<MenuHeader>
{
	...
	public float subscriptionPixels
	{
		get
		{
			return (!SubscriptionManager.instance.isSubscribed) ? 0f : 46f;
		}
	}
	...
	private void EPDKEJEFKBL()
	{
		...
		GDBGLCLOKPA.transform.parent.gameObject.SetActive(false);
		FLJDOPCHLMM.gameObject.SetActive(true);
		FLJDOPCHLMM.isEnabled = false;
		FGOEDANMKED.gameObject.SetActive(true);
	}
