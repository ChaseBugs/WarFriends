...
public class EnemyController : AIObjectG<SoldierBehaviour>, NKMKCCDCHCL, KMNCIJPDFPJ, KMHCMCBGFFC
{
	...
	public bool canBeFreezed
	{
		get
		{
			return IPIKDCMLOID != AJGFGCHCAPA.Vehicle && IPIKDCMLOID != AJGFGCHCAPA.Helicopter && IPIKDCMLOID != AJGFGCHCAPA.MachineGunPoint;
		}
	}
	...
	private bool NBPAEDNBFGE
	{
		get
		{
			return IPIKDCMLOID == AJGFGCHCAPA.ObstacleHiding || IPIKDCMLOID == AJGFGCHCAPA.CornerHiding || IPIKDCMLOID == AJGFGCHCAPA.Rusher || IPIKDCMLOID == AJGFGCHCAPA.Swat || IPIKDCMLOID == AJGFGCHCAPA.MachineGunPoint || IPIKDCMLOID == AJGFGCHCAPA.Minigunner || IPIKDCMLOID == AJGFGCHCAPA.EngineerTurret || IPIKDCMLOID == AJGFGCHCAPA.RusherSpare;
		}
	}
	...
	public SoldierBehaviour soldierBehaviour
	{
		get
		{
			return (SoldierBehaviour)ACLDFHJKBLI;
		}
	}
	...
	public SoldierMeshChanger meshChanger
	{
		get
		{
			return mSoldierSoldierMeshChanger;
		}
	}
	...
	private void EEAIPPGKEDM()
	{
		...
		KJMFJFCDEDH.Show("game-engi-progress-fill", true, HudObjectIndicator.NPPEJJKFCON.EngineerIco, (fraction != PlayerController.OGMBJPKOPCB.fraction) ? Colours.redProgress : Colours.blue);
	...
	private void ThrowGrenadeFromCover(bool DNDCODMGGEE)
	{
		...
		mSoldierAnimationController.ThrowGrenadeFromCover(DNDCODMGGEE, false);
	}

	public void KGJNBKNHNDB(DestroyableObject.CJLNCMMNPBM OHJPPHFEAFI)
	{
		EFFIOHJHFHI.MakeTrigger(false);
	...
	private void ShootFromBazookaFromCover(bool DNDCODMGGEE)
	{
		...
		mSoldierAnimationController.ShootFromBazoka(DNDCODMGGEE, false);
	}
	...
	private void FHEMDCFGMMF(DestroyableObject OJIHGNJFPFM, DestroyableObject.CJLNCMMNPBM OHJPPHFEAFI)
	{
		...
		try
		{
			...
			if (OHJPPHFEAFI.FJLBLLLEELD == DestroyableObject.MANAGOLPMLH.Explosion && !PFKPCFCEDME)
			{
				...
				GetComponentInChildren<Animation>().enabled = false;
				EFFIOHJHFHI.EnableRagdol(false);
	...
	private Vector3 ACAEFIMAAMB()
	{
		...
		NavMeshHit hit;
		if (NavMesh.SamplePosition(zero, out hit, 5f, 1))
		{
			return hit.position;
		}
		return ACAEFIMAAMB();
	...
	private Vector3 LOKJIECGOIB()
	{
		...
		for (int i = 0; i < HJFHCPBADLD.corners.Length - 1; i++)
		{
			...
			float enter;
			if (plane.Raycast(ray, out enter) && enter <= magnitude)
			{
	...
	private void FHNJLIDDPAO()
	{
		...
		{
			JPAEIIEPIPC = 0f;
			KJMFJFCDEDH.Show("game-engi-progress-fill", false, HudObjectIndicator.NPPEJJKFCON.EngineerIco);
			...
			SpawningManager.instance.Spawn(turret, fraction, false, enemyPointEngineerTurret.turretPosition);
	...
	private void PPFONPMPOHD()
	{
		KJMFJFCDEDH.Disable();
		EFFIOHJHFHI.SetAllRigidBodiesToKinematic(true);
		TweenPosition.Begin(base.gameObject, 1f, base.transform.position, base.transform.position - Vector3.up, false);
		CDCOFHGPJHN(1f);
	}
	...
	private void ShootFromCover(bool DNDCODMGGEE, float GINCPOHPFBN)
	{
		...
		if (soldierBehaviour.KHKAAGCIJPO != null)
		{
			mSoldierAnimationController.ShotFromCover(DNDCODMGGEE, 1f, GINCPOHPFBN, false, soldierBehaviour.KHKAAGCIJPO.BEACDIILBEG);
		}
	}
	...
	public void OnShot()
	{
		...
		case Weapon.IGGFMJAKLGO.Colt:
			...
			if (IPIKDCMLOID == AJGFGCHCAPA.CornerHiding)
			{
				mSoldierAnimationController.ShotFromCover(!KAKIEBINCDL, 1f, 0.5f, false, soldierBehaviour.KHKAAGCIJPO.BEACDIILBEG);
			}
			if (IPIKDCMLOID == AJGFGCHCAPA.Walk)
			{
	...
	private void PICEPHBPBNC()
	{
		if (!MLFKPOMJMOP)
		{
			...
			if (PDDOJAMAOHB > HJEJNIJPFHF && !KGDPPCIEJEI(false, true, 16f))
			{
				GenerateNextShootTime();
			}
		}
