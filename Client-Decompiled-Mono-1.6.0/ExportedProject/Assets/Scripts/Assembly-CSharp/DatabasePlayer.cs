...
public class DatabasePlayer : KIGEPEJPEKB
{
	...
	public bool isInBeginnersLeague
	{
		get
		{
			return beginnersLeague > 0 && statisticsData != null && statisticsData.mmmGames > 0;
		}
	}
	...
	public bool isInfluencer
	{
		get
		{
			return !string.IsNullOrEmpty(influencerLink);
		}
	}

	public string influencerString
	{
		get
		{
			return (!string.IsNullOrEmpty(influencerAlias)) ? influencerAlias : influencerLink;
		}
	}
	...
	public bool isInLeague
	{
		get
		{
			return remainingMatches == 0 && leagueDivision != "placement";
		}
	}

	public bool canPlayerSendLogs
	{
		get
		{
			return sendLogsValue == 1;
		}
	}
	...
	public bool isGameCenterConnected
	{
		get
		{
			return !string.IsNullOrEmpty(gameCenterId);
		}
	}
	...
	public bool isGooglePlayConnected
	{
		get
		{
			return !string.IsNullOrEmpty(googlePlayId);
		}
	}
	...
	public string visualsDebugLog
	{
		get
		{
			...
			if (playerVisuals.Count != 4)
			{
				...
				return string.Format("visuals: wrong number {0} -{1}", playerVisuals.Count, arg);
			}
			return string.Format("visuals: {0} {1} {2} {3}", playerVisuals[0].equippedID, playerVisuals[1].equippedID, playerVisuals[2].equippedID, playerVisuals[3].equippedID);
		}
	}

	public string debugBasicInformation
	{
		get
		{
			return string.Format("DatabasePlayer Name: {0}  Id: {1}  Level: {2}  SquadRank: {3}", (name != null) ? name : "null", (id != null) ? id : "null", level, squadRank);
		}
	}

	public string debugSquadInformation
	{
		get
		{
			return string.Format("Squad: {0}  SquadPoints: {1}  Reputations: {2}  SquadRank: {3}", (squadName != null) ? squadName : "null", squadPoints, reputation, squadRank);
		}
	}
	...
	public string debugLeagueInformation
	{
		get
		{
			...
			{
				return string.Format("Beginners League:{0}  LeagueMedals: {1}  GLobalMedals: {2}", beginnersLeague, medalsBalance, skill);
			}
	...
	public string CompareToOtherInstance(DatabasePlayer secondInstance)
	{
		...
		string text = string.Format("Comparing player {0} with id {1}:\n", name, id);
		if (name != secondInstance.name)
		{
			text += string.Format("Different names \"{0}\" and \"{1}\"\n", name, secondInstance.name);
		}
		if (level != secondInstance.level)
		{
			text += string.Format("Different levels \"{0}\" and \"{1}\"\n", level, secondInstance.level);
		}
		...
		{
			text += string.Format("Different medals \"{0}\" and \"{1}\"\n", skill, secondInstance.skill);
		}
		if (armyPower != secondInstance.armyPower)
		{
			text += string.Format("Different army power \"{0}\" and \"{1}\"\n", armyPower, secondInstance.armyPower);
		}
		if (squadPoints != secondInstance.squadPoints)
		{
			text += string.Format("Different squad points \"{0}\" and \"{1}\"\n", squadPoints, secondInstance.squadPoints);
		}
		if (leagueTier != secondInstance.leagueTier)
		{
			text += string.Format("Different league \"{0}\" and \"{1}\"\n", leagueTier, secondInstance.leagueTier);
		}
		if (visualsDebugLog != secondInstance.visualsDebugLog)
		{
			text += string.Format("Different player visuals \"{0}\" and \"{1}\"\n", visualsDebugLog, secondInstance.visualsDebugLog);
		}
		return text;
	}
	...
	public string CompareToOtherInstance(FHIPGDADNFG secondInstance)
	{
		...
		{
			text += string.Format("Different names \"{0}\" and \"{1}\"\n", name, secondInstance.MHPNDNJDPGE);
		}
		if (level != secondInstance.BCCDDJMPBLL)
		{
			text += string.Format("Different levels \"{0}\" and \"{1}\"\n", level, secondInstance.BCCDDJMPBLL);
		}
		...
		{
			text += string.Format("Different medals \"{0}\" and \"{1}\"\n", skill, secondInstance.CJDHKJFKNAH);
		}
		if (armyPower != secondInstance.AMFJHAPGIJN)
		{
			text += string.Format("Different army power \"{0}\" and \"{1}\"\n", armyPower, secondInstance.AMFJHAPGIJN);
		}
		if (visualsDebugLog != secondInstance.EFOPLPDLDEI)
		{
			text += string.Format("Different player visuals \"{0}\" and \"{1}\"\n", visualsDebugLog, secondInstance.EFOPLPDLDEI);
		}
		return text;
	}
