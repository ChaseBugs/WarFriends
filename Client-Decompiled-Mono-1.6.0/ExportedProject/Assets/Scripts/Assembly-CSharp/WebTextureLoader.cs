...
public class WebTextureLoader : Core_BaseScript
{
	private class GIILGEGBNHD
	{
		...
		public string MJMCBGNCJLE { get; private set; }

		public string INFLHPGMEOB { get; private set; }
		...
		{
			string path = string.Format("{0}{1}", MJMCBGNCJLE, INFLHPGMEOB);
			if (File.Exists(path))
			{
				byte[] data = File.ReadAllBytes(path);
	...
	private sealed class NHECBNIHAJO
	{
		...
		{
			return MFHFAIMOLMB == string.Format("Squad Buddy Warcard {0}", IGMKJDOKFNF);
		}
		...
		{
			return MFHFAIMOLMB == string.Format("#Mission Rewards# 5 Heroic Missions completed. Gained {0} arena tickets", IGMKJDOKFNF);
		}
	...
	private string CBLPPLJOGGJ
	{
		get
		{
			return string.Format("{0}/{1}/", Application.persistentDataPath, mFolder);
		}
	}
	...
	{
		return string.Format("Different names \"{0}\" and \"{1}\"\n", Application.persistentDataPath, mFolder);
	}
	...
	public void DODCHBGAIPF(string HKODIHDPOGE, string INFLHPGMEOB)
	{
		...
		string path = string.Format("Error - sheet {0} doesnt have column {1}", DPNGKJOIEDP(), INFLHPGMEOB);
		if (File.Exists(path))
		{
			if (PHFHHDKLCLA != null)
			{
	...
	public bool IsDownloaded(string HKODIHDPOGE, string INFLHPGMEOB)
	{
		...
		string path = string.Format("{0}{1}", CBLPPLJOGGJ, INFLHPGMEOB);
		return File.Exists(path);
	}
