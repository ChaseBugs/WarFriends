...
public class BigArenaButton : Core_BaseScript
{
	...
	private Transform AHHAGCNANDK(int DFHAAIFFLOE)
	{
		...
		{
			ArenaHeroRecord arenaHeroRecord = Singleton<GuiManager>.instance.CHDIOFDMHEF.InstantiateAsChild(KAIMAOHHPEP, KAEFBFKCPHJ.gameObject, string.Format("Hero {0:D2}", DFHAAIFFLOE)) as ArenaHeroRecord;
	...
	private void CCMPHFEHDAF(List<FHIPGDADNFG> JFGDAKMCJAM)
	{
		...
		if (BHJIMFDGCGE.Count > 1)
		{
			...
			LIKAPAMDGKA.AlignToPos(false, UIDraggablePanel.Alingment.BottomRight);
		}
	}
	...
	{
		KAEFBFKCPHJ.MakeEmpty();
		DMLOHHIMOCN.gameObject.SetActive(false);
		GLNLMHGELLB.SetActive(false);
		if (DNDBLHFDJDG == null)
		{
	...
	private float KFBMHNDKAMA(int DFHAAIFFLOE)
	{
		...
		{
			ruleRecordForHeight.PJICLEMEBID(DONAJDGHBDP[DFHAAIFFLOE], true, 50, 45);
	...
	private float DJNGFDODCDE(int DFHAAIFFLOE)
	{
		...
		{
			ruleRecordForHeight.Initialize(DONAJDGHBDP[DFHAAIFFLOE], false, 560, 70);
	...
	{
		KAEFBFKCPHJ.MakeEmpty();
		DMLOHHIMOCN.gameObject.SetActive(true);
		GLNLMHGELLB.SetActive(true);
		if (DNDBLHFDJDG == null)
		{
	...
	private Transform PNDGOBGCMIB(int DFHAAIFFLOE)
	{
		if (DFHAAIFFLOE >= 0 && DFHAAIFFLOE < DONAJDGHBDP.Count)
		{
			ArenaRuleRecord arenaRuleRecord = Singleton<GuiManager>.instance.CHDIOFDMHEF.InstantiateAsChild(KGCMIPLLOEK, GOPNFFACMJH.gameObject, string.Format("Rule {0:D2}", DFHAAIFFLOE)) as ArenaRuleRecord;
			if (arenaRuleRecord != null)
			{
				arenaRuleRecord.Initialize(DONAJDGHBDP[DFHAAIFFLOE], false, 560, 70);
	...
	private void PGDECBFDPGC()
	{
		...
		BJMFENAFNFJ.AlignToPos(true, UIDraggablePanel.Alingment.BottomRight, true);
	}
	...
	private void OADAHBHCGNN()
	{
		...
		BJMFENAFNFJ.AlignToPos(true, UIDraggablePanel.Alingment.TopLeft, true);
	}
	...
	private float MPADLOJPLHK(int DFHAAIFFLOE)
	{
		if (DFHAAIFFLOE >= 1 && DFHAAIFFLOE < DONAJDGHBDP.Count)
		{
			HHMJALHHHLO().Initialize(DONAJDGHBDP[DFHAAIFFLOE], true, -193, 88);
	...
	private Transform BKEGAEBBGPE(int DFHAAIFFLOE)
	{
		if (DFHAAIFFLOE >= 0 && DFHAAIFFLOE < MLCJPKJBAHL.Count)
		{
			WarArenaSmallRewardRecord warArenaSmallRewardRecord = Singleton<GuiManager>.instance.CHDIOFDMHEF.InstantiateAsChild(CCNKBJEPPHJ, KCCOKFDFHGI.gameObject, string.Format("Reward {0:D2}", DFHAAIFFLOE)) as WarArenaSmallRewardRecord;
			if (warArenaSmallRewardRecord != null)
			{
				warArenaSmallRewardRecord.Initialize(MLCJPKJBAHL[DFHAAIFFLOE]);
				return warArenaSmallRewardRecord.transform;
