...
public class Tank : Vehicle<TankBehaviour>
{
	...
	private void JKFLPFCLMIB()
	{
		...
		if (!LCDBACDIODH && KPBAJMBFKLF && LAADDLGBFIG.isMine && IMGHHHHHPKF != null && PCDILCOEIKD != null)
		{
			...
			if (INDBNCLMMNC == ILEKLILGKAK - 1 && !NBPAEDNBFGE && IMGHHHHHPKF.position.PlanarDistance(base.transform.position) < 1314f)
			{
				...
				KLILEECEACC.gameObject.SetActive(false);
				DFNHJNMBPHM.gameObject.SetActive(true);
	...
	public virtual void HBANGBMJNGO(bool OPGMAEDPFML)
	{
		...
		EnemyController enemyController = GBCMGMGAFLL(Singleton<LevelBehaviourManager>.instance.DHKKHDABING[0].behaviour, JHGIAHLDJJL, true);
		enemyController = GBCMGMGAFLL(Singleton<LevelBehaviourManager>.instance.DHKKHDABING[0].behaviour, CKPKEGIKOLK, false);
	}
	...
	private void IDBDPGJPOIN()
	{
		...
		if (!LCDBACDIODH && KPBAJMBFKLF && LAADDLGBFIG.isMine && IMGHHHHHPKF != null && PCDILCOEIKD != null)
		{
			...
			if (INDBNCLMMNC == ILEKLILGKAK - 1 && !NBPAEDNBFGE && IMGHHHHHPKF.position.PlanarDistance(base.transform.position) < 854f)
			{
				NBPAEDNBFGE = false;
				GLBAGFHENDK = base.transform.forward;
				KLILEECEACC.gameObject.SetActive(true);
				DFNHJNMBPHM.gameObject.SetActive(false);
				KLILEECEACC.transform.position = NGCDDPCAGJB.position;
				KLILEECEACC.transform.rotation = NGCDDPCAGJB.rotation;
	...
	private void OFDAFFJLHCH(EnemyPointVehicle PLMMBKGLGAB, bool KANKDLBGMLK)
	{
		...
		if (enemyController != null)
		{
			...
			SpawningManager.BMNEPICFHNO().LMJOPLCEBAA(enemyController, fraction, true, PLMMBKGLGAB.FBGFMDOHAEG());
	...
	public void EOFGLKGHHKG(Vector3 PLEBANKIBPP)
	{
		if (LAADDLGBFIG.isMine)
		{
			...
			{
				TweenPosition tweenPosition = TweenPosition.Begin(base.gameObject, 607f, KLILEECEACC.transform.position + 1652f * Vector3.up - GLBAGFHENDK * 420f, false);
				tweenPosition.NumOfRepetitions = 7;
				tweenPosition.style = UITweener.Style.PingPong;
			}
		}
	...
	public virtual void GEINAEKMOOP()
	{
		...
		if (!LCDBACDIODH)
		{
			...
			if (LAADDLGBFIG.isMine)
			{
				IDAOCGPNDBI(CKPKEGIKOLK, false);
				KFOJKAMLMED(JHGIAHLDJJL, false);
			}
			PJJJKFOANDO.enabled = true;
			PJJJKFOANDO.BBBLJNFEACC();
			KAEAMDOJMIF.enabled = true;
	...
	private void JFADLFOOGBD(EnemyPointVehicle PLMMBKGLGAB, bool KANKDLBGMLK)
	{
		...
		if (enemyController != null)
		{
			...
			int actualLevelForIndex = ACLDFHJKBLI.upgradeSlots.GetActualLevelForIndex(PFGLIAHPMCK.slotUpgradeindex);
			enemyController.SpawnByCard((float)actualLevelForIndex / (float)ACLDFHJKBLI.upgradeSlots.maxLevelOfUnit, ACLDFHJKBLI.cardId);
			SpawningManager.instance.JLGCKNOKGBL(enemyController, fraction, false, PLMMBKGLGAB.PFOBADFGAKF());
			PLMMBKGLGAB.IDNNIHBIMNO = enemyController;
			enemyController.NLGBGOGCNKP = PLMMBKGLGAB;
	...
	public virtual void HNIGDPCNFGH(bool OPGMAEDPFML)
	{
		...
		EnemyController enemyController = GBCMGMGAFLL(Singleton<LevelBehaviourManager>.instance.DHKKHDABING[0].behaviour, JHGIAHLDJJL, false);
		enemyController = GBCMGMGAFLL(Singleton<LevelBehaviourManager>.instance.DHKKHDABING[0].behaviour, CKPKEGIKOLK, true);
	}
	...
	public virtual void NJJCFALPMFH()
	{
		...
		if (!LCDBACDIODH)
		{
			...
			if (LAADDLGBFIG.isMine)
			{
				EOPABEGJABM(CKPKEGIKOLK, true);
				EOPABEGJABM(JHGIAHLDJJL, false);
			}
			PJJJKFOANDO.enabled = true;
	...
	private void EOPABEGJABM(EnemyPointVehicle PLMMBKGLGAB, bool KANKDLBGMLK)
	{
		...
		if (enemyController != null)
		{
			...
			int actualLevelForIndex = ACLDFHJKBLI.upgradeSlots.GetActualLevelForIndex(PFGLIAHPMCK.slotUpgradeindex);
			enemyController.SpawnByCard((float)actualLevelForIndex / (float)ACLDFHJKBLI.upgradeSlots.maxLevelOfUnit, ACLDFHJKBLI.cardId);
			SpawningManager.LDGAKJJINEO().DEFBPHBPFCF(enemyController, fraction, false, PLMMBKGLGAB.PFOBADFGAKF(), false);
			PLMMBKGLGAB.IDNNIHBIMNO = enemyController;
			enemyController.NLGBGOGCNKP = PLMMBKGLGAB;
	...
	public override void OnInstancied()
	{
		...
		if (!LCDBACDIODH)
		{
			...
			if (LAADDLGBFIG.isMine)
			{
				IDAOCGPNDBI(CKPKEGIKOLK, true);
				IDAOCGPNDBI(JHGIAHLDJJL, true);
			}
			PJJJKFOANDO.enabled = true;
			PJJJKFOANDO.Reset();
			KAEAMDOJMIF.enabled = true;
	...
	public virtual void MNKNLEFILJI()
	{
		...
		if (!LCDBACDIODH)
		{
			...
			if (LAADDLGBFIG.isMine)
			{
				EIHNPKDOJIM(CKPKEGIKOLK, true);
				EOPABEGJABM(JHGIAHLDJJL, true);
			}
			PJJJKFOANDO.enabled = false;
			PJJJKFOANDO.BBBLJNFEACC();
			KAEAMDOJMIF.enabled = true;
	...
	public virtual void HCEGDJNMBAA(bool OPGMAEDPFML)
	{
		...
		EnemyController enemyController = GBCMGMGAFLL(Singleton<LevelBehaviourManager>.instance.DHKKHDABING[1].behaviour, JHGIAHLDJJL, false);
		enemyController = GBCMGMGAFLL(Singleton<LevelBehaviourManager>.instance.DHKKHDABING[1].behaviour, CKPKEGIKOLK, false);
	}
	...
	private void JOGIDAKMMPD(Vector3 MPHCNMDIPAI)
	{
		...
		Vector3 vector = PJJJKFOANDO.transform.InverseTransformDirection((MPHCNMDIPAI - position).normalized);
		TweenPosition tweenPosition = TweenPosition.Begin(PJJJKFOANDO.gameObject, 1919f, GMENJOACKEO, GMENJOACKEO - vector * 1413f, false);
		tweenPosition.NumOfRepetitions = 2;
	...
	private void EIHNPKDOJIM(EnemyPointVehicle PLMMBKGLGAB, bool KANKDLBGMLK)
	{
		...
		if (enemyController != null)
		{
			...
			int actualLevelForIndex = ACLDFHJKBLI.upgradeSlots.GetActualLevelForIndex(PFGLIAHPMCK.slotUpgradeindex);
			enemyController.SpawnByCard((float)actualLevelForIndex / (float)ACLDFHJKBLI.upgradeSlots.maxLevelOfUnit, ACLDFHJKBLI.cardId);
			SpawningManager.OKBAIGKCPKA().DEFBPHBPFCF(enemyController, fraction, true, PLMMBKGLGAB.FBGFMDOHAEG(), false);
			PLMMBKGLGAB.IDNNIHBIMNO = enemyController;
			enemyController.NLGBGOGCNKP = PLMMBKGLGAB;
	...
	private void IDAOCGPNDBI(EnemyPointVehicle PLMMBKGLGAB, bool KANKDLBGMLK)
	{
		...
		if (enemyController != null)
		{
			...
			int actualLevelForIndex = ACLDFHJKBLI.upgradeSlots.GetActualLevelForIndex(PFGLIAHPMCK.slotUpgradeindex);
			enemyController.SpawnByCard((float)actualLevelForIndex / (float)ACLDFHJKBLI.upgradeSlots.maxLevelOfUnit, ACLDFHJKBLI.cardId);
			SpawningManager.instance.Spawn(enemyController, fraction, false, PLMMBKGLGAB.position, false);
			PLMMBKGLGAB.IDNNIHBIMNO = enemyController;
			enemyController.NLGBGOGCNKP = PLMMBKGLGAB;
	...
	private void OIKAILEPPIF(Vector3 MPHCNMDIPAI)
	{
		...
		Vector3 vector = PJJJKFOANDO.transform.InverseTransformDirection((MPHCNMDIPAI - position).normalized);
		TweenPosition tweenPosition = TweenPosition.Begin(PJJJKFOANDO.gameObject, 73f, GMENJOACKEO, GMENJOACKEO - vector * 1076f, false);
	...
	public void PlayShootFeedBack(Vector3 PLEBANKIBPP)
	{
		if (LAADDLGBFIG.isMine)
		{
			...
			{
				TweenPosition tweenPosition = TweenPosition.Begin(base.gameObject, 0.1f, KLILEECEACC.transform.position + 0.01f * Vector3.up - GLBAGFHENDK * 0.01f, false);
				tweenPosition.NumOfRepetitions = 2;
				tweenPosition.style = UITweener.Style.PingPong;
			}
		}
	...
	private void KFOJKAMLMED(EnemyPointVehicle PLMMBKGLGAB, bool KANKDLBGMLK)
	{
		...
		if (enemyController != null)
		{
			...
			int actualLevelForIndex = ACLDFHJKBLI.upgradeSlots.GetActualLevelForIndex(PFGLIAHPMCK.slotUpgradeindex);
			enemyController.SpawnByCard((float)actualLevelForIndex / (float)ACLDFHJKBLI.upgradeSlots.maxLevelOfUnit, ACLDFHJKBLI.cardId);
			SpawningManager.BMNEPICFHNO().LMJOPLCEBAA(enemyController, fraction, false, PLMMBKGLGAB.KOFFAHEAHEN());
			PLMMBKGLGAB.IDNNIHBIMNO = enemyController;
			enemyController.NLGBGOGCNKP = PLMMBKGLGAB;
			enemyController.LDAKCDAPOGH(EnemyController.AJGFGCHCAPA.CornerHiding);
			enemyController.SetMaxHealthAndRefill(ACLDFHJKBLI.upgradeSlots.GetSoldierHpInMechanic(PFGLIAHPMCK.slotUpgradeindex) * PFGLIAHPMCK.scaleHp);
