...
public class FightDialog : GuiElementSingle<FightDialog>, PAIIOKBBHBC
{
	...
	private sealed class ODGOMJNMKCJ : IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool JPBGJLDMOPO()
		{
			...
			case 1u:
				...
				BJGCPDNMHDH.mFightButtonOverlay.SetActive(false);
				goto IL_00c7;
			case 0u:
		...
		public bool MoveNext()
		{
			...
			case 1u:
				...
				BJGCPDNMHDH.mFightButtonOverlay.SetActive(true);
				goto IL_00c7;
			case 0u:
				if (BJGCPDNMHDH.isShowed)
				{
	...
	private void PNHPNABNMON(DatabaseAction IFGAGNBDKBE)
	{
		...
		mFightButtonWaiting.SetActive(false);
		AANECPGDMGM aANECPGDMGM = Singleton<ServerResultsCache>.instance.MKKMNBIBCFM(mSquadNameLabel.text, true);
		LDOKCEMGLGC = true;
		...
		{
			HLHBMMCBHJF hLHBMMCBHJF = NGDKKFFCGCI as HLHBMMCBHJF;
			if (hLHBMMCBHJF != null)
			{
	...
	public override void InitGUIValues()
	{
		...
		if (MBCCLBJKAOJ == NKHJBLBAAEB.Battle)
		{
			...
			if (num > num2)
			{
				UnityEngine.Debug.Log(string.Format("Received old game invite ( > {0} seconds). Not showing dialog. Opponent = {1}", num2, EPIPGLJBAMM.NCFFGKHPJLH.name));
		...
		if (MBCCLBJKAOJ == NKHJBLBAAEB.COOP)
		{
			...
			if (num3 > num4)
			{
				UnityEngine.Debug.Log(string.Format("Received old game invite ( > {0} seconds). Not showing dialog. Opponent = {1}", num4, EPIPGLJBAMM.NCFFGKHPJLH.name));
				NGDKKFFCGCI.OJFJOJONNJK();
				return;
			}
		...
		if (MBCCLBJKAOJ == NKHJBLBAAEB.SquadInvite)
		{
			...
			if (aANECPGDMGM != null)
			{
				LDOKCEMGLGC = true;
				mFightButtonWaiting.SetActive(false);
				mFightButtonOverlay.SetActive(false);
			}
			else
			{
	...
	public void ShowDialog(HHFHFANGCEJ PNAKMCNGGHE)
	{
		...
		ICFCHEHNHEN iCFCHEHNHEN = PNAKMCNGGHE as ICFCHEHNHEN;
		if (iCFCHEHNHEN != null)
		{
		...
		HLHBMMCBHJF hLHBMMCBHJF = PNAKMCNGGHE as HLHBMMCBHJF;
		if (hLHBMMCBHJF != null)
		{
			FEFIGBKPAML = hLHBMMCBHJF;
	...
	public virtual void NGHAPNEFBED()
	{
		...
		if (MBCCLBJKAOJ == NKHJBLBAAEB.COOP)
		{
			...
			mPooledEnemiesGrid.MakeEmpty();
			mPooledEnemiesGrid.init(ELAHDCBBIJI.Count, OGFALDKGPJK, NDFKCJMDNAE, mDraggableEnemies);
			mDraggableEnemies.AlignToPos(false);
		}
		if (MBCCLBJKAOJ == NKHJBLBAAEB.COOP)
		{
			LAKGIKENAKP = 319f;
			...
			{
				LDOKCEMGLGC = false;
				mFightButtonWaiting.SetActive(true);
				mFightButtonOverlay.SetActive(false);
			}
			else
			{
