...
public class CardManagerDeathmatchOffline : Singleton<CardManagerDeathmatchOffline>, GDCLLGDCODD
{
	...
	public bool noViableCardCooldown
	{
		get
		{
	...
	public bool startGameCooldown
	{
		get
		{
			return TimeManager.realTimeWithoutPauses < JEBCDMLHIHC;
		}
	}
	...
	public void InitCards()
	{
		...
		Card[] componentsInChildren = gameObject.GetComponentsInChildren<Card>(true);
	...
	public void ACGOIAPAHNA()
	{
		...
		Card[] componentsInChildren = gameObject.GetComponentsInChildren<Card>(false);
		Card[] array = componentsInChildren;
	...
	public Card GetCardInstance(string ICDKHLOBKIE)
	{
		...
		Debug.LogError(string.Format("Card ID \"{0}\" is not pressent in card dictionary.", ICDKHLOBKIE));
		return null;
	}
	...
	public bool UseMissionEventCard(string ICDKHLOBKIE)
	{
		...
		if (card == null)
		{
			Debug.LogError(string.Format("Mission Event card not found! ({0})", ICDKHLOBKIE));
			return true;
		}
		PlayerController enemyOf = PlayerController.GetEnemyOf(PlayerController.OGMBJPKOPCB.fraction);
		ILEBMLBPINN(enemyOf, card);
	...
	public void Load()
	{
		foreach (KeyValuePair<string, Card> item in GNIOLPEKCIL)
		{
			...
			if (row == null)
			{
				Debug.LogError(string.Format("There is no definition for card {0}", item.Value.id));
			}
			item.Value.Init(row);
		}
	}
	...
	public void RemoveCard(Card CEKHIJECPNP)
	{
		if (!LHFEAHHMHPG.Remove(CEKHIJECPNP))
		{
			Debug.LogError(string.Format("Remove card (ID = {0}) failed!", CEKHIJECPNP.id));
		}
	}
