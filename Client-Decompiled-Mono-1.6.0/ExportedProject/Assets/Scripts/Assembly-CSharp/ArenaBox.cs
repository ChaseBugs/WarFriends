...
public class ArenaBox : Core_BaseScript
{
	...
	{
		mWinCount.gameObject.SetActive(false);
		mDetails.gameObject.SetActive(false);
		mLootboxLabel.gameObject.SetActive(false);
		MPBKFPEAGBG.ShowLootBox(FJLBLLLEELD);
		MPBKFPEAGBG.ShowAnimation();
		mOpenBoxCollider.gameObject.SetActive(true);
	...
	{
		BGHKLHIIFEO = false;
		mHintPanel.gameObject.SetActive(false);
	...
	{
		SoundsManager.Instance.PlayButtonClickedSound();
		mOpenBoxCollider.gameObject.SetActive(false);
