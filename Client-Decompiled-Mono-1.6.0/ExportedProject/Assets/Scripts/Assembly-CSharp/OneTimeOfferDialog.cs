...
public class OneTimeOfferDialog : GuiElementSingle<OneTimeOfferDialog>, PAIIOKBBHBC
{
	...
	private void HLILIDEAFNC(LevelBehaviour IOIKKIIFOCB, int DBOLKNNJGMH, bool LFPLGLHPFEM)
	{
		...
		NGMDHEGFIKE[1].InitializeStat(IOIKKIIFOCB, false);
	...
	private void LFMIJOFOMBB(LevelBehaviour IOIKKIIFOCB, int DBOLKNNJGMH, bool LFPLGLHPFEM)
	{
		...
		JHONGBLEHLJ.text = IOIKKIIFOCB.unitName;
		NGMDHEGFIKE[1].NCLDABJGBLH(IOIKKIIFOCB, false);
		NGMDHEGFIKE[0].LPMJFHACLIP(IOIKKIIFOCB, false);
	...
	private void EFLICKDALHA(LevelBehaviour IOIKKIIFOCB, int DBOLKNNJGMH, bool LFPLGLHPFEM)
	{
		...
		JHONGBLEHLJ.text = IOIKKIIFOCB.unitName;
		NGMDHEGFIKE[0].InitializeStat(IOIKKIIFOCB, false);
		NGMDHEGFIKE[1].FMKICOJKNDJ(IOIKKIIFOCB, false);
	...
	private void EBNGJJNCLEH()
	{
		...
		CDJJCFKGGNM.gameObject.SetActive(false);
		...
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 1152f, new Vector3(1344f, 775f, 1089f), new Vector3(1074f, 270f, 507f), false);
	...
	{
		IPJGCHLDPNN.SetActive(true);
		DNCJEBDPBNA.SetActive(true);
		DPHDJEAHODH.SetActive(false);
		CBOBGFENGGF.SetActive(false);
		JBFNFECHDCB.SetActive(true);
		MDAJJIAMDGH = null;
		IFFDIHCPKFE = null;
		FGIMNJOOAOB = null;
		GBKLFKHMFCP = true;
	...
	private void HKFMHKCOGJE(GameObject KHAHPAKDIKE)
	{
		...
		else if (IFFDIHCPKFE != null)
		{
			...
			if (flag)
			{
				...
				GuiScreenSingle<ArmyScreen>.instance.PGBNFKHKNGE(IFFDIHCPKFE, OKGNPCBCIDN, true);
			}
		}
		if (flag)
		{
	...
	private void KNLGMIPMFJA()
	{
		...
		CDJJCFKGGNM.gameObject.SetActive(true);
	}
	...
	private void OHAECFDEPHF(GameObject KHAHPAKDIKE)
	{
		...
		else if (IFFDIHCPKFE != null)
		{
			...
			{
				Singleton<Wallet>.instance.BKLFEOMJMFC(num2);
				GuiScreenSingle<ArmyScreen>.instance.ILAIPFIKGKE(IFFDIHCPKFE, OKGNPCBCIDN, true);
			}
		}
		if (flag)
		{
	...
	private void PNDGCJBPLNE(GameObject KHAHPAKDIKE)
	{
		...
		else if (IFFDIHCPKFE != null)
		{
			...
			{
				Singleton<Wallet>.instance.BKLFEOMJMFC(num2);
				GuiScreenSingle<ArmyScreen>.instance.IDDINDLFCMJ(IFFDIHCPKFE, OKGNPCBCIDN, true);
			}
		}
		if (flag)
		{
	...
	private void KIAFINJNLBO()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 160f, new Vector3(1583f, 348f, 714f), new Vector3(1655f, 515f, 1222f), false);
	...
	private void OABCPHJCHGB()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 1650f, new Vector3(1662f, 1618f, 1517f), false);
		tweenPosition.method = UITweener.Method.Linear;
	...
	private void PEMEANCDHOO()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 1278f, new Vector3(181f, 347f, 1381f), new Vector3(191f, 1376f, 498f), false);
		tweenPosition.method = UITweener.Method.Linear;
		tweenPosition.onFinished = MIINHOGPFKG;
	}
	...
	private void FBIMBHOCMEP(GameObject KHAHPAKDIKE)
	{
		...
		else if (IFFDIHCPKFE != null)
		{
			...
			{
				Singleton<Wallet>.instance.CDIDDJGINID(num2);
				GuiScreenSingle<ArmyScreen>.instance.EFGAIHKNKGG(IFFDIHCPKFE, OKGNPCBCIDN, true);
			}
		}
		if (flag)
		{
	...
	private void NODNEDBONHG(LevelBehaviour IOIKKIIFOCB, int DBOLKNNJGMH, bool LFPLGLHPFEM)
	{
		...
		NGMDHEGFIKE[0].HHMJOKAFENM(IOIKKIIFOCB, false);
	...
	private void EHLPFIKFJJP(WeaponLevelsSetup JMMJHCOKCGG, int DBOLKNNJGMH, bool LFPLGLHPFEM)
	{
		...
		int num = Mathf.Max(0, weaponPowerX - weaponPowerX2);
		Debug.Log(string.Format("Weapon Power This: {0},   Equipped {1}: {2},   Difference: {3}", weaponPowerX, slotForCategory.weaponLevelsSetup.weaponName, weaponPowerX2, num));
		BKFGHNPPHEK.text = MEJMLNDFDBP.GMIPFLIEOHD(weaponPowerX);
		BFIKGAJAFPD.SetActive(num > 0);
		if (num > 0)
		{
			MCPCIHEBDDB.text = string.Format("+{0}", MEJMLNDFDBP.GMIPFLIEOHD(num));
	...
	{
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 1156f, new Vector3(600f, 978f, 180f), false);
