...
public class ArmyLeftBuffDialog : Core_BaseScript
{
	...
	public void HLGALFANPBC()
	{
		...
		OJKOPKONMAH();
		KIOOEACJNOD();
		UIPanel[] componentsInChildren = CEOENOGINLL.GetComponentsInChildren<UIPanel>(true);
	...
	public void OPPAFPHDKNL()
	{
		...
		TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 1197f, new Vector3(266f, 399f, 291f), new Vector3(66f, 1724f, 997f), false).onFinished = delegate
		{
			if (CNBJMDECEJG)
			{
				TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 2f, new Vector3(0f, 0f, 0f)).onFinished = null;
		...
		OJKOPKONMAH();
		ADCAPGFOEPJ(false);
		UIPanel[] componentsInChildren = CEOENOGINLL.GetComponentsInChildren<UIPanel>(false);
		foreach (UIPanel uIPanel in componentsInChildren)
		{
	...
	public void DOKNIFNNCDH()
	{
		...
		TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 243f, new Vector3(1489f, 459f, 1454f), new Vector3(1674f, 250f, 1010f), false).onFinished = delegate
		{
			if (CNBJMDECEJG)
			{
				TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 2f, new Vector3(0f, 0f, 0f)).onFinished = null;
	...
	public void PBAGIJOFIMD()
	{
		...
		TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 887f, new Vector3(201f, 1263f, 1906f), new Vector3(663f, 667f, 288f), false).onFinished = delegate
		{
			if (CNBJMDECEJG)
			{
				TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 2f, new Vector3(0f, 0f, 0f)).onFinished = null;
	...
	private void FKNPOALIFEH(GameObject KHAHPAKDIKE)
	{
		if (!EHAMCACFNHK.GJLADCENHAL())
		{
			...
			RequestBuffer requestBuffer = RequestBufferManager.instance.GetRequestBuffer();
			requestBuffer.AddRequest(DatabaseAction.GetPlayersFromHitList, IFFDIHCPKFE.upgradeSlots.GetSheetName(), 0, 0, string.Empty, true);
	...
	private void EKHICMLFNJP()
	{
		...
		NFHCAIGDBKJ.text = ((!isUnlocked) ? Localization.Localize("ID_NOTAVAILABLEATTHEMOMENT") : string.Format("{0} {1} {2}/ {3}[-]", IFFDIHCPKFE.unitBuffName, num, Colours.stringGray, num2));
		if (isUnlocked)
		{
			...
			FPODNLJCDFN.text = string.Format("{0} {1}/[-] {2}", scraps, Colours.stringWhite, missingScraps);
			CKJKHGAAJOP.repositionNow = true;
			OIEEABDKINI[0].color = ((!IJAFILEMINJ) ? Colours.grayButton : Color.white);
			OIEEABDKINI[1].color = ((!IJAFILEMINJ) ? Colours.grayButton : Color.white);
			CHOKNMFJOJC.enabled = IJAFILEMINJ;
	...
	private void JDPOCLPNCCA(GameObject KHAHPAKDIKE)
	{
		if (!EHAMCACFNHK.HCDEDBJPHIL())
		{
			...
			int currentParts = IFFDIHCPKFE.upgradeSlots.upgradeSlotElite.currentParts;
			RequestBuffer requestBuffer = RequestBufferManager.instance.GetRequestBuffer();
			requestBuffer.AddRequest((DatabaseAction)82, IFFDIHCPKFE.upgradeSlots.GetSheetName(), 0, 1, string.Empty, true);
	...
	public void AHLCOFPIHPF()
	{
		if (CNBJMDECEJG)
		{
			...
			TweenPosition.Begin(CEOENOGINLL, JCAFFELIFAO / 697f, new Vector3(1496f, 315f, 1290f), false).onFinished = HONBONFEJCA;
			UIPanel[] componentsInChildren = CEOENOGINLL.GetComponentsInChildren<UIPanel>(true);
	...
	private void ADCAPGFOEPJ(bool EOGFKBJCKHB = true)
	{
		if (CEOENOGINLL.activeSelf)
		{
			...
			KBOKNFLJCCM(IDFCJJLDIJC, EOGFKBJCKHB, false);
