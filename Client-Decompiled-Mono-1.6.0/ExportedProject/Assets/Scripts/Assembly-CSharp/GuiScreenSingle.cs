	...
	public static T instance
	{
		get
		{
			...
			lock (LNLEHLKLCKG)
			{
				if (MKELPFGCKKG == null)
				{
					T[] componentsInChildren = Singleton<GuiManager>.instance.IJMDOGNBGIL.GetComponentsInChildren<T>(true);
					if (componentsInChildren.Length <= 0)
					{
						Debug.LogError(string.Format("GuiScreenSingle<{0}>.instance not found!", typeof(T)));
						return (T)null;
					}
