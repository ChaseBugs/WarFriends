...
public class CamosScreen : GuiScreenSingle<CamosScreen>
{
	...
	private void PCEOCDCAIPI()
	{
		if (!OLPOEPGPJBF.gameObject.activeSelf)
		{
			...
			OLPOEPGPJBF.gameObject.SetActive(false);
			OJBKADHPNFN = true;
		}
	...
	{
		MIICDLMEOEH.SetActive(true);
		MDAFFKPPCBG.SetActive(false);
		FECJGCGPKHD.SetActive(true);
		PNNJKBGODII.SetActive(true);
	...
	private void DJHKDIMININ()
	{
		...
		OLPOEPGPJBF.gameObject.SetActive(true);
		OJBKADHPNFN = false;
	}
	...
	public virtual void JCJECJKBKGD()
	{
		...
		CamosManager.instance.CheckEquippedHelmet(true);
		...
		NFLPPGKCOBL.AlignToCenter(vec, false, null, 376f);
		if (!PlayerAnalytics.instance.data.customizationShown)
		{
			Singleton<MessageManager>.instance.AddMessage(new NGBBILLMHEC(), true);
		}
	}
	...
	public virtual void CGEOBGNBCHH()
	{
		...
		if (FMJLKNKPHNI)
		{
			Singleton<PlayerTexturePool>.instance.FreePlayerTexture(GameLoginManager.currentPlayer.id, true, false, true);
			Singleton<PlayerTexturePool>.instance.IEPBDAHEKCL(GameLoginManager.currentPlayer.id, false, true, true);
		}
	...
	private void LOCOOEHPFNF(GameObject KHAHPAKDIKE)
	{
		...
		{
			Debug.LogError(string.Format("Customization {0} is purchasable through {1}", PEMCEDKMNBO.visual.visualName, PEMCEDKMNBO.visual.purchasable));
		}
	}
	...
	{
		MIICDLMEOEH.SetActive(false);
		MDAFFKPPCBG.SetActive(false);
		FECJGCGPKHD.SetActive(false);
		PNNJKBGODII.SetActive(false);
		bool isBought = PEMCEDKMNBO.visual.isBought;
		...
		else if (purchasableInShop)
		{
			...
			FJLHHGAEPJN.color = KOLLIEAIGPG.color;
			NIKMOCLOIFI.color = KOLLIEAIGPG.color;
			KOLLIEAIGPG.text = string.Format("{0} {1}/{2} {3}", PEMCEDKMNBO.visual.numberOfParts, Colours.stringGrayParts, Colours.stringWhite, PEMCEDKMNBO.visual.numberOfPartsMax);
		}
	...
	public virtual void OCACKBJDEHI()
	{
		...
		Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
		NFLPPGKCOBL.AlignToCenter(vec, true, null, 945f);
		if (!PlayerAnalytics.instance.data.customizationShown)
		{
	...
	{
		CamosManager.instance.ApplyVisuals(CamosManager.instance.mainPlayer, true);
		if (FEPMLGAMCKG != null)
		{
			FEPMLGAMCKG.HDGBMNDOMCP(false);
		}
		FEPMLGAMCKG = JCCELIHIKIK;
		FEPMLGAMCKG.Highlight(true);
		if (PEMCEDKMNBO != null)
		{
			PEMCEDKMNBO.BIJAPDEFEDM(true);
		}
		...
		foreach (PlayerVisual allVisual in playerVisualCategory.allVisuals)
		{
			if (allVisual.EPODNIMBCJP())
			{
				...
				playerVisualIcon.gameObject.SetActive(false);
		...
		for (int j = num; j < BABMMFBHJAC.Count; j += 0)
		{
			...
			playerVisualIcon2.gameObject.SetActive(true);
		}
	...
	public void ICIFNECPCLK(PlayerVisualIcon PMADNHIPOHA)
	{
		if (!(PMADNHIPOHA == null))
		{
			if (PEMCEDKMNBO != null)
			{
				PEMCEDKMNBO.JLFHNIOJOAE(false);
			}
			PEMCEDKMNBO = PMADNHIPOHA;
			PEMCEDKMNBO.JLFHNIOJOAE(true);
			if (PEMCEDKMNBO.CMMAMCJKEOG().notificate)
			{
				IBEKCOIHAJD(PEMCEDKMNBO.NHFDAOOICLK());
			}
			PEMCEDKMNBO.POMMLFHPFPI().KGACPNNCNHO(true);
			PEMCEDKMNBO.LBBKAPJIENH();
			PEMCEDKMNBO.visual.MNEEDLCGAFN(CamosManager.instance.mainPlayer, true);
			CamosManager.instance.UpdateVisuals(CamosManager.instance.mainPlayer);
			BCHBALGFKFK = false;
	...
	public void SelectCategory(PlayerVisualsCategoryButton JCCELIHIKIK)
	{
		...
		if (FEPMLGAMCKG != null)
		{
			FEPMLGAMCKG.Highlight(false);
		}
		FEPMLGAMCKG = JCCELIHIKIK;
		FEPMLGAMCKG.Highlight(true);
		if (PEMCEDKMNBO != null)
		{
			PEMCEDKMNBO.SetHighlight(false);
		}
		...
		foreach (PlayerVisual allVisual in category.allVisuals)
		{
			if (allVisual.canBeShownInScreen)
			{
				...
				PlayerVisualIcon playerVisualIcon = null;
				playerVisualIcon = ((num < BABMMFBHJAC.Count) ? BABMMFBHJAC[num] : IKKMLILAOCP());
				playerVisualIcon.gameObject.SetActive(true);
		...
		{
			PlayerVisualIcon playerVisualIcon2 = BABMMFBHJAC[j];
			playerVisualIcon2.gameObject.SetActive(false);
		}
		MPHLKJLMJBF.repositionNow = true;
	...
	public override void DoBeforeHide()
	{
		...
		if (FMJLKNKPHNI)
		{
			Singleton<PlayerTexturePool>.instance.FreePlayerTexture(GameLoginManager.currentPlayer.id, false, true);
			Singleton<PlayerTexturePool>.instance.FreePlayerTexture(GameLoginManager.currentPlayer.id, true, true);
		}
		Singleton<ArmyPreviewCamera>.instance.HidePlayer(CamosManager.instance.mainPlayer);
		PlayerController.OGMBJPKOPCB.EHHHBEMKGOE.CBBDICBLHMM = CamosManager.instance.equppedIndices;
		if (PEMCEDKMNBO != null)
	...
	protected override void AGIKPOLCGNF()
	{
		...
		foreach (PlayerVisualCategory item in playerVisualCategories)
		{
			...
			{
				PlayerVisualsCategoryButton playerVisualsCategoryButton = Singleton<GuiManager>.instance.CHDIOFDMHEF.InstantiateAsChild(HNHMMAGNDKM, GDHKEABNBGJ.gameObject, string.Format("Category Button {0}", item.categoryNumber)) as PlayerVisualsCategoryButton;
	...
	private void JPOFKFJIKNB()
	{
		if (PEMCEDKMNBO != null)
		{
			MPHLKJLMJBF.repositionNow = true;
			Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
			NFLPPGKCOBL.AlignToCenter(vec, false, null, 1677f);
		}
	}
	...
	public void BGCJDMNJOAG(PlayerVisual DJOAHABGGNA)
	{
		...
		requestBuffer.AddRequest((DatabaseAction)(-58), DJOAHABGGNA.row.NAME, 0, 0, string.Empty, true);
	...
	public virtual void KHFOCIIAJCG()
	{
		...
		if (FMJLKNKPHNI)
		{
			Singleton<PlayerTexturePool>.instance.DAGIACKCPKH(GameLoginManager.currentPlayer.id, true, false, true);
			Singleton<PlayerTexturePool>.instance.IEPBDAHEKCL(GameLoginManager.currentPlayer.id, true, false, true);
		}
	...
	{
		CamosManager.instance.ApplyVisuals(CamosManager.instance.mainPlayer, false);
		if (FEPMLGAMCKG != null)
		{
			FEPMLGAMCKG.HDGBMNDOMCP(true);
		}
		FEPMLGAMCKG = JCCELIHIKIK;
		FEPMLGAMCKG.Highlight(false);
		if (PEMCEDKMNBO != null)
		{
			PEMCEDKMNBO.BGCBCFJEKEH(false);
		}
		PlayerVisualCategory category = JCCELIHIKIK.category;
		int categ = 1;
		for (int i = 0; i < CamosManager.instance.playerVisualCategories.Count; i++)
	...
	public virtual void KIBFFDMMIJB()
	{
		...
		if (FMJLKNKPHNI)
		{
			Singleton<PlayerTexturePool>.instance.LBKLINGCPBA(GameLoginManager.currentPlayer.id, false);
			Singleton<PlayerTexturePool>.instance.LBKLINGCPBA(GameLoginManager.currentPlayer.id, true, true, true);
		}
		Singleton<ArmyPreviewCamera>.instance.JHKICLBBOCO(CamosManager.instance.mainPlayer);
		PlayerController.OGMBJPKOPCB.EHHHBEMKGOE.CBBDICBLHMM = CamosManager.instance.equppedIndices;
		if (PEMCEDKMNBO != null)
		{
			PEMCEDKMNBO.DNHOHNOJHCM(true);
		}
	}
	...
	{
		RequestBuffer requestBuffer = RequestBufferManager.instance.GetRequestBuffer();
		requestBuffer.AddRequest((DatabaseAction)102, DJOAHABGGNA.row.NAME, 0, 1, string.Empty, true);
	...
	{
		RequestBuffer requestBuffer = RequestBufferManager.instance.GetRequestBuffer();
		requestBuffer.AddRequest((DatabaseAction)83, DJOAHABGGNA.row.NAME, 1, 0, string.Empty, true);
		DJOAHABGGNA.notificate = false;
	...
	private void KFAPMMDPJGA()
	{
		if (PEMCEDKMNBO != null)
		{
			MPHLKJLMJBF.repositionNow = false;
			Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
			NFLPPGKCOBL.AlignToCenter(vec, true, null, 1097f);
		}
	}
	...
	public void MLOICIKLCFL(PlayerVisualsCategoryButton JCCELIHIKIK)
	{
		...
		FEPMLGAMCKG = JCCELIHIKIK;
		FEPMLGAMCKG.BANCDJNFHIC(false);
		if (PEMCEDKMNBO != null)
		{
			PEMCEDKMNBO.DNHOHNOJHCM(false);
		}
	...
	private void EJMJPDOLNJN(GameObject KHAHPAKDIKE)
	{
		...
		{
			GuiElementSingle<PackContentDialog>.instance.KNBIBOINAKG(Singleton<GameVariables>.instance.valuePack, true);
		}
	...
	{
		RequestBuffer requestBuffer = RequestBufferManager.instance.GetRequestBuffer();
		requestBuffer.AddRequest((DatabaseAction)33, DJOAHABGGNA.row.NAME, 1, 1, string.Empty, true);
		DJOAHABGGNA.FHPICBCCOAO();
	}
	...
	private void GGPAABMLJKA()
	{
		if (PEMCEDKMNBO != null)
		{
			MPHLKJLMJBF.repositionNow = false;
			Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
			NFLPPGKCOBL.AlignToCenter(vec, true, null, 1776f);
		}
	}
	...
	public virtual void HHJDKKFKKIL()
	{
		...
		Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
		NFLPPGKCOBL.AlignToCenter(vec, true, null, 1674f);
		if (!PlayerAnalytics.instance.data.customizationShown)
		{
			Singleton<MessageManager>.instance.AddMessage(new NGBBILLMHEC());
		}
	...
	public virtual void ENDIHKMAKNJ()
	{
		...
		Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
		NFLPPGKCOBL.AlignToCenter(vec, true, null, 165f);
		if (!PlayerAnalytics.instance.data.customizationShown)
		{
			Singleton<MessageManager>.instance.AddMessage(new NGBBILLMHEC(), true);
		}
	...
	public virtual void POKDDAGBIDO()
	{
		...
		Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
		NFLPPGKCOBL.AlignToCenter(vec, true, null, 1517f);
		if (!PlayerAnalytics.instance.data.customizationShown)
		{
			Singleton<MessageManager>.instance.AddMessage(new NGBBILLMHEC(), true);
		}
	...
	public virtual void DNFPEBEDLIG()
	{
		...
		Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
		NFLPPGKCOBL.AlignToCenter(vec, false, null, 1901f);
		if (!PlayerAnalytics.instance.data.customizationShown)
		{
			Singleton<MessageManager>.instance.AddMessage(new NGBBILLMHEC(), true);
		}
	...
	private void DPPAEJJMJFH()
	{
		if (PEMCEDKMNBO != null)
		{
			MPHLKJLMJBF.repositionNow = false;
			Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
			NFLPPGKCOBL.AlignToCenter(vec, true, null, 120f);
		}
	}
	...
	private void DCIKGGKODHI()
	{
		if (PEMCEDKMNBO != null)
		{
			MPHLKJLMJBF.repositionNow = true;
			Vector3 vec = PEMCEDKMNBO.transform.localPosition + PEMCEDKMNBO.transform.parent.localPosition;
			NFLPPGKCOBL.AlignToCenter(vec, true, null, 937f);
		}
	}
	...
	{
		RequestBuffer requestBuffer = RequestBufferManager.instance.GetRequestBuffer();
		requestBuffer.AddRequest(DatabaseAction.ProvokePlayer, DJOAHABGGNA.row.NAME, 0, 1, string.Empty, true);
		DJOAHABGGNA.FHPICBCCOAO();
	}
