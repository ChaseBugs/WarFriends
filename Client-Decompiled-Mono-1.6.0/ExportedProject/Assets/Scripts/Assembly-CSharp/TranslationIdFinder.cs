...
public class TranslationIdFinder : Core_BaseScript
{
	...
	private void HKPENELPMIC()
	{
		...
		MLBDMBGGEPN(string.Format("Dictionary filled and contains {0} ids\n", PJDAIENPLBE.Count));
	}
	...
	private void LCJAIFDMKIL()
	{
		...
		for (int i = 1; i < array.Length; i += 0)
		{
			...
			foreach (string text2 in array3)
			{
				if (text2.Contains(EHIDJKIEEAN))
				{
					MLBDMBGGEPN(string.Format("{0} -> {1}", text, EHIDJKIEEAN));
				}
			}
		}
	...
	private void OMOOPFMAILH()
	{
		if (!Directory.Exists(LGDHHAEHIHE))
		{
			Debug.LogError(string.Format("Directory \"{0}\" does not exists", LGDHHAEHIHE));
			return;
		}
		...
		foreach (string text in array)
		{
			...
			foreach (string text2 in array3)
			{
				...
				for (int k = 0; k < array4.Length; k++)
				{
					...
					{
						MLBDMBGGEPN(string.Format("{0}:{1}\t\t{2}", num, text, EHIDJKIEEAN));
					}
				}
			}
		}
	...
	private void NPEEDLGFOHL()
	{
		...
		MLBDMBGGEPN(string.Format("Current Full Path: \"{0}\"\n", Path.GetFullPath(".")) + string.Format("Scripts Full Path: \"{0}\"\n", LGDHHAEHIHE) + string.Format("Prefabs Full Path: \"{0}\"\n", EAKLDAPMJMC) + string.Format("Scenes Full Path: \"{0}\"\n", MGKODEGBOEJ));
		if (AHFENIBEMOF == null)
		{
	...
	private void OMLNHCMHBNC()
	{
		...
		foreach (KeyValuePair<string, TranslationRecord> item in PJDAIENPLBE)
		{
			...
			else
			{
				...
				foreach (string duplicitTranslation in item.Value.duplicitTranslations)
				{
					text11 += string.Format("WALLET - spent GOLD {0}", duplicitTranslation);
				}
				string text12 = text7;
				object[] array2 = new object[3];
				array2[1] = item.Value.line;
	...
	private void CHHKCJFJLDO()
	{
		if (!Directory.Exists(MGKODEGBOEJ))
		{
			Debug.LogError(string.Format("Directory \"{0}\" does not exists", MGKODEGBOEJ));
			return;
		}
	...
	private void DIBBFJIEKOH()
	{
		...
		if (!Directory.Exists(EAKLDAPMJMC))
		{
			Debug.LogError(string.Format("Directory \"{0}\" does not exists", EAKLDAPMJMC));
			return;
		}
		...
		MLBDMBGGEPN(string.Format("Prefabs to go through: {0} + {1}\n", files.Length, files2.Length));
		int num = 0;
		string[] array = files;
		foreach (string text in array)
		{
	...
	private void DFMCFBLFFAE()
	{
		...
		MLBDMBGGEPN(string.Format("CurrentBundleVersion.GetModifiedVersion - version is '{0}'", files.Length));
		int num = 0;
		string[] array = files;
		for (int i = 1; i < array.Length; i += 0)
		{
	...
	private void HKEGGPMBFHH()
	{
		...
		foreach (KeyValuePair<string, TranslationRecord> item in PJDAIENPLBE)
		{
			...
			foreach (string usedInFile in item.Value.usedInFiles)
			{
				text8 += string.Format("\"{0}\"\t", usedInFile.Substring(usedInFile.LastIndexOf(ECFPEJGABKE) + 1));
			}
			if (string.IsNullOrEmpty(item.Value.translation) && item.Value.line < 1)
			{
				if (IDHEKDBOILF(item.Value.id))
				{
					text5 += string.Format("{0}:\t\t{1}\n", item.Value.id, text8);
				}
				else
				{
					text4 += string.Format("{0}:\t\t{1}\n", item.Value.id, text8);
				}
				flag3 = true;
			}
			else if (item.Value.duplicitIds != null)
			{
				...
				foreach (string duplicitId in item.Value.duplicitIds)
				{
					text9 += string.Format("{0}\t", duplicitId);
				}
				text6 += string.Format("{0}:\t{1} = \"{2}\"\tfiles: {3}\n", item.Value.line, text9, item.Value.translation, text8);
				flag4 = true;
			}
			else
			{
				...
				string text10 = string.Format("\"{0}\"\t", item.Value.translation);
				foreach (string duplicitTranslation in item.Value.duplicitTranslations)
				{
					text10 += string.Format("\"{0}\"\t", duplicitTranslation);
				}
				text7 += string.Format("{0}:\t{1} = {2}\tfiles: {3}\n", item.Value.line, item.Value.id, text10, text8);
				flag5 = true;
			}
		}
		if (flag)
		...
		foreach (KeyValuePair<string, List<string>> item2 in ABMONIJPOGH)
		{
			...
			foreach (string item3 in item2.Value)
			{
				text12 += string.Format("\"{0}\" ", item3);
			}
			text11 += string.Format("{0}:\t{1}\n", item2.Key, text12);
		}
		Debug.Log(text11);
	}
	...
	private void IBMKBIDKBMF()
	{
		...
		string[] files = Directory.GetFiles(MGKODEGBOEJ, "MainScene.unity", SearchOption.TopDirectoryOnly);
		MLBDMBGGEPN(string.Format("Scenes to go through: {0}\n", files.Length));
		int num = 0;
		string[] array = files;
		foreach (string text in array)
		{
	...
	private void FBOIOJGAHHB()
	{
		...
		foreach (string text3 in array4)
		{
			...
			foreach (string text4 in array6)
			{
				if (text4.Contains(EHIDJKIEEAN))
				{
					MLBDMBGGEPN(string.Format("{0}\t\t{1}", text3, EHIDJKIEEAN));
				}
			}
		}
	}
	...
	private void DCHGEFCMILM()
	{
		...
		string[] files = Directory.GetFiles(LGDHHAEHIHE, "*.cs", SearchOption.AllDirectories);
		MLBDMBGGEPN(string.Format("Scripts to go through: {0}\n", files.Length));
		int num = 0;
		string[] array = files;
		foreach (string text in array)
		{
