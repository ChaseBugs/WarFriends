...
public class WeaponScreen : GuiScreenSingle<WeaponScreen>
{
	...
	public WeaponGuiRecord selectedIcon { get; private set; }

	public List<WeaponGuiRecord> weaponIcons { get; private set; }

	public List<WeaponEquipSlotButton> weaponCategoryButtons { get; private set; }
	...
	private void IHNMNOOEHGM()
	{
		...
		NFLPPGKCOBL.AlignToCenter(zero, true, null, 728f);
	}
	...
	private void AHDPLLLMFJB(WeaponLevelsSetup JMMJHCOKCGG)
	{
		...
		PlayerInventory.InventorySlot slotForCategory = PlayerInventory.instance.GetSlotForCategory(oGPCPMGOAMA.JMMJHCOKCGG.BHCEOOLEHHG);
		WeaponCategory hJFKNAOICJO = ((slotForCategory != null) ? slotForCategory.category : oGPCPMGOAMA.JMMJHCOKCGG.BHCEOOLEHHG);
	...
	{
		int num = Singleton<OfferManager>.instance.ReduceWeaponDeliveryTime(MBNLFKHLDBH.BHCEOOLEHHG, MBNLFKHLDBH, true);
	...
	{
		int num = Singleton<OfferManager>.instance.ReduceWeaponDeliveryTime(MBNLFKHLDBH.BHCEOOLEHHG, MBNLFKHLDBH, false);
	...
	internal void APGDCFFDPOF(WeaponLevelsSetup MBNLFKHLDBH, bool CDMKCIKHIMM)
	{
		...
		int num3 = Singleton<OfferManager>.instance.DiscountedWeaponDeliveryCost(MBNLFKHLDBH.BHCEOOLEHHG, MBNLFKHLDBH, false);
	...
	private void COPMJOAJKFI(WeaponCategory FABOAIMLLOI, string PGJNIAIHKDB)
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (!(weaponGuiRecord == null))
			{
				...
				{
					weaponGuiRecord.MDBADNAJFBN(false);
				}
	...
	protected override void AGIKPOLCGNF()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			weaponEquipSlotButton.gameObject.name = string.Format("{0} {1} Button", i, PlayerInventory.instance.inventorySlots[i].category);
	...
	private void ELOHOAPJIMK()
	{
		...
		IEIKMKFHDOJ.onReposition = delegate
		{
			...
			NFLPPGKCOBL.AlignToCenter(zero, true);
		};
	}
	...
	public void CMMGDOIBBOH()
	{
		...
		TweenPosition.Begin(GCDLCFADJFO.gameObject, MFAFIELBJGF * 742f, Vector3.zero, Vector3.zero, false).onFinished = delegate
		{
	...
	private void FPHAGAEIDBF(WeaponCategory FABOAIMLLOI, string PGJNIAIHKDB)
	{
		...
		if (nameGuiRecord != null)
		{
			nameGuiRecord.name = string.Format("{0} 00 {1}", PGJNIAIHKDB, FABOAIMLLOI.ToString());
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (!(weaponGuiRecord == null))
			{
				...
				{
					weaponGuiRecord.SetEquiped(true);
				}
				if (weaponLevelsSetup == null && weaponLevelsSetup2.unlockLevelIndex == 0)
				{
					SavingLastSelected.instance.SaveLastWeapon(weaponLevelsSetup2);
	...
	{
		Debug.Log("Clicked on category " + FABOAIMLLOI);
		int num = 0;
	...
	private void APOOIEKEBIC(UITweener GCNILAHBDMJ)
	{
		...
		TweenPosition.Begin(GCDLCFADJFO.gameObject, MFAFIELBJGF * 665f, new Vector3(1587f, 1789f, 845f), false);
	}
	...
	private void KGGIGGPKCJF(DatabaseAction IFGAGNBDKBE)
	{
		...
		case DatabaseAction.CreateFullAccount:
			UpdateGui(PAAMHOJOFFP().GMEFFFKMDCE(), false);
			break;
		}
	}
	...
	internal void NHAPEPLDGBB(WeaponLevelsSetup MBNLFKHLDBH, bool CDMKCIKHIMM)
	{
		...
		int num4 = Singleton<OfferManager>.instance.DiscountedWeaponDeliveryCost(MBNLFKHLDBH.BHCEOOLEHHG, MBNLFKHLDBH, true);
	...
	{
		Debug.Log("Warbucks" + FABOAIMLLOI);
		int num = 1;
	...
	private void GFKMDLNPNHP(DatabaseAction IFGAGNBDKBE)
	{
		...
		case DatabaseAction.GetSquadWarsDivision:
			BDODGNOMDMF(selectedIcon.EAJJDMMGMKK(), false);
			break;
		}
	}
	...
	public void AFCNHKNBDIH(WeaponLevelsSetup MBNLFKHLDBH, ABDDBOABKOK ILOAGGPDJCD)
	{
		...
		if (MBNLFKHLDBH.BHCEOOLEHHG == ~(WeaponCategory.Primary | WeaponCategory.SniperRifle | WeaponCategory.RocketLauncher | WeaponCategory.Grenade))
		{
			...
			if (num == 1)
			{
				Singleton<MessageManager>.instance.AddMessage(new FNGCABFGGLA(MBNLFKHLDBH), true);
			}
		}
	}
	...
	private void DKCHELDAACD(WeaponLevelsSetup JMMJHCOKCGG)
	{
		...
		selectedIcon.PJEEJBPFNFA(false);
		HBBEDANBMOI();
		OHMLGALJLBA();
	}
	...
	protected override void Update()
	{
		...
		if (selectedIcon != null && selectedIcon.weaponSetup != null)
		{
			...
			{
				Debug.Log(string.Format("Weapon state changed from \"{0}\" to \"{1}\"", EMECGGIPPNB.ToString().ToUpper(), selectedIcon.weaponSetup.weaponState.ToString().ToUpper()));
				EMECGGIPPNB = selectedIcon.weaponSetup.weaponState;
				UpdateGui(selectedIcon.weaponSetup, false);
			}
	...
	private void JLHCKLBGOFA(DatabaseAction IFGAGNBDKBE)
	{
		...
		case DatabaseAction.InstantBuyUnit:
			BDODGNOMDMF(FMFHOPFBMOO().ADOICOAEIFA(), false);
			break;
		}
	}
