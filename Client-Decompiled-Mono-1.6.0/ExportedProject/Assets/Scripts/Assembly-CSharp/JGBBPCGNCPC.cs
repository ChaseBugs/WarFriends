...
public class JGBBPCGNCPC
{
	...
	public class Content
	{
		...
		public string debugString
		{
			get
			{
				...
				if (type == NNDPOJCACLP.PowerBand)
				{
					return string.Format("Special pack content: type: {0} id: {1} seconds: {2}{3}", type, visual.id, amount, text);
				}
				if (type == NNDPOJCACLP.Customization)
				{
					return string.Format("Special pack content: type: {0} id: {1}{2}", type, visual.id, text);
				}
				if (type == NNDPOJCACLP.Unit)
				{
					return string.Format("Special pack content: type: {0} id: {1}{2}", type, unit.unitName, text);
				}
				...
				{
					return string.Format("Special pack content: type: {0} rarity: {1} amount: {2}{3}", type, cardpackType, amount, text);
				}
				if (type == NNDPOJCACLP.Warcard)
				{
					return string.Format("Special pack content: type: {0} rarity: {1} amount: {2}{3}", type, warcardType, amount, text);
				}
				return string.Format("Special pack content: type: {0} amount: {1}{2}", type, amount, text);
			}
		}
		...
		public bool HNCDLFFBMAA()
		{
			...
			default:
				...
				if (JMBKPALDGAL < KCIPIGHOBIO.Count)
				{
					...
					AAPOFJBNMHM.BKLMJCEPNMK(KCIPIGHOBIO[JMBKPALDGAL], 361f, false);
	...
	public Tuple<float, string> AMFIAMAGMIN
	{
		get
		{
	...
	public string GOBNFILDHBH
	{
		get
		{
			return (AMFIAMAGMIN != null) ? AMFIAMAGMIN.Value2 : string.Empty;
		}
	}
	...
	public string MJADIIIJPEA
	{
		get
		{
			...
			string text = string.Format("Debugging pack id: {0} with sale: {1} store price: {2} store country: {3} and content:\n", KJBPJOMHNKB, GAFOMGDLAFD, AMFIAMAGMIN.Value2, LDJCPHDIOOD);
