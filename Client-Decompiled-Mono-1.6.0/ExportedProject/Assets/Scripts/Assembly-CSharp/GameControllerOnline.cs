...
public abstract class GameControllerOnline : IGameController
{
	private sealed class NCEKIFIDMCF : IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool KHNDCDGBFJH()
		{
			...
			case 0u:
			{
				...
				HMAIEJHCHDE.gameObject.SetActive(false);
			...
			case 1u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogError(string.Format("Showing squad details dialog with SQUADNAME = "));
		...
		public bool GAPIFMKNJBN()
		{
			...
			case 0u:
			{
				...
				HMAIEJHCHDE.gameObject.SetActive(true);
				HGMAPLKJONO = BJGCPDNMHDH.PKMFODGDMON(HMAIEJHCHDE.fraction, KCDAPPPFIKG, HMAIEJHCHDE.EHHHBEMKGOE.KIOGGEECBGB);
			...
			case 1u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogError(string.Format("Id"));
					foreach (KeyValuePair<int, PlayerController> item in PlayerController.JFGDAKMCJAM)
					{
		...
		public bool OGLKGILLKIO()
		{
			...
			case 1u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogError(string.Format("GLM: OnDataLoaded - configurations.  is tutorial? "));
					foreach (KeyValuePair<int, PlayerController> item in PlayerController.JFGDAKMCJAM)
					{
		...
		public bool HKPECDLGFJB()
		{
			...
			case 1u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogError(string.Format("Buffer already sent!"));
					foreach (KeyValuePair<int, PlayerController> item in PlayerController.JFGDAKMCJAM)
					{
		...
		public bool MoveNext()
		{
			...
			case 1u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogError(string.Format("Waiting too long for all playersFinish to loading"));
					foreach (KeyValuePair<int, PlayerController> item in PlayerController.JFGDAKMCJAM)
					{
						UnityEngine.Debug.Log(string.Format("Player {0} state: {1}", item.Value.EHHHBEMKGOE.MHPNDNJDPGE, item.Value.LMIHBNGHEHM.EHFIJJBPEEG));
					}
				}
				goto IL_03b0;
			case 2u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogWarning(string.Format("Waiting too long to start match, Match start: {0} photon time: {1}", BJGCPDNMHDH.NFDLBMBMMLE, PhotonNetwork.time));
				}
				goto IL_0472;
			default:
				{
		...
		public bool NJBCJDFKGHO()
		{
			...
			case 1u:
				...
				{
					AOKGPJDNJJO = Time.realtimeSinceStartup;
					UnityEngine.Debug.LogError(string.Format("FB hosted app link: https://fb.me/1157647734253183"));
					foreach (KeyValuePair<int, PlayerController> item in PlayerController.JFGDAKMCJAM)
					{
	...
	public virtual string loadingWaitingCards
	{
		get
		{
	...
	public virtual string loadingWaiting
	{
		get
		{
			return Localization.Localize("ID_STATE_WAITINGFOROPPONENT");
		}
	}
	...
	public override bool canShowPause
	{
		get
		{
			return !MatchManager.isReconnect && MatchManager.allPlayersFinishetStartAnimation;
		}
	}

	protected virtual bool JPADCGODJBO
	{
		get
		{
			return false;
	...
	{
		base.Start();
		PhotonView[] componentsInChildren = AAIEPHNDCJP.GetComponentsInChildren<PhotonView>(true);
	...
	public virtual void OnPhotonCustomRoomPropertiesChanged(ExitGames.Client.Photon.Hashtable JLLOEMMPGJA)
	{
		foreach (DictionaryEntry item in JLLOEMMPGJA)
		{
			if (item.Key as string == "MatchStart" && item.Value is double && (double)item.Value != double.PositiveInfinity)
			{
				...
				UnityEngine.Debug.Log(string.Format("Match start time {0} photon time: {1}", NFDLBMBMMLE, PhotonNetwork.time));
			}
		}
	}
	...
	private void FDPLIHFPLKB(double JGBFNKBFCKM)
	{
		...
		UnityEngine.Debug.Log(string.Format("Sync match start time {0} photon time: {1}", NFDLBMBMMLE, PhotonNetwork.time));
	}
	...
	public override void FinishChoosingCards()
	{
		...
		{
			LoadingDialog.ShowLoading(loadingWaitingCards, true).DGPEHPHGDEF = EGMEJKOEAIH;
		}
	}
	...
	protected void StartCameraAnimationFinishedRPC(int INFLHPGMEOB)
	{
		...
		UnityEngine.Debug.Log(string.Format("StartCameraAnimationFinishedRPC for player {0} ", INFLHPGMEOB));
		if (player != null)
		{
	...
	protected void OnJoinedRoom()
	{
		...
		UnityEngine.Debug.Log(string.Format("Joined room: {0}, name: {1},  userId: {2}, allConnected: {3}", PhotonNetwork.room.name, PhotonNetwork.player.name, PhotonNetwork.player.ID, DEGCDEAANEL));
		PhotonPlayer[] playerList = PhotonNetwork.playerList;
		foreach (PhotonPlayer photonPlayer in playerList)
		{
			UnityEngine.Debug.Log(string.Format("Player id: {0}, player userId {1}", photonPlayer.name, photonPlayer.userId));
		}
	...
	{
		PhotonView[] componentsInChildren = KHLGDCHJJPB.GetComponentsInChildren<PhotonView>(true);
	...
	private void KBKKFBFIAML(PhotonPlayer KHLGDCHJJPB)
	{
		...
		Singleton<ArmyPreviewCamera>.instance.RenderToTexture(HMAIEJHCHDE, true, PlayerTexturePool.DFJJOANIJID.Classic);
	...
	protected virtual void PKCOLNDDFPO()
	{
		...
		if (gameIsRunning && (MatchManager.isReconnect || !MatchManager.allPlayersFinishetStartAnimation))
		{
			if (Time.realtimeSinceStartup > MHHFPDGIPGA)
			{
				...
				UnityEngine.Debug.Log(string.Format("CheckForReconnect: isReconnect: {0}", MatchManager.reconnectState));
			}
	...
	{
		UnityEngine.Debug.LogError(string.Format("JoinRoomFailedAfterReconnect: {0} GameIsRunning: {1}", PNAKMCNGGHE, gameIsRunning));
	...
	{
		UnityEngine.Debug.LogError(string.Format("OnPhotonJoinRoomFailed: {0}, message {1}", CJLLLNBEDHO[0], CJLLLNBEDHO[1]));
		if (MatchManager.isReconnect)
		{
