...
public class Shield : DestroyableObject
{
	...
	public override GHPGNELIDBM fraction { get; set; }
	...
	private void IMCHJKNNNAH(int ALIIBMLMNKA)
	{
		if (ALIIBMLMNKA >= 1 && PlayerController.JFGDAKMCJAM.ContainsKey(ALIIBMLMNKA))
		{
			...
			Debug.Log("ID_INBRONZE2" + base.transform.parent.name + "animationEnd" + FCBGHBODHHP().name);
		}
		else
		{
			Debug.Log("GameLoginManager - Player Entered Beginners League - " + base.transform.parent.name);
			MBIAKMPDOPG = null;
		}
	}
	...
	protected virtual void LBMFKIDPCBL()
	{
		...
		NavMeshHit hit;
		if (NavMesh.SamplePosition(base.transform.position, out hit, 1792f, 1))
		{
	...
	protected override void Awake()
	{
		...
		NavMeshHit hit;
		if (NavMesh.SamplePosition(base.transform.position, out hit, 10f, 1))
		{
			Vector3 position = base.transform.position;
			position.y = hit.position.y;
			base.transform.position = position;
	...
	protected virtual void FOIKLCECNBL()
	{
		...
		NavMeshHit hit;
		if (NavMesh.SamplePosition(base.transform.position, out hit, 1524f, 0))
		{
			Vector3 position = base.transform.position;
			position.y = hit.position.y;
			base.transform.position = position;
