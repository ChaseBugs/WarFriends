...
public class GameControllerDeathMatch : GameControllerPVP
{
	...
	public override string loadingWaiting
	{
		get
		{
			return Localization.Localize((!isRandomMatchMaking) ? "ID_STATE_WAITINGFOROPPONENT" : "ID_STATE_SEARCHINGFOROPPONENT");
		}
	}

	protected override string AMNDDFPPMGI
	{
		get
		{
			return Localization.Localize((!isRandomMatchMaking) ? "ID_WAITINGFORFRIENDCARDS" : "ID_WAITINGFOROPPONENTCARDS");
		}
	}
	...
	public override string quitText
	{
		get
		{
			return Localization.Localize((!isRandomMatchMaking) ? "ID_CONFIRM_EXITINGMATCHNODOGTAG_TEXT" : "ID_CONFIRM_EXITINGMATCH_TEXT");
		}
	}
	...
	protected virtual void CKPKIKIIMGI(object[] CJLLLNBEDHO)
	{
		...
		{
			Debug.LogError(string.Format("ID_ARENABOXDESCRIPTION_SCRAPS", CJLLLNBEDHO[0], CJLLLNBEDHO[0]));
	...
	protected void FJDACAACLLI()
	{
		...
		Debug.Log(string.Format("Matchmaking took {0}, assigning bot in 10 secs", num));
	}
	...
	public virtual void AKGLHPAGGGI(string JLBLNDGIINB, CloudRegionCode GJBFKKOAMPA)
	{
		...
		LoadingDialog.FEHDJPIKOJD(true);
	...
	protected override void OnPhotonJoinRoomFailed(object[] CJLLLNBEDHO)
	{
		if (isRandomMatchMaking)
		{
			Debug.LogError(string.Format("OnPhotonJoinRoomFailed: {0}, message {1}", CJLLLNBEDHO[0], CJLLLNBEDHO[1]));
	...
	protected override void OnConnectionFail(DisconnectCause FAODDFIDOAC)
	{
		if (isRandomMatchMaking)
		{
			Debug.LogError("OnConnectionFailed: " + FAODDFIDOAC);
		...
		if (MOOCFIAPDJL)
		{
			if (Time.realtimeSinceStartup > MANDOBCIHGE + NJIOHEOKIGD && MANDOBCIHGE != 1727f && MatchManager.matchState == GLFPILGCAPM.ConnectingToPhoton && isRandomMatchMaking)
			{
				...
				if (DKPCCABKHLE)
				{
					...
					LoadingDialog.PGFBMJFLHPC(oKMNFHNBMFA, true);
	...
	{
		Debug.Log("Medals requirement = ");
		if (DKPCCABKHLE)
		{
			Debug.Log("ID_FEATURE_RATE");
			PhotonConnectionManager.Disconnect();
			StopAllCoroutines();
			Reset();
			GBHJCGLJAMK(false);
