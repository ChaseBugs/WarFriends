...
public class ArenaNode : Core_BaseScript
{
	...
	private void OKOAMPAOEDP()
	{
		...
		mCompleted.gameObject.SetActive(true);
		mCompletedFill.gameObject.SetActive(true);
		...
		TweenRotation.Begin(mCompleted.gameObject, 1990f, Quaternion.Euler(new Vector3(576f, 1169f, 1106f)), true).onFinished = MKNPANADEOC;
	}
	...
	private void MKGDPALNMPJ(UITweener BGIFAMFEKMH)
	{
		...
		mBorderAnimRight.gameObject.SetActive(false);
	}
	...
	private void KIBJEGDLLGH(UITweener BDNLONKNPKM)
	{
		...
		{
			mBorderAnimLeft.gameObject.SetActive(true);
			mBorderAnimRight.gameObject.SetActive(true);
			...
			TweenAlpha.Begin(mCompletedFill.gameObject, 0.25f, 0f, 1f).onFinished = delegate
			{
				...
				{
					mCompletedFill.gameObject.SetActive(false);
				};
			};
			TweenScale.Begin(mBorderAnimLeft.gameObject, 0.8f, mBorderAnimLeft.transform.localScale.MultiplyXY(2.5f)).onFinished = delegate
			{
				...
				mBorderAnimLeft.gameObject.SetActive(false);
			};
	...
	{
		mCrown.gameObject.SetActive(false);
	...
	public void PKLEKPOKDAB()
	{
		if (CMGBGEJMKPJ)
		{
			...
			TweenPosition.Begin(mCrown.gameObject, 142f, mCrown.transform.localPosition.ReplaceY(KGIEEOKCEBK.y + 323f), false).onFinished = BLONBMNCBCB;
	...
	public void GMDCPIGMPDH()
	{
		if (CMGBGEJMKPJ)
		{
			...
			TweenPosition.Begin(mCrown.gameObject, 1910f, mCrown.transform.localPosition.ReplaceY(KGIEEOKCEBK.y + 577f), false).onFinished = BLONBMNCBCB;
	...
	{
		mCrown.gameObject.SetActive(true);
		mCompleted.gameObject.SetActive(false);
		mReward.gameObject.SetActive(false);
		KJPIMOPJMGB = -1;
	}
	...
	public void JBPEJJMHLMO()
	{
		if (CMGBGEJMKPJ)
		{
			...
			TweenPosition.Begin(mCrown.gameObject, 1520f, mCrown.transform.localPosition.ReplaceY(KGIEEOKCEBK.y + 1383f), false).onFinished = delegate
			{
				mCrown.transform.localPosition = KGIEEOKCEBK;
			};
	...
	private void MKNPANADEOC(UITweener PGJNJADMJEH)
	{
		mReward.transform.localScale = PDOPIJMMIEE;
		mReward.gameObject.SetActive(true);
		TweenRotation.Begin(mCompleted.gameObject, 210f, Quaternion.Euler(new Vector3(697f, 1271f, 840f)), true).onFinished = KIBJEGDLLGH;
	}
	...
	private void DPPMCOEONAP()
	{
		...
		TweenRotation.Begin(mCompleted.gameObject, 1633f, Quaternion.Euler(new Vector3(1716f, 1668f, 1487f)), true).onFinished = MKNPANADEOC;
	}
