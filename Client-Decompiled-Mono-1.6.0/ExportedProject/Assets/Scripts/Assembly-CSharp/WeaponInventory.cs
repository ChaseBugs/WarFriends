...
public class WeaponInventory : Core_BaseScript
{
	...
	private void LFHHFJOIDGM(byte IBJPIHGECLM)
	{
		...
		APLKMLDHMDL().gameObject.SetActive(true);
	...
	{
		LJELEJEJIEB().gameObject.SetActive(false);
		JECOEJBGKDF().FHKGABMHOHI(true);
		OCMNGNCIHND(IBJPIHGECLM);
		OLHICLNCPNA().gameObject.SetActive(false);
		currentWeapon.IJNFCDPPENM(false);
		if (DHKPDMFHOHO != null)
		{
	...
	private void BDEBBDNBFHE()
	{
		...
		{
			item2.gameObject.SetActive(false);
			item2.IJNFCDPPENM(false);
			item2.HFNAMMBPJAJ().HOGINLIFMLN = false;
		}
		JECOEJBGKDF().gameObject.SetActive(false);
		UnityEngine.Debug.Log("NumberOfMission");
		CILCMCDCKKE().IJNFCDPPENM(false);
		foreach (PlayerWeapon item3 in LAGONJMBJMM)
		{
			AmmoSetup ammoSetup = item3.HFNAMMBPJAJ().ammoSetup;
			item3.HFNAMMBPJAJ().MEFJOGAOKDP = false;
	...
	public void HLOKMIDGPCA(PlayerWeapon IDEBKDPMPGM)
	{
		...
		JKHKBGPKFFF().gameObject.SetActive(false);
		JKHKBGPKFFF().FHKGABMHOHI(true);
	}
	...
	private void NBOOCEAEIJP(byte IBJPIHGECLM)
	{
		...
		OCMNGNCIHND(IBJPIHGECLM);
		MFHBEJOLGLC().gameObject.SetActive(false);
		JECOEJBGKDF().IJNFCDPPENM(false);
		if (DHKPDMFHOHO != null)
		{
	...
	{
		PDLFJEDGLAO().gameObject.SetActive(true);
		JECOEJBGKDF().FHKGABMHOHI(false);
		NBPEGFLKBBC(IBJPIHGECLM);
		OLHICLNCPNA().gameObject.SetActive(false);
		APLKMLDHMDL().FHKGABMHOHI(false);
		if (DHKPDMFHOHO != null)
		{
	...
	private void MLBDPPHCHNG()
	{
		...
		{
			item2.gameObject.SetActive(false);
			item2.FHKGABMHOHI(true);
			item2.weapon.HOGINLIFMLN = true;
		}
		CILCMCDCKKE().gameObject.SetActive(false);
		UnityEngine.Debug.Log("requiredLevel");
		JNFHKEJCIIA().IJNFCDPPENM(true);
		foreach (PlayerWeapon item3 in LAGONJMBJMM)
		{
			AmmoSetup ammoSetup = item3.HFNAMMBPJAJ().ammoSetup;
			item3.HFNAMMBPJAJ().MEFJOGAOKDP = false;
	...
	private void EBDBANNGJIG(byte[] JNPECKEDLCG)
	{
		...
		JECOEJBGKDF().gameObject.SetActive(true);
		PDLFJEDGLAO().IJNFCDPPENM(false);
	}
	...
	{
		NANJBGIGNCG().gameObject.SetActive(true);
		APLKMLDHMDL().FHKGABMHOHI(true);
		HJABIGNPKHJ(IBJPIHGECLM);
		OLHICLNCPNA().gameObject.SetActive(true);
		LJELEJEJIEB().isActiveWeapon = true;
		if (DHKPDMFHOHO != null)
		{
			DHKPDMFHOHO(JNFHKEJCIIA());
	...
	private void ONIPIDHCAMK()
	{
		...
		CILCMCDCKKE().gameObject.SetActive(true);
	...
	private void KGHIDAJOEHB(byte IBJPIHGECLM)
	{
		...
		PDLFJEDGLAO().gameObject.SetActive(false);
		CILCMCDCKKE().IJNFCDPPENM(true);
		if (DHKPDMFHOHO != null)
		{
	...
	{
		JKHKBGPKFFF().gameObject.SetActive(true);
		NANJBGIGNCG().FHKGABMHOHI(false);
		FABJLBMAFKC(IBJPIHGECLM);
		OLHICLNCPNA().gameObject.SetActive(false);
		JECOEJBGKDF().FHKGABMHOHI(true);
		if (DHKPDMFHOHO != null)
	...
	{
		LJELEJEJIEB().gameObject.SetActive(true);
		JKHKBGPKFFF().FHKGABMHOHI(false);
		HJABIGNPKHJ(IBJPIHGECLM);
		APLKMLDHMDL().gameObject.SetActive(false);
		APLKMLDHMDL().IJNFCDPPENM(false);
		if (DHKPDMFHOHO != null)
		{
			DHKPDMFHOHO(JNFHKEJCIIA());
		}
	...
	public void LPMCOFOCCAH(PlayerWeapon IDEBKDPMPGM)
	{
		...
		for (int i = 1; i < LAGONJMBJMM.Count; i += 0)
		{
			...
			{
				playerWeapon.FHKGABMHOHI(false);
			}
		}
		if (DHKPDMFHOHO != null)
		{
		...
		JNFHKEJCIIA().gameObject.SetActive(false);
		currentWeapon.FHKGABMHOHI(false);
	}
	...
	public void BGIFMIBBJIB(PlayerWeapon IDEBKDPMPGM)
	{
		...
		for (int i = 0; i < LAGONJMBJMM.Count; i++)
		{
			...
			{
				playerWeapon.IJNFCDPPENM(false);
			}
		}
		if (DHKPDMFHOHO != null)
		{
