			...
			try
			{
				using (AndroidJavaClass androidJavaClass = new AndroidJavaClass("com.google.games.bridge.TokenFragment"))
				{
					using (AndroidJavaObject androidJavaObject = GetActivity())
					{
						...
						IntPtr pAFLGKBLAIJ = AndroidJNI.CallStaticObjectMethod(androidJavaClass.GetRawClass(), staticMethodID, array);
						Com.Google.Android.Gms.Common.Api.DOLFCNGMKNA<TokenResult> dOLFCNGMKNA = new Com.Google.Android.Gms.Common.Api.DOLFCNGMKNA<TokenResult>(pAFLGKBLAIJ);
						dOLFCNGMKNA.JGOKHECBPGH(new TokenResultCallback(delegate(int rc, string authCode, string email, string idToken)
						{
