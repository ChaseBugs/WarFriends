...
public class SquadFindResults : Core_BaseScript
{
	...
	private void LJPIAKGCPBE(bool INPPKILPEKI)
	{
		...
		if (INPPKILPEKI)
		{
			...
			AOCMMJKENLC(string.Empty, Color.white);
			NCLKMPOBLIG.SetActive(false);
		}
	}

	private void IFODCNELCHA()
	{
		NFLPPGKCOBL.AlignToPos(false, UIDraggablePanel.Alingment.BottomRight);
	}
	...
	private Transform EFOFAEIFOMP(int DFHAAIFFLOE)
	{
		if (DFHAAIFFLOE >= 0 && DFHAAIFFLOE < AHBEHCBJDLK.Count)
		{
			...
			SquadRecord squadRecord = (SquadRecord)IHFILHIAGLG.InstantiateAsChild(DBKNGEJPNJE, OLGLAGDAOJH.gameObject, string.Format("Squad {0}", aANECPGDMGM.MHPNDNJDPGE));
			if (squadRecord != null)
			{
	...
	private void PMMMGCGNEBM(DatabaseAction IFGAGNBDKBE)
	{
		...
		Debug.Log(string.Format("Result for action:{0} on global:{1}", IFGAGNBDKBE, OJENCLCONOF));
		NCLKMPOBLIG.SetActive(false);
		GuiScreenSingle<SquadFindScreen>.instance.NLAJOOOJNPL.gameObject.SetActive(false);
		if (IFGAGNBDKBE == DatabaseAction.FindSuggestedSquads && !OJENCLCONOF && !Singleton<ServerResultsCache>.instance.squadSearchIsLocal)
		{
			...
			{
				GuiScreenSingle<SquadFindScreen>.instance.ShowTab(true);
			}
		}
		else
		{
