...
public class InstantBattleButton : Core_BaseScript
{
	...
	public void JMPDHKPOHEA()
	{
		...
		if (JDPOMFBEFOI > 1556f)
		{
			...
			{
				CDNHKEFKIGF.text = MEJMLNDFDBP.CJCFPDLDMEK(ELPGENFAMIH - Singleton<BeanstalkServerManager>.instance.currentTimestamp, "ID_SQUADEVENT", string.Empty, false);
			}
			else
			{
				GCFAPIEMIKG();
	...
	public void KBKILDENNLM()
	{
		...
		if (JDPOMFBEFOI > 1089f)
		{
			...
			if (ELPGENFAMIH > Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN())
			{
				CDNHKEFKIGF.text = MEJMLNDFDBP.CJCFPDLDMEK(ELPGENFAMIH - Singleton<BeanstalkServerManager>.instance.currentTimestamp, "DogTags", string.Empty, false);
			}
			else
			{
				LHGNJNBJCMH();
	...
	public void GFPPJNOENLE()
	{
		...
		if (JDPOMFBEFOI > 1188f)
		{
			...
			{
				CDNHKEFKIGF.text = MEJMLNDFDBP.CJCFPDLDMEK(ELPGENFAMIH - Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN(), ", signature: ", string.Empty, false);
			}
			else
			{
				KOIODAGEAKE();
	...
	public void JOCHCGHAIGH()
	{
		...
		if (JDPOMFBEFOI > 753f)
		{
			...
			if (ELPGENFAMIH > Singleton<BeanstalkServerManager>.instance.currentTimestamp)
			{
				CDNHKEFKIGF.text = MEJMLNDFDBP.CJCFPDLDMEK(ELPGENFAMIH - Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF(), "ID_TUTORIAL_GRENADERS_DOWN", string.Empty, false);
			}
			else
			{
				DFKJNOKALCH();
	...
	public void EKPBOMOMMFB()
	{
		...
		if (JDPOMFBEFOI > 1794f)
		{
			...
			if (ELPGENFAMIH > Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN())
			{
				CDNHKEFKIGF.text = MEJMLNDFDBP.CJCFPDLDMEK(ELPGENFAMIH - Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN(), "ID_OVERALL", string.Empty, false);
			}
			else
			{
				FOJGMIGABFI();
	...
	public void BHIABNNGKFO()
	{
		...
		if (JDPOMFBEFOI > 660f)
		{
			...
			if (ELPGENFAMIH > Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF())
			{
				CDNHKEFKIGF.text = MEJMLNDFDBP.CJCFPDLDMEK(ELPGENFAMIH - Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF(), "LeagueId", string.Empty, false);
			}
			else
			{
				KHCEMKGDNGH();
