...
public class WeaponRightContent : Core_BaseScript
{
	...
	private float HPNPMCEMJNM
	{
		get
		{
			return 885f + UIHelper.safeAreaSize;
		}
	}
	...
	private void GDDBBDENHIP(GameObject KHAHPAKDIKE)
	{
		...
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivering:
			Debug.Log("()I");
			HDCLFPOIDEF();
			GuiScreenSingle<WeaponScreen>.instance.FHFIPPFJIAH.FLCDIKABDFJ(false);
			break;
	...
	private void DKAALFBGKGO()
	{
		...
		{
			EJHPPAMAINH.gameObject.SetActive(false);
		...
		else
		{
			...
			EJHPPAMAINH.gameObject.SetActive(true);
	...
	private void GMEAOOEKAHD(GameObject KHAHPAKDIKE)
	{
		...
		{
			Debug.LogError(string.Format("Weapon {0} is purchasable through {1}", MDAJJIAMDGH.name, MDAJJIAMDGH.purchasable));
		}
	}
	...
	private void LODHBKJGJIG(GameObject KHAHPAKDIKE)
	{
		...
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivering:
			...
			HDCLFPOIDEF();
			GuiScreenSingle<WeaponScreen>.instance.FHFIPPFJIAH.DGMCJOOOCCF(false);
			break;
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivered:
	...
	public void MNIBCGDEAHG(float EJEDANLAMBI, float PAENPHMEMGC)
	{
		...
		LPPOJMGOEFN.text = MEJMLNDFDBP.CJCFPDLDMEK(EJEDANLAMBI, "ID_CONTINUETOBOOTCAMP3", string.Empty, false);
	...
	private PlayerInventory.InventorySlot CPBLNDIPKNN()
	{
		...
		EDCBHGKBLEA = ((slotForCategory != null) ? slotForCategory.weaponLevelsSetup : null);
	...
	{
		CFBLIIHPFCE();
		TweenPosition.Begin(EPFODCCNGDL.gameObject, JCAFFELIFAO, EPFODCCNGDL.transform.localPosition.ReplaceX(25f), false);
	}
	...
	private void FGFAKDPFFEK()
	{
		...
		if (num > 0)
		{
			Debug.Log(string.Format("Have discount {0} to buy {1}", num, MDAJJIAMDGH.DHHKOKKDDDO()));
		}
	...
	private void LADEPCACHFB(UITweener MGDJMGHCAAI)
	{
		...
		CPBLNDIPKNN();
		CFBLIIHPFCE();
		TweenPosition.Begin(BEJIFJIKHPJ.gameObject, JCAFFELIFAO, BEJIFJIKHPJ.transform.localPosition.ReplaceX(172f), false);
	...
	private void LPJPADDLOOH()
	{
		...
		DGEAABIFDDP.color = MEJMLNDFDBP.KFBODMGGLMO(MDAJJIAMDGH.critical, (!(EDCBHGKBLEA == null)) ? EDCBHGKBLEA.critical : MDAJJIAMDGH.critical, true);
		AGKINDPECEN.text = MEJMLNDFDBP.GMIPFLIEOHD(MDAJJIAMDGH.burstSize);
		AGKINDPECEN.color = MEJMLNDFDBP.KFBODMGGLMO(MDAJJIAMDGH.burstSize, (!(EDCBHGKBLEA == null)) ? EDCBHGKBLEA.burstSize : MDAJJIAMDGH.burstSize, true);
		...
		DLBIGOAMMKH.color = MEJMLNDFDBP.KFBODMGGLMO(MDAJJIAMDGH.accuracy, (!(EDCBHGKBLEA == null)) ? EDCBHGKBLEA.accuracy : MDAJJIAMDGH.accuracy, true);
	...
	private void BBPHKDHJKNO()
	{
		if (!FINPLBLOADG.gameObject.activeSelf)
		{
			...
			FINPLBLOADG.gameObject.SetActive(false);
			OJBKADHPNFN = false;
		}
	...
	{
		CFBLIIHPFCE();
		TweenPosition.Begin(EPFODCCNGDL.gameObject, JCAFFELIFAO, EPFODCCNGDL.transform.localPosition.ReplaceX(1205f), false);
	}
	...
	private void PAIDNKHKLAF(UITweener MGDJMGHCAAI)
	{
		EPFODCCNGDL.transform.localPosition = EPFODCCNGDL.transform.localPosition.ReplaceX(HPNPMCEMJNM);
		NDNNJMAKKBD();
		TweenPosition.Begin(EPFODCCNGDL.gameObject, JCAFFELIFAO, EPFODCCNGDL.transform.localPosition.ReplaceX(1704f), false);
	...
	private void HMMDGGMLHKP(GameObject KHAHPAKDIKE)
	{
		...
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivering:
			...
			HDCLFPOIDEF();
			GuiScreenSingle<WeaponScreen>.instance.FHFIPPFJIAH.FLCDIKABDFJ(true);
			break;
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivered:
			Debug.Log("NO");
			ANDKFPNFAFJ();
	...
	private void PHFJIFCBKBB()
	{
		...
		if (weaponLevelsSetup != null)
		{
			...
			else
			{
				...
				GuiScreenSingle<WeaponScreen>.instance.MBKJEPDJFKJ(weaponLevelsSetup, true);
			}
		}
		Singleton<Wallet>.instance.CDIDDJGINID(num2);
		Singleton<Wallet>.instance.MBAFMBJFOKH(num3);
	...
	private void IDINCHPIGPI()
	{
		if (!FINPLBLOADG.gameObject.activeSelf)
		{
			...
			FINPLBLOADG.gameObject.SetActive(true);
			OJBKADHPNFN = true;
		}
	...
	private void CJAJCGPAJCH(UITweener MGDJMGHCAAI)
	{
		...
		CFBLIIHPFCE();
		TweenPosition.Begin(EPFODCCNGDL.gameObject, JCAFFELIFAO, EPFODCCNGDL.transform.localPosition.ReplaceX(340f), false);
	...
	private void HLPDFLOCAJF(UITweener MGDJMGHCAAI)
	{
		...
		CPBLNDIPKNN();
		NDNNJMAKKBD();
		TweenPosition.Begin(BEJIFJIKHPJ.gameObject, JCAFFELIFAO, BEJIFJIKHPJ.transform.localPosition.ReplaceX(38f), false);
	...
	private void IEPOJIONKOG()
	{
		...
		FIJBAGNGAHI.color = MEJMLNDFDBP.KFBODMGGLMO(MDAJJIAMDGH.rateOfFirePerMinute, (!(EDCBHGKBLEA == null)) ? EDCBHGKBLEA.rateOfFirePerMinute : MDAJJIAMDGH.rateOfFirePerMinute, true);
		NHABLPAHLAG.text = ((!(MDAJJIAMDGH.shotVelocity > 661f)) ? Localization.Localize("0") : MEJMLNDFDBP.MAJBCINJAGF(MDAJJIAMDGH.shotVelocity));
		NHABLPAHLAG.color = MEJMLNDFDBP.KFBODMGGLMO(MDAJJIAMDGH.shotVelocity, (!(EDCBHGKBLEA == null)) ? EDCBHGKBLEA.shotVelocity : MDAJJIAMDGH.shotVelocity, true);
	}
	...
	public void KKNDJBAGDAP(bool LGIIPHPACIC = true)
	{
		...
		{
			TweenPosition.Begin(EPFODCCNGDL.gameObject, JCAFFELIFAO, EPFODCCNGDL.transform.localPosition.ReplaceX(DFJENEHLENC()), false).onFinished = delegate
			{
				NDNNJMAKKBD();
				TweenPosition.Begin(EPFODCCNGDL.gameObject, JCAFFELIFAO, EPFODCCNGDL.transform.localPosition.ReplaceX(265f));
			};
		...
		{
			TweenPosition.Begin(mBlackmarketButton.gameObject, JCAFFELIFAO, mBlackmarketButton.transform.localPosition.ReplaceX(HPNPMCEMJNM), false).onFinished = CJAJCGPAJCH;
		}
	...
	private void IKKLKOPEMNK(GameObject KHAHPAKDIKE)
	{
		...
		{
			GuiElementSingle<PackContentDialog>.instance.KNBIBOINAKG(JGBBPCGNCPC.EGFMBMHOAHA(MDAJJIAMDGH.purchasableValue), true);
		}
		else
		{
	...
	private void LAIBHGOBADE(GameObject KHAHPAKDIKE)
	{
		...
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivering:
			...
			HDCLFPOIDEF();
			GuiScreenSingle<WeaponScreen>.instance.FHFIPPFJIAH.NCDHKBKPPMG(true);
			break;
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivered:
	...
	private void FNKNBGDDLNH(GameObject KHAHPAKDIKE)
	{
		...
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivering:
			...
			HDCLFPOIDEF();
			GuiScreenSingle<WeaponScreen>.instance.FHFIPPFJIAH.HJOGMENNJAJ(true);
			break;
		case WeaponLevelsSetup.JLANLLEABDC.WeaponDelivered:
