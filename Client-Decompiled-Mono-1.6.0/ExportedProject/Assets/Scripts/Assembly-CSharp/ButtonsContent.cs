...
public class ButtonsContent : Core_BaseScript
{
	...
	private void OJDKJDJOGGP()
	{
		if (!GuiScreenSingle<EndScreen>.instance.isSinglePlayer && !Singleton<GameController>.instance.isRandomMatchMaking)
		{
			...
			MNEFEENCGOP.text = ((!string.IsNullOrEmpty(matchTimeStatus)) ? string.Format("{0} ({1})", MatchManager.matchStatusAfterMatch, matchTimeStatus) : MatchManager.matchStatusAfterMatch);
		...
		if (Singleton<GameController>.instance.isCoop)
		{
			if (KKIIENBCDPC.gameObject.activeInHierarchy)
			{
				...
				FDDILBBJNMM.text = ((!string.IsNullOrEmpty(matchTimeStatus2)) ? string.Format("{0} ({1})", MatchManager.matchStatusAfterMatch, matchTimeStatus2) : MatchManager.matchStatusAfterMatch);
	...
	private void MAFPGDJMAPD()
	{
		...
		component = TweenPosition.Begin(DFDFBOAKFID, 103f * JCAFFELIFAO, DFDFBOAKFID.transform.localPosition, DFDFBOAKFID.transform.localPosition, false);
	...
	private void KIBKOFBBMDJ()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(DFDFBOAKFID, JCAFFELIFAO, DFDFBOAKFID.transform.localPosition, DFDFBOAKFID.transform.localPosition.ReplaceY(503f), false);
