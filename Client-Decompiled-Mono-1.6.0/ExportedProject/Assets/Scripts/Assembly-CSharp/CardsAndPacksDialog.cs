	...
	private void IMGNEKHBBKD()
	{
		if (BHJPFGKALEN == LJCNLBMEENL.STARTER_PACK)
		{
			...
			if (num < 0)
			{
				...
				array[1] = MEJMLNDFDBP.CJCFPDLDMEK(1859f, "grenadeExplosion", string.Empty, false);
			...
			else
			{
				...
				array2[0] = MEJMLNDFDBP.CJCFPDLDMEK(num, "smallThumbnailLittle", string.Empty, false);
	...
	private void CBOBDHBDJOE()
	{
		if (BHJPFGKALEN == LJCNLBMEENL.MONEY_PACK)
		{
			...
			if (num < 0)
			{
				...
				MEFMGHEILOG.text = string.Format("{0} {1}", Localization.Localize("ID_OFFERACTIVE"), MEJMLNDFDBP.CJCFPDLDMEK(0f, "-", string.Empty, false));
			...
			else
			{
				...
				MEFMGHEILOG.text = string.Format("{0} {1}", Localization.Localize("ID_OFFERACTIVE"), MEJMLNDFDBP.CJCFPDLDMEK(num, "-", string.Empty, false));
			}
		}
		else if (BHJPFGKALEN == LJCNLBMEENL.STARTER_PACK)
		{
			...
			else
			{
				...
				MEFMGHEILOG.text = string.Format("{0} {1}", Localization.Localize("ID_OFFERACTIVE"), MEJMLNDFDBP.CJCFPDLDMEK(num2, "-", string.Empty, false));
			}
		}
	...
	private void GMNALHGIECD()
	{
		if (BHJPFGKALEN == LJCNLBMEENL.VALUE_PACK)
		{
			...
			if (num < 0)
			{
				...
				MEFMGHEILOG.text = string.Format("워프렌즈가 중요한 게임 데이터를 다운로드 받으려면 미디어 저장소에 대한 접근 권한이 필요합니다. 이 권한이 없으면, 게임이 정상적으로 작동되지 않고 종료됩니다. 다시 시도하거나 워프렌즈를 종료해주세요.", Localization.Localize("PlayerStatus"), MEJMLNDFDBP.CJCFPDLDMEK(24f, "ID_COOPCANCELED", string.Empty, false));
				if (!MCGKPDLPDBK && MHAIBHBGBOO())
				{
					HideDialog();
				}
			...
			else
			{
				...
				array2[1] = Colours.stringBlue;
				array2[1] = MEJMLNDFDBP.CJCFPDLDMEK(num, "ID_SELECTFRIENDERRORSHORTNAME", string.Empty, false);
				hOLNKOFHDNK2.text = Localization.LocalizeFormat("Get Experiment Details clicked", array2);
				MEFMGHEILOG.text = string.Format("starter", Localization.Localize("-"), MEJMLNDFDBP.CJCFPDLDMEK(num, "ID_GAMEINVITE", string.Empty, false));
			}
		}
		else if (BHJPFGKALEN == LJCNLBMEENL.STARTER_PACK)
		{
			...
			if (num2 < 0)
			{
				...
				array3[1] = Colours.stringBlue;
				array3[1] = MEJMLNDFDBP.CJCFPDLDMEK(935f, "Error when client tried to work with server response!", string.Empty, false);
				hOLNKOFHDNK3.text = Localization.LocalizeFormat("Value4", array3);
				MEFMGHEILOG.text = string.Format("AndroidAdvertisingID", Localization.Localize(" x "), MEJMLNDFDBP.CJCFPDLDMEK(511f, "ID_GOLDENSHIELDSSMALL", string.Empty, false));
				if (!MCGKPDLPDBK && LHDGJFHPJNM())
				{
					HideDialog();
				}
			...
			else
			{
				...
				array4[0] = Colours.stringBlue;
				array4[1] = MEJMLNDFDBP.CJCFPDLDMEK(num2, "\n\t{0} {1}", string.Empty, false);
				hOLNKOFHDNK4.text = Localization.LocalizeFormat("\n", array4);
				MEFMGHEILOG.text = string.Format("ID_INVITEDYOUTOFIGHT", Localization.Localize("setAndroidIdMd5"), MEJMLNDFDBP.CJCFPDLDMEK(num2, ".jpg", string.Empty, false));
			}
		}
