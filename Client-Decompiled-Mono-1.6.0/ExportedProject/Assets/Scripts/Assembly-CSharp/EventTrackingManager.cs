...
public class EventTrackingManager : Singleton<EventTrackingManager>
{
	...
	public bool isAdVideoPlaying
	{
		get
		{
			return fuseboxxService != null && fuseboxxService.MBLILPODHPD;
		}
	}
	...
	{
		UnityEngine.Debug.Log(string.Format("FuseBox Event VisualGained for rarity {0}", PDEDEOKMDJD));
	...
	{
		UnityEngine.Debug.Log(string.Format("FuseBox Event OpenLootbox for rarity {0}", PDEDEOKMDJD));
	...
	private void AFBHOJNGDAC()
	{
		...
		UnityEngine.Debug.Log(string.Format("FuseBox Event Level Up for level {0}", data.GetPlayerLevel()));
		Hashtable hashtable = new Hashtable();
		hashtable.Add("Total_Sessions", data.numberOfSessions);
	...
	public void RegisterBattleEndedEvent(bool CFLLLHDNHEN, bool EGEMJPLEKOC, int IOMFMJOBEOI, int EHCGJPCDPJJ, int MNCBMEGPIJE)
	{
		...
		if (Singleton<GameController>.instance.isDeathMatch)
		{
			...
			DAAFBKMOHFE("War_Battle_End_Android_Performance", "Tier", string.Format("{0}_processorT{1}_resT{2}", Singleton<MapManager>.instance.currentMap.name, SystemInfo.processorCount, num5), new Hashtable
			{
				{
					"Min_fps",
					AFPSCounter.Instance.fpsCounter.lastMinimumValue.ToString()
	...
	{
		UnityEngine.Debug.Log(string.Format("Event Tracking Manager - Tutorial Game Ended, Deploy clicks: {0}, Deployed units: {1}", StatsManager.instance.matchStats.deployClicks, StatsManager.instance.matchStats.unitsDeployed));
		DAAFBKMOHFE("Tutorial_Step_Complete", "StepId", BOCEKNFGLPC, new Hashtable
		{
			{ "Sniper_Tutorial_Played", 0 },
			{ "Sniper_Tutorial_Duration", 0 },
	...
	{
		UnityEngine.Debug.Log(string.Format("FuseBox Event DuplicateLootbox for {0}", DADJEBGBFMK));
	...
	{
		PlayerAnalytics.PlayerAnalyticsData data = PlayerAnalytics.instance.data;
		UnityEngine.Debug.Log(string.Format("FuseBox Event BuyLootboxes for id {0}", FNGFKECJEGL));
