...
public class MegaRewardPart : Core_BaseScript
{
	...
	public void EGOHMNMEMLM()
	{
		...
		CPFIIHEMEEH(eIPHAAGCHCP, true, false);
	}
	...
	{
		TweenPosition tweenPosition = TweenPosition.Begin(OLPOEPGPJBF.gameObject, 1589f, OLPOEPGPJBF.transform.localPosition.ReplaceY(1678f), false);
	...
	public void Animate()
	{
		...
		float megaRewardProgress = AssignmentsManager.instance.megaRewardProgress;
		NILECLLJFBG(true);
		DANJPGECBCA.text = string.Format("{0} / [FECA21]{1}", currentMegaRewardPoints, megaRewarPoints);
	...
	private void GIOBCHEFFJN()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(OLPOEPGPJBF.gameObject, 362f, new Vector3(OLPOEPGPJBF.transform.localPosition.x, 1776f, OLPOEPGPJBF.transform.localPosition.z), false);
		tweenPosition.method = UITweener.Method.EaseIn;
		tweenPosition.onFinished = OHOLHLHNPLP;
	}
	...
	{
		DANJPGECBCA.text = string.Format("{0} / [FECA21]{1}[-]", AssignmentsManager.instance.currentMegaRewardPoints, AssignmentsManager.instance.megaRewarPoints);
