...
public class RentalDialog : GuiElementSingle<RentalDialog>, PAIIOKBBHBC
{
	...
	private void FODJOKGKDCJ()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 882f, new Vector3(183f, 976f, 1782f), new Vector3(1079f, 407f, 1916f), false);
	...
	private void EHLPFIKFJJP()
	{
		...
		if (MDAJJIAMDGH == null)
		{
			Debug.LogError(string.Format("Weapon {0} does not exists on client.", KKJCAACJNHG));
			return;
		}
	...
	private void MLKCMOIMKJM(GameObject KHAHPAKDIKE)
	{
		if (AFOFDNJOJHG())
		{
			Singleton<BeanstalkServerManager>.instance.NCKODKEJGHD(false);
			switch (PPBJNCCFBJG)
			{
			case DKHAOLDJLMK.Weapon:
				MDAJJIAMDGH.tryOutWeapon = false;
				LevelManager.instance.EquipRentalWeapon(MDAJJIAMDGH, true);
				break;
			case DKHAOLDJLMK.ArmyUnit:
				...
				break;
			case (DKHAOLDJLMK)8:
				FGIMNJOOAOB.EFPABPANAJE(true);
				LevelManager.instance.EquipRentalVisual(FGIMNJOOAOB, true);
				break;
			}
	...
	private void PEDJFACMHHF(GameObject KHAHPAKDIKE)
	{
		...
		Singleton<BeanstalkServerManager>.instance.AcceptRentalOffer(false);
		DKHAOLDJLMK pPBJNCCFBJG = PPBJNCCFBJG;
		if (pPBJNCCFBJG != DKHAOLDJLMK.ArmyUnit)
		{
			switch (pPBJNCCFBJG)
		...
		{
			MDAJJIAMDGH.tryOutWeapon = false;
			LevelManager.instance.EquipRentalWeapon(MDAJJIAMDGH, false);
		}
		...
		if (pPBJNCCFBJG != DKHAOLDJLMK.ArmyUnit)
		{
			if (pPBJNCCFBJG != DKHAOLDJLMK.ArmyUnit)
			{
				if (pPBJNCCFBJG == DKHAOLDJLMK.ArmyUnit)
				{
					...
					LevelManager.instance.EquipRentalVisual(FGIMNJOOAOB, false);
				}
			}
			else
			{
	...
	private void HLILIDEAFNC()
	{
		...
		if (IFFDIHCPKFE == null)
		{
			Debug.LogError(string.Format("Unit {0} does not exists on client.", KKJCAACJNHG));
			return;
		}
		...
		NGMDHEGFIKE[1].InitializeStat(IFFDIHCPKFE, false);
	...
	private void JDFHFNGHCLG()
	{
		...
		if (FGIMNJOOAOB == null)
		{
			Debug.LogError(string.Format("Power Band {0} does not exists on client.", KKJCAACJNHG));
			return;
		}
		KKBPMPEBKIL = FGIMNJOOAOB.priceWarbucks + FGIMNJOOAOB.priceGold;
	...
	private void LJNAKBBJIIC()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 1430f, new Vector3(1212f, 791f, 198f), new Vector3(1982f, 1286f, 1324f), false);
		tweenPosition.method = UITweener.Method.Linear;
		tweenPosition.onFinished = delegate
		{
			TweenPosition tweenPosition2 = TweenPosition.Begin(CDJJCFKGGNM.gameObject, 0.15f, new Vector3(478f, -180f, -5f));
	...
	{
		Debug.Log(string.Format("Rental Dialog called: type: \"{0}, \"ID: \"{1}\", discount: \"{2}%\", is sale: \"{3}\"", PPBJNCCFBJG, KKJCAACJNHG, OKGNPCBCIDN, BHCBHPPNNBI));
