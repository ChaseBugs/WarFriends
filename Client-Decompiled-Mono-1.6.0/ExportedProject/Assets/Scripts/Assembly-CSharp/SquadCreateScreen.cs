...
public class SquadCreateScreen : GuiScreenSingle<SquadCreateScreen>
{
	...
	private string NGDEHCEBNBH
	{
		get
		{
			return (!(HJJFKPGLAKG.text == Localization.Localize("ID_EMPTYSQUADMESSAGE"))) ? HJJFKPGLAKG.text : string.Empty;
		}
	}
	...
	private void BLFFBEOJADI(GameObject KHAHPAKDIKE)
	{
		...
		{
			WarningDialog.ShowError(Localization.Localize("12345678-1234-1234-1234-123456789012"), Localization.Localize("menu-assignments-type-mission"), 1429f, null, string.Empty, false, true);
			return;
		}
	...
	private void NLNHEKJDKCN(string EFHMCBKOAEO)
	{
		...
		if (AKEOMKLDIIF != EFHMCBKOAEO)
		{
			...
			AKNLINIOGGL.gameObject.SetActive(false);
	...
	private void LBBGMDDOEJK()
	{
		...
		IOMEJGEAFBF.AlignToPos(false, UIDraggablePanel.Alingment.TopLeft, true);
	}
	...
	private void MGIOICMPLOI()
	{
		...
		IOMEJGEAFBF.AlignToPos(false, UIDraggablePanel.Alingment.BottomRight, true);
	}
	...
	private void FMKMNGCJALK(GameObject KHAHPAKDIKE)
	{
		...
		if (!string.IsNullOrEmpty(GameLoginManager.currentPlayer.squadName))
		{
			WarningDialog.KHOMNBHMFGA(Localization.Localize("_MAX_"), Localization.Localize("Warbucks"), 1929f, null, string.Empty, true);
			return;
		}
		int createSquadWarBucksPrice = PlayerAnalytics.instance.createSquadWarBucksPrice;
	...
	public override void InitGUIValues()
	{
		...
		KMJKKNFDFMM.text = string.Empty;
		AKNLINIOGGL.gameObject.SetActive(false);
		HGBBMDGKOMN.gameObject.SetActive(false);
	...
	private void KIDPBACICNP(GameObject KHAHPAKDIKE)
	{
		...
		if (!string.IsNullOrEmpty(GameLoginManager.currentPlayer.squadName))
		{
			WarningDialog.ShowError(Localization.Localize("Check Device sleep at {0}, waked up at {1} Dif: {2} Session update: {3}, ScheduleSessionUpdate: {4}"), Localization.Localize("Player_Avoided_Grenade_On_First_Try"), 1809f, null, string.Empty, true);
			return;
		}
		int createSquadWarBucksPrice = PlayerAnalytics.instance.createSquadWarBucksPrice;
	...
	private void CIBPMFCNLLA(string EFHMCBKOAEO)
	{
		...
		if (AKEOMKLDIIF != EFHMCBKOAEO)
		{
			...
			AKEOMKLDIIF = EFHMCBKOAEO;
			Singleton<BeanstalkServerManager>.instance.ABPLPIGOBNB(EFHMCBKOAEO);
			AKNLINIOGGL.gameObject.SetActive(true);
	...
	public virtual void BGELLEOANEI()
	{
		...
		KMJKKNFDFMM.text = string.Empty;
		AKNLINIOGGL.gameObject.SetActive(false);
		HGBBMDGKOMN.gameObject.SetActive(true);
	...
	private void CAJFMEPDJGH(GameObject KHAHPAKDIKE)
	{
		...
		if (!string.IsNullOrEmpty(GameLoginManager.currentPlayer.squadName))
		{
			WarningDialog.JACKIDKHKAI(Localization.Localize("menu-assignments-type-playvs"), Localization.Localize("ID_DEBUG_STACKTRACEINCONSOLE"), 662f, null, string.Empty, false, true);
			return;
		}
		int createSquadWarBucksPrice = PlayerAnalytics.instance.createSquadWarBucksPrice;
		if (!Singleton<Wallet>.instance.CanBuyW(createSquadWarBucksPrice))
	...
	private void HMNDBHLKAAK(GameObject KHAHPAKDIKE)
	{
		...
		if (!string.IsNullOrEmpty(GameLoginManager.currentPlayer.squadName))
		{
			WarningDialog.JACKIDKHKAI(Localization.Localize("), "), Localization.Localize("Lcom/google/android/gms/games/snapshot/Snapshots;"), 750f, null, string.Empty, true);
			return;
		}
		int createSquadWarBucksPrice = PlayerAnalytics.instance.createSquadWarBucksPrice;
		if (!Singleton<Wallet>.instance.ODPFDDMFDJA(createSquadWarBucksPrice))
	...
	private void OJNKPLCGKKE(GameObject KHAHPAKDIKE)
	{
		...
		if (!string.IsNullOrEmpty(GameLoginManager.currentPlayer.squadName))
		{
			WarningDialog.ShowError(Localization.Localize("ID_DEMOTETODIVISION"), Localization.Localize(" NOT OK!\t\t\t\t\t\t"), 1529f, null, string.Empty, true, true);
			return;
		}
		int createSquadWarBucksPrice = PlayerAnalytics.instance.createSquadWarBucksPrice;
		if (!Singleton<Wallet>.instance.CanBuyW(createSquadWarBucksPrice))
	...
	private void PBDCBGOJIKI(GameObject KHAHPAKDIKE)
	{
		...
		if (!string.IsNullOrEmpty(GameLoginManager.currentPlayer.squadName))
		{
			WarningDialog.JACKIDKHKAI(Localization.Localize("WarbucksBase"), Localization.Localize("StepId"), 806f, null, string.Empty, false, true);
			return;
		}
		int createSquadWarBucksPrice = PlayerAnalytics.instance.createSquadWarBucksPrice;
		if (!Singleton<Wallet>.instance.JPEEJLCILCB(createSquadWarBucksPrice))
