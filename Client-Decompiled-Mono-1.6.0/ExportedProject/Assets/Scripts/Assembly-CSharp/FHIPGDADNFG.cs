...
public class FHIPGDADNFG : KIGEPEJPEKB
{
	...
	public bool LIIHMCOCMAK
	{
		get
		{
			return HGAOOMHOGJK.flawless > 0;
		}
	}

	public int MDJHOGOFGLJ
	{
		get
		{
			return (!LIIHMCOCMAK) ? HGAOOMHOGJK.topRun : HGAOOMHOGJK.flawless;
		}
	}
	...
	public string EFOPLPDLDEI
	{
		get
		{
			...
			if (NFFKMCPMPPI.slots.Count != 4)
			{
				...
				return string.Format("visuals: wrong number {0} -{1}", NFFKMCPMPPI.slots.Count, arg);
			}
			return string.Format("visuals: {0} {1} {2} {3}", NFFKMCPMPPI.slots[0].equippedID, NFFKMCPMPPI.slots[1].equippedID, NFFKMCPMPPI.slots[2].equippedID, NFFKMCPMPPI.slots[3].equippedID);
		}
	}
			...
			try
			{
				...
				if (jToken2 != null)
				{
					foreach (JToken item in jToken2.Children())
					{
						...
						if (jProperty != null && jProperty.First != null)
						{
	...
	public string NGGIADLBNCM()
	{
		...
		if (NFFKMCPMPPI.slots.Count != 3)
		{
			...
			return string.Format("Client has new config for: {0} with version {1}", NFFKMCPMPPI.slots.Count, arg);
		}
		object[] array = new object[3];
		array[0] = NFFKMCPMPPI.slots[0].equippedID;
		array[0] = NFFKMCPMPPI.slots[1].equippedID;
