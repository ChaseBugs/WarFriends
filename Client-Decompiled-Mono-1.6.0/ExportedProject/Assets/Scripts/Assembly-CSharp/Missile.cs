...
public class Missile : PhysicalAmmo
{
	...
	{
		ILCMFMCIOLM.SetActive(true);
		switch (EIIMIEGIOPN.CJKLGCEPKGO)
		{
		case MissileSetup.JANBEEJAKMD.RpgBasic:
			BLKDOPLONOM.SetActive(true);
			GEPPEKGCPBD.SetActive(false);
			...
			MCHCHMHGFGA.SetActive(false);
			break;
		case MissileSetup.JANBEEJAKMD.Tank:
			BLKDOPLONOM.SetActive(true);
			GEPPEKGCPBD.SetActive(true);
			...
			MCHCHMHGFGA.SetActive(true);
			break;
		case MissileSetup.JANBEEJAKMD.Small:
			BLKDOPLONOM.SetActive(false);
			GEPPEKGCPBD.SetActive(true);
	...
	public virtual void OGKGIBDMNPI()
	{
		...
		StopAllCoroutines();
		JDGADCDMFHM = null;
		ILCMFMCIOLM.gameObject.SetActive(true);
		JCKOBELDCGD = false;
	}
	...
	private void CCAHEOEDCIB(Vector3 FAPCLFPNKDE)
	{
		if (!PFKPCFCEDME)
		{
			...
			FFPLNCHHFDP.Stop();
			ILCMFMCIOLM.SetActive(true);
			MIDOLDHLMAF((!CAKABHIPCDF) ? 1264f : 1558f, true);
			JCKOBELDCGD = true;
		}
	}
	...
	public override void OnInstancied()
	{
		...
		StopAllCoroutines();
		JDGADCDMFHM = null;
		ILCMFMCIOLM.gameObject.SetActive(false);
		JCKOBELDCGD = false;
	}
