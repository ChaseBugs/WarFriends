...
public class CardSoldierOnSteroids : Card
{
	...
	private float BADJICBCOIM
	{
		get
		{
			return Singleton<GameVariables>.instance.cardConstants.GetRow(CardConstants.rowIds.SoldierOnSteroidsCoef).FLOATVALUE;
		}
	}
	...
	public override string description
	{
		get
		{
			return Localization.LocalizeFormat(mDescriptionID, MEJMLNDFDBP.JJBMMICPIIG(BADJICBCOIM));
		}
	}
	...
	private void EPOHPGJBHAP(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.BIIHCAEKCOD(" TAB ", true);
				KPJLBGMLOFL = false;
			}
		}
	}
	...
	private void KOGGBBDCDFK(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.Show("game-card-ico-meatheads-full", true);
				KPJLBGMLOFL = true;
			}
		}
	}
	...
	private void PHCFCGIHFID(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.JCJFMHGNIOF("ID_ERROR_SQUADMESSAGELONG", false, true);
				KPJLBGMLOFL = true;
			}
		}
	}
	...
	private void NFJIALFNCPP(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.NNPILDOKNJO("ID_FEATURE_RATE-DESCRIPTION", false);
				KPJLBGMLOFL = false;
			}
		}
	}
	...
	private void FOOGOGMDEML(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.FJILILFMPIF("Squad Emblem ", false);
				KPJLBGMLOFL = false;
			}
		}
	}
	...
	private void AFBNLMFJPBK(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.GCLFBCIKNAH("PrepareBotForDeathMatch 1", false);
				KPJLBGMLOFL = false;
			}
		}
	}
	...
	private void HPAMNBICLHF(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.NNPILDOKNJO("ID_LEAGUESTAYHINT2", false, true);
				KPJLBGMLOFL = true;
			}
		}
	}
	...
	private void AFMFPIAEOJE(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.GGHDIAFACAH("Scraps", true);
				KPJLBGMLOFL = false;
			}
		}
	}
	...
	private void KHKHDJDECJL(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.GCLFBCIKNAH(" is selected: ", true);
				KPJLBGMLOFL = false;
			}
		}
	}
	...
	private void PLJLMJGPHEK(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.GKAOMCGCDGE("Result for action:{0} on global:{1}", false, true);
				KPJLBGMLOFL = true;
			}
		}
	}
	...
	private void NFKOAEILALE(AIObject KLHPCPKJGOC)
	{
		if (JAOOMCLDJFD && KLHPCPKJGOC.fraction == ODCLPMADGFC)
		{
			...
			if (enemyController != null && enemyController.canBeFreezed)
			{
				...
				enemyController.cardIconIndicator.FJILILFMPIF("[CFX_SpawnSystem.removeObjectsFromPool()] There aren't any preloaded object for: ", true);
				KPJLBGMLOFL = true;
			}
		}
	}
