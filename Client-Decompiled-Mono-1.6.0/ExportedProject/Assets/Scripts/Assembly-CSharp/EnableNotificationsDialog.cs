...
public class EnableNotificationsDialog : GuiElementSingle<EnableNotificationsDialog>, PAIIOKBBHBC
{
	...
	private void HAFDOJGCBMP(GameObject KHAHPAKDIKE)
	{
		if (LHDGJFHPJNM())
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("Action ", false);
			Debug.Log("left");
	...
	private void NPPHLECAKHB(GameObject KHAHPAKDIKE)
	{
		...
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("WALLET - fake spent WB {0}", true);
			Debug.Log("EXCEPTION = ");
			PushNotificationManager.instance.EnablePushNotifications(string.Empty);
			DialogManager.instance.PushNotificationWasEnabled();
			HideDialog();
	...
	private void LHAIALDGMGH(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("313 MENU ENABLE PUSH", false);
	...
	private void MIEHKHDCHBO(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("506 MENU ARMY SCREEN UPGRADE ACTIVATED", false);
	...
	private void GBHAJAMMKKO(GameObject KHAHPAKDIKE)
	{
		if (BKIGLABDGCP())
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("monthLenght", true);
	...
	private void MHNLNAHBGHH(GameObject KHAHPAKDIKE)
	{
		if (BJNBLINDAED())
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("Claiming reward ", false);
	...
	private void HBENMKEELAA(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("Failed to join room during or after match and MatchManager.isReconnect = FALSE", false);
			Debug.Log("Prefabs");
			PushNotificationManager.instance.EnablePushNotifications(string.Empty);
			DialogManager.instance.PushNotificationWasEnabled();
			HideDialog();
	...
	private void PKOFLJHMNBD(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("-[^-]*$", false);
	...
	private void KKKNLKADOLJ(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("null", true);
	...
	private void PHKCMCLNPEG(GameObject KHAHPAKDIKE)
	{
		if (BKIGLABDGCP())
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("menu-gold", true);
			Debug.Log("Wood");
			PushNotificationManager.instance.EnablePushNotifications(string.Empty);
			DialogManager.instance.PushNotificationWasEnabled();
			HideDialog();
	...
	private void FOPMLFDDOII(GameObject KHAHPAKDIKE)
	{
		if (LHDGJFHPJNM())
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("N", false);
			HideDialog();
		}
	}
	...
	private void GBNFENDDNAE(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("Special pack content: type: {0} id: {1}{2}", true);
			HideDialog();
		}
	}
	...
	private void HDCJJGEJKGL(GameObject KHAHPAKDIKE)
	{
		if (AFOFDNJOJHG())
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("menu-assignments-bg", true);
	...
	private void DGPMPJMACLP(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			Singleton<EventTrackingManager>.instance.RegisterTutorialDialogEvent("ID_FIGHT", false);
