...
public class UpgradesManager : Singleton<UpgradesManager>
{
		...
		public bool BCFKDABEHHO()
		{
			...
			default:
				...
				if (FDCGIINEHPB < LevelManager.instance.behaviours.Count)
				{
					...
					PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(BFFOCDLAJMC.PrepareVisualsForGameCoroutine(NDOIPCDEIKG.unitUpgrades, NDOIPCDEIKG.isEquipped, false));
					if (!FDKOKMKIIMG)
					{
						CLPPIAGLCJJ = 1;
					}
		...
		public bool HJHJCCFGJIH()
		{
			...
			default:
				...
				if (FDCGIINEHPB < LevelManager.instance.behaviours.Count)
				{
					BFFOCDLAJMC = LevelManager.instance.behaviours[FDCGIINEHPB];
					NDOIPCDEIKG = KBBDGCLPOCJ[FDCGIINEHPB];
					PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(BFFOCDLAJMC.PrepareVisualsForGameCoroutine(NDOIPCDEIKG.unitUpgrades, NDOIPCDEIKG.isEquipped, false, true));
					if (!FDKOKMKIIMG)
					{
						CLPPIAGLCJJ = 0;
					}
		...
		public bool HMLAEHJKPJE()
		{
			...
			default:
				...
				if (FDCGIINEHPB < LevelManager.instance.behaviours.Count)
				{
					BFFOCDLAJMC = LevelManager.instance.behaviours[FDCGIINEHPB];
					NDOIPCDEIKG = KBBDGCLPOCJ[FDCGIINEHPB];
					PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(BFFOCDLAJMC.PrepareVisualsForGameCoroutine(NDOIPCDEIKG.unitUpgrades, NDOIPCDEIKG.isEquipped, true, true));
					if (!FDKOKMKIIMG)
					{
						CLPPIAGLCJJ = 0;
					}
		...
		public bool PJJPPMIBNLN()
		{
			...
			default:
				...
				if (FDCGIINEHPB < LevelManager.instance.behaviours.Count)
				{
					BFFOCDLAJMC = LevelManager.instance.behaviours[FDCGIINEHPB];
					NDOIPCDEIKG = KBBDGCLPOCJ[FDCGIINEHPB];
					PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(BFFOCDLAJMC.PrepareVisualsForGameCoroutine(NDOIPCDEIKG.unitUpgrades, NDOIPCDEIKG.isEquipped, true));
					if (!FDKOKMKIIMG)
					{
						CLPPIAGLCJJ = 0;
					}
		...
		public bool DDLMLOJGJMA()
		{
			...
			default:
				...
				if (FDCGIINEHPB < LevelManager.instance.behaviours.Count)
				{
					...
					PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(BFFOCDLAJMC.PrepareVisualsForGameCoroutine(NDOIPCDEIKG.unitUpgrades, HMPNHIBCGKD, true, true));
					if (!FDKOKMKIIMG)
					{
						CLPPIAGLCJJ = 1;
					}
		...
		public bool MoveNext()
		{
			...
			default:
				...
				if (FDCGIINEHPB < LevelManager.instance.behaviours.Count)
				{
					...
					PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(BFFOCDLAJMC.PrepareVisualsForGameCoroutine(NDOIPCDEIKG.unitUpgrades, HMPNHIBCGKD, false));
					if (!FDKOKMKIIMG)
					{
						CLPPIAGLCJJ = 1;
					}
	...
	public void LMLELPFGNHG()
	{
		foreach (LevelBehaviour behaviour in LevelManager.instance.behaviours)
		{
			...
			behaviour.upgradeSlots.NullCardVisuals(true, true);
			behaviour.Unload();
		}
	}
	...
	public void EDKEHIEOCIG()
	{
		foreach (LevelBehaviour behaviour in LevelManager.instance.behaviours)
		{
			behaviour.upgradeSlots.NullMineVisuals();
			behaviour.upgradeSlots.NullOpponentVisuals();
			behaviour.upgradeSlots.NullCardVisuals(false, true);
			behaviour.Unload();
		}
	}
	...
	public void BGGIGDFAHPM()
	{
		foreach (LevelBehaviour behaviour in LevelManager.instance.behaviours)
		{
			behaviour.upgradeSlots.NullMineVisuals();
			behaviour.upgradeSlots.NullOpponentVisuals();
			behaviour.upgradeSlots.NullCardVisuals(true, false);
			behaviour.Unload();
		}
	}
	...
	public void AFMPNNPHFPK()
	{
		foreach (LevelBehaviour behaviour in LevelManager.instance.behaviours)
		{
			behaviour.upgradeSlots.NullMineVisuals();
			behaviour.upgradeSlots.NullOpponentVisuals();
			behaviour.upgradeSlots.NullCardVisuals(false, false);
			behaviour.Unload();
		}
	}
