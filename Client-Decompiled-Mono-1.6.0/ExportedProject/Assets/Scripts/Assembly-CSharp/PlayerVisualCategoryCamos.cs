	...
	public class PlayerVisualCamo : PlayerVisual
	{
		...
		public SkinnedMeshRenderer JGLGCBIJFNL()
		{
			if (mMeshBody == null)
			{
				...
				if (gameObject != null)
				{
					...
					{
						Debug.LogError(string.Format("PlayerVisualCamo Error - SkinnedMeshRenderer count is 0 at {0}!", modelPath));
					}
				}
				else
				{
					Debug.LogError(string.Format("Camo load error: {0}", modelPath));
				}
		...
		public virtual void LAOELJCICBN(KMNCIJPDFPJ DGJFKDADPHE, bool OGKCIFGLKJJ = false)
		{
			...
			if (Application.isPlaying)
			{
				...
				{
					DGJFKDADPHE.equippedHeadAccesory.HGGLCMFPPIB(DGJFKDADPHE, true);
				}
		...
		public SkinnedMeshRenderer EEPAACGMAGL()
		{
			if (mMeshBody == null)
			{
				...
				if (gameObject != null)
				{
					SkinnedMeshRenderer[] componentsInChildren = gameObject.GetComponentsInChildren<SkinnedMeshRenderer>(false);
					if (componentsInChildren.Length == 0)
					{
		...
		{
			MeshFilter[] componentsInChildren = NPAMLHLLIKC.GetComponentsInChildren<MeshFilter>(true);
		...
		{
			MeshFilter[] componentsInChildren = NPAMLHLLIKC.GetComponentsInChildren<MeshFilter>(false);
			hairMeshes = new List<MeshFilter>();
			MeshFilter[] array = componentsInChildren;
			foreach (MeshFilter meshFilter in array)
			{
		...
		public SkinnedMeshRenderer PMJEBJCPKJA()
		{
			if (mPreviewMeshBody == null)
			{
				...
				if (gameObject != null)
				{
					...
					{
						Debug.LogError(string.Format("PlayerVisualCamo Error - SkinnedMeshRenderer count is 0 at {0}!", previewModelPath));
					}
				}
				else
				{
					Debug.LogError(string.Format("Camo load error: {0}", previewModelPath));
				}
				FFDOGJEAPFA(gameObject);
			}
			return mPreviewMeshBody;
		...
		public virtual void ABKPCPCCCPD(KMNCIJPDFPJ DGJFKDADPHE, bool OGKCIFGLKJJ = false)
		{
			...
			if (Application.isPlaying)
			{
				...
				if (DGJFKDADPHE.equippedHeadAccesory != null)
				{
					DGJFKDADPHE.equippedHeadAccesory.MNEEDLCGAFN(DGJFKDADPHE, true);
				}
				if (DGJFKDADPHE.equippedPowerBand != null)
				{
		...
		public SkinnedMeshRenderer BEGPEOCKPPK()
		{
			if (mPreviewMeshBody == null)
			{
				...
				{
					Debug.LogError(string.Format("Squad icon: {0} already displayed", previewModelPath));
				}
				LOLKCEHHJAF(gameObject);
			}
			return mPreviewMeshBody;
		...
		public virtual void KGACOPBGFOA(KMNCIJPDFPJ DGJFKDADPHE, bool OGKCIFGLKJJ = false)
		{
			...
			if (Application.isPlaying)
			{
				...
				if (DGJFKDADPHE.equippedPowerBand != null)
				{
					DGJFKDADPHE.equippedPowerBand.MNEEDLCGAFN(DGJFKDADPHE, true);
				}
			}
		}
		...
		public virtual void JDCPEMMKMJA(KMNCIJPDFPJ DGJFKDADPHE, bool OGKCIFGLKJJ = false)
		{
			...
			if (Application.isPlaying)
			{
				...
				if (DGJFKDADPHE.equippedHelmet != null)
				{
					DGJFKDADPHE.equippedHelmet.HGGLCMFPPIB(DGJFKDADPHE, true);
				}
				if (DGJFKDADPHE.equippedHeadAccesory != null)
				{
					DGJFKDADPHE.equippedHeadAccesory.HGGLCMFPPIB(DGJFKDADPHE, true);
				...
				if (DGJFKDADPHE.equippedPowerBand != null)
				{
					DGJFKDADPHE.equippedPowerBand.HGGLCMFPPIB(DGJFKDADPHE, true);
				}
			}
		}
		...
		public SkinnedMeshRenderer GBOFHMKJFEG()
		{
			if (mMeshBody == null)
			{
				...
				if (gameObject != null)
				{
					...
					{
						Debug.LogError(string.Format("JoinNowSquadMessage-{0}", modelPath));
					}
				}
				else
				{
