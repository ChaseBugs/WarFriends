...
public class UIFont : MonoBehaviour
{
	...
	public int size
	{
		get
		{
			return (mReplacement != null) ? mReplacement.size : ((!isDynamic) ? mFont.charSize : mAlteredDynamicSize);
		}
	}
	...
	private int mAlteredDynamicSize
	{
		get
		{
			return AlterFontHeight(mDynamicFontSize);
		}
	}
	...
	private void Trim()
	{
		...
		if (texture != null && mSprite != null)
		{
			...
			Rect rect2 = ((mAtlas.coordinates == UIAtlas.Coordinates.Pixels) ? mSprite.outer : NGUIMath.ConvertToPixels(mSprite.outer, texture.width, texture.height, true));
	...
	public float RecalculateDynamicOffset(int fontSize)
	{
		...
		if (mDynamicFont != null && !mDynamicFontOffsets.TryGetValue(fontSize, out value))
		{
			...
			CharacterInfo info;
			mDynamicFont.GetCharacterInfo('j', out info, fontSize, mDynamicFontStyle);
			float num = info.vert.yMin + 0.02f * info.vert.yMax;
			...
			for (int i = 0; i < length; i++)
			{
				...
				else
				{
					...
					if (!mDynamicFont.GetCharacterInfo(c, out mChar, printDynamicFontSize, mDynamicFontStyle))
					{
						...
						{
							Debug.LogError(string.Format("Character {0} could not be found in font texture!", c));
							continue;
						}
					}
