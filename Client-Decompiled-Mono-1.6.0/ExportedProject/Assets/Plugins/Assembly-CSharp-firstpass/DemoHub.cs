...
internal class DemoHub : Hub
{
	...
	public void TaskWithException()
	{
		...
		{
			taskWithExceptionResult = string.Format("Error: {0}", error.ErrorMessage);
		});
	}
	...
	public void GenericTaskWithException()
	{
		...
		{
			genericTaskWithExceptionResult = string.Format("Error: {0}", error.ErrorMessage);
		});
	}
	...
	public void SynchronousException()
	{
		...
		{
			synchronousExceptionResult = string.Format("Error: {0}", error.ErrorMessage);
		});
	}
	...
	public void PassingDynamicComplex(object person)
	{
		...
		{
			invokingHubMethodWithDynamicResult = string.Format("The person's age is {0}", result.ReturnValue);
		}, person);
	}
