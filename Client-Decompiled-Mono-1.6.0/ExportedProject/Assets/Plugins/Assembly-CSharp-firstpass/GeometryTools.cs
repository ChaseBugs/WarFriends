...
public static class GeometryTools
{
	...
	public static Vector3[] BaseballFieldShape(float angle, float radius)
	{
		...
		array[num + 1] = 0.7f * (Quaternion.AngleAxis(1f * angle, Vector3.up) * Vector3.forward);
		array[num + 2] = 0.7f * (Quaternion.AngleAxis(-1f * angle, Vector3.up) * Vector3.forward);
		array[num + 3] = radius * 0.99f * (Quaternion.AngleAxis(-1f * angle, Vector3.up) * Vector3.forward);
		return array;
	}
