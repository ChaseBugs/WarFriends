...
namespace Org.BouncyCastle.Asn1
{
	public class Asn1StreamParser
	{
		...
		internal Asn1Object ReadTaggedObject(bool constructed, int tag)
		{
			...
			if (_in is IndefiniteLengthInputStream)
			{
				return (asn1EncodableVector.Count != 1) ? new BerTaggedObject(false, tag, BerSequence.FromVector(asn1EncodableVector)) : new BerTaggedObject(true, tag, asn1EncodableVector[0]);
			}
			return (asn1EncodableVector.Count != 1) ? new DerTaggedObject(false, tag, DerSequence.FromVector(asn1EncodableVector)) : new DerTaggedObject(true, tag, asn1EncodableVector[0]);
		}
