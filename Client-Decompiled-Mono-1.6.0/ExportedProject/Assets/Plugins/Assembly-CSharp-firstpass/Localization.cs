...
public class Localization : MonoBehaviour
{
	...
	public static bool isEnglish
	{
		get
		{
			return instance.mLanguage.ToLower() == "en";
		}
	}

	public static float languageConstant
	{
		get
		{
			return (!(instance == null) && languageMultiplier != null && !string.IsNullOrEmpty(instance.currentLanguage) && languageMultiplier.ContainsKey(instance.currentLanguage)) ? languageMultiplier[instance.currentLanguage] : 1f;
		}
	}
	...
	private TextAsset GetAssetForLanguage(string language)
	{
		...
		if (textAsset == null)
		{
			Debug.LogError(string.Format("Can not find text asset for {0}", language));
		}
		return textAsset;
	}
	...
	public static string LocalizeFormat(string keyFormat, params object[] args)
	{
		...
		if (text == keyFormat)
		{
			AnalyticsHelper.LeaveBreadcrumb(string.Format("Localization do not exists or is the same as key: {0}", keyFormat));
			return keyFormat;
		}
		try
		{
		...
		{
			AnalyticsHelper.LeaveBreadcrumb(string.Format("Translation ID {0} contains errors ({1}).", keyFormat, text));
