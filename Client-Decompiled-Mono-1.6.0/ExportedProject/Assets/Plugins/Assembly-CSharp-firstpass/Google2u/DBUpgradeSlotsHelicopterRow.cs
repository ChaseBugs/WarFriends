...
{
	[Serializable]
	public class DBUpgradeSlotsHelicopterRow : IGoogle2uRow
	{
		public int TIER;
		...
		public DBUpgradeSlotsHelicopterRow()
		{
		}

		public DBUpgradeSlotsHelicopterRow(string _GOOGLEFU_ID, string _TIER, string _HP, string _DAMAGE, string _BATCHSIZEMIN, string _BATCHSIZEMAX, string _SHOTFREQUENCYMIN, string _SHOTFREQUENCYMAX, string _REALSHOTPROBABILITY, string _SEATS, string _NEXTUPGRADEPRICE, string _NEXTUPGRADEPRICEGOLD, string _DELIVERYTIME, string _DPS, string _ARMYPOWER, string _SPECIAL, string _SOLDIERHP, string _ELITE)
		{
			int result;
			...
			int result9;
			if (int.TryParse(_SEATS, out result9))
			{
				SEATS = result9;
			}
			else
			{
				Debug.LogError("Failed To Convert SEATS string: " + _SEATS + " to int");
			}
			int result10;
			if (int.TryParse(_NEXTUPGRADEPRICE, out result10))
			{
			...
			int result11;
			if (int.TryParse(_NEXTUPGRADEPRICEGOLD, out result11))
			{
			...
			float result12;
			if (float.TryParse(_DELIVERYTIME, out result12))
			{
			...
			float result13;
			if (float.TryParse(_DPS, out result13))
			{
				DPS = result13;
			}
			else
			...
			float result14;
			if (float.TryParse(_ARMYPOWER, out result14))
			{
				ARMYPOWER = result14;
			}
			else
			...
			float result15;
			if (float.TryParse(_SPECIAL, out result15))
			{
				SPECIAL = result15;
			}
			else
			...
			float result16;
			if (float.TryParse(_SOLDIERHP, out result16))
			{
				SOLDIERHP = result16;
			}
			else
