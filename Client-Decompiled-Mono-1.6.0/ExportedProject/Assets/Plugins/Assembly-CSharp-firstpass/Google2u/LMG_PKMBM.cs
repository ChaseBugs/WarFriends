...
namespace Google2u
{
	public class LMG_PKMBM : Google2uComponentBase, IGoogle2uDB
	{
		public enum rowIds
		{
			ROW_1 = 0,
		...
		public List<LMG_PKMBMRow> Rows = new List<LMG_PKMBMRow>();
		...
		{
			Rows.Add(new LMG_PKMBMRow(input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7], input[8], input[9], input[10]));
		}
		...
		public LMG_PKMBMRow GetRow(rowIds in_RowID)
		{
			LMG_PKMBMRow result = null;
			try
			{
				result = Rows[(int)in_RowID];
			}
		...
		public LMG_PKMBMRow GetRow(string in_RowString)
		{
			LMG_PKMBMRow result = null;
