...
namespace Google2u
{
	public class LMG_GSh : Google2uComponentBase, IGoogle2uDB
	{
		public enum rowIds
		{
			ROW_1 = 0,
		...
		public List<LMG_GShRow> Rows = new List<LMG_GShRow>();
		...
		{
			Rows.Add(new LMG_GShRow(input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7], input[8]));
		}
		...
		public LMG_GShRow GetRow(rowIds in_RowID)
		{
			LMG_GShRow result = null;
			try
			{
				result = Rows[(int)in_RowID];
			}
		...
		public LMG_GShRow GetRow(string in_RowString)
		{
			LMG_GShRow result = null;
