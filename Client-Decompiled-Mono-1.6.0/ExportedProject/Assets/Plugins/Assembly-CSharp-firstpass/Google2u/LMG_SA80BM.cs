...
namespace Google2u
{
	public class LMG_SA80BM : Google2uComponentBase, IGoogle2uDB
	{
		public enum rowIds
		{
			ROW_1 = 0,
		...
		public List<LMG_SA80BMRow> Rows = new List<LMG_SA80BMRow>();
		...
		{
			Rows.Add(new LMG_SA80BMRow(input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7], input[8], input[9], input[10]));
		}
		...
		public LMG_SA80BMRow GetRow(rowIds in_RowID)
		{
			LMG_SA80BMRow result = null;
			try
			{
				result = Rows[(int)in_RowID];
			}
		...
		public LMG_SA80BMRow GetRow(string in_RowString)
		{
			LMG_SA80BMRow result = null;
