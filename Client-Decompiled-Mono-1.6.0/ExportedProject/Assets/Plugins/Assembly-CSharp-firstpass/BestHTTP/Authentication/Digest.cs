			...
			try
			{
				...
				case AuthenticationTypes.Digest:
				{
					...
					else if (text3.Contains("auth-int"))
					{
						...
						string text4 = string.Format("{0}:{1}:{2}", request.MethodType.ToString().ToUpper(), request.CurrentUri.PathAndQuery, array.CalculateMD5Hash()).CalculateMD5Hash();
						empty2 = string.Format("{0}:{1}:{2}:{3}:{4}:{5}", empty, Nonce, text2, text, text3, text4).CalculateMD5Hash();
					}
					else
					{
						...
						empty2 = string.Format("{0}:{1}:{2}:{3}:{4}:{5}", empty, Nonce, text2, text, text3, text5).CalculateMD5Hash();
					}
					string text6 = string.Format("Digest username=\"{0}\", realm=\"{1}\", nonce=\"{2}\", uri=\"{3}\", cnonce=\"{4}\", response=\"{5}\"", credentials.UserName, Realm, Nonce, request.Uri.PathAndQuery, text, empty2);
					if (text3 != null)
					{
