...
namespace BestHTTP.SocketIO
{
	public sealed class Socket : ISocket
	{
		...
		public void On(SocketIOEventTypes type, SocketIOCallback callback)
		{
			...
			EventCallbacks.Register(nameFor, callback, false, AutoDecodePayload);
		}
		...
		void ISocket.OnPacket(Packet packet)
		{
			...
			case SocketIOEventTypes.Error:
			{
				...
				if (success)
				{
					...
					Error error = ((dictionary == null || !dictionary.ContainsKey("code")) ? new Error(SocketIOErrors.Custom, packet.Payload) : new Error((SocketIOErrors)Convert.ToInt32(dictionary["code"]), dictionary["message"] as string));
