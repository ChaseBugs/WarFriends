...
namespace BestHTTP.SocketIO
{
	public sealed class HandshakeData
	{
		...
		internal void Start()
		{
			...
			{
				HandshakeRequest = new HTTPRequest(new Uri(string.Format("{0}?EIO={1}&transport=polling&t={2}-{3}{4}&b64=true", Manager.Uri.ToString(), 4, Manager.Timestamp, Manager.RequestCounter++, Manager.Options.BuildQueryParams())), OnHandshakeCallback);
		...
		private void OnHandshakeCallback(HTTPRequest req, HTTPResponse resp)
		{
			...
			case HTTPRequestStates.Finished:
				...
				{
					RaiseOnError(string.Format("Handshake request finished Successfully, but the server sent an error. Status Code: {0}-{1} Message: {2} Uri: {3}", resp.StatusCode, resp.Message, resp.DataAsText, req.CurrentUri));
				}
				break;
