...
namespace BestHTTP.SocketIO
{
	public sealed class Packet
	{
		...
		public bool HasAllAttachment
		{
			get
			{
				return Attachments != null && Attachments.Count == AttachmentCount;
			}
		}
		...
		public bool ReconstructAttachmentAsBase64()
		{
			...
			return PlaceholderReplacer(delegate(string json, Dictionary<string, object> obj)
			{
				...
				Payload = Payload.Replace(json, string.Format("\"{0}\"", Convert.ToBase64String(Attachments[index])));
				IsDecoded = false;
			});
		}
		...
		internal string Encode()
		{
			...
			{
				stringBuilder.Append(((int)TransportEvent/*cast due to .constrained prefix*/).ToString());
			}
			...
			{
				stringBuilder.Append(((int)SocketIOEvent/*cast due to .constrained prefix*/).ToString());
			}
			if (SocketIOEvent == SocketIOEventTypes.BinaryEvent || SocketIOEvent == SocketIOEventTypes.BinaryAck)
			{
