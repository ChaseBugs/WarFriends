...
public class UIRoot : MonoBehaviour
{
	...
	public int activeHeight
	{
		get
		{
			...
			if (scalingStyle == Scaling.FixedWidthShrinkOnly)
			{
				...
				return manualHeight + UIHelper.sizeMod;
			}
	...
	public float pixelSizeAdjustment
	{
		get
		{
			return GetPixelSizeAdjustment(Screen.height, Screen.width);
		}
	}

	public float fontPixelSizeAdjustment
	{
		get
		{
			return (float)Mathf.Min(1536, Screen.height) / (float)activeHeight * Localization.languageConstant;
		}
	}
	...
	public float GetPixelSizeAdjustment(int height, int width)
	{
		...
		if (scalingStyle == Scaling.FixedWidthShrinkOnly)
		{
			...
			if (num > 1f)
			{
				...
				float num3 = (float)activeHeight / (float)manualHeight;
				result *= num3;
			}
			return result;
		}
