...
namespace CodeStage.AdvancedFPSCounter.CountersData
{
	...
	public class FPSCounterData : BaseCounterData
	{
		...
		protected override void CacheCurrentColor()
		{
			...
			colorCached = string.Format("[{0}]FPS: ", arg);
			colorCachedMin = string.Format("\n[{0}]MIN: ", arg);
			colorCachedMax = string.Format("[{0}]MAX: ", arg);
			colorCachedAvg = string.Format(" [{0}]AVG: ", arg);
		}

		protected void CacheWarningColor()
		{
			string arg = AFPSCounter.Color32ToHex2(colorWarning);
			colorWarningCached = string.Format("[{0}]FPS: ", arg);
			colorWarningCachedMin = string.Format("\n[{0}]MIN: ", arg);
			colorWarningCachedMax = string.Format("[{0}]MAX: ", arg);
			colorWarningCachedAvg = string.Format(" [{0}]AVG: ", arg);
		}

		protected void CacheCriticalColor()
		{
			string arg = AFPSCounter.Color32ToHex2(colorCritical);
			colorCriticalCached = string.Format("[{0}]FPS: ", arg);
			colorCriticalCachedMin = string.Format("\n[{0}]MIN: ", arg);
			colorCriticalCachedMax = string.Format("[{0}]MAX: ", arg);
			colorCriticalCachedAvg = string.Format(" [{0}]AVG: ", arg);
		}
