...
namespace CodeStage.AntiCheat.ObscuredTypes
{
	public static class ConfigurationSaver
	{
		...
		private static string GetFilePath(string fileName)
		{
			...
			return string.Format("{0}{1}.dat", mCachedPath, fileName);
		}
		...
		public static bool LoadConfigFromFile(string className, Google2uComponentBase sheet, string sheetVersion)
		{
			...
			try
			{
				using (StreamReader fs = File.OpenText(filePath))
				{
					...
					{
						string message = string.Format("Wrong data for sheet {0} saved version is {1} and should be {2}", className, text2, sheetVersion);
						...
						if (num2 > 0)
						{
							...
							if (sheet.HasColumn(text5))
							{
								...
								while (num2 > 0)
								{
									if (num4 >= list.Count)
									{
										Debug.LogError(string.Format("column {0} in sheet {1} has more saved records than IDs", text5, className));
										break;
									}
									int rowIndex2 = sheet.GetRowIndex(list[num4]);
									if (rowIndex2 < 0)
									{
										Debug.LogError(string.Format("column {0} in sheet {1} has mismach in saved data", text5, className));
										break;
									}
									...
									Debug.LogError(string.Format("column {0} in sheet {1} has more saved records", text5, className));
									break;
								}
							}
						}
			...
			{
				string message2 = string.Format("Exception - parse sheet {0}: {1}", className, ex.Message);
				AnalyticsHelper.LogHandledException(new Exception(message2));
