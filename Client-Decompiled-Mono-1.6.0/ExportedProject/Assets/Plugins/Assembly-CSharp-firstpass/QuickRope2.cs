...
public class QuickRope2 : MonoBehaviour
{
	...
	public Vector3[] JointPositions
	{
		get
		{
			...
			{
				array[i] = Joints[i].transform.position;
			}
			return array;
		}
	}
	...
	public Quaternion[] JointRotations
	{
		get
		{
			...
			array[0] = Quaternion.LookRotation(Joints[0].transform.position - Joints[1].transform.position);
			for (int i = 1; i < array.Length; i++)
			{
				array[i] = Quaternion.LookRotation(Joints[i - 1].transform.position - Joints[i].transform.position);
			}
			return array;
		}
	}
	...
	public void ExtendRope(float velocity)
	{
		...
		if (Vector3.Distance(Joints[1].transform.position, Joints[0].transform.position) > distBetweenJoints * 1.5f)
		{
			...
			if (JointPrefab != null)
			{
				gameObject = UnityEngine.Object.Instantiate(JointPrefab, Joints[1].transform.position - (Joints[1].transform.position - Joints[0].transform.position).normalized * distBetweenJoints, Quaternion.LookRotation(Joints[0].transform.position - Joints[1].transform.position));
	...
	private void UpdateFreeFall()
	{
		...
		else if (Vector3.Distance(Joints[1].transform.position, Joints[0].transform.position) > distBetweenJoints)
		{
			...
			if (JointPrefab != null)
			{
				gameObject = UnityEngine.Object.Instantiate(JointPrefab, Joints[1].transform.position - (Joints[1].transform.position - Joints[0].transform.position).normalized * distBetweenJoints, Quaternion.identity);
	...
	public void GenerateJointObjects()
	{
		...
		for (int i = 1; i < SplinePoints.Count - 1; i++)
		{
			...
			if (JointPrefab != null)
			{
				gameObject = UnityEngine.Object.Instantiate(JointPrefab, SplinePoints[i], calculatedRotations[i]);
				if (alternateJoints)
				{
