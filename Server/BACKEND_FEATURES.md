...
- **Google Play RTDN is now an authenticated durable wake-up path.** The provider-only route
  ...
  client `products.get` path, while pending refund reviews remain durable manual-review records
  until a protected ReviewRefund operator workflow exists.
