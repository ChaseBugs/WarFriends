...
- **Out of scope** — intentionally excluded by the project owner; it is not part of backend
  completion and must not hold the remaining implementation count open.
...
- **Game Center now has an explicit live ownership-proof path for a patched client.** The stock
  1.6.0 `GameCenterPassword` is only the deterministic hash of `GameCenterId + "banana"`, so it is
  retained as legacy compatibility and is never reinterpreted as Apple authority. An opt-in
  `ProviderProof` carries the current GameKit URL, signature, salt, and UInt64 millisecond timestamp;
  the server binds the configured bundle ID and exact player ID, requires a recent timestamp,
  downloads only from configured HTTPS hosts, validates the downloaded certificate through its
  signed issuer chain to a Node-trusted root, respects bounded cache headers, and verifies the
  RSASSA-PKCS1-v1_5/SHA-256 signature before create, link, update, or login. Proof bytes are never
  persisted. Google Play live provider proof remains an explicit unresolved integration.
- **Facebook now has an explicit live ownership-proof path for a patched client.** The recovered
  SDK exposes a real user access token, but stock `FacebookPassword` is only a hash of the Facebook
  ID. Opt-in `ProviderProof` JSON carries that SDK token separately. The backend inspects it through
  Meta's configured versioned `debug_token` endpoint using a backend-only app access token, and
  requires a valid USER token bound to the exact configured App ID and requested Facebook user ID
  with coherent live issue, token-expiry, and optional data-access-expiry timestamps. The user/app
  tokens are never stored or logged. Provider transport/schema failures remain retryable server
  failures and clear only that login's throttle reservation instead of locking out a valid player.
...
|---|---|---|---|---|---|
| P0 | Transport and boot configuration | `GetConfigurations`, all request routing | Implemented | BestHTTP form posts, URL action/version routes, action consistency check, JSON tooling route, exact `POST /api/check.php` body required by `CheckIsOnline`, Client/Server local port 8080 agreement with Android ADB reverse guidance, executable cross-project connectivity/action-157 tests, disposable MongoDB-backed `check -> config -> CreateAccount -> GetPlayerData` smoke, action-34 restoration of the exact persisted `Buffers` snapshot before the stock callback clears it, and exact three-segment raw configuration response without the parser-breaking empty trailing segment; the smoke proves the restored BufferId is a durable replay rather than a newly executed action-98 request; action 157 preserves exact stock selector/cache strings, accepts documented cache aliases only when all copies agree, and keeps malformed requests on a callback-safe zero-cache no-publication path | Recover exact response keys for every individual action and deploy-time production base host |
