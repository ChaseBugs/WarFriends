...
function cloneArena(value: WarArenaState): WarArenaState {
  // Keep the action service's cloning entry point, but make it delegate to the same complete
  // proof used by persisted reads and custom full-document publishers.
  return validatedWarArenaState(value)!;
}
