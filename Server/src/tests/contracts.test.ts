...
test("platform identity authority must match the exact connected player mirror", () => {
  ...
  assert.equal(validatedIdentityDisplayName("Contract Player"), "Contract Player");
  ...
  assert.throws(
    () => validatedIdentityDocument({ ...identity, displayName: "line\nbreak" }),
    /Stored platform identity authority is invalid/,
  );
  assert.throws(
    () => validatedIdentityDisplayName("x".repeat(101)),
...
test("moderation report contract preserves recovered fields and requires authentication", () => {
  ...
  for (const reportedPlayerId of [
    "", " target-1", "target-1 ", "target\n1", "x".repeat(129), null, 123,
  ]) {
    assert.throws(() => normalizeReportInput({
      ReportedPlayerId: reportedPlayerId,
      ReportType: "2",
      Message: "review this report",
    }, true), /Reported player is required/);
  }
  for (const message of [" ", "x".repeat(1_001), "hidden\0text", null, 123]) {
    assert.throws(() => normalizeReportInput({
      ReportedPlayerId: "target-1",
      ReportType: "2",
      Message: message,
  ...
  const untrustedEvidence = normalizeReportInput({
    ReportedPlayerId: "target-1",
    ReportType: "2",
    MyArmyPower: "9".repeat(65),
    MyRank: Number.NaN,
    OpponentRank: "rank\nforged",
    TimeOfMatch: "12.5",
  }, false);
  assert.deepEqual(untrustedEvidence.evidence, { TimeOfMatch: "12.5" });
