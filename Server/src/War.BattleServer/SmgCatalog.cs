...
public interface IBurstStage { int BurstSize{get;} float CadenceSeconds{get;} float BurstLockSeconds{get;} }
public sealed record SmgStage(string SourceId,int Index,int ClipSize,int ReserveAmmo,float ReloadSeconds,
...
public sealed record SmgBinding(string SourceId,int InventoryIndex,int AnimationFamily,Vector3 ShotOffset,
...
public sealed record SmgContentManifest(int Version,string SceneRevision,string StatsRevision,string BindingsRevision);

/// <summary>Immutable source package for all recovered PlayerBurstWeapon SMGs.</summary>
public sealed class SmgCatalog
{
    private static readonly (string Id,int Count)[] Expected=
    [
        ("Google2u.SMG_CPW",36),("Google2u.SMG_MP5",26),("Google2u.SMG_MP7",56),
        ("Google2u.SMG_P90",66),("Google2u.SMG_UMP45",46),("Google2u.SMG_MP5_elite",26),
        ("Google2u.SMG_P90Elite",66),("Google2u.SMG_Vector",76)
    ];
    private readonly IReadOnlyDictionary<string,SmgStage[]> stages;
    private readonly IReadOnlyDictionary<string,SmgBinding> bindings;
    ...
    public IReadOnlyCollection<SmgBinding> Bindings=>Array.AsReadOnly(Expected.Select(x=>Binding(x.Id)).ToArray());
    public uint AlliesBulletMask {get;}
    public uint EnemiesBulletMask {get;}
    private SmgCatalog(Dictionary<string,SmgStage[]> stages,Dictionary<string,SmgBinding> bindings,
    ...
    {this.stages=stages;this.bindings=bindings;SceneRevision=scene;StatsRevision=stats;BindingsRevision=bindingRevision;
     PackageRevision=Convert.ToHexStringLower(SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(
         "WarFriends/smg-content/v1\n"+scene+"\n"+stats+"\n"+bindingRevision)));
     AlliesBulletMask=allies;EnemiesBulletMask=enemies;}
    public SmgStage Stage(string sourceId,int upgrade)=>stages.TryGetValue(sourceId,out var lane)&&upgrade>=0&&upgrade<lane.Length
        ? lane[upgrade]:throw new InvalidDataException("Unsupported SMG or upgrade index.");
    public SmgBinding Binding(string sourceId)=>bindings.TryGetValue(sourceId,out var row)?row:
        throw new InvalidDataException("Unsupported SMG binding.");
        ...
        Func<Vector3,Vector3,float,ShotCollision?> trace)
    {
        ...
        if(projectileId==0||owner==null||owner.Length==0||!PlayerHitbox.Finite(authoritativeMuzzle)||
           !PlayerHitbox.Finite(target)||!float.IsFinite(criticalRoll)||criticalRoll is <0 or >1||trace==null)
            throw new InvalidDataException("Invalid SMG projectile authority.");
        float damage=stage.Damage*(criticalRoll<stage.CriticalProbability?binding.CriticalMultiplier:1);
        if(!float.IsFinite(damage)||damage is <0 or >100_000_000)throw new InvalidDataException("Invalid SMG damage authority.");
    ...
    public static SmgCatalog Load(string manifestPath)
    {
        ...
        var manifest=JsonSerializer.Deserialize<SmgContentManifest>(manifestBytes,json)??throw new InvalidDataException("Missing SMG manifest.");
        if(manifest.Version!=1||!Hash(manifest.SceneRevision)||!Hash(manifest.StatsRevision)||!Hash(manifest.BindingsRevision))
            throw new InvalidDataException("Invalid SMG content manifest.");
        ...
        byte[] bindingBytes=Read(Path.Combine(directory,"recovered-smg-bindings.json"),2,1_000_000);
        if(Convert.ToHexStringLower(SHA256.HashData(source))!=manifest.StatsRevision||
           Convert.ToHexStringLower(SHA256.HashData(bindingBytes))!=manifest.BindingsRevision)
            throw new InvalidDataException("SMG package revision mismatch.");
        try
        {
            ...
            if(root.GetProperty("client").GetString()!="1.4.0"||root.GetProperty("mainSceneSha256").GetString()!=manifest.SceneRevision)
                throw new InvalidDataException("SMG source provenance mismatch.");
            ...
            var stageRows=new Dictionary<string,SmgStage[]>(StringComparer.Ordinal);
            foreach(var expected in Expected)
            {
                var definition=definitions[expected.Id];var rows=sheets[expected.Id];
                if(rows.GetArrayLength()!=expected.Count||Integer(definition,"BURSTSIZE",1,32)!=3)
                    throw new InvalidDataException("Incomplete SMG upgrade lane.");
                ...
                var lane=new SmgStage[expected.Count];int i=0;
                foreach(var row in rows.EnumerateArray())lane[i]=new(expected.Id,i++,Integer(row,"CLIPSIZE",1,1000),
                    ...
                    cadence,critical,player,overtime,3,burstLock);
            ...
            if(bindingRoot.GetProperty("client").GetString()!="1.4.0"||bindingRoot.GetProperty("source").GetString()!="Assets/Scenes/MainScene.unity"||
               bindingRoot.GetProperty("sha256").GetString()!=manifest.SceneRevision||bindingRoot.GetProperty("weaponCount").GetInt32()!=8)
                throw new InvalidDataException("SMG binding provenance mismatch.");
            ...
            if(allies==0||enemies==0)throw new InvalidDataException("Missing SMG bullet masks.");
            var bindingRows=new Dictionary<string,SmgBinding>(StringComparer.Ordinal);var indexes=new HashSet<int>();
            foreach(var row in bindingRoot.GetProperty("weapons").EnumerateArray())
            {
                ...
                if(!stageRows.ContainsKey(id)||!indexes.Add(index)||row.GetProperty("playerWeaponType").GetString()!="PlayerBurstWeapon"||
                   row.GetProperty("weaponType").GetString()!="LightMachinegun"||row.GetProperty("burst").GetInt32()!=3||
                ...
                if(family is not (0 or 5 or 13))throw new InvalidDataException("Unsupported SMG animation family.");
                string muzzle=row.GetProperty("muzzlePath").GetString()??"",bulletRevision=row.GetProperty("bulletSha256").GetString()??"";
                if(muzzle.Length is <1 or >512||!Hash(bulletRevision))throw new InvalidDataException("Invalid SMG asset identity.");
                bindingRows.Add(id,new(id,index,family,Vector(row.GetProperty("shotOffset")),3,
            ...
            if(bindingRows.Count!=8||!bindingRows.Keys.ToHashSet(StringComparer.Ordinal).SetEquals(Expected.Select(x=>x.Id)))
                throw new InvalidDataException("Incomplete SMG binding set.");
        ...
        {throw new InvalidDataException("Malformed SMG content package.",e);}
    }
    private static bool Hash(string? value)=>value!=null&&Regex.IsMatch(value,@"\A[0-9a-f]{64}\z");
    private static byte[] Read(string path,int min,int max){using var stream=File.OpenRead(path);if(stream.Length<min||stream.Length>max)throw new InvalidDataException("Invalid SMG file size.");var bytes=new byte[checked((int)stream.Length)];stream.ReadExactly(bytes);return bytes;}
    private static float Number(JsonElement row,string key,float min,float max){float value=row.GetProperty(key).GetSingle();if(!float.IsFinite(value)||value<min||value>max)throw new InvalidDataException("Invalid SMG number.");return value;}
    private static int Integer(JsonElement row,string key,int min,int max){if(!row.GetProperty(key).TryGetInt32(out int value)||value<min||value>max)throw new InvalidDataException("Invalid SMG integer.");return value;}
    private static Vector3 Vector(JsonElement value){if(value.GetArrayLength()!=3)throw new InvalidDataException("Invalid SMG vector.");var result=new Vector3(value[0].GetSingle(),value[1].GetSingle(),value[2].GetSingle());if(!float.IsFinite(result.X)||!float.IsFinite(result.Y)||!float.IsFinite(result.Z))throw new InvalidDataException("Invalid SMG vector.");return result;}
}
