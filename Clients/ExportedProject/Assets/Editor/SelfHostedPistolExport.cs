...
// Read-only source export for the six recovered PlayerClickWeapon pistols.
public static class SelfHostedPistolExport
{
    public static void Run()
    {
        string output=Environment.GetEnvironmentVariable("WAR_PISTOL_OUTPUT");
        if(string.IsNullOrEmpty(output))throw new InvalidOperationException("Set WAR_PISTOL_OUTPUT.");
        ...
        var levels=roots.SelectMany(g=>g.GetComponentsInChildren<LevelManager>(true)).Single();var rows=new List<object>();
        foreach(var setup in levels.GetComponentsInChildren<WeaponLevelsSetup>(true))
        {
            ...
            if(!id.StartsWith("Google2u.Pistol_",StringComparison.Ordinal))continue;
            int index=setup.indexInWeaponInventory;
            if(index<0||index>=inventory.allWeapons.Count)throw new InvalidOperationException("Invalid pistol index "+id);
            var controller=inventory.allWeapons[index] as PlayerClickWeapon;var weapon=controller==null?null:controller.GetComponent<Pistol>();
            ...
            if(controller==null||gun==null||bullet==null||weapon.spawnPoint==null||weapon.bulletPrefab==null)
                throw new InvalidOperationException("Incomplete pistol binding "+id);
            ...
            if(!AssetDatabase.TryGetGUIDAndLocalFileIdentifier(weapon.bulletPrefab,out guid,out fileId))
                throw new InvalidOperationException("Missing pistol projectile identity "+id);
            rows.Add(new{id=id,inventoryIndex=index,setupPath=PathOf(setup.transform),playerWeaponType=controller.GetType().FullName,
                ...
                autofire=controller.autofire,showScope=controller.showScope,scopeNum=controller.scopeNum,
                firstShotWaitTime=controller.firstShotWaitTime,fast=gun.fastBullet,friendKill=weapon.friendKill,
                shotType=(int)weapon.shotType,ignoreLayersMask=weapon.ignoreLayersMask,animationFamily=(int)weapon.weaponType,
                reloadable=weapon.reloadableWeapon,infiniteAmmo=weapon.infiniteAmmo,pistolReserveOverride=weapon.ammoLeft,
        ...
        if(rows.Count!=6)throw new InvalidOperationException("Unexpected pistol count "+rows.Count);
        ...
        Debug.Log("PISTOL_EXPORT_PASSED weapons="+rows.Count);EditorApplication.Exit(0);
    }
