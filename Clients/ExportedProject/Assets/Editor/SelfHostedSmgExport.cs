...
// Read-only source export for the eight recovered PlayerBurstWeapon SMGs.
public static class SelfHostedSmgExport
{
    public static void Run()
    {
        string output=Environment.GetEnvironmentVariable("WAR_SMG_OUTPUT");
        if(string.IsNullOrEmpty(output))throw new InvalidOperationException("Set WAR_SMG_OUTPUT.");
        ...
        foreach(var setup in levels.GetComponentsInChildren<WeaponLevelsSetup>(true))
        {
            ...
            if(!id.StartsWith("Google2u.SMG_",StringComparison.Ordinal))continue;
            int index=setup.indexInWeaponInventory;
            if(index<0||index>=inventory.allWeapons.Count)throw new InvalidOperationException("Invalid SMG index "+id);
            ...
            if(controller==null||gun==null||bullet==null||weapon.spawnPoint==null||weapon.bulletPrefab==null)
                throw new InvalidOperationException("Incomplete SMG binding "+id);
            ...
            if(!AssetDatabase.TryGetGUIDAndLocalFileIdentifier(weapon.bulletPrefab,out guid,out fileId))
                throw new InvalidOperationException("Missing SMG projectile identity "+id);
        ...
        if(rows.Count!=8)throw new InvalidOperationException("Unexpected SMG count "+rows.Count);
        ...
        Debug.Log("SMG_EXPORT_PASSED weapons="+rows.Count);EditorApplication.Exit(0);
    }
