		...
		case DailyReward.GoldCard:
		{
			...
			for (int j = 0; j < warcards.Length; j++)
			{
				...
				if (num4 > j)
				{
					...
					switch (cardType)
					{
						case CardManager.CardType.Gold: warcardsQuestion[j].color = Colours.questionGold; break;
						case CardManager.CardType.Silver: warcardsQuestion[j].color = Colours.questionSilver; break;
						default: warcardsQuestion[j].color = Colours.questionBronze; break;
					}
				}
			}
