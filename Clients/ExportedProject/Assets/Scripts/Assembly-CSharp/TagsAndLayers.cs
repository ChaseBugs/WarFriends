...
public class TagsAndLayers : Singleton<TagsAndLayers>
{
	...
	public LayerMask GetBulletMask(Fractions fraction, int ignoreLayersMask)
	{
		switch (fraction)
		{
			case Fractions.Enemies:
				return (int)enemiesBulletMask & ~ignoreLayersMask;
			case Fractions.Allies:
				return (int)alliesBulletMask & ~ignoreLayersMask;
			default:
				return (int)bulletMask & ~ignoreLayersMask;
		}
	}
	...
	public int GetFractionBulletLayer(Fractions fraction, bool isFlying)
	{
		...
		{
			case Fractions.Enemies:
				return (!isFlying) ? destroyableEntitiesEnemiesLayerNumber : destroyableEntitiesEnemiesMechLayerNumber;
			case Fractions.Allies:
				return (!isFlying) ? destroyableEntitiesAlliesLayerNumber : destroyableEntitiesAlliesMechLayerNumber;
			default:
				return destroyableEntitiesLayerNumber;
		}
	}
}
