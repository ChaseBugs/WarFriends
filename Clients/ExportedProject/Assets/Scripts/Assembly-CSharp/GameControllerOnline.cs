...
public abstract class GameControllerOnline : IGameController
{
	...
	private void OnDisconnectedFromPhoton()
	{
		...
		if (controller != null && controller.mainController == this)
		{
	...
	private GameController.GameEndReason GetReasonForClient(GameController.GameEndReason endReas)
	{
		switch (endReas)
		{
			case GameController.GameEndReason.Killed:
				return GameController.GameEndReason.Win;
			case GameController.GameEndReason.Win:
				return GameController.GameEndReason.Killed;
			case GameController.GameEndReason.Forfeit:
				return GameController.GameEndReason.WinByForfeit;
			case GameController.GameEndReason.WinByForfeit:
				return GameController.GameEndReason.Forfeit;
			default:
				return endReas;
		}
	}
	...
	{
		// Match and reconnect state belong to the player created during startup.
		if (PlayerController.currentPlayer == null || PlayerController.currentPlayer.networkStatus == null)
			return;
		...
		if (PlayerController.currentPlayer != null && PlayerController.currentPlayer.networkStatus != null && PlayerController.currentPlayer.networkStatus.matchState == PlayerNetworkStatus.MatchState.Rematch && !mBothPlayersConnected && Time.realtimeSinceStartup > MatchManager.rematchExpireTime)
		{
			Quit();
			LoadingDialog.Hide();
			MatchManager.matchState = MatchState.GameCancelled;
