...
internal class GainSkillshotsAssignment : Assignment
{
	...
	public override string hintText
	{
		get
		{
			...
			SkillShot.SkillShotType key = (SkillShot.SkillShotType)Convert.ToInt32(data[2]);
			return GameVariables.skillshotNames[key].Value3;
		}
	}
	...
	{
		data = Assignment.GetAloneObjectFor(10, 3, databaseAssignment.target);
		...
		data[0] = Convert.ToInt32(data[0]) * skillShot["COUNT"].ToObject<int>();
		data[1] = Singleton<SkillShotManager>.instance.GetSkillshotDefinition(skillshot).menuName;
		data[2] = (int)skillshot;
	}
	...
	public override float Update()
	{
		SkillShot.SkillShotType key = (SkillShot.SkillShotType)Convert.ToInt32(data[2]);
		int gained = 0;
		if (Singleton<ScoreManager>.instance.skillShotCounts.ContainsKey(key))
		{
			gained = Singleton<ScoreManager>.instance.skillShotCounts[key];
		}
		else
		{
