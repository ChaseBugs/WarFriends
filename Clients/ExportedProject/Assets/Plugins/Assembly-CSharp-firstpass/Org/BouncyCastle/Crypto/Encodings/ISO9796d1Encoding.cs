...
public class ISO9796d1Encoding : IAsymmetricBlockCipher
{
	...
	private byte[] EncodeBlock(byte[] input, int inOff, int inLen)
	{
		...
		for (int j = array.Length - 2 * num2; j != array.Length; j += 2)
		{
			...
			array[j] = (byte)((shadows[(int)((uint)((b & 0xFF)) >> (4))] << 4) | shadows[b & 0xF]);
			array[j + 1] = b;
		}
	...
	private byte[] DecodeBlock(byte[] input, int inOff, int inLen)
	{
		...
		bytes[0] = (byte)((shadows[(int)((uint)((bytes[1] & 0xFF)) >> (4))] << 4) | shadows[bytes[1] & 0xF]);
		bool flag = false;
		int num3 = 0;
		for (int num4 = bytes.Length - 1; num4 >= bytes.Length - 2 * num2; num4 -= 2)
		{
			int num5 = (shadows[(int)((uint)((bytes[num4] & 0xFF)) >> (4))] << 4) | shadows[bytes[num4] & 0xF];
