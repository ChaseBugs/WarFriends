...
public class tk2dCamera : MonoBehaviour
{
	...
	[Obsolete]
	public float zoomScale => 1f / Mathf.Max(0.01f, zoomFactor);
	...
	private Matrix4x4 GetProjectionMatrixForOverride(tk2dCamera settings, tk2dCameraResolutionOverride currentOverride, float pixelWidth, float pixelHeight, bool halfTexelOffset, out Rect screenExtents, out Rect unscaledScreenExtents)
	{
		...
		else if (UnityCamera.rect != CameraSettings.rect)
		{
		...
		float num18 = 1f / ZoomFactor;
		bool flag = Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == (RuntimePlatform)5 || Application.platform == RuntimePlatform.WindowsEditor;
		...
		unscaledScreenExtents = new Rect(zero.x * num20, zero.y * num20, (float)nativeResolutionWidth * num20, (float)nativeResolutionHeight * num20);
		return OrthoOffCenter(scaleForOverride, num17 * (num + num19) * num18, num17 * (num3 + num19) * num18, num17 * (num2 - num19) * num18, num17 * (num4 - num19) * num18, UnityCamera.nearClipPlane, UnityCamera.farClipPlane);
	}
