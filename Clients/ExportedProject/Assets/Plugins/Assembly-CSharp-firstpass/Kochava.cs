...
public class Kochava : MonoBehaviour
{
		...
		try
		{
			...
			if (PlayerPrefs.HasKey("session_tracking"))
			{
				try
				{
					...
					sessionTracking = (KochSessionTracking)(int)Enum.Parse(typeof(KochSessionTracking), text4, ignoreCase: true);
	...
	private IEnumerator Init_KV(string postData)
	{
		...
		if (serverResponse.ContainsKey("blacklist"))
		{
			...
			if (serverResponse["blacklist"].GetType().GetElementType() == typeof(string))
			{
				...
				{
					Exception e3 = ex3;
					Log("Failed parsing device_identifier blacklist received from server: " + e3, KochLogLevel.warning);
				}
			}
			try
			{
		...
		if (serverResponse.ContainsKey("whitelist") && serverResponse["whitelist"].GetType().GetElementType() == typeof(string))
		{
			...
			try
			{
				...
				for (int i2 = devIdWhitelistArr.Length - 1; i2 >= 0; i2--)
				{
					if (devIdWhitelistArr[i2] == "location")
					{
						doReportLocation = true;
					}
					result = ((i2 == 0) ? (result + devIdWhitelistArr[i2]) : (result + devIdWhitelistArr[i2] + ","));
				}
			}
			catch (Exception ex5)
			{
		...
		if (serverResponse.ContainsKey("eventname_blacklist"))
		{
			if (serverResponse["eventname_blacklist"].GetType().GetElementType() == typeof(string))
			{
				try
				{
					...
					for (int i3 = eventNameBlacklistArr.Length - 1; i3 >= 0; i3--)
					{
						eventNameBlacklist.Add(eventNameBlacklistArr[i3]);
					}
				}
				catch (Exception ex6)
				{
					Exception e5 = ex6;
					Log("Failed parsing eventname_blacklist received from server: " + e5, KochLogLevel.warning);
				}
			}
		...
		if (Application.platform == RuntimePlatform.Android)
		{
			...
			using (AndroidJavaClass androidUnityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
			{
				...
				if (androidActivity != null)
				{
					...
					androidHelperClass.CallStatic<string>("GetExternalKochavaInfo_Android", new object[5]
					{
						androidContext,
						whitelist,
						device_id_delay,
		...
		using (AndroidJavaClass androidUnityPlayer2 = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
		{
		AndroidJavaObject androidActivity2 = androidUnityPlayer2.GetStatic<AndroidJavaObject>("currentActivity");
		...
		androidHelperClass2.CallStatic<string>("GetExternalLocationReport_Android", new object[3] { locationAccuracy, locationTimeout, locationStaleness });
	...
	public IEnumerator KochavaAttributionTimerFired(int delayTime)
	{
		...
		else
		{
			...
			{
				Exception e2 = ex2;
				Log("Failed Deserialize JSON attribution data chunk: " + e2, KochLogLevel.warning);
			}
			int retry = 0;
