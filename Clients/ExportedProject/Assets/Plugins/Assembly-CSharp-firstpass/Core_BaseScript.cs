...
public class Core_BaseScript : MonoBehaviour
{
	...
	protected LocalStorage _localStorage
	{
		get
		{
			...
			{
				mLocalStorage = GetLocalStorage();
				if (mLocalStorage == null && Debug.isDebugBuild)
				{
					Debug.LogError($"BaseScript: You forget add the LocalStorage prefab object to '{Application.loadedLevelName}' scene");
				}
			}
	...
	protected virtual LocalStorage GetLocalStorage()
	{
		return UnityEngine.Object.FindObjectOfType(typeof(LocalStorage)) as LocalStorage;
	}
