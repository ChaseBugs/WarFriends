...
public abstract class PeerBase
{
	...
	internal string ClientVersion => ppeer.ClientVersion;

	internal Type SocketImplementation => ppeer.SocketImplementation;

	public string ServerAddress { get; internal set; }

	internal string HttpUrlParameters { get; set; }

	internal bool TrafficStatsEnabled => ppeer.TrafficStatsEnabled;

	internal TrafficStats TrafficStatsIncoming => ppeer.TrafficStatsIncoming;

	internal TrafficStats TrafficStatsOutgoing => ppeer.TrafficStatsOutgoing;

	internal TrafficStatsGameLevel TrafficStatsGameLevel => ppeer.TrafficStatsGameLevel;

	internal bool crcEnabled => ppeer.CrcEnabled;

	internal IPhotonPeerListener Listener => ppeer.Listener;

	internal DebugLevel debugOut => ppeer.DebugOut;

	internal int sentCountAllowance => ppeer.SentCountAllowance;

	internal int DisconnectTimeout => ppeer.DisconnectTimeout;

	internal int timePingInterval => ppeer.TimePingInterval;

	internal byte ChannelCount => ppeer.ChannelCount;

	internal int limitOfUnreliableCommands => ppeer.LimitOfUnreliableCommands;

	public byte QuickResendAttempts => ppeer.QuickResendAttempts;

	public NetworkSimulationSet NetworkSimulationSettings => networkSimulationSettings;

	internal int CommandLogSize => ppeer.CommandLogSize;
	...
	protected internal bool IsIpv6 => rt != null && rt.AddressResolvedAsIpv6;

	internal static int outgoingStreamBufferSize => PhotonPeer.OutgoingStreamBufferSize;

	internal bool IsSendingOnlyAcks => ppeer.IsSendingOnlyAcks;

	internal int mtu => ppeer.MaximumTransferUnit;

	internal int rhttpMinConnections => ppeer.RhttpMinConnections;

	internal int rhttpMaxConnections => ppeer.RhttpMaxConnections;
