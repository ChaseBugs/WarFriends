...
public class Protocol16 : IProtocol
{
	...
	internal override string protocolType => "GpBinaryV16";

	internal override byte[] VersionBytes => versionBytes;

	private bool SerializeCustom(StreamBuffer dout, object serObject)
	{
		if (Protocol.TypeDict.TryGetValue(serObject.GetType(), out var value))
		{
			...
			SerializeShort(dout, num, setType: false);
			dout.Position += num;
	...
	private object DeserializeCustom(StreamBuffer din, byte customTypeCode)
	{
		...
		if (Protocol.CodeDict.TryGetValue(customTypeCode, out var value))
		{
	...
	private void SerializeParameterTable(StreamBuffer stream, Dictionary<byte, object> parameters)
	{
		...
		SerializeShort(stream, (short)parameters.Count, setType: false);
