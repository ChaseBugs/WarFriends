...
public class Image : MaskableGraphic, ICanvasRaycastFilter, ISerializationCallbackReceiver, ILayoutElement
{
	...
	private void GenerateSlicedSprite(VertexHelper toFill)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			for (int k = 0; k < 3; k++)
			{
				...
				{
					int num2 = k + 1;
					AddQuad(toFill, new Vector2(s_VertScratch[j].x, s_VertScratch[k].y), new Vector2(s_VertScratch[num].x, s_VertScratch[num2].y), base.color, new Vector2(s_UVScratch[j].x, s_UVScratch[k].y), new Vector2(s_UVScratch[num].x, s_UVScratch[num2].y));
				}
			}
		}
	}
	...
	private void GenerateTiledSprite(VertexHelper toFill)
	{
		...
		for (float num9 = y; num9 < num4; num9 += num2)
		{
			...
			AddQuad(toFill, new Vector2(num3, num9) + pixelAdjustedRect.position, new Vector2(pixelAdjustedRect.width, num10) + pixelAdjustedRect.position, base.color, new Vector2(vector5.x, uvMin.y), new Vector2(vector.z, uvMax.y));
		}
		uvMax = vector5;
		for (float num11 = x; num11 < num3; num11 += num)
		{
			...
			AddQuad(toFill, new Vector2(num11, num4) + pixelAdjustedRect.position, new Vector2(num12, pixelAdjustedRect.height) + pixelAdjustedRect.position, base.color, new Vector2(uvMin.x, vector5.y), new Vector2(uvMax.x, vector.w));
		}
		...
		AddQuad(toFill, new Vector2(num3, 0f) + pixelAdjustedRect.position, new Vector2(pixelAdjustedRect.width, y) + pixelAdjustedRect.position, base.color, new Vector2(vector5.x, vector.y), new Vector2(vector.z, uvMin.y));
		AddQuad(toFill, new Vector2(0f, num4) + pixelAdjustedRect.position, new Vector2(x, pixelAdjustedRect.height) + pixelAdjustedRect.position, base.color, new Vector2(vector.x, vector5.y), new Vector2(uvMin.x, vector.w));
		AddQuad(toFill, new Vector2(num3, num4) + pixelAdjustedRect.position, new Vector2(pixelAdjustedRect.width, pixelAdjustedRect.height) + pixelAdjustedRect.position, base.color, new Vector2(vector5.x, vector5.y), new Vector2(vector.z, vector.w));
	}
