...
public class NetworkManager : MonoBehaviour
{
	...
	public bool sendPeerInfo
	{
		get
		{
			return m_SendPeerInfo;
		}
		set
		{
			m_SendPeerInfo = value;
		}
	}
	...
	public int numPlayers
	{
		get
		{
			...
			foreach (NetworkConnection localConnection in NetworkServer.localConnections)
			{
				...
				foreach (PlayerController playerController2 in localConnection.playerControllers)
				{
	...
	private void Awake()
	{
		...
		if (m_NetworkAddress != string.Empty)
		{
	...
	internal void OnServerConnectInternal(NetworkMessage netMsg)
	{
		...
		if (networkSceneName != string.Empty && networkSceneName != m_OfflineScene)
		{
			StringMessage msg = new StringMessage(networkSceneName);
	...
	private void OnServerAddPlayerInternal(NetworkConnection conn, short playerControllerId)
	{
		...
		else
		{
			...
			GameObject player = ((!(startPosition != null)) ? ((GameObject)Object.Instantiate(m_PlayerPrefab, Vector3.zero, Quaternion.identity)) : ((GameObject)Object.Instantiate(m_PlayerPrefab, startPosition.position, startPosition.rotation)));
	...
	public void StartMatchMaker()
	{
		...
		SetMatchHost(m_MatchHost, m_MatchPort, https: true);
	}
	...
	public void SetMatchHost(string newHost, int port, bool https)
	{
		...
		m_MatchHost = newHost;
		m_MatchPort = port;
		matchMaker.baseUri = new Uri(text + m_MatchHost + ":" + m_MatchPort);
	}
