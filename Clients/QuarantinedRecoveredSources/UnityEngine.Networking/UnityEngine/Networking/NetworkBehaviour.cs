...
public class NetworkBehaviour : MonoBehaviour
{
	...
	protected class Invoker
	{
		...
		{
			return string.Concat(invokeType, ":", invokeClass, ":", invokeFunction.Method.Name);
		}
	}
	...
	protected static void RegisterCommandDelegate(Type invokeClass, int cmdHash, CmdDelegate func)
	{
		if (!s_CmdHandlerDelegates.ContainsKey(cmdHash))
		{
			...
			if (LogFilter.logDev)
			{
				Debug.Log("RegisterCommandDelegate hash:" + cmdHash + " " + func.Method.Name);
			}
		}
	}
	...
	protected static void RegisterRpcDelegate(Type invokeClass, int cmdHash, CmdDelegate func)
	{
		if (!s_CmdHandlerDelegates.ContainsKey(cmdHash))
		{
			...
			if (LogFilter.logDev)
			{
				Debug.Log("RegisterRpcDelegate hash:" + cmdHash + " " + func.Method.Name);
			}
		}
	}
	...
	protected static void RegisterEventDelegate(Type invokeClass, int cmdHash, CmdDelegate func)
	{
		if (!s_CmdHandlerDelegates.ContainsKey(cmdHash))
		{
			...
			if (LogFilter.logDev)
			{
				Debug.Log("RegisterEventDelegate hash:" + cmdHash + " " + func.Method.Name);
			}
		}
	}
	...
	internal static void DumpInvokers()
	{
		...
		{
			Debug.Log(string.Concat("  Invoker:", s_CmdHandlerDelegate.Value.invokeClass, ":", s_CmdHandlerDelegate.Value.invokeFunction.Method.Name, " ", s_CmdHandlerDelegate.Value.invokeType, " ", s_CmdHandlerDelegate.Key));
		}
	}
	...
	internal static string GetCmdHashHandlerName(int cmdHash)
	{
		...
		Invoker invoker = s_CmdHandlerDelegates[cmdHash];
		return string.Concat(invoker.invokeType, ":", invoker.invokeFunction.Method.Name);
	}
