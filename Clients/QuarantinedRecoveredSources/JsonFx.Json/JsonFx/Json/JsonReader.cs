...
public class JsonReader
{
	...
	protected readonly JsonReaderSettings Settings = new JsonReaderSettings();
	...
	{
		Settings = settings;
		using (StreamReader streamReader = new StreamReader(input, detectEncodingFromByteOrderMarks: true))
		{
	...
	{
		return (T)Read(typeof(T), typeIsHint: false);
	}
	...
	protected virtual object ReadObject(Type objectType)
	{
		...
		do
		{
			...
			case JsonToken.UnquotedName:
			{
				...
				{
					type3 = TypeCoercionUtility.GetMemberInfo(memberMap, text, out memberInfo);
				}
				else
				{
					type3 = type;
				...
				object obj2 = Read(type3, typeIsHint: false);
				...
				else
				{
					...
					Settings.Coercion.SetMemberValue(obj, type3, memberInfo, obj2);
				}
				goto IL_0270;
			}
