...
public class EcmaScriptIdentifier : IJsonSerializable
{
	...
	{
		identifier = ((!string.IsNullOrEmpty(ident)) ? EnsureValidIdentifier(ident, nested: true) : string.Empty);
	}
	...
	{
		return EnsureValidIdentifier(varExpr, nested, throwOnEmpty: true);
	}
	...
	public static bool IsValidIdentifier(string varExpr, bool nested)
	{
		...
		if (nested)
		{
			...
			{
				if (!IsValidIdentifier(varExpr2, nested: false))
				{
					return false;
				}
			}
