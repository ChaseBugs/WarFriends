...
internal class UserAuthorizationDialog : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (Screen.width < 385 || Screen.height < 155)
		{
			...
			Application.ReplyToUserAuthorizationRequest(reply: false);
		}
	...
	private void DoUserAuthorizationDialog(int windowID)
	{
		...
		{
			Application.ReplyToUserAuthorizationRequest(reply: true, remember: true);
		}
		GUILayout.Space(5f);
		if (GUILayout.Button("Allow"))
		{
			Application.ReplyToUserAuthorizationRequest(reply: true);
		}
		GUILayout.EndHorizontal();
		GUILayout.FlexibleSpace();
	}
