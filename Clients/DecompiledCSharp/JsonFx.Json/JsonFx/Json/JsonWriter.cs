...
public class JsonWriter : IDisposable
{
		...
		if (value is IJsonSerializable)
		{
			try
			{
				if (isProperty)
				{
					...
					{
						throw new JsonSerializationException($"The maxiumum depth of {settings.MaxDepth} was exceeded. Check for cycles in object graph.");
					}
					WriteLine();
				}
	...
	{
		if (!(value.GetEnumerator() is IDictionaryEnumerator dictionaryEnumerator))
		{
			throw new JsonSerializationException($"Types which implement Generic IDictionary<TKey, TValue> must have an IEnumerator which implements IDictionaryEnumerator. ({value.GetType()})");
		}
		bool flag = false;
		Writer.Write('{');
		depth++;
	...
	{
		if (!(Attribute.GetCustomAttribute(member, typeof(DefaultValueAttribute)) is DefaultValueAttribute defaultValueAttribute))
		{
			return false;
		}
