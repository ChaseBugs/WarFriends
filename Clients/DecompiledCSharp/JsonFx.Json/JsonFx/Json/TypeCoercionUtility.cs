...
public class TypeCoercionUtility
{
	...
	public virtual object InstantiateObject(Type objectType, out Dictionary<string, MemberInfo> memberMap)
	{
		...
		{
			throw new JsonTypeCoercionException($"Interfaces, Abstract classes, and unsupported ValueTypes cannot be deserialized. ({objectType.FullName})");
		}
		ConstructorInfo constructor = objectType.GetConstructor(Type.EmptyTypes);
		if (constructor == null)
		{
			throw new JsonTypeCoercionException($"Only objects with default constructors can be deserialized. ({objectType.FullName})");
		}
		object result;
		try
		{
	...
	public virtual object CoerceType(Type targetType, object value)
	{
		...
		{
			throw new JsonTypeCoercionException($"Error converting {value.GetType().FullName} to {targetType.FullName}", innerException);
		}
	}
