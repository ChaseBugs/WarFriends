...
public abstract class PipeStream : Stream
{
	...
	internal Stream Stream
	{
		get
		{
			...
			if (stream == null)
			{
				stream = new FileStream(handle.DangerousGetHandle(), (!CanRead) ? FileAccess.Write : ((!CanWrite) ? FileAccess.Read : FileAccess.ReadWrite), ownsHandle: true, buffer_size, IsAsync);
			}
			return stream;
		}
		set
