...
internal class CompilationContext
{
	...
	private class HoistedVariableDetector : ExpressionVisitor
	{
		...
		{
			if (parameter_to_lambda.TryGetValue(parameter, out var value))
			{
				if (hoisted_map == null)
				{
					hoisted_map = new Dictionary<LambdaExpression, List<ParameterExpression>>();
				}
				if (!hoisted_map.TryGetValue(value, out var value2))
				{
	...
	public List<ParameterExpression> GetHoistedLocals(LambdaExpression lambda)
	{
		...
		hoisted_map.TryGetValue(lambda, out var value);
		return value;
	}

	public object[] CreateHoistedLocals(int unit)
	{
		return new object[GetHoistedLocals(units[unit].Lambda)?.Count ?? 0];
	}
