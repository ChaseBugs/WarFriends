...
public sealed class SqliteDataReader : DbDataReader
{
	...
	public override int RecordsAffected => (_rowsAffected >= 0) ? _rowsAffected : 0;
	...
	internal DataTable GetSchemaTable(bool wantUniqueInfo, bool wantDefaultValue)
	{
		...
		for (int i = 0; i < _fieldCount; i++)
		{
			...
			empty = _command.Connection._sql.ColumnOriginalName(_activeStatement, i);
			if (!string.IsNullOrEmpty(empty))
			{
				dataRow[SchemaTableColumn.BaseColumnName] = empty;
			}
			dataRow[SchemaTableColumn.IsExpression] = string.IsNullOrEmpty(empty);
			dataRow[SchemaTableColumn.IsAliased] = string.Compare(GetName(i), empty, ignoreCase: true, CultureInfo.InvariantCulture) != 0;
			...
			if (!string.IsNullOrEmpty(empty))
			{
				_command.Connection._sql.ColumnMetaData((string)dataRow[SchemaTableOptionalColumn.BaseCatalogName], (string)dataRow[SchemaTableColumn.BaseTableName], empty, out dataType, out var collateSequence, out var notNull, out var primaryKey, out var autoIncrement);
				...
				if (wantUniqueInfo)
				{
					...
					foreach (DataRow row in dataTable2.Rows)
					{
						...
						{
							if (string.Compare((string)row2["COLUMN_NAME"], empty, ignoreCase: true, CultureInfo.InvariantCulture) == 0)
							{
