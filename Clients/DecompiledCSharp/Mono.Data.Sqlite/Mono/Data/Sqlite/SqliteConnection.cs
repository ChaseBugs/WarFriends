...
public sealed class SqliteConnection : DbConnection, ICloneable
{
	...
	public override void Open()
	{
		...
		try
		{
			...
			_defaultIsolation = (System.Data.IsolationLevel)(int)Enum.Parse(typeof(System.Data.IsolationLevel), FindKey(items, "Default IsolationLevel", "Serializable"), ignoreCase: true);
			...
			SQLiteDateFormats fmt = (SQLiteDateFormats)(int)Enum.Parse(typeof(SQLiteDateFormats), FindKey(items, "DateTimeFormat", "ISO8601"), ignoreCase: true);
			if (flag)
			{
	...
	private DataTable Schema_Columns(string strCatalog, string strTable, string strColumn)
	{
		...
		string text = ((string.Compare(strCatalog, "temp", ignoreCase: true, CultureInfo.InvariantCulture) != 0) ? "sqlite_master" : "sqlite_temp_master");
	...
	private DataTable Schema_Triggers(string catalog, string table, string triggerName)
	{
		...
		string text = ((string.Compare(catalog, "temp", ignoreCase: true, CultureInfo.InvariantCulture) != 0) ? "sqlite_master" : "sqlite_temp_master");
	...
	private DataTable Schema_Tables(string strCatalog, string strTable, string strType)
	{
		...
		using (SqliteCommand sqliteCommand = new SqliteCommand(string.Format(CultureInfo.InvariantCulture, "SELECT [type], [name], [tbl_name], [rootpage], [sql], [rowid] FROM [{0}].[{1}] WHERE [type] LIKE 'table'", strCatalog, text), this))
		{
			...
			while (sqliteDataReader.Read())
			{
				...
				if ((string.Compare(strType, text2, ignoreCase: true, CultureInfo.InvariantCulture) == 0 || strType == null) && (string.Compare(sqliteDataReader.GetString(2), strTable, ignoreCase: true, CultureInfo.InvariantCulture) == 0 || strTable == null))
				{
					DataRow dataRow = dataTable.NewRow();
					dataRow["TABLE_CATALOG"] = strCatalog;
					dataRow["TABLE_NAME"] = sqliteDataReader.GetString(2);
