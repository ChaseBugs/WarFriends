		...
		if (!m_Dragging && (vector != Vector2.zero || m_Velocity != Vector2.zero))
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				else if (m_Inertia)
				{
					...
					int index2;
					int index = (index2 = i);
					float num = reference[index2];
					reference[index] = num * Mathf.Pow(m_DecelerationRate, unscaledDeltaTime);
					if (Mathf.Abs(m_Velocity[i]) < 1f)
					{
						m_Velocity[i] = 0f;
					}
					int index3 = (index2 = i);
					num = anchoredPosition[index2];
					anchoredPosition[index3] = num + m_Velocity[i] * unscaledDeltaTime;
				}
				else
				{
					m_Velocity[i] = 0f;
