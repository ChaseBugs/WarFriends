...
public class Json
{
	internal class ObjectDecoder
	{
		...
		private object decode<T>(object decodedJsonObject, string rootElement = null) where T : new()
		{
			if (rootElement != null)
			{
				if (!(decodedJsonObject is IDictionary dictionary))
				{
		...
		private static Dictionary<string, Action<object, object>> getMembersWithSetters(object obj)
		{
			...
			{
				if (propertyInfo.PropertyType.Namespace.StartsWith("System") && propertyInfo.CanWrite && propertyInfo.GetSetMethod(nonPublic: true) != null)
				{
	...
	internal class Deserializer
	{
		...
		private object parseNumber(char[] json, ref int index)
		{
			...
			if (!text.Contains(".") && long.TryParse(text, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result))
			{
				return result;
			}
	...
	internal class Serializer
	{
		...
		private void serializeObject(object value)
		{
			...
			{
				Utils.logObject($"failed to serialize {value} with error: {ex.Message}");
			}
		}
