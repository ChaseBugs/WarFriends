...
public class HerdSimCore : MonoBehaviour
{
	...
	public void Disable(bool disableModel, bool disableCollider)
	{
		if (_enabled)
		{
			...
			if (disableModel)
			{
				_model.gameObject.SetActive(value: false);
			}
			if (disableCollider)
			{
	...
	public void Enable()
	{
		if (!_enabled)
		{
			...
			{
				_model.gameObject.SetActive(value: true);
			}
			if (!_collider.gameObject.activeInHierarchy)
			{
				_collider.gameObject.SetActive(value: true);
			}
	...
	public void OnDrawGizmos()
	{
		...
		if (_leader == this)
		{
			...
			Gizmos.DrawIcon(_collider.transform.position, "leader.png", allowScaling: false);
		}
		...
		if (_scared)
		{
			Gizmos.DrawIcon(_collider.transform.position, "scared.png", allowScaling: false);
		}
		if (_dead)
		{
			Gizmos.DrawIcon(_collider.transform.position, "dead.png", allowScaling: false);
		}
	}
}
