...
public static class tk2dSpriteGeomGen
{
	...
	public static void SetTiledSpriteGeom(Vector3[] pos, Vector2[] uv, int offset, out Vector3 boundsCenter, out Vector3 boundsExtents, tk2dSpriteDefinition spriteDef, Vector3 scale, Vector2 dimensions, tk2dBaseSprite.Anchor anchor, float colliderOffsetZ, float colliderExtentZ)
	{
		...
		boundsCenter.Set(vector.x * 0.5f + vector2.x, vector.y * 0.5f + vector2.y, colliderOffsetZ);
		boundsExtents.Set(vector.x * 0.5f, vector.y * 0.5f, colliderExtentZ);
		int num3 = 0;
		Vector3 vector3 = Vector3.Scale(spriteDef.untrimmedBoundsData[1], scale);
		...
		for (int i = 0; i < num2; i++)
		{
			...
			for (int j = 0; j < num; j++)
			{
				float num4 = 1f;
				float num5 = 1f;
				if (Mathf.Abs(vector4.x + vector3.x) > Mathf.Abs(vector.x))
				{
					num4 = vector.x % vector3.x / vector3.x;
				}
				if (Mathf.Abs(vector4.y + vector3.y) > Mathf.Abs(vector.y))
				{
					num5 = vector.y % vector3.y / vector3.y;
				}
				Vector3 vector5 = vector4 + zero;
				if (num4 != 1f || num5 != 1f)
				{
					Vector2 zero3 = Vector2.zero;
					Vector2 vector6 = new Vector2(num4, num5);
					Vector3 vector7 = new Vector3(Mathf.Lerp(spriteDef.positions[0].x, spriteDef.positions[3].x, zero3.x) * scale.x, Mathf.Lerp(spriteDef.positions[0].y, spriteDef.positions[3].y, zero3.y) * scale.y, spriteDef.positions[0].z * scale.z);
					...
					reference = vector5 + new Vector3(vector7.x, vector7.y, vector7.z);
					ref Vector3 reference2 = ref pos[offset + num3 + 1];
					reference2 = vector5 + new Vector3(vector8.x, vector7.y, vector7.z);
					ref Vector3 reference3 = ref pos[offset + num3 + 2];
					reference3 = vector5 + new Vector3(vector7.x, vector8.y, vector7.z);
					ref Vector3 reference4 = ref pos[offset + num3 + 3];
					reference4 = vector5 + new Vector3(vector8.x, vector8.y, vector7.z);
					if (spriteDef.flipped == tk2dSpriteDefinition.FlipMode.Tk2d)
					{
						Vector2 vector9 = new Vector2(Mathf.Lerp(spriteDef.uvs[0].x, spriteDef.uvs[3].x, zero3.y), Mathf.Lerp(spriteDef.uvs[0].y, spriteDef.uvs[3].y, zero3.x));
						...
						reference5 = new Vector2(vector9.x, vector9.y);
						ref Vector2 reference6 = ref uv[offset + num3 + 1];
						reference6 = new Vector2(vector9.x, vector10.y);
						ref Vector2 reference7 = ref uv[offset + num3 + 2];
						reference7 = new Vector2(vector10.x, vector9.y);
						ref Vector2 reference8 = ref uv[offset + num3 + 3];
						reference8 = new Vector2(vector10.x, vector10.y);
					}
					else if (spriteDef.flipped == tk2dSpriteDefinition.FlipMode.TPackerCW)
					{
						Vector2 vector11 = new Vector2(Mathf.Lerp(spriteDef.uvs[0].x, spriteDef.uvs[3].x, zero3.y), Mathf.Lerp(spriteDef.uvs[0].y, spriteDef.uvs[3].y, zero3.x));
						Vector2 vector12 = new Vector2(Mathf.Lerp(spriteDef.uvs[0].x, spriteDef.uvs[3].x, vector6.y), Mathf.Lerp(spriteDef.uvs[0].y, spriteDef.uvs[3].y, vector6.x));
						ref Vector2 reference9 = ref uv[offset + num3];
						reference9 = new Vector2(vector11.x, vector11.y);
						ref Vector2 reference10 = ref uv[offset + num3 + 2];
						reference10 = new Vector2(vector12.x, vector11.y);
						ref Vector2 reference11 = ref uv[offset + num3 + 1];
						reference11 = new Vector2(vector11.x, vector12.y);
						ref Vector2 reference12 = ref uv[offset + num3 + 3];
						reference12 = new Vector2(vector12.x, vector12.y);
					}
					else
					{
						Vector2 vector13 = new Vector2(Mathf.Lerp(spriteDef.uvs[0].x, spriteDef.uvs[3].x, zero3.x), Mathf.Lerp(spriteDef.uvs[0].y, spriteDef.uvs[3].y, zero3.y));
						...
						reference13 = new Vector2(vector13.x, vector13.y);
						ref Vector2 reference14 = ref uv[offset + num3 + 1];
						reference14 = new Vector2(vector14.x, vector13.y);
						ref Vector2 reference15 = ref uv[offset + num3 + 2];
						reference15 = new Vector2(vector13.x, vector14.y);
						ref Vector2 reference16 = ref uv[offset + num3 + 3];
						reference16 = new Vector2(vector14.x, vector14.y);
					}
				}
				else
				{
					ref Vector3 reference17 = ref pos[offset + num3];
					reference17 = vector5 + Vector3.Scale(spriteDef.positions[0], scale);
					ref Vector3 reference18 = ref pos[offset + num3 + 1];
					reference18 = vector5 + Vector3.Scale(spriteDef.positions[1], scale);
					ref Vector3 reference19 = ref pos[offset + num3 + 2];
					reference19 = vector5 + Vector3.Scale(spriteDef.positions[2], scale);
					ref Vector3 reference20 = ref pos[offset + num3 + 3];
					reference20 = vector5 + Vector3.Scale(spriteDef.positions[3], scale);
