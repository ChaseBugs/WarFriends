...
public class UIPopupList : MonoBehaviour
{
	...
	protected void Highlight(UILabel lbl, bool instant)
	{
		...
		if (atlasSprite != null)
		{
			...
			if (lbl.font.isDynamic)
			{
				vector += new Vector3(0f, Mathf.Round(lbl.transform.localScale.y * 0.1f), 0f);
			}
	...
	protected void AnimatePosition(UIWidget widget, bool placeAbove, float bottom)
	{
	...
	protected virtual void OnClick()
	{
		if (mChild == null && atlas != null && font != null && items.Count > 0)
		{
			...
			float num5 = ((!font.isDynamic) ? 0f : Mathf.Round(textLabel.transform.localScale.y * 0.1f));
			mHighlight.cachedTransform.localScale = new Vector3(a - (border.x + padding.x) * 2f + (atlasSprite.inner.xMin - atlasSprite.outer.xMin) * num4, num2 + num5 + num * num4, 1f);
			...
			{
				transform2.localPosition = new Vector3(bounds.min.x, (bounds.max.y - num3 - border.y + transform2.transform.localScale.y) * 0.5f, bounds.min.z);
			}
		}
		else
		{
