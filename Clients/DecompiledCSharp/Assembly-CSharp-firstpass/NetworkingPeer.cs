...
public class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	public static Dictionary<Type, List<MethodInfo>> cachedMethods = new Dictionary<Type, List<MethodInfo>>();
	...
	public void DebugReturn(DebugLevel level, string message)
	{
		...
		case DebugLevel.ERROR:
			...
			if (message.StartsWith("Receive issue: MessageSize too long. MTU"))
			{
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		case 231:
			if (operationResponse.ReturnCode != 0)
			{
				Debug.LogError(AuthValues.ToString());
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 209:
		{
			...
			if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
			{
				Debug.Log(string.Concat("Ev OwnershipRequest ", photonView.ownershipTransfer, ". ActorNr: ", num, " takes from: ", num4, ". Current owner: ", photonView.ownerId, " isOwnerActive: ", photonView.isOwnerActive, ". MasterClient: ", mMasterClientId, ". This client's player: ", PhotonNetwork.player.ToStringFull()));
			}
	...
	protected internal void ExecuteRpc(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
	{
		...
		int num6 = 0;
		int num7 = 0;
		if (photonView.cachedMonoBehaviour.ContainsKey(text))
		{
			num7++;
			MethodInfo mInfo = photonView.cachedMonoBehaviour[text].mInfo;
			MonoBehaviour mBehaviour = photonView.cachedMonoBehaviour[text].mBehaviour;
			...
			if (parameters.Length == array2.Length)
			{
				...
				{
					num6++;
					object obj2 = mInfo.Invoke(mBehaviour, array);
					if (mInfo.ReturnType == typeof(IEnumerator))
					{
						mBehaviour.StartCoroutine((IEnumerator)obj2);
					}
				}
			}
			else if (parameters.Length - 1 == array2.Length)
			{
				if (CheckTypeMatch(parameters, array2) && parameters[parameters.Length - 1].ParameterType == typeof(PhotonMessageInfo))
				{
					...
					object obj3 = mInfo.Invoke(mBehaviour, array3);
					if (mInfo.ReturnType == typeof(IEnumerator))
					{
						mBehaviour.StartCoroutine((IEnumerator)obj3);
					}
				}
			}
			...
			{
				num6++;
				object obj4 = mInfo.Invoke(mBehaviour, new object[1] { array });
				if (mInfo.ReturnType == typeof(IEnumerator))
				{
					mBehaviour.StartCoroutine((IEnumerator)obj4);
				}
			}
		}
		if (num6 == 1)
		...
		foreach (Type type in array2)
		{
			...
			text2 = ((type != null) ? (text2 + type.Name) : (text2 + "null"));
		}
		if (num6 == 0)
		{
			if (num7 == 0)
	...
	public void RegisterPooledPhotonView(PhotonView netView)
	{
		if (netView.viewIdField != 0)
		{
			if (photonViewList.ContainsKey(netView.viewID) && netView != photonViewList[netView.viewID])
			{
				Debug.LogError($"PhotonView ID duplicate found: {netView.viewID}. New: {netView} old: {photonViewList[netView.viewID]}. Maybe one wasn't destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.");
			}
	...
	public void ChangePhotonView(PhotonView netView)
	{
		if (netView.viewIdField != 0)
		{
			photonViewList[netView.viewID] = netView;
