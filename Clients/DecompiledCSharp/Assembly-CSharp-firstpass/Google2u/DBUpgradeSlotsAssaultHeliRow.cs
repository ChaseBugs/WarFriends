...
[Serializable]
public class DBUpgradeSlotsAssaultHeliRow : IGoogle2uRow
{
	public int TIER;
	...
	public DBUpgradeSlotsAssaultHeliRow()
	{
	}

	public DBUpgradeSlotsAssaultHeliRow(string _GOOGLEFU_ID, string _TIER, string _HP, string _DAMAGE, string _BATCHSIZEMIN, string _BATCHSIZEMAX, string _SHOTFREQUENCYMIN, string _SHOTFREQUENCYMAX, string _REALSHOTPROBABILITY, string _NEXTUPGRADEPRICE, string _NEXTUPGRADEPRICEGOLD, string _DELIVERYTIME, string _DPS, string _ARMYPOWER, string _SPECIAL, string _SOLDIERHP, string _GLASSHP, string _SPEED, string _ELITE)
	{
		...
		if (int.TryParse(_NEXTUPGRADEPRICE, out var result9))
		{
		...
		if (int.TryParse(_NEXTUPGRADEPRICEGOLD, out var result10))
		{
		...
		if (float.TryParse(_DELIVERYTIME, out var result11))
		{
		...
		if (float.TryParse(_DPS, out var result12))
		{
			DPS = result12;
		}
		else
		...
		if (float.TryParse(_ARMYPOWER, out var result13))
		{
			ARMYPOWER = result13;
		}
		else
		...
		if (float.TryParse(_SPECIAL, out var result14))
		{
			SPECIAL = result14;
		}
		else
		...
		if (float.TryParse(_SOLDIERHP, out var result15))
		{
			SOLDIERHP = result15;
		}
		else
		...
		if (float.TryParse(_GLASSHP, out var result16))
		{
			GLASSHP = result16;
		}
		else
		{
			Debug.LogError("Failed To Convert GLASSHP string: " + _GLASSHP + " to float");
		}
		if (float.TryParse(_SPEED, out var result17))
		{
			SPEED = result17;
		}
		else
		...
		if (float.TryParse(_ELITE, out var result18))
		{
			ELITE = result18;
		}
		else
	...
	public override string ToString()
	{
		...
		empty = empty + "{SOLDIERHP : " + SOLDIERHP + "} ";
		empty = empty + "{GLASSHP : " + GLASSHP + "} ";
		empty = empty + "{SPEED : " + SPEED + "} ";
