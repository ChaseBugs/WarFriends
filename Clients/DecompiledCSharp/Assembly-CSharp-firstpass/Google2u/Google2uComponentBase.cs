...
public class Google2uComponentBase : MonoBehaviour
{
	...
	public virtual int getColumnCount => -1;
	...
	public bool HasColumn(string fieldName)
	{
		...
		if (!mRowProperties.TryGetValue(fieldName, out var value))
		{
	...
	public void SetValue(int rowIndex, string columnName, string value)
	{
		...
		if (row != null)
		{
			...
			catch (Exception)
			{
				...
				{
					Debug.LogError($"Can not convert {value} for column {columnName} to type {propInfo.PropertyType} in sheet {GetType()}");
				}
				return;
			}
		}
	...
	private object GetRow(int index)
	{
		...
		if (mRows != null)
		{
			...
			Debug.LogError($"Row index {index} out of range for sheet {base.gameObject.name} {GetType()}");
		}
		return null;
	}
}
