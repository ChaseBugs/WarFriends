...
public class RopeTubeRenderer
{
		...
		for (int i = 0; i < points.Length; i++)
		{
			if (radiuses != null)
			{
				...
				for (int j = 0; j < edgeCount + 1; j++)
				{
					...
					reference = _transform.InverseTransformPoint(points[i] + rotations[i] * circleLookup[j] * radiuses[i]);
					ref Vector3 reference2 = ref normals[num];
					reference2 = _transform.InverseTransformDirection(rotations[i] * circleLookup[j]);
					num++;
				}
			}
			else
			{
				...
				{
					ref Vector3 reference3 = ref vertices[num];
					reference3 = _transform.InverseTransformPoint(points[i] + rotations[i] * circleLookup[k] * radius);
					ref Vector3 reference4 = ref normals[num];
					reference4 = _transform.InverseTransformDirection(rotations[i] * circleLookup[k]);
					num++;
				}
			}
		}
		ref Vector3 reference5 = ref vertices[0];
		reference5 = _transform.InverseTransformPoint(points[0]);
		ref Vector3 reference6 = ref vertices[vertices.Length - 1];
		reference6 = _transform.InverseTransformPoint(points[points.Length - 1]);
		ref Vector3 reference7 = ref normals[0];
		reference7 = _transform.InverseTransformDirection(rotations[0] * Vector3.forward);
		ref Vector3 reference8 = ref normals[targetVertexCount - 1];
		reference8 = _transform.InverseTransformDirection(rotations[0] * -Vector3.forward);
		num = 1;
		for (int l = 0; l < edgeCount + 1; l++)
		{
			ref Vector3 reference9 = ref vertices[num];
			reference9 = vertices[num + edgeCount + 1];
			ref Vector3 reference10 = ref normals[num];
			reference10 = normals[0];
			num++;
		}
		num = vertices.Length - edgeCount - 2;
		for (int m = 0; m < edgeCount + 1; m++)
		{
			ref Vector3 reference11 = ref vertices[num];
			reference11 = vertices[num - edgeCount - 1];
			ref Vector3 reference12 = ref normals[num];
			reference12 = normals[targetVertexCount - 1];
			num++;
		}
	...
	private void UpdateCircleLookup()
	{
		...
		for (int i = 0; i < circleLookup.Length; i++)
		{
			...
			ref Vector3 reference = ref circleLookup[i];
			reference = new Vector3(0f, Mathf.Cos(f), Mathf.Sin(f));
		}
	}
	...
	private void UpdateUVs()
	{
		...
		int num3 = 0;
		ref Vector2 reference = ref uvs[num3++];
		reference = new Vector2(capUVRect.width * 0.5f + capUVRect.x, capUVRect.height * 0.5f + capUVRect.y);
		for (int i = 0; i < edgeCount + 1; i++)
		{
			float f = (float)i * num2 * ((float)Math.PI * 2f) + (float)Math.PI / 2f;
			ref Vector2 reference2 = ref uvs[num3++];
			reference2 = new Vector2(uvs[0].x + Mathf.Cos(f) * 0.5f * capUVRect.width, uvs[0].y + Mathf.Sin(f) * 0.5f * capUVRect.height);
		}
		for (int j = 0; j < points.Length; j++)
		{
			...
			for (int k = 0; k < edgeCount + 1; k++)
			{
				...
				ref Vector2 reference3 = ref uvs[num3++];
				reference3 = new Vector2(bodyUVRect.x + num4 * bodyUVRect.width, bodyUVRect.y + num5 * bodyUVRect.height);
			}
		}
		for (int l = 0; l < edgeCount + 1; l++)
		{
			ref Vector2 reference4 = ref uvs[num3++];
			reference4 = uvs[l + 1];
		}
		ref Vector2 reference5 = ref uvs[num3++];
		reference5 = uvs[0];
	}
	...
	private void UpdateTangents()
	{
		...
		{
			ref Vector4 reference = ref tangents[num++];
			reference = new Vector4(vector.x, vector.y, vector.z, 1f);
		}
		for (int j = 0; j < rotations.Length; j++)
		{
			...
			if (calculateTangents)
			{
				for (int k = 0; k < edgeCount + 1; k++)
				{
					ref Vector4 reference2 = ref tangents[num++];
					reference2 = new Vector4(vector.x, vector.y, vector.z, 1f);
				}
			}
		}
		...
		for (int l = 0; l < edgeCount + 2; l++)
		{
			...
			reference3 = new Vector4(vector.x, vector.y, vector.z, 1f);
		}
	}
	...
	private void UpdateColors()
	{
		...
		reference = pointColors[0];
		for (int i = 0; i < edgeCount + 1; i++)
		{
			ref Color reference2 = ref colors[num++];
			reference2 = pointColors[0];
		}
		for (int j = 0; j < points.Length; j++)
		{
			for (int k = 0; k < edgeCount + 1; k++)
			{
				ref Color reference3 = ref colors[num++];
				reference3 = pointColors[j];
			}
		}
		for (int l = 0; l < edgeCount + 1; l++)
		{
			ref Color reference4 = ref colors[num++];
			reference4 = pointColors[pointColors.Length - 1];
		}
		ref Color reference5 = ref colors[num++];
		reference5 = pointColors[pointColors.Length - 1];
		mesh.colors = colors;
	}
