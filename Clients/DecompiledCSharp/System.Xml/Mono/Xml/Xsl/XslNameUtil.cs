...
internal class XslNameUtil
{
	public static XmlQualifiedName[] FromListString(string names, XPathNavigator current)
	{
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			{
				array2[num++] = FromString(array[j], current, useDefaultXmlns: true);
			}
		}
		return array2;
	}
	...
	{
		return FromString(name, current, useDefaultXmlns: false);
	}
	...
	public static XmlQualifiedName FromString(string name, XmlNamespaceManager ctx)
	{
		...
		if (num > 0)
		{
			return new XmlQualifiedName(name.Substring(num + 1), ctx.LookupNamespace(name.Substring(0, num), atomizedNames: false));
		}
		if (num < 0)
		{
			return new XmlQualifiedName(name, string.Empty);
