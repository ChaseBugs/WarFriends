...
internal class XsdValidatingReader : XmlReader, IHasXmlParserContext, IHasXmlSchemaInfo, IXmlLineInfo
{
	...
	private void AssessStartElementSchemaValidity()
	{
		...
		if (text2 != null)
		{
			...
			else
			{
				...
				if (xsiType is XmlSchemaComplexType { IsAbstract: not false })
				{
	...
	public override bool Read()
	{
		...
		{
			if (reader is DTDValidatingReader { DTD: null } dTDValidatingReader)
			{
		...
		case XmlNodeType.SignificantWhitespace:
			...
			if (Context.ActualType is XmlSchemaComplexType { ContentType: var contentType } xmlSchemaComplexType)
			{
				switch (contentType)
				{
				case XmlSchemaContentType.ElementOnly:
