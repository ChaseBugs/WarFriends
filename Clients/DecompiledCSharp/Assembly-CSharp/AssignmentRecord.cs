...
public class AssignmentRecord : Core_BaseScript
{
	...
	private void SetInitialLook(Assignment.State state, bool firstShow = false)
	{
		...
		rewardPart.SetActive(state switch
		{
			Assignment.State.Finishing => firstShow, 
			Assignment.State.InProgress => true, 
			_ => false, 
		});
		...
		claimPart.SetActive(state switch
		{
			Assignment.State.Finishing => !firstShow, 
			Assignment.State.Done => true, 
			_ => false, 
		});
