...
public class IntroductionDealsMoneyPack : IntroductionDealsItem
{
	...
	{
		GuiElementSingle<CardsAndPacksDialog>.instance.ShowDialog(CardsAndPacksDialog.DialogType.MONEY_PACK);
		SetShowTimeEnd(0.5f);
	}
	...
	{
		base.InitGuiValues();
		float num = Singleton<GameVariables>.instance.SaleOfPack(CardPack.Money);
	...
	private IEnumerator TimerWorker()
	{
		while (true)
		{
			int moneyPackTimeOffer = PlayerAnalytics.instance.data.moneyPackDeadline - Singleton<BeanstalkServerManager>.instance.currentTimestamp;
			timer.text = string.Format("{0} {1}{2}", Localization.Localize("ID_OFFERACTIVE"), Colours.stringBlue, MiscTools.PrintableTime(moneyPackTimeOffer, "-", string.Empty));
