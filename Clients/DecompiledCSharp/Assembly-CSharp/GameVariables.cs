...
public class GameVariables : Singleton<GameVariables>
{
	...
	{
		return beginnersNumber switch
		{
			1 => Localization.Localize("ID_BEGINNERSLEAGUE3"), 
			2 => Localization.Localize("ID_BEGINNERSLEAGUE2"), 
			3 => Localization.Localize("ID_BEGINNERSLEAGUE1"), 
			_ => leagueNames[League.Bronze3].Value1, 
		};
	}
	...
	public Color LeagueColorHighlight(LeagueType type)
	{
		return type switch
		{
			LeagueType.Promote => Colours.greenLeague, 
			LeagueType.Demote => Colours.redLeague, 
			LeagueType.Top => Colours.yellowLeague, 
			_ => Colours.blue, 
		};
	}
	...
	public Color LeaguePositionColorLeagueRecordBg(LeagueType type)
	{
		return type switch
		{
			LeagueType.Promote => Colours.greenLeagueBg, 
			LeagueType.Demote => Colours.redLeagueBg, 
			LeagueType.Top => Colours.goldLeagueBg, 
			_ => Colours.grayLeagueBg, 
		};
	}
	...
	public CardPacksRow GetCardPackRow(CardPack packName)
	{
		...
		{
			CardPack.Gold => Singleton<GameVariables>.instance.cardPacks.GetRow(CardPacks.rowIds.GOLD_CARDPACK), 
			CardPack.Silver => Singleton<GameVariables>.instance.cardPacks.GetRow(CardPacks.rowIds.SILVER_CARDPACK), 
			_ => Singleton<GameVariables>.instance.cardPacks.GetRow(CardPacks.rowIds.BRONZE_CARDPACK), 
		};
	}
	...
	public int ReputationPointsForWarcard(CardManager.CardType rarity)
	{
		...
		{
			CardManager.CardType.Buddy => (int)cardConstants.GetRow(CardConstants.rowIds.RepPointsBuddyCard).FLOATVALUE, 
			CardManager.CardType.Gold => (int)cardConstants.GetRow(CardConstants.rowIds.RepPointsGoldCard).FLOATVALUE, 
			CardManager.CardType.Silver => (int)cardConstants.GetRow(CardConstants.rowIds.RepPointsSilverCard).FLOATVALUE, 
			_ => (int)cardConstants.GetRow(CardConstants.rowIds.RepPointsBronzeCard).FLOATVALUE, 
		};
	}
