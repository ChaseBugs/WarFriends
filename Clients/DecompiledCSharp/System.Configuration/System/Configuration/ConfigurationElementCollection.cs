...
public abstract class ConfigurationElementCollection : ConfigurationElement, ICollection, IEnumerable
{
	...
	private bool IsBasic => CollectionType == ConfigurationElementCollectionType.BasicMap || CollectionType == ConfigurationElementCollectionType.BasicMapAlternate;

	private bool IsAlternate => CollectionType == ConfigurationElementCollectionType.AddRemoveClearMapAlternate || CollectionType == ConfigurationElementCollectionType.BasicMapAlternate;
	...
	protected internal override bool SerializeElement(XmlWriter writer, bool serializeCollectionKey)
	{
		...
		else
		{
			if (emitClear)
			{
				writer.WriteElementString(clearElementName, string.Empty);
				flag = true;
			}
			if (removed != null)
			{
			...
			{
				ConfigurationElement configurationElement2 = (ConfigurationElement)list[k];
				configurationElement2.SerializeToXmlElement(writer, addElementName);
			}
	...
	protected override bool OnDeserializeUnrecognizedElement(string elementName, XmlReader reader)
	{
		...
		else
		{
			...
			{
				ConfigurationElement origElement = CreateNewElementInternal(null);
				ConfigurationRemoveElement configurationRemoveElement = new ConfigurationRemoveElement(origElement, this);
	...
	protected internal override void Unmerge(ConfigurationElement sourceElement, ConfigurationElement parentElement, ConfigurationSaveMode updateMode)
	{
		...
		for (int i = 0; i < configurationElementCollection.Count; i++)
		{
			...
			{
				ConfigurationElement configurationElement4 = CreateNewElementInternal(null);
				configurationElement4.Unmerge(configurationElement, null, ConfigurationSaveMode.Full);
				BaseAdd(configurationElement4);
			}
		}
		...
		else
		{
			...
			for (int j = 0; j < configurationElementCollection2.Count; j++)
			{
				...
				if (configurationElementCollection.IndexOfKey(elementKey2) == -1)
				{
					...
					removed.Add(configurationElement5);
				}
			}
		}
	}
