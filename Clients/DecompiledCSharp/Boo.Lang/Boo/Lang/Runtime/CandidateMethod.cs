...
public class CandidateMethod
{
	...
	public int[] ArgumentScores => _argumentScores;

	public bool VarArgs => _varArgs;

	public int MinimumArgumentCount => (!_varArgs) ? Parameters.Length : (Parameters.Length - 1);

	public ParameterInfo[] Parameters => _method.GetParameters();

	public Type VarArgsParameterType => GetParameterType(Parameters.Length - 1).GetElementType();

	public bool DoesNotRequireConversions => !RequiresConversions;

	private bool RequiresConversions => Array.Exists(_argumentScores, RequiresConversion);
