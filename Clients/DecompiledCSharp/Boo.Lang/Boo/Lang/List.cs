...
public class List<T> : IEnumerable, ICollection, IEnumerable<T>, IList<T>, ICollection<T>, IEquatable<List<T>>, IList
{
	...
	public List<T> Multiply(int count)
	{
		...
		return NewConcreteList(array, takeOwnership: true);
	}
	...
	public List<T> Collect(Predicate<T> condition)
	{
		...
		List<T> list = NewConcreteList(new T[0], takeOwnership: true);
	...
	private List<T> InnerGetRange(int begin, int end)
	{
		...
		return NewConcreteList(new T[0], takeOwnership: true);
	}
	...
	{
		List<T> list = lhs.NewConcreteList(lhs.ToArray(), takeOwnership: true);
		list.Extend(rhs);
		return list;
	}
}
