...
public sealed class NetworkServer
{
	...
	internal void InternalListenRelay(string relayIp, int relayPort, NetworkID netGuid, SourceID sourceId, NodeID nodeId, int listenPort)
	{
		...
		Update();
		NetworkTransport.ConnectAsNetworkHost(m_ServerId, relayIp, relayPort, netGuid, sourceId, nodeId, out var error);
		m_RelaySlotId = 0;
		if (LogFilter.logDebug)
		{
	...
	public static bool SendToAll(short msgType, MessageBase msg)
	{
		...
		ConnectionArray connectionArray = instance.m_Connections;
		bool flag = true;
		for (int i = connectionArray.LocalIndex; i < connectionArray.Count; i++)
		{
			NetworkConnection networkConnection = connectionArray.Get(i);
	...
	{
		ConnectionArray connectionArray = instance.m_Connections;
		if (connectionArray.ContainsPlayer(player, out var conn))
		{
	...
	internal void InternalUpdate()
	{
		...
		do
		{
			networkEventType = NetworkTransport.ReceiveFromHost(m_ServerId, out var connectionId, out var channelId, m_MsgBuffer, (ushort)m_MsgBuffer.Length, out var receivedSize, out error);
			if (networkEventType != NetworkEventType.Nothing && LogFilter.logDev)
			{
				Debug.Log(string.Concat("Server event: host=", m_ServerId, " event=", networkEventType, " error=", error));
			}
			...
			case NetworkEventType.ConnectEvent:
			{
				...
				NetworkTransport.GetConnectionInfo(m_ServerId, connectionId, out var address, out var _, out var _, out var _, out var _);
				NetworkConnection networkConnection3 = (NetworkConnection)Activator.CreateInstance(s_NetworkConnectionClass);
				networkConnection3.SetHandlers(m_MessageHandlers);
				networkConnection3.Initialize(address, m_ServerId, connectionId, m_HostTopology);
				networkConnection3.SetMaxDelay(m_MaxDelay);
				m_Connections.Add(connectionId, networkConnection3);
				networkConnection3.InvokeHandlerNoData(32);
				if (m_SendPeerInfo)
				{
					SendNetworkInfo(networkConnection3);
				}
				SendCrc(networkConnection3);
				break;
			}
			...
			{
				NetworkConnection networkConnection2 = m_Connections.GetUnsafe(connectionId);
				if (error != 0 && error != 6)
				{
					GenerateDisconnectError(networkConnection2, error);
				}
				m_Connections.Remove(connectionId);
				if (networkConnection2 != null)
				{
					networkConnection2.InvokeHandlerNoData(33);
					for (int i = 0; i < networkConnection2.playerControllers.Count; i++)
					{
						if (networkConnection2.playerControllers[i].gameObject != null && LogFilter.logWarn)
						{
					...
					networkConnection2.RemoveObservers();
					networkConnection2.Dispose();
				}
	...
	{
		ConnectionArray connectionArray = instance.m_Connections;
		NetworkConnection networkConnection = connectionArray.Get(connectionId);
		if (networkConnection != null)
		{
			networkConnection.Send(msgType, msg);
		}
	...
	public static void ResetConnectionStats()
	{
		...
		for (int i = connectionArray.LocalIndex; i < connectionArray.Count; i++)
		{
			connectionArray.Get(i)?.ResetStats();
		}
	}
	...
	private static void SendCrc(NetworkConnection targetConnection)
	{
		...
		foreach (string key in NetworkCRC.singleton.scripts.Keys)
		{
			...
			{
				name = key,
				channel = (byte)NetworkCRC.singleton.scripts[key]
			});
		}
	...
	public void SendNetworkInfo(NetworkConnection targetConnection)
	{
		...
		for (int i = 0; i < m_Connections.Count; i++)
		{
			...
			if (networkConnection != null)
			{
				...
				NetworkTransport.GetConnectionInfo(m_ServerId, networkConnection.connectionId, out var address, out var port, out var _, out var _, out var _);
