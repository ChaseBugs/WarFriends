...
public sealed class NetworkIdentity : MonoBehaviour
{
	...
	internal void OnStartServer()
	{
		...
		{
			Debug.LogError(string.Concat("Object has non-zero netId ", netId, " for ", base.gameObject, " !!1"));
		}
	}
	...
	internal void HandleCommand(int cmdHash, NetworkReader reader)
	{
		...
		if (!flag)
		{
			...
			if (LogFilter.logError)
			{
				Debug.LogError(string.Concat("Found no receiver for incoming command [", cmdHashHandlerName2, "] on ", base.gameObject, ",  the server and client should have the same NetworkBehaviour instances [netId=", netId, "]."));
			}
		}
	}
	...
	internal void UNetUpdate()
	{
		...
		for (int j = 0; j < NetworkServer.numChannels; j++)
		{
			...
			for (int k = 0; k < m_NetworkBehaviours.Length; k++)
			{
				...
				{
					Debug.LogWarning(string.Concat("Large state update of ", s_UpdateWriter.Position - position, " bytes (max is ", NetworkServer.maxPacketSize, " for netId:", netId, " from script:", networkBehaviour2));
				}
			}
			if (flag)
			{
