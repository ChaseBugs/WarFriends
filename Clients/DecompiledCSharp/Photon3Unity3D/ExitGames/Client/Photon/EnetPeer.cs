...
internal class EnetPeer : PeerBase
{
	...
	private Encryptor encryptor => ppeer.encryptor;

	private Decryptor decryptor => ppeer.decryptor;
				...
				lock (sentReliableCommands)
				{
					...
					foreach (NCommand sentReliableCommand in sentReliableCommands)
					{
						...
						if (sentReliableCommand.commandSentCount > base.sentCountAllowance || timeInt > sentReliableCommand.timeoutTime)
						{
							...
							if (CommandLog != null)
							{
								CommandLog.Enqueue(new CmdLogSentReliable(sentReliableCommand, timeInt, roundTripTime, roundTripTimeVariance, triggeredTimeout: true));
								CommandLogResize();
							}
	...
	{
		channels.TryGetValue(command.commandChannelID, out var value);
		if (value == null)
		{
			if ((int)base.debugOut >= 1)
			{
