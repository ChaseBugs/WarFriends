...
public class EndpointPermission
{
	...
	{
		return obj is EndpointPermission endpointPermission && port == endpointPermission.port && transport == endpointPermission.transport && string.Compare(hostname, endpointPermission.hostname, ignoreCase: true) == 0;
	}
	...
	internal EndpointPermission Intersect(EndpointPermission perm)
	{
		...
		EndpointPermission endpointPermission = new EndpointPermission(text, num, transportType);
		endpointPermission.hasWildcard = true;
		endpointPermission.resolved = true;
