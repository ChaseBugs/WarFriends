	...
	public override IPermission CreatePermission()
	{
		...
		else
		{
			if (string.Compare(m_access, "Accept", ignoreCase: true) != 0)
			{
				...
				throw new ArgumentException(string.Format(text3, m_access));
			}
		...
		if (string.Compare(m_port, "All", ignoreCase: true) != 0)
		{
			...
			catch
			{
				...
				throw new ArgumentException(string.Format(text4, m_port));
			}
		...
		try
		{
			transport = (TransportType)(int)Enum.Parse(typeof(TransportType), m_transport, ignoreCase: true);
		}
		catch
		{
			string text5 = global::Locale.GetText("The parameter value for 'Transport', '{1}, is invalid.");
			throw new ArgumentException(string.Format(text5, m_transport));
		}
