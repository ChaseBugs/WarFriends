...
public class HttpWebRequest : WebRequest, ISerializable
{
	...
	static HttpWebRequest()
	{
		...
		if (ConfigurationSettings.GetConfig("system.net/settings") is System.Net.NetConfig { MaxResponseHeadersLength: var num })
		{
			if (num != -1)
			{
				num *= 64;
	...
	{
		if (webResponse.GetResponseStream() is System.Net.WebConnectionStream { Connection: var connection })
		{
	...
	internal void SetResponseData(System.Net.WebConnectionData data)
	{
		lock (locker)
		{
			...
			try
			{
				...
				{
					if (is_ntlm_auth && authCompleted && webResponse != null && webResponse.StatusCode < HttpStatusCode.BadRequest && webResponse.GetResponseStream() is System.Net.WebConnectionStream { Connection: var connection })
					{
