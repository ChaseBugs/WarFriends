...
internal class Win32EventLog : System.Diagnostics.EventLogImpl
{
	...
	private int OldestEventLogEntry
	{
		...
		{
			int OldestRecord = 0;
			int oldestEventLogRecord = PInvoke.GetOldestEventLogRecord(ReadHandle, ref OldestRecord);
	...
	{
		int num = PInvoke.ClearEventLog(ReadHandle, null);
		if (num != 1)
		{
			throw new Win32Exception(Marshal.GetLastWin32Error());
		}
	...
	protected override int GetEntryCount()
	{
		...
		int numberOfEventLogRecords = PInvoke.GetNumberOfEventLogRecords(ReadHandle, ref NumberOfRecords);
	...
	protected override EventLogEntry GetEntry(int index)
	{
		...
		string category = FormatCategory(source, num5);
		return new EventLogEntry(category, num5, index2, eventID, source, message, userName, machineName, (EventLogEntryType)entryType, timeGenerated, timeWritten, array2, array, num3);
	}
	...
	public override void WriteEntry(string[] replacementStrings, EventLogEntryType type, uint instanceID, short category, byte[] rawData)
	{
		...
		try
		{
			int num = PInvoke.ReportEvent(intPtr, (ushort)type, (ushort)category, instanceID, IntPtr.Zero, (ushort)replacementStrings.Length, (uint)rawData.Length, replacementStrings, rawData);
			if (num != 1)
			{
				throw new Win32Exception(Marshal.GetLastWin32Error());
			}
	...
	private void ReadEventLog(int index, byte[] buffer, ref int bytesRead, ref int minBufferNeeded)
	{
		for (int i = 0; i < 3; i++)
		{
			int num = PInvoke.ReadEventLog(ReadHandle, (ReadFlags)6, index, buffer, buffer.Length, ref bytesRead, ref minBufferNeeded);
			if (num != 1)
			{
	...
	{
		int num = PInvoke.CloseEventLog(hEventLog);
		if (num != 1)
		{
			throw new Win32Exception(Marshal.GetLastWin32Error());
		}
	...
	{
		int num = PInvoke.DeregisterEventSource(hEventLog);
		if (num != 1)
		{
			throw new Win32Exception(Marshal.GetLastWin32Error());
		}
	...
	private static string FetchMessage(string msgDll, uint messageID, string[] replacementStrings)
	{
		...
		try
		{
			for (int i = 0; i < replacementStrings.Length; i++)
			{
				...
				reference = Marshal.StringToHGlobalAuto(replacementStrings[i]);
			}
	...
	public override void EnableNotification()
	{
		...
		_notifyThread = new Thread(NotifyEventThread);
