...
internal class NativeDapiProtection
{
	...
	[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
	private struct CRYPTPROTECT_PROMPTSTRUCT(uint flags)
	{
		private int cbSize = Marshal.SizeOf(typeof(CRYPTPROTECT_PROMPTSTRUCT));

		private uint dwPromptFlags = flags;

		private IntPtr hwndApp = IntPtr.Zero;

		private string szPrompt = null;
	}
