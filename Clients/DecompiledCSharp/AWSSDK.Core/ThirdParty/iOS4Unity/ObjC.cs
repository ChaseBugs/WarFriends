...
public static class ObjC
{
	...
	{
		return ToNSArray(items.Select((NSObject i) => i.Handle).ToArray());
	}

	public static IntPtr ToNSArray(string[] items)
	{
		return ToNSArray(items.Select((string s) => ToNSString(s)).ToArray());
	}
	...
	public unsafe static IntPtr ToNSString(string str)
	{
		...
		fixed (char* ptr = str + (IntPtr)(RuntimeHelpers.OffsetToStringData / 2))
		{
			return MessageSendIntPtr(receiver, InitWithCharacters_lengthSelector.Handle, (IntPtr)ptr, str.Length);
		}
	}
	...
	{
		return MessageSendIntPtr(GetClass("NSDate"), DateWithTimeIntervalSinceReferenceDateSelector.Handle, (date.Ticks - 631139040000000000L) / 10000000);
	}
