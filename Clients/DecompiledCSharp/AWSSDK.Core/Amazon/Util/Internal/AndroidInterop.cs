...
public class AndroidInterop
{
	public static T CallStaticJavaMethod<T>(string className, string methodName, params object[] parameters)
	{
		...
		if (typeFromUnityEngine != null)
		{
			...
			if (methodInfo != null)
			{
				return (T)methodInfo.Invoke(obj, new object[2] { methodName, parameters });
			}
		}
		return default(T);
	}
	...
	public static T CallMethod<T>(object androidJavaObject, string methodName, params object[] parameters)
	{
		return (T)(from x in androidJavaObject.GetType().GetMethods()
			where x.Name == "Call"
			select x).First((MethodInfo x) => x.ContainsGenericParameters).MakeGenericMethod(typeof(T)).Invoke(androidJavaObject, new object[2] { methodName, parameters });
	}
	...
	public static object CallMethod(object androidJavaObject, string methodName, params object[] parameters)
	{
		...
		return (from x in androidJavaObject.GetType().GetMethods()
			where x.Name == "Call"
			select x).First((MethodInfo x) => x.ContainsGenericParameters).MakeGenericMethod(typeFromUnityEngine).Invoke(androidJavaObject, new object[2] { methodName, parameters });
	}
	...
	public static T GetStaticJavaField<T>(string className, string methodName)
	{
		...
		if (typeFromUnityEngine != null)
		{
			...
			if (methodInfo != null)
			{
				return (T)methodInfo.Invoke(obj, new object[1] { methodName });
			}
		}
		return default(T);
	}
	...
	public static object GetStaticJavaField(string className, string methodName)
	{
		...
		if (typeFromUnityEngine != null)
		{
			...
			if (methodInfo != null)
			{
				return methodInfo.Invoke(obj, new object[1] { methodName });
			}
		}
		return null;
	}
	...
	public static T GetJavaField<T>(object androidJavaObject, string methodName)
	{
		return (T)(from x in androidJavaObject.GetType().GetMethods()
			where x.Name == "Get"
			select x).First((MethodInfo x) => x.ContainsGenericParameters).MakeGenericMethod(typeof(T)).Invoke(androidJavaObject, new object[1] { methodName });
	}
