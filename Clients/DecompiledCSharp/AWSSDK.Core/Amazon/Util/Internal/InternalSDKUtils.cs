...
public static class InternalSDKUtils
{
	...
	internal const string CoreVersionNumber = "3.3.14.1";
	...
	public static string BuildUserAgentString(string serviceSdkVersion)
	{
		...
		IEnvironmentInfo service = ServiceFactory.Instance.GetService<IEnvironmentInfo>();
		return string.Format(CultureInfo.InvariantCulture, "{0}/{1} aws-sdk-core/{2} {3} OS/{4} {5}", _userAgentBaseName, serviceSdkVersion, "3.3.14.1", service.FrameworkUserAgent, service.PlatformUserAgent, _customData).Trim();
	}
