...
public class DefaultRetryPolicy : RetryPolicy
{
	...
	private static readonly HashSet<string> _coreCLRRetryErrorMessages = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "The server returned an invalid or unrecognized response", "The connection with the server was terminated abnormally", "An error occurred while sending the request.", "Failed sending data to the peer" };

	private ICollection<string> _errorCodesToRetryOn = new HashSet<string> { "Throttling", "ThrottlingException", "ProvisionedThroughputExceededException", "RequestTimeout" };
	...
	public override void NotifySuccess(IExecutionContext executionContext)
	{
		if (executionContext.RequestContext.ClientConfig.ThrottleRetries && _retryCapacity != null)
		{
			_capacityManagerInstance.TryReleaseCapacity(executionContext.RequestContext.Retries > 0, _retryCapacity);
		}
	}
	...
	private bool RetryForExceptionSync(Exception exception)
	{
		...
		if (exception is AmazonServiceException ex)
		{
			...
			if (IsInnerException<WebException>(exception, out var inner) && WebExceptionStatusesToRetryOn.Contains(inner.Status))
			{
				return true;
			}
		}
