...
public abstract class AmazonServiceClient : IDisposable
{
	...
	{
		ThrowIfDisposed();
		ExecutionContext executionContext = new ExecutionContext(new RequestContext(Config.LogMetrics, Signer)
		{
			ClientConfig = Config,
	...
	protected IAsyncResult BeginInvoke<TRequest>(TRequest request, IMarshaller<IRequest, AmazonWebServiceRequest> marshaller, ResponseUnmarshaller unmarshaller, AsyncOptions asyncOptions, Action<AmazonWebServiceRequest, AmazonWebServiceResponse, Exception, AsyncOptions> callbackHelper) where TRequest : AmazonWebServiceRequest
	{
		...
		AsyncExecutionContext executionContext = new AsyncExecutionContext(new AsyncRequestContext(Config.LogMetrics, Signer)
		{
			ClientConfig = Config,
			Marshaller = marshaller,
			OriginalRequest = request,
	...
	protected static TResponse EndInvoke<TResponse>(IAsyncResult result) where TResponse : AmazonWebServiceResponse
	{
		...
		if (!(result is RuntimeAsyncResult runtimeAsyncResult))
		{
	...
	private void BuildRuntimePipeline()
	{
		...
		httpHandler = ((AWSConfigs.HttpClient != AWSConfigs.HttpClientOption.UnityWWW) ? new HttpHandler<string>(new UnityWebRequestFactory(), this) : new HttpHandler<string>(new UnityWwwRequestFactory(), this));
	...
	public static Uri ComposeUrl(IRequest iRequest)
	{
		...
		string text2 = AWSSDKUtils.UrlEncode(text, path: true) + stringBuilder;
		Uri uri = new Uri(endpoint.AbsoluteUri + text2);
