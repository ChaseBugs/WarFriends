...
public class VectorLine
{
	...
	public Color color => m_lineColors[0];
	...
	public float lineWidth
	{
		...
		set
		{
			...
			{
				RedoLine(use1Pixel: true);
			}
			else if (m_1pixelLine && value != 1f)
			{
				RedoLine(use1Pixel: false);
			}
		}
	}
	...
	{
		points3 = linePoints;
		Color[] colors = SetColor(Color.white, LineType.Discrete, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, LineType.Discrete, Joins.None, use2Dlines: false, usePoints: false);
	}

	public VectorLine(string lineName, Vector3[] linePoints, Color color, Material lineMaterial, float width)
	{
		points3 = linePoints;
		Color[] colors = SetColor(color, LineType.Discrete, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, LineType.Discrete, Joins.None, use2Dlines: false, usePoints: false);
	}
	...
	{
		points3 = linePoints;
		Color[] colors = SetColor(Color.white, lineType, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, lineType, Joins.None, use2Dlines: false, usePoints: false);
	}

	public VectorLine(string lineName, Vector3[] linePoints, Color color, Material lineMaterial, float width, LineType lineType)
	{
		points3 = linePoints;
		Color[] colors = SetColor(color, lineType, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, lineType, Joins.None, use2Dlines: false, usePoints: false);
	}
	...
	public VectorLine(string lineName, Vector3[] linePoints, Material lineMaterial, float width, LineType lineType, Joins joins)
	{
		points3 = linePoints;
		Color[] colors = SetColor(Color.white, lineType, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, lineType, joins, use2Dlines: false, usePoints: false);
	}
	...
	public VectorLine(string lineName, Vector2[] linePoints, Material lineMaterial, float width)
	{
		points2 = linePoints;
		Color[] colors = SetColor(Color.white, LineType.Discrete, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, LineType.Discrete, Joins.None, use2Dlines: true, usePoints: false);
	}
	...
	public VectorLine(string lineName, Vector2[] linePoints, Material lineMaterial, float width, LineType lineType)
	{
		points2 = linePoints;
		Color[] colors = SetColor(Color.white, lineType, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, lineType, Joins.None, use2Dlines: true, usePoints: false);
	}
	...
	public VectorLine(string lineName, Vector2[] linePoints, Material lineMaterial, float width, LineType lineType, Joins joins)
	{
		points2 = linePoints;
		Color[] colors = SetColor(Color.white, lineType, linePoints.Length, usePoints: false);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, lineType, joins, use2Dlines: true, usePoints: false);
	}
	...
	{
		points2 = linePoints;
		Color[] colors = SetColor(Color.white, LineType.Continuous, linePoints.Length, usePoints: true);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, LineType.Continuous, Joins.None, use2Dlines: true, usePoints: true);
	}

	protected VectorLine(bool usePoints, string lineName, Vector2[] linePoints, Color color, Material lineMaterial, float width)
	{
		points2 = linePoints;
		Color[] colors = SetColor(color, LineType.Continuous, linePoints.Length, usePoints: true);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, LineType.Continuous, Joins.None, use2Dlines: true, usePoints: true);
	}
	...
	protected VectorLine(bool usePoints, string lineName, Vector3[] linePoints, Material lineMaterial, float width)
	{
		points3 = linePoints;
		Color[] colors = SetColor(Color.white, LineType.Continuous, linePoints.Length, usePoints: true);
		SetupMesh(ref lineName, lineMaterial, colors, ref width, LineType.Continuous, Joins.None, use2Dlines: false, usePoints: true);
	}
	...
	private void BuildMesh(Color[] colors)
	{
		...
		if (m_1pixelLine)
		{
			...
			if (m_isPoints)
			{
				...
				{
					ref Color32 reference = ref m_lineColors[i];
					reference = colors[i];
				}
			}
			else
			{
				...
				{
					ref Color32 reference2 = ref m_lineColors[num];
					reference2 = colors[j];
					ref Color32 reference3 = ref m_lineColors[num + 1];
					reference3 = colors[j];
					num += 2;
				}
			}
		...
		{
			num2 = (m_isPoints ? m_pointsLength : (m_continuous ? (m_pointsLength - 1) : (m_pointsLength / 2)));
			for (int k = 0; k < num2; k++)
			{
				ref Vector2 reference4 = ref m_lineUVs[num];
				reference4 = new Vector2(0f, 1f);
				ref Vector2 reference5 = ref m_lineUVs[num + 1];
				reference5 = new Vector2(0f, 0f);
				ref Vector2 reference6 = ref m_lineUVs[num + 2];
				reference6 = new Vector2(1f, 1f);
				ref Vector2 reference7 = ref m_lineUVs[num + 3];
				reference7 = new Vector2(1f, 0f);
				num += 4;
			}
			num = 0;
			for (int l = 0; l < num2; l++)
			{
				ref Color32 reference8 = ref m_lineColors[num];
				reference8 = colors[l];
				ref Color32 reference9 = ref m_lineColors[num + 1];
				reference9 = colors[l];
				ref Color32 reference10 = ref m_lineColors[num + 2];
				reference10 = colors[l];
				ref Color32 reference11 = ref m_lineColors[num + 3];
				reference11 = colors[l];
				num += 4;
			}
		}
		m_mesh.MarkDynamic();
	...
	private void SetupTriangles()
	{
		...
		{
			int num = (m_useMeshQuads ? 4 : 6);
			if (m_continuous)
			{
				m_triangleCount = (m_isPoints ? (m_triangleCount = m_pointsLength * num) : (m_triangleCount = (m_pointsLength - 1) * num));
		...
		int num2 = 0;
		int num3 = 0;
		num2 = (m_isPoints ? (m_pointsLength * 4) : (m_continuous ? ((m_pointsLength - 1) * 4) : (m_pointsLength * 2)));
		if (m_1pixelLine)
		{
			num2 = (m_isPoints ? m_pointsLength : (m_continuous ? ((m_pointsLength - 1) * 2) : m_pointsLength));
			if (m_continuous)
			{
				int num4 = 0;
				if (!m_isPoints)
			...
			m_mesh.SetIndices(array, m_isPoints ? MeshTopology.Points : MeshTopology.Lines, 0);
			return;
		}
		if (m_useMeshQuads)
		{
	...
	private void AddEndCap()
	{
		...
		for (int k = num - 8; k < num - 4; k++)
		{
			...
			reference2 = m_lineColors[num - 12];
		}
		ref Vector2 reference3 = ref m_lineUVs[num - 8];
		reference3 = new Vector2(0f, 0.25f);
		ref Vector2 reference4 = ref m_lineUVs[num - 7];
		reference4 = new Vector2(0f, 0f);
		ref Vector2 reference5 = ref m_lineUVs[num - 6];
		reference5 = new Vector2(1f, 0.25f);
		ref Vector2 reference6 = ref m_lineUVs[num - 5];
		reference6 = new Vector2(1f, 0f);
		if (capType == EndCap.Mirror)
		{
			ref Vector2 reference7 = ref m_lineUVs[num - 4];
			reference7 = new Vector2(1f, 0.25f);
			ref Vector2 reference8 = ref m_lineUVs[num - 3];
			reference8 = new Vector2(1f, 0f);
			ref Vector2 reference9 = ref m_lineUVs[num - 2];
			reference9 = new Vector2(0f, 0.25f);
			ref Vector2 reference10 = ref m_lineUVs[num - 1];
			reference10 = new Vector2(0f, 0f);
		}
		else
		{
			ref Vector2 reference11 = ref m_lineUVs[num - 4];
			reference11 = new Vector2(0f, 1f);
			ref Vector2 reference12 = ref m_lineUVs[num - 3];
			reference12 = new Vector2(0f, 0.75f);
			ref Vector2 reference13 = ref m_lineUVs[num - 2];
			reference13 = new Vector2(1f, 1f);
			ref Vector2 reference14 = ref m_lineUVs[num - 1];
			reference14 = new Vector2(1f, 0.75f);
		}
		m_mesh.vertices = m_lineVertices;
		m_mesh.uv = m_lineUVs;
		m_mesh.colors32 = m_lineColors;
	...
	{
		return SetCameraRenderTexture(renderTexture, Color.black, useOrtho: false);
	}
	...
	{
		return SetCamera(CameraClearFlags.Depth, useOrtho: false);
	}
	...
	{
		return SetCamera(clearFlags, useOrtho: false);
	}
	...
	{
		return SetCamera(thisCamera, CameraClearFlags.Depth, useOrtho: false);
	}
	...
	{
		return SetCamera(thisCamera, clearFlags, useOrtho: false);
	}
	...
	private void SetEndCapColors()
	{
		...
		{
			int num = (m_continuous ? (m_drawStart * 4) : (m_drawStart * 2));
			for (int i = 0; i < 4; i++)
			{
				ref Color32 reference = ref m_lineColors[i + m_vertexCount];
				reference = m_lineColors[i + num];
			}
		}
		...
		int num3 = num2 * (m_continuous ? 4 : 2) - 8;
		if (num3 < -4)
		{
			num3 = -4;
		}
		for (int j = 4; j < 8; j++)
		{
			...
			reference2 = m_lineColors[j + num3];
		}
	}
	...
	{
		int max = ((!m_isPoints) ? (m_continuous ? (pointsLength - 1) : (pointsLength / 2)) : pointsLength);
	...
	public void SetColors(Color[] lineColors)
	{
		...
		if (m_1pixelLine)
		{
			...
			else
			{
				...
				{
					ref Color32 reference2 = ref m_lineColors[num];
					reference2 = lineColors[j];
					ref Color32 reference3 = ref m_lineColors[num + 1];
					reference3 = lineColors[j];
					num += 2;
				}
			}
		}
		else
		{
			...
			{
				ref Color32 reference4 = ref m_lineColors[num];
				reference4 = lineColors[k];
				ref Color32 reference5 = ref m_lineColors[num + 1];
				reference5 = lineColors[k];
				ref Color32 reference6 = ref m_lineColors[num + 2];
				reference6 = lineColors[k];
				ref Color32 reference7 = ref m_lineColors[num + 3];
				reference7 = lineColors[k];
				num += 4;
			}
		}
		if (m_capType != EndCap.None)
	...
	public void SetColorsSmooth(Color[] lineColors)
	{
		...
		else
		{
			...
			if (m_1pixelLine)
			{
				...
				ref Color32 reference2 = ref m_lineColors[num + 1];
				reference2 = lineColors[start];
				num += 2;
				for (int i = start + 1; i < end; i++)
				{
					ref Color32 reference3 = ref m_lineColors[num];
					reference3 = lineColors[i - 1];
					ref Color32 reference4 = ref m_lineColors[num + 1];
					reference4 = lineColors[i];
					num += 2;
				}
			}
			else
			{
				...
				reference5 = lineColors[start];
				ref Color32 reference6 = ref m_lineColors[num + 1];
				reference6 = lineColors[start];
				ref Color32 reference7 = ref m_lineColors[num + 2];
				reference7 = lineColors[start];
				ref Color32 reference8 = ref m_lineColors[num + 3];
				reference8 = lineColors[start];
				num += 4;
				for (int j = start + 1; j < end; j++)
				{
					ref Color32 reference9 = ref m_lineColors[num];
					reference9 = lineColors[j - 1];
					ref Color32 reference10 = ref m_lineColors[num + 1];
					reference10 = lineColors[j - 1];
	...
	{
		start = ((m_minDrawIndex != 0) ? (m_continuous ? m_minDrawIndex : (m_minDrawIndex / 2)) : 0);
		if (m_maxDrawIndex <= 0)
		{
			return;
		}
	...
	{
		SetWidths(lineWidths, null, lineWidths.Length, doFloat: true);
	}

	public void SetWidths(int[] lineWidths)
	{
		SetWidths(null, lineWidths, lineWidths.Length, doFloat: false);
	}
	...
	private bool CheckLine()
	{
		...
		if (m_useMeshQuads)
		{
			...
			else
			{
				...
				if (m_drawStart > 0)
				{
					ref Vector3 reference = ref m_lineVertices[m_drawStart * 4 - 1];
					reference = m_lineVertices[m_drawStart * 4];
					ref Vector3 reference2 = ref m_lineVertices[m_drawStart * 4 - 2];
					reference2 = m_lineVertices[m_drawStart * 4];
				}
				if (m_drawEnd > 0 && m_drawEnd < m_pointsLength - 1)
				{
					ref Vector3 reference3 = ref m_lineVertices[m_drawEnd * 4];
					reference3 = m_lineVertices[m_drawEnd * 4 - 1];
					ref Vector3 reference4 = ref m_lineVertices[m_drawEnd * 4 + 1];
					reference4 = m_lineVertices[m_drawEnd * 4 - 1];
				}
				if (m_minDrawIndex > 0)
				{
					ref Vector3 reference5 = ref m_lineVertices[m_minDrawIndex * 4 - 1];
					reference5 = m_lineVertices[m_minDrawIndex * 4];
					ref Vector3 reference6 = ref m_lineVertices[m_minDrawIndex * 4 - 2];
					reference6 = m_lineVertices[m_minDrawIndex * 4];
				}
				if (m_maxDrawIndex > 0 && m_maxDrawIndex < m_pointsLength - 1)
				{
					ref Vector3 reference7 = ref m_lineVertices[m_maxDrawIndex * 4];
					reference7 = m_lineVertices[m_maxDrawIndex * 4 - 1];
					ref Vector3 reference8 = ref m_lineVertices[m_maxDrawIndex * 4 + 1];
					reference8 = m_lineVertices[m_maxDrawIndex * 4 - 1];
				}
			}
		}
		...
		else
		{
			...
			if (m_drawStart > 0)
			{
				ref Vector3 reference9 = ref m_lineVertices[m_drawStart * 4 - 1];
				reference9 = m_lineVertices[m_drawStart * 4];
			}
			if (m_drawEnd > 0 && m_drawEnd < m_pointsLength - 1)
			{
				ref Vector3 reference10 = ref m_lineVertices[m_drawEnd * 4];
				reference10 = m_lineVertices[m_drawEnd * 4 - 1];
			}
			if (m_minDrawIndex > 0)
			{
				ref Vector3 reference11 = ref m_lineVertices[m_minDrawIndex * 4 - 1];
				reference11 = m_lineVertices[m_minDrawIndex * 4];
			}
			if (m_maxDrawIndex > 0 && m_maxDrawIndex < m_pointsLength - 1)
			{
				ref Vector3 reference12 = ref m_lineVertices[m_maxDrawIndex * 4];
				reference12 = m_lineVertices[m_maxDrawIndex * 4 - 1];
			}
		}
		if (m_capType != EndCap.None)
		{
			if (m_capType <= EndCap.Mirror)
			{
				...
				if (m_is2D)
				{
					...
					ref Vector3 reference13 = ref m_lineVertices[m_vertexCount];
					reference13 = m_lineVertices[num] + vector;
					ref Vector3 reference14 = ref m_lineVertices[m_vertexCount + 1];
					reference14 = m_lineVertices[num + 1] + vector;
				}
				else
				{
					...
					ref Vector3 reference15 = ref m_lineVertices[m_vertexCount];
					reference15 = cam3D.ScreenToWorldPoint(vector2 + vector3);
					ref Vector3 reference16 = ref m_lineVertices[m_vertexCount + 1];
					reference16 = cam3D.ScreenToWorldPoint(cam3D.WorldToScreenPoint(m_lineVertices[num + 1]) + vector3);
				}
				ref Vector3 reference17 = ref m_lineVertices[m_vertexCount + 2];
				reference17 = m_lineVertices[num];
				ref Vector3 reference18 = ref m_lineVertices[m_vertexCount + 3];
				reference18 = m_lineVertices[num + 1];
			}
			if (m_capType >= EndCap.Both)
			{
				int num3 = m_drawEnd;
				...
				ref Vector3 reference19 = ref m_lineVertices[m_vertexCount + 4];
				reference19 = m_lineVertices[num4 - 2];
				ref Vector3 reference20 = ref m_lineVertices[m_vertexCount + 5];
				reference20 = m_lineVertices[num4 - 1];
				if (m_is2D)
				{
					...
					ref Vector3 reference21 = ref m_lineVertices[m_vertexCount + 6];
					reference21 = m_lineVertices[num4 - 2] + vector4;
					ref Vector3 reference22 = ref m_lineVertices[m_vertexCount + 7];
					reference22 = m_lineVertices[num4 - 1] + vector4;
				}
				else
				{
					...
					ref Vector3 reference23 = ref m_lineVertices[m_vertexCount + 6];
					reference23 = cam3D.ScreenToWorldPoint(cam3D.WorldToScreenPoint(m_lineVertices[num4 - 2]) + vector6);
					ref Vector3 reference24 = ref m_lineVertices[m_vertexCount + 7];
					reference24 = cam3D.ScreenToWorldPoint(vector5 + vector6);
				}
			}
	...
	public void Draw(Transform thisTransform)
	{
		...
		Matrix4x4 thisMatrix = (flag ? thisTransform.localToWorldMatrix : Matrix4x4.identity);
		zDist = (useOrthoCam ? ((float)(101 - m_depth)) : ((float)(screenHeight / 2) + (100f - (float)m_depth) * 0.0001f));
		int end = 0;
		SetupDrawStartEnd(out var start, out end);
		if (m_is2D)
		{
	...
	private void Line2D(int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix)
	{
		...
		if (capLength == 0f)
		{
			...
			for (int k = start; k < end; k += num2)
			{
				...
				reference3 = vector2 - vector3;
				ref Vector3 reference4 = ref m_lineVertices[idx + 3];
				reference4 = vector2 + vector3;
				idx += 4;
		...
		for (int l = m_minDrawIndex; l < end; l += num2)
		{
			...
			ref Vector3 reference5 = ref m_lineVertices[idx];
			reference5 = vector - vector4;
			ref Vector3 reference6 = ref m_lineVertices[idx + 1];
			reference6 = vector + vector4;
			if (smoothWidth && l < end - num2)
			{
				vector4 = v1 * m_lineWidths[widthIdx + 1];
			}
			ref Vector3 reference7 = ref m_lineVertices[idx + 2];
			reference7 = vector2 - vector4;
			ref Vector3 reference8 = ref m_lineVertices[idx + 3];
			reference8 = vector2 + vector4;
			idx += 4;
			widthIdx += widthIdxAdd;
		}
	}
	...
	private void Line3DContinuous(int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix)
	{
		...
		if (m_1pixelLine)
		{
			Vector3 vector = (useTransformMatrix ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(points3[start])) : cam3D.WorldToScreenPoint(points3[start]));
			vector.z = ((vector.z < 0.15f) ? (0f - zDist) : zDist);
			int num = start * 2;
			for (int i = start; i < end; i++)
			{
				Vector3 vector2 = vector;
				vector = (useTransformMatrix ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(points3[i + 1])) : cam3D.WorldToScreenPoint(points3[i + 1]));
				vector.z = ((vector.z < 0.15f) ? (0f - zDist) : zDist);
		...
		Vector3 vector3 = (useTransformMatrix ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(points3[start])) : cam3D.WorldToScreenPoint(points3[start]));
		vector3.z = ((vector3.z < 0.15f) ? (0f - zDist) : zDist);
		float num2 = 0f;
		int widthIdx = 0;
		widthIdxAdd = 0;
		if (m_lineWidths.Length > 1)
		...
		{
			Vector3 pos = vector3;
			vector3 = (useTransformMatrix ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(points3[j + 1])) : cam3D.WorldToScreenPoint(points3[j + 1]));
			...
			ref Vector3 reference = ref m_lineVertices[idx];
			reference = pos - vector4;
			ref Vector3 reference2 = ref m_lineVertices[idx + 1];
			reference2 = pos + vector4;
			...
			ref Vector3 reference3 = ref m_lineVertices[idx + 2];
			reference3 = vector3 - vector4;
			ref Vector3 reference4 = ref m_lineVertices[idx + 3];
			reference4 = vector3 + vector4;
			idx += 4;
			widthIdx += widthIdxAdd;
		}
		if (m_joins == Joins.Weld)
	...
	private void Line3DDiscrete(int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix)
	{
		...
		if (m_1pixelLine)
		{
			for (int i = start; i <= end; i++)
			{
				Vector3 vector = (useTransformMatrix ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(points3[i])) : cam3D.WorldToScreenPoint(points3[i]));
				vector.z = ((vector.z < 0.15f) ? (0f - zDist) : zDist);
		...
		for (int j = start; j < end; j += 2)
		{
			...
			pos.z = ((pos.z < 0.15f) ? (0f - zDist) : zDist);
			if (pos.x == vector2.x && pos.y == vector2.y)
			{
				Skip(ref idx, ref widthIdx, ref pos);
				continue;
			}
			vector2.z = ((vector2.z < 0.15f) ? (0f - zDist) : zDist);
			v1.x = vector2.y;
			v1.y = pos.x;
			v2.x = pos.y;
			v2.y = vector2.x;
			...
			ref Vector3 reference = ref m_lineVertices[idx];
			reference = pos - vector3;
			ref Vector3 reference2 = ref m_lineVertices[idx + 1];
			reference2 = pos + vector3;
	...
	public void Draw3D(Transform thisTransform)
	{
		...
		int num = 0;
		SetupDrawStartEnd(out var start, out var end);
		bool flag = !(thisTransform == null);
		Matrix4x4 matrix4x = (flag ? thisTransform.localToWorldMatrix : Matrix4x4.identity);
		if (m_1pixelLine)
		{
			if (m_continuous)
			{
				...
				if (flag)
				{
					for (int i = start; i < end; i++)
					{
						...
						reference = matrix4x.MultiplyPoint3x4(points3[i]);
						ref Vector3 reference2 = ref m_lineVertices[num2 + 1];
						reference2 = matrix4x.MultiplyPoint3x4(points3[i + 1]);
						num2 += 2;
					}
				}
				else
				{
					for (int j = start; j < end; j++)
					{
						ref Vector3 reference3 = ref m_lineVertices[num2];
						reference3 = points3[j];
						ref Vector3 reference4 = ref m_lineVertices[num2 + 1];
						reference4 = points3[j + 1];
						num2 += 2;
					}
				}
			}
			else if (flag)
			{
				for (int k = start; k <= end; k++)
				{
					ref Vector3 reference5 = ref m_lineVertices[k];
					reference5 = matrix4x.MultiplyPoint3x4(points3[k]);
				}
			}
			else
			{
				...
				{
					ref Vector3 reference6 = ref m_lineVertices[l];
					reference6 = points3[l];
				}
			}
			if (CheckLine())
			{
		...
		for (int m = start; m < end; m += num4)
		{
			...
			ref Vector3 reference7 = ref m_screenPoints[num3];
			reference7 = vector - vector3;
			ref Vector3 reference8 = ref m_screenPoints[num3 + 1];
			reference8 = vector + vector3;
			ref Vector3 reference9 = ref m_lineVertices[num3];
			reference9 = cam3D.ScreenToWorldPoint(m_screenPoints[num3]);
			ref Vector3 reference10 = ref m_lineVertices[num3 + 1];
			reference10 = cam3D.ScreenToWorldPoint(m_screenPoints[num3 + 1]);
			if (smoothWidth && m < end - num4)
			{
				vector3 = normalized * m_lineWidths[num + 1];
			}
			ref Vector3 reference11 = ref m_screenPoints[num3 + 2];
			reference11 = vector2 - vector3;
			ref Vector3 reference12 = ref m_screenPoints[num3 + 3];
			reference12 = vector2 + vector3;
			ref Vector3 reference13 = ref m_lineVertices[num3 + 2];
			reference13 = cam3D.ScreenToWorldPoint(m_screenPoints[num3 + 2]);
			ref Vector3 reference14 = ref m_lineVertices[num3 + 3];
			reference14 = cam3D.ScreenToWorldPoint(m_screenPoints[num3 + 3]);
			num3 += 4;
			num += widthIdxAdd;
		}
		if (m_joins == Joins.Weld)
	...
	private void DrawPoints(Transform thisTransform)
	{
		...
		if (m_1pixelLine)
		{
			if (!m_is2D)
			{
				for (int i = start; i <= end; i++)
				{
					...
					reference = (flag ? cam3D.WorldToScreenPoint(matrix4x.MultiplyPoint3x4(points3[i])) : cam3D.WorldToScreenPoint(points3[i]));
					if (m_lineVertices[i].z < 0.15f)
					{
						ref Vector3 reference2 = ref m_lineVertices[i];
			...
			else
			{
				for (int j = start; j <= end; j++)
				{
					ref Vector3 reference3 = ref m_lineVertices[j];
					reference3 = (flag ? matrix4x.MultiplyPoint3x4(points2[j]) : ((Vector3)points2[j]));
		...
		if (!m_is2D)
		{
			for (int k = start; k <= end; k++)
			{
				Vector3 pos = (flag ? cam3D.WorldToScreenPoint(matrix4x.MultiplyPoint3x4(points3[k])) : cam3D.WorldToScreenPoint(points3[k]));
				if (pos.z < 0.15f)
				{
					Skip(ref idx, ref widthIdx, ref pos);
					continue;
				...
				ref Vector3 reference4 = ref m_lineVertices[idx];
				reference4 = pos + v2;
				ref Vector3 reference5 = ref m_lineVertices[idx + 1];
				reference5 = pos - v1;
				ref Vector3 reference6 = ref m_lineVertices[idx + 2];
				reference6 = pos + v1;
				ref Vector3 reference7 = ref m_lineVertices[idx + 3];
				reference7 = pos - v2;
				idx += 4;
				widthIdx += widthIdxAdd;
			}
		...
		else
		{
			for (int l = start; l <= end; l++)
			{
				Vector3 pos = (flag ? matrix4x.MultiplyPoint3x4(points2[l]) : ((Vector3)points2[l]));
				pos.z = zDist;
				v1.x = (v1.y = (v2.y = m_lineWidths[widthIdx]));
				v2.x = 0f - m_lineWidths[widthIdx];
				ref Vector3 reference8 = ref m_lineVertices[idx];
				reference8 = pos + v2;
				ref Vector3 reference9 = ref m_lineVertices[idx + 1];
				reference9 = pos - v1;
				ref Vector3 reference10 = ref m_lineVertices[idx + 2];
				reference10 = pos + v1;
				ref Vector3 reference11 = ref m_lineVertices[idx + 3];
				reference11 = pos - v2;
				idx += 4;
				widthIdx += widthIdxAdd;
			}
	...
	private void DrawPoints3D(Transform thisTransform)
	{
		...
		if (m_1pixelLine)
		{
			...
			else
			{
				for (int j = start; j <= end; j++)
				{
					ref Vector3 reference2 = ref m_lineVertices[j];
					reference2 = points3[j];
				}
			}
			m_mesh.vertices = m_lineVertices;
			m_mesh.RecalculateBounds();
		...
		for (int k = start; k <= end; k++)
		{
			Vector3 vector = (flag ? cam3D.WorldToScreenPoint(matrix4x.MultiplyPoint3x4(points3[k])) : cam3D.WorldToScreenPoint(points3[k]));
			if (vector.z < 0.15f)
			{
				vector = Vector3.zero;
				Skip(ref idx, ref widthIdx, ref vector);
			...
			v1.x = (v1.y = (v2.y = m_lineWidths[widthIdx]));
			v2.x = 0f - m_lineWidths[widthIdx];
			ref Vector3 reference3 = ref m_lineVertices[idx];
			reference3 = cam3D.ScreenToWorldPoint(vector + v2);
			ref Vector3 reference4 = ref m_lineVertices[idx + 1];
			reference4 = cam3D.ScreenToWorldPoint(vector - v1);
			ref Vector3 reference5 = ref m_lineVertices[idx + 2];
			reference5 = cam3D.ScreenToWorldPoint(vector + v1);
			ref Vector3 reference6 = ref m_lineVertices[idx + 3];
			reference6 = cam3D.ScreenToWorldPoint(vector - v2);
			idx += 4;
			widthIdx += widthIdxAdd;
		}
		m_mesh.vertices = m_lineVertices;
	...
	public void SetTextureScale(Transform thisTransform, float textureScale, float offset)
	{
		...
		int num = (m_continuous ? (pointsLength - 1) : pointsLength);
	...
	public static void SetDepth(Transform thisTransform, int depth)
	{
		...
		thisTransform.position = new Vector3(thisTransform.position.x, thisTransform.position.y, useOrthoCam ? ((float)(101 - depth)) : ((float)(screenHeight / 2) + (100f - (float)depth) * 0.0001f));
	}
	...
	public static Vector3[] BytesToVector3Array(byte[] lineBytes)
	{
		...
		for (int i = 0; i < lineBytes.Length; i += 12)
		{
			...
			reference = new Vector3(ConvertToFloat(lineBytes, i), ConvertToFloat(lineBytes, i + 4), ConvertToFloat(lineBytes, i + 8));
		}
		return array;
	}
	...
	public static Vector2[] BytesToVector2Array(byte[] lineBytes)
	{
		...
		for (int i = 0; i < lineBytes.Length; i += 8)
		{
			...
			reference = new Vector2(ConvertToFloat(lineBytes, i), ConvertToFloat(lineBytes, i + 4));
		}
		return array;
	}
	...
	public void MakeRect(Vector3 topLeft, Vector3 bottomRight, int index)
	{
		if (m_continuous)
		{
			...
			{
				ref Vector2 reference = ref points2[index];
				reference = new Vector2(topLeft.x, topLeft.y);
				ref Vector2 reference2 = ref points2[index + 1];
				reference2 = new Vector2(bottomRight.x, topLeft.y);
				ref Vector2 reference3 = ref points2[index + 2];
				reference3 = new Vector2(bottomRight.x, bottomRight.y);
				ref Vector2 reference4 = ref points2[index + 3];
				reference4 = new Vector2(topLeft.x, bottomRight.y);
				ref Vector2 reference5 = ref points2[index + 4];
				reference5 = new Vector2(topLeft.x, topLeft.y);
			}
			else
			{
				ref Vector3 reference6 = ref points3[index];
				reference6 = new Vector3(topLeft.x, topLeft.y, topLeft.z);
				ref Vector3 reference7 = ref points3[index + 1];
				reference7 = new Vector3(bottomRight.x, topLeft.y, topLeft.z);
				ref Vector3 reference8 = ref points3[index + 2];
				reference8 = new Vector3(bottomRight.x, bottomRight.y, bottomRight.z);
				ref Vector3 reference9 = ref points3[index + 3];
				reference9 = new Vector3(topLeft.x, bottomRight.y, bottomRight.z);
				ref Vector3 reference10 = ref points3[index + 4];
				reference10 = new Vector3(topLeft.x, topLeft.y, topLeft.z);
			}
		}
		...
		{
			ref Vector2 reference11 = ref points2[index];
			reference11 = new Vector2(topLeft.x, topLeft.y);
			ref Vector2 reference12 = ref points2[index + 1];
			reference12 = new Vector2(bottomRight.x, topLeft.y);
			ref Vector2 reference13 = ref points2[index + 2];
			reference13 = new Vector2(bottomRight.x, topLeft.y);
			ref Vector2 reference14 = ref points2[index + 3];
			reference14 = new Vector2(bottomRight.x, bottomRight.y);
			ref Vector2 reference15 = ref points2[index + 4];
			reference15 = new Vector2(bottomRight.x, bottomRight.y);
			ref Vector2 reference16 = ref points2[index + 5];
			reference16 = new Vector2(topLeft.x, bottomRight.y);
			ref Vector2 reference17 = ref points2[index + 6];
			reference17 = new Vector2(topLeft.x, bottomRight.y);
			ref Vector2 reference18 = ref points2[index + 7];
			reference18 = new Vector2(topLeft.x, topLeft.y);
		}
		else
		{
			ref Vector3 reference19 = ref points3[index];
			reference19 = new Vector3(topLeft.x, topLeft.y, topLeft.z);
			ref Vector3 reference20 = ref points3[index + 1];
			reference20 = new Vector3(bottomRight.x, topLeft.y, topLeft.z);
			ref Vector3 reference21 = ref points3[index + 2];
			reference21 = new Vector3(bottomRight.x, topLeft.y, topLeft.z);
			ref Vector3 reference22 = ref points3[index + 3];
			reference22 = new Vector3(bottomRight.x, bottomRight.y, bottomRight.z);
			ref Vector3 reference23 = ref points3[index + 4];
			reference23 = new Vector3(bottomRight.x, bottomRight.y, bottomRight.z);
			ref Vector3 reference24 = ref points3[index + 5];
			reference24 = new Vector3(topLeft.x, bottomRight.y, bottomRight.z);
			ref Vector3 reference25 = ref points3[index + 6];
			reference25 = new Vector3(topLeft.x, bottomRight.y, bottomRight.z);
			ref Vector3 reference26 = ref points3[index + 7];
			reference26 = new Vector3(topLeft.x, topLeft.y, topLeft.z);
		}
	}
		...
		else
		{
			...
			if (m_continuous)
			{
				...
				if (m_is2D)
				{
					...
					{
						ref Vector2 reference = ref points2[index + num3];
						reference = vector + new Vector2(0.5f + Mathf.Cos(num2) * xRadius, 0.5f + Mathf.Sin(num2) * yRadius);
						num2 += num;
					}
					if (!m_isPoints)
					{
						ref Vector2 reference2 = ref points2[index + num3];
						reference2 = points2[index + (num3 - segments)];
					}
				}
				else
				{
					...
					for (num3 = 0; num3 < segments; num3++)
					{
						ref Vector3 reference3 = ref points3[index + num3];
						reference3 = origin + matrix4x.MultiplyPoint3x4(new Vector3(Mathf.Cos(num2) * xRadius, Mathf.Sin(num2) * yRadius, 0f));
						num2 += num;
					}
					if (!m_isPoints)
					{
						ref Vector3 reference4 = ref points3[index + num3];
						reference4 = points3[index + (num3 - segments)];
					}
				}
			}
			else if (m_is2D)
			{
				...
				{
					ref Vector2 reference5 = ref points2[index + num4];
					reference5 = vector2 + new Vector2(0.5f + Mathf.Cos(num2) * xRadius, 0.5f + Mathf.Sin(num2) * yRadius);
					num2 += num;
					num4++;
					ref Vector2 reference6 = ref points2[index + num4];
					reference6 = vector2 + new Vector2(0.5f + Mathf.Cos(num2) * xRadius, 0.5f + Mathf.Sin(num2) * yRadius);
				}
			}
			else
			{
				...
				{
					ref Vector3 reference7 = ref points3[index + num5];
					reference7 = origin + matrix4x2.MultiplyPoint3x4(new Vector3(Mathf.Cos(num2) * xRadius, Mathf.Sin(num2) * yRadius, 0f));
					num2 += num;
					num5++;
					ref Vector3 reference8 = ref points3[index + num5];
					reference8 = origin + matrix4x2.MultiplyPoint3x4(new Vector3(Mathf.Cos(num2) * xRadius, Mathf.Sin(num2) * yRadius, 0f));
				}
			}
		}
	}
	...
	public void MakeCurve(Vector3 anchor1, Vector3 control1, Vector3 anchor2, Vector3 control2, int segments, int index)
	{
		...
		if (m_continuous)
		{
			int num = (m_isPoints ? segments : (segments + 1));
			if (m_is2D)
			{
				...
				for (int i = 0; i < num; i++)
				{
					ref Vector2 reference = ref points2[index + i];
					reference = GetBezierPoint(ref anchor3, ref control3, ref anchor4, ref control4, (float)i / (float)segments);
				}
			}
			else
			{
				for (int j = 0; j < num; j++)
				{
					ref Vector3 reference2 = ref points3[index + j];
					reference2 = GetBezierPoint3D(ref anchor1, ref control1, ref anchor2, ref control2, (float)j / (float)segments);
				}
			}
			return;
		}
		...
		if (m_is2D)
		{
			...
			{
				ref Vector2 reference3 = ref points2[index + num2++];
				reference3 = GetBezierPoint(ref anchor5, ref control5, ref anchor6, ref control6, (float)k / (float)segments);
				ref Vector2 reference4 = ref points2[index + num2++];
				reference4 = GetBezierPoint(ref anchor5, ref control5, ref anchor6, ref control6, (float)(k + 1) / (float)segments);
			}
		}
		else
		{
			...
			{
				ref Vector3 reference5 = ref points3[index + num2++];
				reference5 = GetBezierPoint3D(ref anchor1, ref control1, ref anchor2, ref control2, (float)l / (float)segments);
				ref Vector3 reference6 = ref points3[index + num2++];
				reference6 = GetBezierPoint3D(ref anchor1, ref control1, ref anchor2, ref control2, (float)(l + 1) / (float)segments);
			}
		}
	}
	...
	private void MakeSpline(Vector2[] splinePoints2, Vector3[] splinePoints3, int segments, int index, bool loop)
	{
		...
		else
		{
			...
			for (i = 0; i < num3; i++)
			{
				...
				if (m_continuous)
				{
					if (m_is2D)
					{
						...
						{
							ref Vector2 reference = ref points2[num2++];
							reference = GetSplinePoint(ref splinePoints2[num6], ref splinePoints2[i], ref splinePoints2[num7], ref splinePoints2[num8], num9);
						}
					}
					else
					{
						for (num9 = num5; num9 <= 1f; num9 += num4)
						{
							ref Vector3 reference2 = ref points3[num2++];
							reference2 = GetSplinePoint3D(ref splinePoints3[num6], ref splinePoints3[i], ref splinePoints3[num7], ref splinePoints3[num8], num9);
						}
					}
				}
				else if (m_is2D)
				{
					for (num9 = num5; num9 <= 1f; num9 += num4)
					{
						ref Vector2 reference3 = ref points2[num2++];
						reference3 = GetSplinePoint(ref splinePoints2[num6], ref splinePoints2[i], ref splinePoints2[num7], ref splinePoints2[num8], num9);
						if (num2 > index + 1 && num2 < index + segments * 2)
						{
							ref Vector2 reference4 = ref points2[num2++];
							reference4 = points2[num2 - 2];
						}
					}
				}
				else
				{
					for (num9 = num5; num9 <= 1f; num9 += num4)
					{
						ref Vector3 reference5 = ref points3[num2++];
						reference5 = GetSplinePoint3D(ref splinePoints3[num6], ref splinePoints3[i], ref splinePoints3[num7], ref splinePoints3[num8], num9);
						if (num2 > index + 1 && num2 < index + segments * 2)
						{
							ref Vector3 reference6 = ref points3[num2++];
							reference6 = points3[num2 - 2];
						}
					}
				}
				num5 = num9 - 1f;
			...
			if ((m_continuous && num2 < index + (segments + 1)) || (!m_continuous && num2 < index + segments * 2))
			{
				if (m_is2D)
				{
					ref Vector2 reference7 = ref points2[num2];
					reference7 = GetSplinePoint(ref splinePoints2[num6], ref splinePoints2[i - 1], ref splinePoints2[num7], ref splinePoints2[num8], 1f);
				}
				else
				{
					ref Vector3 reference8 = ref points3[num2];
					reference8 = GetSplinePoint3D(ref splinePoints3[num6], ref splinePoints3[i - 1], ref splinePoints3[num7], ref splinePoints3[num8], 1f);
				}
			}
		}
	}
		...
		for (int j = 0; j < text.Length; j++)
		{
			...
			if (VectorChar.data[num6] != null)
			{
				...
				if (m_is2D)
				{
					...
					{
						ref Vector2 reference = ref points2[num5++];
						reference = Vector2.Scale(VectorChar.data[num6][k] + new Vector2(num3, num4), vector) + (Vector2)startPos;
					}
				}
				else
				{
					...
					{
						ref Vector3 reference2 = ref points3[num5++];
						reference2 = Vector3.Scale((Vector3)VectorChar.data[num6][l] + new Vector3(num3, num4, 0f), vector) + startPos;
					}
				}
				num3 += charSpacing;
			}
	...
	public void MakeCube(Vector3 position, float xSize, float ySize, float zSize, int index)
	{
		...
		ySize /= 2f;
		zSize /= 2f;
		ref Vector3 reference = ref points3[index];
		reference = position + new Vector3(0f - xSize, ySize, 0f - zSize);
		ref Vector3 reference2 = ref points3[index + 1];
		reference2 = position + new Vector3(xSize, ySize, 0f - zSize);
		ref Vector3 reference3 = ref points3[index + 2];
		reference3 = position + new Vector3(xSize, ySize, 0f - zSize);
		ref Vector3 reference4 = ref points3[index + 3];
		reference4 = position + new Vector3(xSize, ySize, zSize);
		ref Vector3 reference5 = ref points3[index + 4];
		reference5 = position + new Vector3(xSize, ySize, zSize);
		ref Vector3 reference6 = ref points3[index + 5];
		reference6 = position + new Vector3(0f - xSize, ySize, zSize);
		ref Vector3 reference7 = ref points3[index + 6];
		reference7 = position + new Vector3(0f - xSize, ySize, zSize);
		ref Vector3 reference8 = ref points3[index + 7];
		reference8 = position + new Vector3(0f - xSize, ySize, 0f - zSize);
		ref Vector3 reference9 = ref points3[index + 8];
		reference9 = position + new Vector3(0f - xSize, 0f - ySize, 0f - zSize);
		ref Vector3 reference10 = ref points3[index + 9];
		reference10 = position + new Vector3(0f - xSize, ySize, 0f - zSize);
		ref Vector3 reference11 = ref points3[index + 10];
		reference11 = position + new Vector3(xSize, 0f - ySize, 0f - zSize);
		ref Vector3 reference12 = ref points3[index + 11];
		reference12 = position + new Vector3(xSize, ySize, 0f - zSize);
		ref Vector3 reference13 = ref points3[index + 12];
		reference13 = position + new Vector3(0f - xSize, 0f - ySize, zSize);
		ref Vector3 reference14 = ref points3[index + 13];
		reference14 = position + new Vector3(0f - xSize, ySize, zSize);
		ref Vector3 reference15 = ref points3[index + 14];
		reference15 = position + new Vector3(xSize, 0f - ySize, zSize);
		ref Vector3 reference16 = ref points3[index + 15];
		reference16 = position + new Vector3(xSize, ySize, zSize);
		ref Vector3 reference17 = ref points3[index + 16];
		reference17 = position + new Vector3(0f - xSize, 0f - ySize, 0f - zSize);
		ref Vector3 reference18 = ref points3[index + 17];
		reference18 = position + new Vector3(xSize, 0f - ySize, 0f - zSize);
		ref Vector3 reference19 = ref points3[index + 18];
		reference19 = position + new Vector3(xSize, 0f - ySize, 0f - zSize);
		ref Vector3 reference20 = ref points3[index + 19];
		reference20 = position + new Vector3(xSize, 0f - ySize, zSize);
		ref Vector3 reference21 = ref points3[index + 20];
		reference21 = position + new Vector3(xSize, 0f - ySize, zSize);
		ref Vector3 reference22 = ref points3[index + 21];
		reference22 = position + new Vector3(0f - xSize, 0f - ySize, zSize);
		ref Vector3 reference23 = ref points3[index + 22];
		reference23 = position + new Vector3(0f - xSize, 0f - ySize, zSize);
		ref Vector3 reference24 = ref points3[index + 23];
		reference24 = position + new Vector3(0f - xSize, 0f - ySize, 0f - zSize);
	}
	...
	public Vector2 GetPoint(float distance)
	{
		...
		for (int num = (m_continuous ? m_drawEnd : ((m_drawEnd + 1) / 2)); distance > m_distances[i] && i < num; i++)
		{
		}
		if (m_continuous)
		{
	...
	public static void SetEndCap(string name, EndCap capType, Material material, params Texture2D[] textures)
	{
		...
		Texture2D texture2D = new Texture2D(width, height * 4, TextureFormat.ARGB32, mipmap: false);
	...
	public bool Selected(Vector2 p, int extraDistance, out int index)
	{
		...
		int num2 = (m_continuous ? (m_drawStart - num) : (m_drawStart / 2 - num));
		int num3 = m_drawEnd;
		bool flag = !(m_useTransform == null);
		Matrix4x4 matrix4x = (flag ? m_useTransform.localToWorldMatrix : Matrix4x4.identity);
		if (m_isPoints)
		{
			...
			for (int j = m_drawStart; j <= num3; j++)
			{
				...
				Vector2 vector = (flag ? cam3D.WorldToScreenPoint(matrix4x.MultiplyPoint3x4(points3[j])) : cam3D.WorldToScreenPoint(points3[j]));
