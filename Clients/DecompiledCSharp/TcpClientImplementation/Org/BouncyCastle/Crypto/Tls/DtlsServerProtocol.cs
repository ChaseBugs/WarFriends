...
public class DtlsServerProtocol(SecureRandom secureRandom) : DtlsProtocol(secureRandom)
{
	...
	protected virtual void ProcessCertificateVerify(ServerHandshakeState state, byte[] body, TlsHandshakeHash prepareFinishHash)
	{
		...
		try
		{
			byte[] hash = ((!TlsUtilities.IsTlsV12(serverContext)) ? serverContext.SecurityParameters.SessionHash : prepareFinishHash.GetFinalHash(digitallySigned.Algorithm.Hash));
