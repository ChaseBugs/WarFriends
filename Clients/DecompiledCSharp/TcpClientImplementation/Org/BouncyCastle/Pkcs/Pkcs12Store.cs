...
public class Pkcs12Store
{
	...
	public void Load(Stream input, char[] password)
	{
		...
		if (password != null && pfx.MacData != null)
		{
			...
			byte[] a = CalculatePbeMac(algorithmID.ObjectID, salt, intValue, password, wrongPkcs12Zero: false, octets);
			...
			if (!Arrays.ConstantTimeAreEqual(a, digest))
			{
				...
				a = CalculatePbeMac(algorithmID.ObjectID, salt, intValue, password, wrongPkcs12Zero: true, octets);
				if (!Arrays.ConstantTimeAreEqual(a, digest))
				{
					throw new IOException("PKCS12 key store MAC invalid - wrong password or corrupted file.");
				}
	...
	public string GetCertificateAlias(X509Certificate cert)
	{
		...
		{
			X509CertificateEntry x509CertificateEntry = (X509CertificateEntry)cert2.Value;
		...
		{
			X509CertificateEntry x509CertificateEntry2 = (X509CertificateEntry)keyCert.Value;
	...
	{
		if (!(PbeUtilities.CreateEngine(algId.ObjectID) is IBufferedCipher bufferedCipher))
		{
