...
- Validate both Player League competition mirrors (`MedalsBalance` and `Skill`) as nonnegative safe integers before returning a ranked division or settling its reward positions; one corrupt member must abort the whole settlement before its first write.
