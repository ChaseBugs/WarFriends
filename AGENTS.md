...
- Validate the complete ranked-PvP win-streak pair before settlement or boot projection: `WinCount` must be exactly 0-9, the zero count must pair only with timestamp zero, and a positive count needs a non-future Unix second that fits the recovered client's signed `int`. Never clamp a corrupt count into the maximum WarBucks tier or expose a future timestamp through the client's `isInfinite` path.
- Validate the paid-VIP lootbox countdown as an exact 1-4 integer before settlement, progression migration, or boot projection; only an absent legacy field defaults to four. Before duplicate conversion, reject a selected visual whose persisted parts exceed its catalog target instead of clamping unproven ownership into WarBucks.
