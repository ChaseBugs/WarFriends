...
public class Kochava : MonoBehaviour
{
	...
	private static string idfaKey = "kochava_key_idfa";

	private static string idfvKey = "kochava_key_idfv";

	private static string androidIdKey = "kochava_key_androidId";

	private static string androidAdvertisingIdKey = "kochava_key_androidAdvertisingId";
	...
	public static List<LogEvent> EventLog => _S._EventLog;

	public static int eventQueueLength => _S.eventQueue.Count;

	public static float eventPostingTime => _S._eventPostingTime;
		...
		try
		{
			try
			{
				...
				using (AndroidJavaClass androidJavaClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
				{
					...
					AndroidJavaObject androidJavaObject2 = androidJavaObject.Call<AndroidJavaObject>("getApplicationContext", new object[0]);
					AndroidJavaClass androidJavaClass2 = new AndroidJavaClass("com.kochava.android.tracker.lite.KochavaSDKLite");
					string text = androidJavaClass2.CallStatic<string>("GetExternalKochavaDeviceIdentifiers_Android", new object[2] { androidJavaObject2, AdidSupressed });
			...
			if (PlayerPrefs.HasKey("blacklist"))
			{
				try
				{
					string text3 = PlayerPrefs.GetString("blacklist");
					devIdBlacklist = new List<string>();
					string[] array = JsonReader.Deserialize<string[]>(text3);
					for (int num = array.Length - 1; num >= 0; num--)
					{
						devIdBlacklist.Add(array[num]);
					}
					Log("Loaded device_id blacklist from persistent storage: " + text3, KochLogLevel.debug);
				}
				catch (Exception ex2)
				{
			...
			if (PlayerPrefs.HasKey("session_tracking"))
			{
				try
				{
					string text4 = PlayerPrefs.GetString("session_tracking");
					sessionTracking = (KochSessionTracking)Enum.Parse(typeof(KochSessionTracking), text4, ignoreCase: true);
					Log("Loaded session tracking mode from persistent storage: " + text4, KochLogLevel.debug);
				}
				catch (Exception ex4)
				{
			...
			if (num3 - num2 > num4)
			{
				...
				dictionary.Add("sdk_version", "Unity3D-20160914");
			...
			CollectIdentifiers(hardwareIdentifierData);
		}
		catch (Exception ex5)
		{
	...
	private IEnumerator Init_KV(string postData)
	{
		...
		if (serverResponse.ContainsKey("flags"))
		{
			...
			if (dictionary.ContainsKey("session_tracking"))
			{
				...
				{
					Log("Failed System.Enum.Parse of KochSessionTracking: " + ex2, KochLogLevel.warning);
				}
			}
		...
		if (serverResponse.ContainsKey("blacklist"))
		{
			...
			if (serverResponse["blacklist"].GetType().GetElementType() == typeof(string))
			{
				...
				{
					Log("Failed parsing device_identifier blacklist received from server: " + ex3, KochLogLevel.warning);
				}
			}
			try
			{
			...
			{
				Log("Failed saving device_identifier blacklist to persistent storage: " + ex4, KochLogLevel.warning);
			}
		}
		if (serverResponse.ContainsKey("whitelist") && serverResponse["whitelist"].GetType().GetElementType() == typeof(string))
		{
			...
			try
			{
				...
				{
					if (array2[num11] == "location")
					{
						doReportLocation = true;
					}
					text2 = ((num11 == 0) ? (text2 + array2[num11]) : (text2 + array2[num11] + ","));
				}
			}
			catch (Exception ex5)
			{
				Log("Failed parsing device_identifier whitelist received from server: " + ex5, KochLogLevel.warning);
			}
			text2 += "}";
		...
		if (serverResponse.ContainsKey("eventname_blacklist"))
		{
			if (serverResponse["eventname_blacklist"].GetType().GetElementType() == typeof(string))
			{
				try
				{
					...
					{
						eventNameBlacklist.Add(array3[num12]);
					}
				}
				catch (Exception ex6)
				{
					Log("Failed parsing eventname_blacklist received from server: " + ex6, KochLogLevel.warning);
				}
			}
		...
		double num13 = CurrentTime();
		double num14 = double.Parse(PlayerPrefs.GetString("last_location_time"));
		if (!(num13 - num14 > (double)(locationStaleness * 60)))
		{
			yield break;
		}
		using (AndroidJavaClass androidJavaClass3 = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
		{
			...
			androidJavaObject3.Call<AndroidJavaObject>("getApplicationContext", new object[0]);
			AndroidJavaClass androidJavaClass4 = new AndroidJavaClass("com.kochava.android.tracker.lite.KochavaSDKLite");
			androidJavaClass4.CallStatic<string>("GetExternalLocationReport_Android", new object[3] { locationAccuracy, locationTimeout, locationStaleness });
	...
	private void DeviceInformationCallback(string deviceInfo)
	{
		try
		{
			...
			CollectIdentifiers(hardwareIntegrationData);
		}
		catch (Exception ex)
		{
	...
	private void initInitial()
	{
		...
		finally
		{
			...
			if (retrieveAttribution)
			{
				...
				{
					string s = PlayerPrefs.GetString("getattribution_wait");
					num2 = int.Parse(s);
				}
	...
	public void ScanWatchlistChanges()
	{
		try
		{
			...
			string text = PlayerPrefs.GetString("watchlistProperties");
			Log("retrieve watchlist: " + text);
			Dictionary<string, object> dictionary = null;
			dictionary = JsonReader.Deserialize<Dictionary<string, object>>(text);
			Dictionary<string, object> dictionary2 = new Dictionary<string, object>();
			...
			if (send_id_updates)
			{
				if (!devIdBlacklist.Contains("idfa") && dictionary.ContainsKey("idfa") && hardwareIntegrationData.ContainsKey("IDFA") && dictionary["idfa"].ToString() != hardwareIntegrationData["IDFA"].ToString())
				{
					...
					PlayerPrefs.SetString(idfaKey, hardwareIdentifierData["IDFA"].ToString());
				}
				if (!devIdBlacklist.Contains("adid") && dictionary.ContainsKey("adid") && hardwareIntegrationData.ContainsKey("adid") && dictionary["adid"].ToString() != hardwareIntegrationData["adid"].ToString())
				{
					...
					PlayerPrefs.SetString(androidAdvertisingIdKey, hardwareIdentifierData["adid"].ToString());
				}
			}
			if (dictionary2.Count > 0)
			{
				string text2 = JsonWriter.Serialize(dictionary);
				string text3 = JsonWriter.Serialize(dictionary2);
				Log("final watchlist: " + text2);
				Log("changeData: " + text3);
	...
	private void postEvent(Dictionary<string, object> data, bool isInitial)
	{
		...
		if (data.TryGetValue("is_genuine", out var value) && value is bool && !(bool)value)
		{
			AnalyticsHelper.LogCustomError("Piracy Check", "is_genuine == false");
		}
	...
	private IEnumerator processQueue()
	{
		...
		if (requestArray.Count > 0)
		{
			...
			{
				Log("Event posting response processing failure: " + ex2, KochLogLevel.error);
			}
		}
		queueIsProcessing = false;
	}
	...
	private void loadQueue()
	{
		try
		{
			...
			string value = PlayerPrefs.GetString("kochava_queue_storage");
			int num = 0;
			QueuedEvent[] array = JsonReader.Deserialize<QueuedEvent[]>(value);
	...
	private IEnumerator clearQueue()
	{
		...
		{
			Log("Failure clearing event queue: " + ex2, KochLogLevel.error);
		}
	}
	...
	public IEnumerator KochavaAttributionTimerFired(int delayTime)
	{
		...
		else
		{
			...
			{
				Log("Failed Deserialize JSON attribution data chunk: " + ex2, KochLogLevel.warning);
			}
			int num = 0;
	...
	public static void CollectIdentifiers(Dictionary<string, object> data)
	{
		if (data.ContainsKey("IDFA"))
		{
			PlayerPrefs.SetString(idfaKey, data["IDFA"].ToString());
		}
		if (data.ContainsKey("IDFV"))
		{
			PlayerPrefs.SetString(idfvKey, data["IDFV"].ToString());
		}
		if (data.ContainsKey("android_id"))
		{
			PlayerPrefs.SetString(androidIdKey, data["android_id"].ToString());
		}
		if (data.ContainsKey("adid"))
		{
			PlayerPrefs.SetString(androidAdvertisingIdKey, data["adid"].ToString());
		}
	}

	public static string GetKochavaIDFA()
	{
		return PlayerPrefs.GetString(idfaKey, string.Empty);
	}

	public static string GetKochavaIDFV()
	{
		return PlayerPrefs.GetString(idfvKey, string.Empty);
	}

	public static string GetAndroidID()
	{
		return PlayerPrefs.GetString(androidIdKey, string.Empty);
	}

	public static string GetAndroidAdvertisingID()
	{
		return PlayerPrefs.GetString(androidAdvertisingIdKey, string.Empty);
	}
}
