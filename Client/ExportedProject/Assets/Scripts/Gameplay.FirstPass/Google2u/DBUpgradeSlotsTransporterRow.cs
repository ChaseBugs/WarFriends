...
{
	[Serializable]
	public class DBUpgradeSlotsTransporterRow : IGoogle2uRow
	{
		public int TIER;
		...
		public DBUpgradeSlotsTransporterRow()
		{
		}

		public DBUpgradeSlotsTransporterRow(string _GOOGLEFU_ID, string _TIER, string _HP, string _DAMAGE, string _BATCHSIZEMIN, string _BATCHSIZEMAX, string _SHOTFREQUENCYMIN, string _SHOTFREQUENCYMAX, string _REALSHOTPROBABILITY, string _NEXTUPGRADEPRICE, string _NEXTUPGRADEPRICEGOLD, string _DELIVERYTIME, string _DPS, string _ARMYPOWER, string _SPECIAL, string _REPAIRBOTHP, string _SOLDIERHP, string _ELITE)
		{
			...
			if (float.TryParse(_REPAIRBOTHP, out var result15))
			{
				REPAIRBOTHP = result15;
			}
			else
			{
				Debug.LogError("Failed To Convert REPAIRBOTHP string: " + _REPAIRBOTHP + " to float");
			}
			if (float.TryParse(_SOLDIERHP, out var result16))
			{
				SOLDIERHP = result16;
			}
			else
		...
		public override string ToString()
		{
			...
			empty = empty + "{REPAIRBOTHP : " + REPAIRBOTHP + "} ";
