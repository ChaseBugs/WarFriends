...
namespace Google2u
{
	public class SMG_CPW : Google2uComponentBase, IGoogle2uDB
	{
		public enum rowIds
		{
			ROW_1,
		...
		public List<SMG_CPWRow> Rows = new List<SMG_CPWRow>();
		...
		{
			Rows.Add(new SMG_CPWRow(input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7], input[8], input[9]));
		}
		...
		public SMG_CPWRow GetRow(rowIds in_RowID)
		{
			SMG_CPWRow result = null;
			try
			{
				result = Rows[(int)in_RowID];
			}
		...
		public SMG_CPWRow GetRow(string in_RowString)
		{
			SMG_CPWRow result = null;
