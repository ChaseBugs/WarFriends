...
namespace Google2u
{
	public class SMG_CPWBM : Google2uComponentBase, IGoogle2uDB
	{
		public enum rowIds
		{
			ROW_1,
		...
		public List<SMG_CPWBMRow> Rows = new List<SMG_CPWBMRow>();
		...
		{
			Rows.Add(new SMG_CPWBMRow(input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7], input[8], input[9], input[10]));
		}
		...
		public SMG_CPWBMRow GetRow(rowIds in_RowID)
		{
			SMG_CPWBMRow result = null;
			try
			{
				result = Rows[(int)in_RowID];
			}
		...
		public SMG_CPWBMRow GetRow(string in_RowString)
		{
			SMG_CPWBMRow result = null;
