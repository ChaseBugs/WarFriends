...
{
	[Serializable]
	public class GrenadeLauncher_QLZ87BMRow : IGoogle2uRow
	{
		...
		public GrenadeLauncher_QLZ87BMRow()
		{
		}

		public GrenadeLauncher_QLZ87BMRow(string _GOOGLEFU_ID, string _RELOADTIME, string _AMMO, string _EXPLODEDAMAGE, string _MINDAMAGE, string _NEXTUPGRADEPRICE, string _NEXTUPGRADEPRICEGOLD, string _DELIVERYTIME, string _DPS, string _DEADRADIUS, string _ATTACK, string _WEAPONPRICE)
		{
			...
			if (float.TryParse(_EXPLODEDAMAGE, out var result3))
			{
			...
			if (float.TryParse(_DEADRADIUS, out var result9))
			{
				DEADRADIUS = result9;
			}
			else
			...
			if (int.TryParse(_WEAPONPRICE, out var result11))
			{
