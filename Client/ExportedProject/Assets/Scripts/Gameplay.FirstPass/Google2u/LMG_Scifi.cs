...
namespace Google2u
{
	public class LMG_Scifi : Google2uComponentBase, IGoogle2uDB
	{
		public enum rowIds
		{
			ROW_1,
		...
		public List<LMG_ScifiRow> Rows = new List<LMG_ScifiRow>();
		...
		{
			Rows.Add(new LMG_ScifiRow(input[0], input[1], input[2], input[3], input[4], input[5], input[6], input[7], input[8]));
		}
		...
		public LMG_ScifiRow GetRow(rowIds in_RowID)
		{
			LMG_ScifiRow result = null;
			try
			{
				result = Rows[(int)in_RowID];
			}
		...
		public LMG_ScifiRow GetRow(string in_RowString)
		{
			LMG_ScifiRow result = null;
