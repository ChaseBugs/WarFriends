...
public class tk2dSprite : tk2dBaseSprite
{
	...
	protected void OnDestroy()
	{
		...
		{
			DestroyOwnedMesh(meshColliderMesh);
		}
	}

	private static void DestroyOwnedMesh(Mesh ownedMesh)
	{
		// This component is marked ExecuteInEditMode, so Unity also invokes OnDestroy while
		// BuildPipeline swaps serialized scenes. Object.Destroy is valid only during Play Mode
		// and previously produced an editor error for each temporary sprite mesh. Keep deferred
		// destruction at runtime, but release the non-persistent editor mesh immediately when the
		// component is being torn down outside Play Mode.
