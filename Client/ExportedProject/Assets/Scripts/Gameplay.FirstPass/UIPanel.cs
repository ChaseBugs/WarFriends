...
public class UIPanel : MonoBehaviour
{
	...
	private Dictionary<Material, bool> mChanged = new Dictionary<Material, bool>();
	...
	private BetterList<Vector2> mUvs2 = new BetterList<Vector2>();
	...
	public float alpha1
	{
		...
		set
		{
			...
			if ((double)mAlpha < 0.01 && (double)num >= 0.01)
			{
				for (int i = 0; i < mDrawCalls.size; i++)
				{
					UIDrawCall uIDrawCall = mDrawCalls[i];
					MarkMaterialAsChanged(uIDrawCall.material, sort: false, uIDrawCall.isPremultiplied);
				}
	...
	public UIRoot root
	{
		get
		{
			return mRoot ?? (mRoot = NGUITools.FindInParents<UIRoot>(base.gameObject));
		}
		set
		{
			mRoot = value;
	...
	public void MarkMaterialAsChanged(Material mat, bool sort, bool premultiplied)
	{
		if (mat != null)
		{
			...
			if (!mChanged.ContainsKey(mat))
			{
				mChanged.Add(mat, premultiplied);
			}
		}
	}
	...
	public void AddWidget(UIWidget w)
	{
		...
		{
			mWidgets.Add(w);
			if (!mChanged.ContainsKey(w.material))
			{
				mChanged.Add(w.material, w.isPremultiplied);
			}
			mDepthChanged = true;
		}
	}
	...
	{
		if (w != null && w != null && mWidgets.Remove(w) && w.material != null && !mChanged.ContainsKey(w.material))
		{
			mChanged.Add(w.material, w.isPremultiplied);
		}
	}
	...
	private void OnEnable()
	{
		...
		while (num < mWidgets.size)
		{
			...
			if (uIWidget != null)
			{
				MarkMaterialAsChanged(uIWidget.material, sort: true, uIWidget.isPremultiplied);
				num++;
			}
			else
			{
	...
	public void UpdateDrawcalls()
	{
		...
		// WindowsWebPlayer no longer exists in Unity 2018. The branch remains applicable to
		// Windows standalone builds and the editor.
	...
	private void Fill(Material mat, bool isPremultiplied)
	{
		...
		while (num < mWidgets.size)
		{
			...
			if (uIWidget.material == mat && uIWidget.isVisible)
			{
				...
				if (generateNormals)
				{
					uIWidget.WriteToBuffers(mVerts, mUvs, mCols, mNorms, mTans, mUvs2);
				}
				else
				{
					uIWidget.WriteToBuffers(mVerts, mUvs, mCols, null, null, mUvs2);
				}
			}
			num++;
		}
		if (mVerts.size > 0)
		{
			...
			if (mVerts.size > 65000)
			{
				Debug.LogError("Too many vertices on one panel: " + mVerts.size + "name: " + base.gameObject.GetFullName());
			}
			drawCall.Set(mVerts, (!generateNormals) ? null : mNorms, (!generateNormals) ? null : mTans, mUvs, mCols, mAlpha, (!generateUvs2) ? null : mUvs2, isPremultiplied);
		}
		else
		{
	...
	private void LateUpdate()
	{
		...
		if (!isFreezed)
		{
			...
			for (int size2 = mWidgets.size; j < size2; j++)
			{
				...
				if (uIWidget.UpdateGeometry(this, forceVisible) && !mChanged.ContainsKey(uIWidget.material))
				{
					mChanged.Add(uIWidget.material, uIWidget.isPremultiplied);
				}
			}
		}
		if (mChanged.Count != 0 && onChange != null)
		{
			onChange();
		}
		if (mDepthChanged)
		...
		Dictionary<Material, bool>.Enumerator enumerator = mChanged.GetEnumerator();
		while (enumerator.MoveNext())
		{
			Fill(enumerator.Current.Key, enumerator.Current.Value);
		}
		if (mAlphaChanged)
		{
			mAlphaChanged = false;
			int k = 0;
			for (int size3 = drawCalls.size; k < size3; k++)
			{
				UIDrawCall uIDrawCall = drawCalls.buffer[k];
				uIDrawCall.UpdateAlpha(mAlpha);
			}
		}
		UpdateDrawcalls();
		mChanged.Clear();
	...
	public Vector3 CalculateConstrainOffsetTopLeft(Vector2 min, Vector2 max)
	{
		...
		return NGUIMath.ConstrainRectTopLeft(minRect, maxRect, minArea, maxArea);
	}
	...
	public Vector3 CalculateLeftTopAlignOffset(Vector2 min, Vector2 max)
	{
		...
		return new Vector3(vector.x - min.x, vector2.y - max.y, 0f);
	}
	...
	public Vector3 CalculateBottomRightAlignOffset(Vector2 min, Vector2 max)
	{
		...
		return new Vector3(vector2.x - max.x, vector.y - min.y, 0f);
	}
