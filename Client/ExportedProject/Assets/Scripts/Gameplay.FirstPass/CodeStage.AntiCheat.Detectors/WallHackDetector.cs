...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class WallHackDetector : ActDetectorBase
	{
		...
		private void UpdateServiceContainer()
		{
			if (base.enabled && base.gameObject.activeSelf)
			{
				...
				if (serviceContainer == null)
				{
					...
					SingletonSupport.DontDestroyOnLoadIfPlaying((UnityEngine.Object)serviceContainer);
				}
				...
				if (checkWireframe && wfCamera == null)
				{
					if (wfShader == null)
					{
						wfShader = Shader.Find("Unlit/Texture");
					}
					if (wfShader == null)
					{
						Debug.LogError("[ACTk] WallHack Detector can't find 'Unlit/Texture' shader!\nPlease make sure you have it included at the Editor > Project Settings > Graphics.", this);
					...
					else
					{
						...
						{
							shaderTexture = new Texture2D(4, 4, TextureFormat.ARGB32, mipChain: false);
							shaderTexture.filterMode = FilterMode.Point;
							Color[] colors = new Color[8] { wfColor1, wfColor1, wfColor1, wfColor1, wfColor1, wfColor1, wfColor1, wfColor1 };
							Color[] colors2 = new Color[8] { wfColor2, wfColor2, wfColor2, wfColor2, wfColor2, wfColor2, wfColor2, wfColor2 };
							shaderTexture.SetPixels(0, 0, 4, 2, colors);
							shaderTexture.SetPixels(0, 2, 4, 2, colors2);
							shaderTexture.Apply();
						}
						...
						{
							targetTexture = new Texture2D(renderTexture.width, renderTexture.height, TextureFormat.ARGB32, mipChain: false);
		...
		private IEnumerator CaptureFrame()
		{
			...
			targetTexture.ReadPixels(new Rect(0f, 0f, renderTexture.width, renderTexture.height), 0, 0);
			targetTexture.Apply();
