	...
	public struct ObscuredVector3
	{
		...
		public float x
		{
			get
			{
				...
				if (ObscuredCheatingDetector.IsRunning && !fakeValue.Equals((object)initialFakeValue) && Math.Abs(num - fakeValue.x) > ObscuredCheatingDetector.Instance.vector3Epsilon)
				{
		...
		public float y
		{
			get
			{
				...
				if (ObscuredCheatingDetector.IsRunning && !fakeValue.Equals((object)initialFakeValue) && Math.Abs(num - fakeValue.y) > ObscuredCheatingDetector.Instance.vector3Epsilon)
				{
					ObscuredCheatingDetector.Instance.OnCheatingDetected();
				}
				return num;
		...
		public float z
		{
			get
			{
				...
				if (ObscuredCheatingDetector.IsRunning && !fakeValue.Equals((object)initialFakeValue) && Math.Abs(num - fakeValue.z) > ObscuredCheatingDetector.Instance.vector3Epsilon)
				{
					ObscuredCheatingDetector.Instance.OnCheatingDetected();
				}
				return num;
		...
		private Vector3 InternalDecrypt()
		{
			...
			if (ObscuredCheatingDetector.IsRunning && !fakeValue.Equals((object)Vector3.zero) && !CompareVectorsWithTolerance(vector, fakeValue))
			{
				ObscuredCheatingDetector.Instance.OnCheatingDetected();
			}
			return vector;
