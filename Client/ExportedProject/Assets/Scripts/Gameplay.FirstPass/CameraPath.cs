...
public class CameraPath : MonoBehaviour
{
	...
	public bool shouldInterpolateNextPath => nextPath != null && interpolateNextPath;
	...
	public void RecalculateStoredValues()
	{
		...
		if (_points.Count >= 2)
		{
			...
			for (int k = 0; k < _storedValueArraySize - 1; k++)
			{
				...
				Vector3 pathPosition = GetPathPosition(num4, ignoreNormalisation: true);
			...
			for (int l = 0; l < _storedValueArraySize; l++)
			{
				...
				Vector3 pathPosition4 = GetPathPosition(percentage3, ignoreNormalisation: true);
				Vector3 pathPosition5 = GetPathPosition(percentage4, ignoreNormalisation: true);
				ref Vector3 reference = ref _storedPathDirections[l];
				reference = ((pathPosition4 - pathPosition3 + (pathPosition4 - pathPosition5)) * 0.5f).normalized;
			}
	...
	public float GetNearestPoint(Vector3 fromPostition, bool ignoreNormalisation, int refinments)
	{
		...
		for (float num7 = 0f; num7 < 1f; num7 += num2)
		{
			...
			float num8 = Vector3.SqrMagnitude(vector);
			if (num5 > num8)
			{
				num3 = num7;
				num5 = num8;
