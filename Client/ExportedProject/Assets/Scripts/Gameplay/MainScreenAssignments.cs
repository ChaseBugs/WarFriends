...
public class MainScreenAssignments : Core_BaseScript
{
	...
	[FormerlySerializedAs("COEILMPDGMI")]
	public GameObject LBGJODDBIEG;

	[FormerlySerializedAs("NEMILEOBFKC")]
	public GameObject CNAGOEBGOME;

	[FormerlySerializedAs("CHBGHEOONPD")]
	...
	[FormerlySerializedAs("DAINJJHFBHH")]
	public WinStreakCounter EFKOEBDKHPG;

	[FormerlySerializedAs("DFCAIKODFNO")]
	...
	[FormerlySerializedAs("HEHFLIBKPFI")]
	public UILabel MODGICHIGBH;

	[FormerlySerializedAs("LFKAJELEKII")]
	public UISprite[] OCNPMCGKMAJ;

	[FormerlySerializedAs("MCHOAODJEPD")]
	public UISprite[] NNDJMFBOGHD;

	[FormerlySerializedAs("DPLALDFFDMH")]
	public UISprite[] HFANCIJEMLL;

	[FormerlySerializedAs("PLNLAPGHIIB")]
	public UISprite DOJIALLMOAH;

	[FormerlySerializedAs("NMIKNPEIPPJ")]
	public BoxCollider NADBNPGOCPD;

	[Header("-Hint")]
	[FormerlySerializedAs("JMKFPMIJEMC")]
	public BoxCollider CLHHOMBGIDK;

	[FormerlySerializedAs("CAMIJBBCGGF")]
	public UISprite HIMOBMKBJJM;

	[FormerlySerializedAs("KBJDABHDLCH")]
	...
	{
		TweenPosition tweenPosition = TweenPosition.Begin(IILAJEMNNEA.gameObject, 794f, new Vector3(613f, 1267f, 773f), useLocal: false);
	...
	private void FBIOJBAECHL()
	{
		...
		{
			LBGJODDBIEG.SetActive(value: true);
			CNAGOEBGOME.gameObject.SetActive(value: false);
			JDFNANMLHKP.gameObject.SetActive(value: true);
		}
		else if (Singleton<NotificationManager>.instance.NotificationStarterAssignments())
		{
			LBGJODDBIEG.SetActive(value: false);
	...
	private void OCJEKNKLDEF()
	{
		...
		IILAJEMNNEA.gameObject.SetActive(value: false);
		...
		TweenPosition tweenPosition = TweenPosition.Begin(IILAJEMNNEA.gameObject, 1987f, new Vector3(841f, 1992f, 1475f), new Vector3(1115f, 1435f, 1571f), useLocal: false);
	...
	{
		IILAJEMNNEA.gameObject.SetActive(value: true);
	...
	private void AHBGAJAFJAF()
	{
		...
		for (int i = 1; i < OCNPMCGKMAJ.Length; i++)
		{
			...
			if (assignment != null)
			{
				...
				HFANCIJEMLL[i].gameObject.SetActive(value: true);
				OCNPMCGKMAJ[i].gameObject.SetActive(value: true);
				if (currentAssignments.Count > i && assignment.currentState == (Assignment.JLANLLEABDC)7)
				{
					NNDJMFBOGHD[i].alpha = 342f;
					HFANCIJEMLL[i].gameObject.SetActive(value: true);
					OCNPMCGKMAJ[i].gameObject.SetActive(value: false);
					num++;
				}
			}
			else
			{
				...
				HFANCIJEMLL[i].gameObject.SetActive(value: false);
				OCNPMCGKMAJ[i].gameObject.SetActive(value: true);
			}
		}
	...
	private void KBKIELPFLBP()
	{
		if (!StarterAssignmentsManager.instance.isActiveAndNotCompleted && Singleton<NotificationManager>.instance.GetNumberOfAssignmentNotifications() > 0)
		{
			LBGJODDBIEG.SetActive(value: false);
			CNAGOEBGOME.gameObject.SetActive(value: false);
			JDFNANMLHKP.gameObject.SetActive(value: false);
		}
		else if (Singleton<NotificationManager>.instance.NotificationStarterAssignments())
		{
			LBGJODDBIEG.SetActive(value: true);
	...
	{
		TweenPosition tweenPosition = TweenPosition.Begin(IILAJEMNNEA.gameObject, 1501f, new Vector3(1232f, 1846f, 464f), useLocal: false);
	...
	private void IABANOPECMI()
	{
		...
		{
			LBGJODDBIEG.SetActive(value: true);
			CNAGOEBGOME.gameObject.SetActive(value: true);
			JDFNANMLHKP.gameObject.SetActive(value: false);
		}
		else if (Singleton<NotificationManager>.instance.NotificationStarterAssignments())
		{
	...
	private void KNNJAFAFBJB()
	{
		...
		int megaRewarPoints = AssignmentsManager.instance.megaRewarPoints;
		bool active = currentMegaRewardPoints >= megaRewarPoints;
		MODGICHIGBH.text = $"{currentMegaRewardPoints} / [F0CE28]{megaRewarPoints}[-]";
		DOJIALLMOAH.fillAmount = AssignmentsManager.instance.megaRewardProgress;
		PNNGFGIJHDD();
		NADBNPGOCPD.gameObject.SetActive(active);
		NADBNPGOCPD.enabled = true;
	...
	private void DKNFOPPLAJA()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(IILAJEMNNEA.gameObject, 1983f, new Vector3(1431f, 376f, 493f), new Vector3(943f, 1730f, 1795f), useLocal: false);
		tweenPosition.method = UITweener.Method.Linear;
	...
	{
		TweenPosition tweenPosition = TweenPosition.Begin(IILAJEMNNEA.gameObject, 715f, new Vector3(1268f, 739f, 1508f), useLocal: false);
		tweenPosition.method = (UITweener.Method)6;
		tweenPosition.onFinished = null;
	}
