...
public class CardShieldDestroyer : Card
{
	...
	private void DNGDKPBDMCN(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].LHHMNLJKOAO().DAIDBENCFIP().LOFMDBHAFAO("), ", 415f, 1153f, GBJOJBKCEKN: false);
		}
	}
	...
	private void NKKBMMHDDEO(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].LHHMNLJKOAO().cardIconIndicator.JBMCFEMBPKE("ID_PLAYERLEAGUEDATAAREBEINGPROCESSED", 1919f, 1389f, GBJOJBKCEKN: false);
		}
	}
	...
	private void HLEHLHAMBOL(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].shield.cardIconIndicator.Show("game-card-ico-shielddestroyer", 3f, 3f, GBJOJBKCEKN: false);
		}
	}
	...
	private void MNHKHKNNIGH(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].PKBPGCGNCAD().DAIDBENCFIP().FFMLINMFADM("extraVip", 1213f, 707f, GBJOJBKCEKN: false);
		}
	}
	...
	private void PGAHKLPADLM(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].JPHAJDIIAAI().cardIconIndicator.Show("MoneyPackDeadline", 1785f, 1582f, GBJOJBKCEKN: false);
		}
	}
	...
	private void NGJDMLCCGHM(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].LMHDAIBCLDP().DHAFFOCDDGA().EEBNPGOCKFD("D2", 362f, 692f, GBJOJBKCEKN: false);
		}
	}
	...
	private void MFPJHHIGADL(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			int index = (int)Math.Round(num, 1);
			NMIADOEBBDH[index].shield.DestroyShield();
			NMIADOEBBDH[index].PHANADCNBFD().DAIDBENCFIP().LOFMDBHAFAO("ID_READYTIME", 1534f, 633f, GBJOJBKCEKN: true);
		}
	}
	...
	private void JDFKGIDIAFD(GHPGNELIDBM IEIKEJDECIG)
	{
		...
		if (NMIADOEBBDH.Count != 0)
		{
			...
			NMIADOEBBDH[index].CGKKGILAAPG().HLAOFDKKACK().BELBLFGCCLJ("CAMOS", 817f, 1891f, GBJOJBKCEKN: false);
		}
	}
