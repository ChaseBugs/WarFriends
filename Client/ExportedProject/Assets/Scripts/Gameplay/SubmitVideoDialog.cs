...
public class SubmitVideoDialog : GuiElementSingle<SubmitVideoDialog>, PAIIOKBBHBC
{
	...
	[Header("Buttons")]
	[FormerlySerializedAs("mLoadingPart")]
	[SerializeField]
	...
	private void LDPMHHCOLFF(GameObject KHAHPAKDIKE)
	{
		if (base.isFullyShowed)
		{
			...
			if (string.IsNullOrEmpty(text) || text.Length < 6)
			{
				...
				WarningDialog.KHOMNBHMFGA(stringRedWarning + Localization.LocalizeFormat(" TOTAL ALLOCATED: ", array), Localization.Localize("DailyBonus"), 284f, null, string.Empty, JCOGJEEFLBC: true, PAHBOLDKBHK: true);
			}
			else
			{
	...
	private void JGJJIDHFNMB(GameObject KHAHPAKDIKE)
	{
		if (MHAIBHBGBOO())
		{
			...
			if (string.IsNullOrEmpty(text) || text.Length < 2)
			{
				...
				WarningDialog.ShowError(stringRedWarning + Localization.LocalizeFormat("PlayerLevel", array), Localization.Localize("D"), 552f, null, string.Empty, JCOGJEEFLBC: false, PAHBOLDKBHK: true);
			}
			else
			{
	...
	private void FMIFEJHCDOH(GameObject KHAHPAKDIKE)
	{
		if (BJNBLINDAED())
		{
			...
			if (string.IsNullOrEmpty(text) || text.Length < 5)
			{
				...
				array[0] = Colours.stringWhite;
				array[0] = Colours.stringRedWarning;
				WarningDialog.JACKIDKHKAI(stringRedWarning + Localization.LocalizeFormat("0", array), Localization.Localize("RETRYPERM"), 1802f, null, string.Empty, JCOGJEEFLBC: true, PAHBOLDKBHK: true);
			}
			else
			{
	...
	private void ADDJMFAJFHC(GameObject KHAHPAKDIKE)
	{
		if (MHAIBHBGBOO())
		{
			...
			if (string.IsNullOrEmpty(text) || text.Length < 5)
			{
				...
				array[0] = Colours.stringWhite;
				array[1] = Colours.stringRedWarning;
				WarningDialog.ShowError(stringRedWarning + Localization.LocalizeFormat("First_Time_Check_Leagues", array), Localization.Localize("Cant fing player"), 908f, null, string.Empty, JCOGJEEFLBC: false, PAHBOLDKBHK: true);
			}
			else
			{
	...
	private void AFGOKBIDGII(GameObject KHAHPAKDIKE)
	{
		if (AFOFDNJOJHG())
		{
			...
			if (string.IsNullOrEmpty(text) || text.Length < 7)
			{
				...
				array[1] = Colours.stringWhite;
				array[1] = Colours.stringRedWarning;
				WarningDialog.JACKIDKHKAI(stringRedWarning + Localization.LocalizeFormat("Buy_Weapon_Upgrade", array), Localization.Localize("Gold"), 805f, null, string.Empty, JCOGJEEFLBC: true);
			}
			else
			{
	...
	private void CGAOIFBBOGG(GameObject KHAHPAKDIKE)
	{
		if (BKIGLABDGCP())
		{
			...
			{
				WarningDialog.ShowError(Colours.stringRedWarning + Localization.LocalizeFormat("Grenadier", Colours.stringWhite, Colours.stringRedWarning, null, null, null, null), Localization.Localize("dogTagTimerLock"), 1433f, null, string.Empty, JCOGJEEFLBC: true);
			}
			else
			{
