...
public class BlackMarketManager : DatabaseSerializedObjectGeneric<BlackMarketManager.BlackMarketOfferData>
{
	...
	public static BlackMarketManager instance
	{
		get
		{
			BJHPKLAEFCI = BJHPKLAEFCI ?? ((BlackMarketManager)SingletonSupport.FindOrCreate(typeof(BlackMarketManager)));
			return BJHPKLAEFCI;
		}
	}

	public int deadline => data.offerEnd;

	public bool isOfferActive => deadline > Singleton<BeanstalkServerManager>.instance.currentTimestamp;
	...
	public void APEDBJKELPE()
	{
		...
		Singleton<MessageManager>.instance.AddMessage(new IBPMPPJBBFG(), LJMMJDNNIFH: true);
		if (PCALLDPDMLC())
		{
			Singleton<EventTrackingManager>.instance.BlackMarketTriger(data.lastTrigger, data.offersTotal);
			foreach (WeaponLevelsSetup item in EEMHAHAINNM)
