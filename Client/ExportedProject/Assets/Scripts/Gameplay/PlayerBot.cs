...
public class PlayerBot : Core_BaseScript
{
	...
	[FormerlySerializedAs("OLFMCFCFJOB")]
	public PlayerBotDiffcultiesRow BPFKMNIHCGP;

	[FormerlySerializedAs("BCHFPEDBKKO")]
	public int PADBFAIBBDG;

	[FormerlySerializedAs("HEFHMIDFBBH")]
	...
	private bool _003CJMGDKHLEJAI_003Ek__BackingField;
	...
	public bool explosivePistol
	{
		...
		set
		{
			_003CJMGDKHLEJAI_003Ek__BackingField = value;
		}
	}

	public bool explosiveTiming => playerController.weaponInventory.LAGONJMBJMM.Count > 2 && playerController.weaponInventory.LAGONJMBJMM[2].KLBAKPENKMM;

	private bool HLMELNMHHDA => playerController.weaponInventory.currentWeapon.weapon.BEACDIILBEG == Weapon.IGGFMJAKLGO.Bazooka || playerController.weaponInventory.currentWeapon.weapon.BEACDIILBEG == Weapon.IGGFMJAKLGO.Grenade || playerController.weaponInventory.currentWeapon.weapon.BEACDIILBEG == Weapon.IGGFMJAKLGO.GrenadeLauncher;
	...
	private void AFIFENPMOFL()
	{
		...
		MapDefinition.DefendPosition defendPosition = JBOHBNKMDDM(LFPNJOCKLPC().EMAMENNDFMB(), DDNDHFGNOGB: true);
		MapDefinition.DefendPosition defendPosition2 = DECKDDPFPAF(AEDFPIGGJOC().CMNFFACHCEE(), DDNDHFGNOGB: true);
		...
		if ((double)defendPosition3.point.IGEDGCNJCAP().healthRatio < 1943.0)
		{
			...
			defendPosition2 = JBOHBNKMDDM(defendPosition2, DDNDHFGNOGB: false);
		...
		{
			defendPosition = DECKDDPFPAF(defendPosition, DDNDHFGNOGB: false);
	...
	private void ELHINJMCFHL()
	{
		...
		FFHGAOENJGJ = UnityEngine.Random.Range(BPFKMNIHCGP.OVERTIMESHIELDMINTIME, BPFKMNIHCGP.OVERTIMESHIELDMAXTIME);
		MapDefinition.DefendPosition defendPosition = JBOHBNKMDDM(FKKKABHIPCI().CMNFFACHCEE(), DDNDHFGNOGB: false);
		MapDefinition.DefendPosition defendPosition2 = JBOHBNKMDDM(playerController.CMNFFACHCEE(), DDNDHFGNOGB: false);
		...
		{
			defendPosition = JBOHBNKMDDM(defendPosition, DDNDHFGNOGB: true);
			defendPosition2 = JBOHBNKMDDM(defendPosition2, DDNDHFGNOGB: false);
			MapDefinition.DefendPosition nCMHGPNPEJM = FBAAHJOOGFI(defendPosition, defendPosition2);
		...
		{
			defendPosition = DECKDDPFPAF(defendPosition, DDNDHFGNOGB: false);
			defendPosition2 = JBOHBNKMDDM(defendPosition2, DDNDHFGNOGB: true);
	...
	private PlayerWeapon GNDKPIEIFCM(GameShootableEntity HCOFJJCIEOD)
	{
		...
		if (flag)
		{
			return AFEDAHEABHD(GOHLBBIDKCA: true);
		}
		if (explosivePistol)
		{
			return OIINLHHPKKM();
	...
	private void OJOJDDMPEAB()
	{
		DMCIGBBNGLM = TimeManager.realTimeWithoutPauses;
		FFHGAOENJGJ = UnityEngine.Random.Range(BPFKMNIHCGP.OVERTIMESHIELDMINTIME, BPFKMNIHCGP.OVERTIMESHIELDMAXTIME);
		MapDefinition.DefendPosition defendPosition = JBOHBNKMDDM(FKKKABHIPCI().CMNFFACHCEE(), DDNDHFGNOGB: true);
		MapDefinition.DefendPosition defendPosition2 = DECKDDPFPAF(LFPNJOCKLPC().currentPlayerPoint, DDNDHFGNOGB: true);
		MapDefinition.DefendPosition defendPosition3 = CHMDBHNKCAP(defendPosition, defendPosition2);
		int num = 0;
		int index = LFPNJOCKLPC().BBNPGJAALCJ().index;
		if ((double)defendPosition3.point.LHHMNLJKOAO().healthRatio < 748.0)
		{
			defendPosition = JBOHBNKMDDM(defendPosition, DDNDHFGNOGB: false);
			defendPosition2 = JBOHBNKMDDM(defendPosition2, DDNDHFGNOGB: false);
	...
	private PlayerWeapon FDILLAGNCIK(GameShootableEntity HCOFJJCIEOD)
	{
		...
		if (flag)
		{
			return BEHHGHCLNGL(GOHLBBIDKCA: true);
		}
		if (FEJONECDPCO())
		{
			return AKBLFHGEONC();
	...
	private PlayerWeapon NDHKMDHEOIH(GameShootableEntity HCOFJJCIEOD)
	{
		...
		if (flag)
		{
			return FIHHJNEKELM(GOHLBBIDKCA: true);
		}
		if (IGFGFJGBACL())
		{
			return NFDMMFBGAPO();
	...
	private PlayerWeapon FNCOONDEMOE(GameShootableEntity HCOFJJCIEOD)
	{
		...
		if ((bool)destroyableObject && aIObject != null && aIObject.ACLDFHJKBLI.botProperties.useExplosive)
		{
			...
			{
				return PPGPHCOCIGH(GOHLBBIDKCA: true);
			}
			return null;
		}
	...
	private void AGFOFBIKJNF()
	{
		DMCIGBBNGLM = TimeManager.CEAFAMFNGCC();
		FFHGAOENJGJ = UnityEngine.Random.Range(BPFKMNIHCGP.OVERTIMESHIELDMINTIME, BPFKMNIHCGP.OVERTIMESHIELDMAXTIME);
		MapDefinition.DefendPosition defendPosition = JBOHBNKMDDM(AEDFPIGGJOC().BBNPGJAALCJ(), DDNDHFGNOGB: true);
		MapDefinition.DefendPosition defendPosition2 = JBOHBNKMDDM(FKKKABHIPCI().CMNFFACHCEE(), DDNDHFGNOGB: false);
		MapDefinition.DefendPosition defendPosition3 = JNMLFCDCCEJ(defendPosition, defendPosition2);
		int num = 0;
	...
	private PlayerWeapon EKFNFLCNAID(GameShootableEntity HCOFJJCIEOD)
	{
		...
		if ((bool)destroyableObject && aIObject != null && aIObject.ACLDFHJKBLI.botProperties.useExplosive)
		{
			...
			{
				return EFLEBOIHINE(GOHLBBIDKCA: true);
			}
			return null;
		}
		...
		return DLKLDPKEANJ(GOHLBBIDKCA: true);
	}
	...
	private void IEAAKLLKCHE()
	{
		DMCIGBBNGLM = TimeManager.CEAFAMFNGCC();
		FFHGAOENJGJ = UnityEngine.Random.Range(BPFKMNIHCGP.OVERTIMESHIELDMINTIME, BPFKMNIHCGP.OVERTIMESHIELDMAXTIME);
		MapDefinition.DefendPosition defendPosition = JBOHBNKMDDM(AEDFPIGGJOC().currentPlayerPoint, DDNDHFGNOGB: true);
		MapDefinition.DefendPosition defendPosition2 = JBOHBNKMDDM(FKKKABHIPCI().CMNFFACHCEE(), DDNDHFGNOGB: true);
		MapDefinition.DefendPosition defendPosition3 = JCJGLCJADJK(defendPosition, defendPosition2);
		int num = 1;
