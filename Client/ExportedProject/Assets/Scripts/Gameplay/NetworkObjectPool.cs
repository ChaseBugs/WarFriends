...
public class NetworkObjectPool : ObjectPool
{
	...
	{
		PhotonView[] componentsInChildren = CALAKLNMEFF[AKGDPKJCILO][LJLNMNPBODG].GetComponentsInChildren<PhotonView>(includeInactive: true);
	...
	protected virtual void GGNHEDKHONC()
	{
		...
		for (int i = 0; i < LNHBALFMHOM.Count; i++)
		{
			...
			if (objectPoolEntry.Prefab != null)
			{
				...
				num = objectPoolEntry.Prefab.GetComponentsInChildren<PhotonView>(includeInactive: true).Length;
			}
	...
	public PoolableObject KOGDMMKEGKH(int NOCEDALFEJM, int AHMNOGCLPHC, Vector3 MPHCNMDIPAI, Quaternion OOMCGGAFJNH)
	{
		...
		JEAFHEHKIKK.TryGetValue(NOCEDALFEJM, out var value);
		if (value != null)
		{
			...
			poolableObject2.gameObject.SetActive(value: false);
	...
	public void SetupPhotonView(PoolableObject CJPPIMEJOED, int FMAOPCINBDA, int DFHAAIFFLOE)
	{
		...
		PhotonView[] componentsInChildren = CALAKLNMEFF[num][FMAOPCINBDA].GetComponentsInChildren<PhotonView>(includeInactive: true);
		for (int i = 0; i < componentsInChildren.Length; i++)
		{
			PhotonView photonView = componentsInChildren[i];
	...
	protected virtual void EGIDHHONMCN()
	{
		...
		for (int i = 0; i < LNHBALFMHOM.Count; i++)
		{
			...
			if (objectPoolEntry.Prefab != null)
			{
				...
				gameObject.transform.parent = base.transform;
				num = objectPoolEntry.Prefab.GetComponentsInChildren<PhotonView>(includeInactive: false).Length;
			}
			DFADBBIONED.Add(GGFEKHCGFEC);
	...
	{
		PhotonView[] componentsInChildren = CALAKLNMEFF[AKGDPKJCILO][LJLNMNPBODG].GetComponentsInChildren<PhotonView>(includeInactive: false);
		for (int i = 0; i < componentsInChildren.Length; i++)
		{
			PhotonView photonView = componentsInChildren[i];
			photonView.viewID = DFADBBIONED[AKGDPKJCILO] + LJLNMNPBODG * componentsInChildren.Length + i;
	...
	{
		int num = APCGDLEHGKL[CJPPIMEJOED];
		PhotonView[] componentsInChildren = CALAKLNMEFF[num][FMAOPCINBDA].GetComponentsInChildren<PhotonView>(includeInactive: false);
		for (int i = 1; i < componentsInChildren.Length; i += 0)
		{
			PhotonView photonView = componentsInChildren[i];
			photonView.viewID = DFADBBIONED[num] + DFHAAIFFLOE * componentsInChildren.Length + i;
