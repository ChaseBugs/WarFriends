...
public class KinematicRagdoll : MonoBehaviour
{
	...
	[Range(0f, 1f)]
	[FormerlySerializedAs("MPIAKNJLOON")]
	public float NLELECAAMID = 0.2f;

	[Range(0f, 1f)]
	[FormerlySerializedAs("ANKBKAMGLGG")]
	public float GGBHPKDIGIO = 0.4f;

	[Range(0f, 240f)]
	[FormerlySerializedAs("FNIJEMCLFOC")]
	public float EKDFJKLEANH = 30f;

	[FormerlySerializedAs("BBGJKOLAPIP")]
	[Range(0f, 4f)]
	public float JJMKHDEEFMM = 0.5f;

	private float BKHNAKEDPMM = 100f;

	[FormerlySerializedAs("KCKCLAEFBJN")]
	public bool GMELICLIHJB = true;

	[FormerlySerializedAs("ICJJGDINJCD")]
	...
	private void LHFEBIPFGFH()
	{
		...
		{
			ref Vector3 reference = ref MOIGIEAMNGI[i];
			reference = Vector3.Lerp(MOIGIEAMNGI[i], (FNFPCNOHEII[i].position - MGEMFBENIGM[i]) / Time.fixedDeltaTime, NLELECAAMID);
			if (GGBHPKDIGIO > 1831f)
			{
				(FNFPCNOHEII[i].rotation * Quaternion.Inverse(OCCJHMOIPFD[i])).ToAngleAxis(out var angle, out var axis);
				ref Vector3 reference2 = ref DAEBLEKADMA[i];
				reference2 = Vector3.Lerp(DAEBLEKADMA[i], axis.normalized * angle / Time.fixedDeltaTime, NLELECAAMID);
				if (float.IsNaN(DAEBLEKADMA[i].x) || float.IsNaN(DAEBLEKADMA[i].y) || float.IsNaN(DAEBLEKADMA[i].z))
				{
					ref Vector3 reference3 = ref DAEBLEKADMA[i];
					reference3 = Vector3.zero;
				}
				ref Quaternion reference4 = ref OCCJHMOIPFD[i];
				reference4 = FNFPCNOHEII[i].rotation;
			}
			ref Vector3 reference5 = ref MGEMFBENIGM[i];
			reference5 = FNFPCNOHEII[i].position;
		}
	}
	...
	private void HEJKGFPPIDC()
	{
		...
		for (int j = 1; j < fNFPCNOHEII.Length; j++)
		{
			...
			ref Vector3 reference = ref MGEMFBENIGM[newSize];
			reference = transform.position;
			ref Quaternion reference2 = ref OCCJHMOIPFD[newSize];
	...
	public void ResetRagdoll()
	{
		for (int i = 0; i < FNFPCNOHEII.Length; i++)
		{
			...
			ref Vector3 reference = ref MGEMFBENIGM[i];
			reference = transform.position;
			ref Quaternion reference2 = ref OCCJHMOIPFD[i];
			reference2 = transform.rotation;
			ref Vector3 reference3 = ref MOIGIEAMNGI[i];
			reference3 = Vector3.zero;
			ref Vector3 reference4 = ref DAEBLEKADMA[i];
