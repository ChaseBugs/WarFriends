	...
	internal class AnswersEventInstanceJavaObject
	{
		...
		public AnswersEventInstanceJavaObject(string eventType, Dictionary<string, object> customAttributes, params string[] args)
		{
			...
			foreach (KeyValuePair<string, object> customAttribute in customAttributes)
			{
				...
				if (value == null)
				{
					Debug.Log($"[Answers] Expected custom attribute value to be non-null. Received: {value}");
				}
				...
				{
					Debug.Log($"[Answers] Expected custom attribute value to be a string or numeric. Received: {value}");
				}
			}
		}
		...
		public void InvokeSafelyAsCurrency(string methodName, string currency)
		{
			if (currency != null)
			{
				...
				AndroidJavaObject androidJavaObject = androidJavaClass.CallStatic<AndroidJavaObject>("getInstance", new object[1] { currency });
