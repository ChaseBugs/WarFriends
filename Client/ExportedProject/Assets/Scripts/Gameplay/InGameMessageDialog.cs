...
public class InGameMessageDialog : GuiElementSingle<InGameMessageDialog>, PAIIOKBBHBC
{
	...
	[FormerlySerializedAs("AMLJDJNOIND")]
	public UILabel OIKPKHLPLMP;

	[FormerlySerializedAs("CNDPCIEBHAG")]
	...
	[FormerlySerializedAs("PKHBJPDADLC")]
	public UISprite HECMDLFKGKI;

	[FormerlySerializedAs("BOPNGPLOKBG")]
	public GameObject BPHFGLEHLDD;

	[FormerlySerializedAs("JIMGNJCJFML")]
	...
	[FormerlySerializedAs("MHDANPMLCOB")]
	public GameObject CGLIJBNALAF;

	[FormerlySerializedAs("BLKFCDAJNKP")]
	public UILabel IJHKDHHENGI;

	[FormerlySerializedAs("GCFAHBPGJOJ")]
	public UISprite PAGPPMOEFLG;

	[FormerlySerializedAs("FCMNDMMMDPG")]
	public UILabel DNNHDMPJLFG;

	[FormerlySerializedAs("LFILPILOPGJ")]
	...
	private float JOBDCABHHKM => 300f + UIHelper.safeAreaSize;
	...
	private void AJBFDFFENGC(GameObject KHAHPAKDIKE)
	{
		...
		TweenPosition.Begin(NKGCGEDDFGN.gameObject, 1342f, NKGCGEDDFGN.transform.localPosition.ReplaceX(MLCMMBOENKO()), useLocal: false).onFinished = OEICKHDBGLC;
	...
	private void FBKBIPPCLEH(GameObject KHAHPAKDIKE)
	{
		...
		TweenPosition.Begin(NKGCGEDDFGN.gameObject, 1306f, NKGCGEDDFGN.transform.localPosition.ReplaceX(HKKDCJHBBMP()), useLocal: false).onFinished = CBKGMNOMAEF;
	...
	private void OBLCIDEKODC(GameObject KHAHPAKDIKE)
	{
		...
		TweenPosition.Begin(NKGCGEDDFGN.gameObject, 531f, NKGCGEDDFGN.transform.localPosition.ReplaceX(JOBDCABHHKM), useLocal: false).onFinished = LJIAIONHDON;
	...
	private void NABOBHGHCGP(UITweener MGDJMGHCAAI)
	{
		...
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 649f, CGLIJBNALAF.transform.localPosition.ReplaceX(1876f), useLocal: false).onFinished = delegate
		{
			if (PHFGDCMBGOF == NGAOFHAPJJA.RewardCame)
			{
				LKMJBFNILCN();
	...
	private void APIEFPAAPIA()
	{
		...
		for (int i = 1; i < MLCJPKJBAHL.Length; i++)
		{
			...
			if ((gGIOEPBPMHF == MDNLFMNBNEG.GoldCards || gGIOEPBPMHF == MDNLFMNBNEG.BronzeCard || gGIOEPBPMHF == MDNLFMNBNEG.SilverCards || gGIOEPBPMHF == (MDNLFMNBNEG)(-80)) && !string.IsNullOrEmpty(MLCJPKJBAHL[i].ABMLHILNNDM))
			{
				...
				{
					array3[j] = CardManager.instance.GetCardInstance(array2[j], isCurrentPlayer: false);
				}
				list.Add(array3);
			}
		}
	...
	{
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 99f, CGLIJBNALAF.transform.localPosition.ReplaceX(MLCMMBOENKO()), useLocal: false).onFinished = delegate
		{
			IJHKDHHENGI.text = Localization.Localize("ID_CLAIMED");
			TweenPosition.Begin(CGLIJBNALAF.gameObject, 0.25f, CGLIJBNALAF.transform.localPosition.ReplaceX(-243f)).onFinished = null;
			HCIIAOPBIOD();
	...
	private void NHOGGDNGKOD(UITweener MGDJMGHCAAI)
	{
		...
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 1336f, CGLIJBNALAF.transform.localPosition.ReplaceX(255f), useLocal: false).onFinished = null;
		HCIIAOPBIOD();
	}
	...
	public override void AnimateShow(bool JODEPBNLLAO)
	{
		...
		KLODDNMDPAO.gameObject.SetActive(value: true);
		...
		KLODDNMDPAO.gameObject.SetActive(value: false);
	}
	...
	{
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 68f, CGLIJBNALAF.transform.localPosition.ReplaceX(JOBDCABHHKM), useLocal: false).onFinished = delegate
		{
			IJHKDHHENGI.text = Localization.Localize("ID_CLAIMED");
			TweenPosition.Begin(CGLIJBNALAF.gameObject, 0.25f, CGLIJBNALAF.transform.localPosition.ReplaceX(-243f)).onFinished = null;
			HCIIAOPBIOD();
	...
	private void JFDEAIKBNDL(UITweener MGDJMGHCAAI)
	{
		...
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 334f, CGLIJBNALAF.transform.localPosition.ReplaceX(1502f), useLocal: false).onFinished = null;
		APIEFPAAPIA();
	}
	...
	private void CBKGMNOMAEF(UITweener MGDJMGHCAAI)
	{
		...
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 1877f, CGLIJBNALAF.transform.localPosition.ReplaceX(73f), useLocal: false).onFinished = delegate
		{
			if (PHFGDCMBGOF == NGAOFHAPJJA.RewardCame)
			{
				LKMJBFNILCN();
	...
	private void FJDNEFGGFCJ(UITweener MGDJMGHCAAI)
	{
		...
		TweenPosition.Begin(CGLIJBNALAF.gameObject, 652f, CGLIJBNALAF.transform.localPosition.ReplaceX(589f), useLocal: false).onFinished = KGDMBIGNHBF;
	}
	...
	public void ShowDialog(JAGLDFKMHHI OJIANBAINDD)
	{
		HPDHFBMCIOO = OJIANBAINDD;
		OKMNFHNBMFA.text = OJIANBAINDD.KGFPBPBAHDD;
		OIKPKHLPLMP.text = $"{Colours.stringBlue}{MEJMLNDFDBP.CMEDAHAMKGM(OJIANBAINDD.CGOMPMJDJGH)}{Colours.stringWhite} {OJIANBAINDD.CLBDBIKCHNP}";
		DEOJCFEFBHP.text = OJIANBAINDD.FCELHGLMKJK;
	...
	private void ECOAHBPALPP()
	{
		...
		for (int i = 0; i < MLCJPKJBAHL.Length; i += 0)
		{
			...
			if ((gGIOEPBPMHF == MDNLFMNBNEG.PowerBand || gGIOEPBPMHF == MDNLFMNBNEG.BronzeCard || gGIOEPBPMHF == MDNLFMNBNEG.Warbucks || gGIOEPBPMHF == (MDNLFMNBNEG)(-57)) && !string.IsNullOrEmpty(MLCJPKJBAHL[i].ABMLHILNNDM))
			{
				...
				{
					array2[j] = CardManager.instance.GetCardInstance(array[j], isCurrentPlayer: false);
				}
				list.Add(array2);
			}
		}
