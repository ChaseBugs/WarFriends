...
public class DroneSteering : Core_BaseScript
{
	...
	[FormerlySerializedAs("IIDEMAELGHH")]
	...
	[FormerlySerializedAs("EALHAMBNHHG")]
	public float ECNBEOHNHCB = 0.1f;

	[FormerlySerializedAs("ONKCLMMPABO")]
	public float JEMGOPDEBLL = 100f;

	[FormerlySerializedAs("CLGJBGHIKED")]
	...
	[CompilerGenerated]
	private bool _003CNPKNFODAHDG_003Ek__BackingField;
	...
	public new bool enabled
	{
		...
		set
		{
			_003CNPKNFODAHDG_003Ek__BackingField = value;
		}
	}
	...
	protected void GCOJHEDPIKL()
	{
		if (FEHCCGEGPLH.isMine)
		{
			...
			{
				GPIIEAHGHEO = FPOLIHANKLO(IMGHHHHHPKF.transform.position, CCBIFGDBDDE: true);
	...
	protected void ANLFCJJCGMG()
	{
		if (FEHCCGEGPLH.isMine)
		{
			...
			if (INDBNCLMMNC < ILEKLILGKAK)
			{
				GPIIEAHGHEO = AEDEJDEANEC(IMGHHHHHPKF.transform.position, CCBIFGDBDDE: true);
				DHFAONHFMCF += GPIIEAHGHEO;
				NGCDDPCAGJB.position += DHFAONHFMCF;
				Vector3 axis = Vector3.Cross(Vector3.up, DHFAONHFMCF);
	...
	protected void FMKLDIJPHIH()
	{
		if (FEHCCGEGPLH.isMine)
		{
			...
			if (INDBNCLMMNC < ILEKLILGKAK)
			{
				GPIIEAHGHEO = JADAEPCNKIP(IMGHHHHHPKF.transform.position, CCBIFGDBDDE: true);
				DHFAONHFMCF += GPIIEAHGHEO;
				NGCDDPCAGJB.position += DHFAONHFMCF;
				Vector3 axis = Vector3.Cross(Vector3.up, DHFAONHFMCF);
	...
	protected void HAMDMFGNIKD()
	{
		if (FEHCCGEGPLH.isMine)
		{
			...
			if (INDBNCLMMNC < ILEKLILGKAK)
			{
				GPIIEAHGHEO = HAKLACHNDBB(IMGHHHHHPKF.transform.position, CCBIFGDBDDE: true);
				DHFAONHFMCF += GPIIEAHGHEO;
				NGCDDPCAGJB.position += DHFAONHFMCF;
				Vector3 axis = Vector3.Cross(Vector3.up, DHFAONHFMCF);
	...
	protected void CMELHMEBEFI()
	{
		if (FEHCCGEGPLH.isMine)
		{
			...
			if (INDBNCLMMNC < ILEKLILGKAK)
			{
				GPIIEAHGHEO = NADCOBPHNGM(IMGHHHHHPKF.transform.position, CCBIFGDBDDE: true);
				DHFAONHFMCF += GPIIEAHGHEO;
				NGCDDPCAGJB.position += DHFAONHFMCF;
				Vector3 axis = Vector3.Cross(Vector3.up, DHFAONHFMCF);
