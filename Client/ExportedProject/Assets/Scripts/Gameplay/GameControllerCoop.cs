...
public class GameControllerCoop : GameControllerOnline
{
	private sealed class NCEKIFIDMCF : IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool MoveNext()
		{
			...
			case 0u:
				...
				GameControllerOnline.HMAIEJHCHDE.Killed += BJGCPDNMHDH.LLCPCOECHFD;
				PHDOCKCBJOF = BJGCPDNMHDH.StartCoroutine(((GameControllerOnline)BJGCPDNMHDH).StartGame());
				if (!FDKOKMKIIMG)
				{
					CLPPIAGLCJJ = 1;
				}
	...
	private sealed class OILCCDFMJEJ : IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool HOOHJGPHHGN()
		{
			...
			case 0u:
				...
				TimeManager.instance.SetupPhysicsTiming(KCGNHJOLNKD: false);
				Singleton<PerformanceManager>.instance.CAPGAAJFBDB(FIKGLFBFBON: false);
				Singleton<GameCamera>.instance.SetTarget(GameControllerOnline.HMAIEJHCHDE);
				Singleton<GuiManager>.instance.ShowGui(GuiScreenSingle<SpectateScreen>.instance);
				PlayerController.OGMBJPKOPCB.gameObject.SetActive(value: true);
				if (TimeManager.instance.CCAFFILOOIA() && BJGCPDNMHDH.gameIsRunning)
	...
	private sealed class MGAAEFEKILD
	{
		...
		{
			KHLGDCHJJPB.gameObject.SetActive(value: true);
			TimeManager.instance.SetupPhysicsTiming(KCGNHJOLNKD: false);
		}
		...
		{
			KHLGDCHJJPB.gameObject.SetActive(value: false);
			TimeManager.instance.SetupPhysicsTiming(KCGNHJOLNKD: false);
		}
	...
	protected override bool JPADCGODJBO => DEGCDEAANEL;

	public override string quitText => Localization.Localize((!JPADCGODJBO) ? "ID_CONFIRM_EXITINGCOOPNODOGTAG_TEXT" : "ID_CONFIRM_EXITINGCOOP_TEXT");
	...
	public override void StartMultiplayerGame(string JLBLNDGIINB, CloudRegionCode GJBFKKOAMPA)
	{
		...
		{
			LoadingDialog.ShowLoading(Localization.Localize("ID_WAITINGFORFRIEND"), DMDICGADGCG: false, BGOHKCCLDJP: true).DGPEHPHGDEF = NMCJNOFAJLL;
		}
	}
	...
	{
		LoadingDialog.SetLook(EDHDMJLHLFA: false);
		LoadingDialog.ShowLoading(Localization.Localize("ID_WAITINGFORFRIEND"), DMDICGADGCG: true, BGOHKCCLDJP: true).DGPEHPHGDEF = NMCJNOFAJLL;
		base.Rematch();
	}
	...
	{
		base.Forfeit();
		DailyMissionsManager.instance.LFFBCGDBGHH().EndMission(isWin: false);
		if (Singleton<PhotonConnectionManager>.instance.isClient)
		{
			Disconnect();
		}
	...
	public virtual void OFJJIBIAJPM(string JLBLNDGIINB, CloudRegionCode GJBFKKOAMPA)
	{
		...
		{
			LoadingDialog.PGFBMJFLHPC(Localization.Localize("gold3"), DMDICGADGCG: true, BGOHKCCLDJP: true).DGPEHPHGDEF = NMCJNOFAJLL;
		}
	}
	...
	private void GAEAGAPHLAM(int EJBEDLHIKDG, float KBJEOEEOEFG, GameController.HKGHCIEPGEL MNMLNIMFMJP)
	{
		...
		if (mGAAEFEKILD.KHLGDCHJJPB != null)
		{
			...
			mGAAEFEKILD.KHLGDCHJJPB.LMIHBNGHEHM.BJIAOHPEDAP(IDEBKDPMPGM: true);
			mGAAEFEKILD.KHLGDCHJJPB.LMIHBNGHEHM.LODPIIGCEKP(IDEBKDPMPGM: false);
	...
	{
		LoadingDialog.FEHDJPIKOJD(EDHDMJLHLFA: false);
		LoadingDialog.FDBHFJFKALH(Localization.Localize("ID_WARNING_OFFEREXPIRED"), DMDICGADGCG: false, BGOHKCCLDJP: true).DGPEHPHGDEF = NMCJNOFAJLL;
		base.Rematch();
	}
	...
	public void FPKHIMHAMJH(DatabasePlayer NCFFGKHPJLH, out string JLBLNDGIINB, out bool MKDDIBOHCIA)
	{
		...
		if (Singleton<GameController>.instance.isCoop && MKDDIBOHCIA)
		{
			LoadingDialog.PGFBMJFLHPC(Localization.Localize("NO any UIRoot - failed "), DMDICGADGCG: true, BGOHKCCLDJP: true, KKLNBPJCLHC: true).DGPEHPHGDEF = NMCJNOFAJLL;
		}
	}
	...
	public virtual void BNDLLKLJIFK()
	{
		...
		LoadingDialog.FDBHFJFKALH(Localization.Localize("Total_Battles"), DMDICGADGCG: true, BGOHKCCLDJP: false, KKLNBPJCLHC: true).DGPEHPHGDEF = NMCJNOFAJLL;
		base.Rematch();
	}
