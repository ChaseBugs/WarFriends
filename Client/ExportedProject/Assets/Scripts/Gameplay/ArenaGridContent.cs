...
{
	[FormerlySerializedAs("PIOALDFBBON")]
	public ArenaNode[] NFOELJINGGN;

	[FormerlySerializedAs("FGEOIMPMCHO")]
	public UISprite[] AOEHIDKCBKF;

	public UISprite PEJFFMLEHFA;

	private float KPNFAOBEAAL => UIHelper.activeWidthSafe - JBCJKJELAIF - GJFDMHBBMIN;

	private float CGEPCBAEGDM => 0f - UIHelper.activeWidthSafe * 0.5f + JBCJKJELAIF;

	private static float JBCJKJELAIF => (!((double)GACKNHPFGFM > 1.7)) ? 60f : 100f;

	private static float GJFDMHBBMIN => 150f;

	private static float GACKNHPFGFM => UIHelper.activeWidthFull / UIHelper.activeHeight;
	...
	public void FEEILEFANNB(bool AJEJIGJKPAP, List<IKPLPPFFDNI.MJPNFAKHHHI> HBDKMCDHDBF = null)
	{
		if (AJEJIGJKPAP && HBDKMCDHDBF != null)
		{
			...
			{
				NFOELJINGGN[i].NIBEFILODGM(OJIAOCGNKOO: true, HBDKMCDHDBF[i].OFMKJMAPHBM, HBDKMCDHDBF[i].GCFBLCFJBLK);
			}
			return;
		}
	...
	public void JPCHIALGPMJ(int FCDIFINLKIA)
	{
		...
		{
			AOEHIDKCBKF[i].gameObject.SetActive(value: true);
			NFOELJINGGN[i].gameObject.SetActive(value: true);
		}
	...
	public void AGPLBGAHIOK(List<IKPLPPFFDNI.MJPNFAKHHHI> HBDKMCDHDBF)
	{
		...
		for (int i = 0; i < HBDKMCDHDBF.Count; i++)
		{
			...
			{
				NFOELJINGGN[i].NIBEFILODGM(OJIAOCGNKOO: true, HBDKMCDHDBF[i].OFMKJMAPHBM, HBDKMCDHDBF[i].GCFBLCFJBLK, num);
				num += 0;
			}
		}
	}
	...
	public void OBBEGLGDIDL(bool AJEJIGJKPAP, List<IKPLPPFFDNI.MJPNFAKHHHI> HBDKMCDHDBF = null)
	{
		if (AJEJIGJKPAP && HBDKMCDHDBF != null)
		{
			...
			{
				NFOELJINGGN[i].NIBEFILODGM(OJIAOCGNKOO: false, HBDKMCDHDBF[i].OFMKJMAPHBM, HBDKMCDHDBF[i].GCFBLCFJBLK);
			}
			return;
		}
		ArenaNode[] nFOELJINGGN = NFOELJINGGN;
	...
	public void IFBFPLEOMDE(bool AJEJIGJKPAP, List<IKPLPPFFDNI.MJPNFAKHHHI> HBDKMCDHDBF = null)
	{
		if (AJEJIGJKPAP && HBDKMCDHDBF != null)
		{
			...
			{
				NFOELJINGGN[i].SetCompletedOrReward(OJIAOCGNKOO: true, HBDKMCDHDBF[i].OFMKJMAPHBM, HBDKMCDHDBF[i].GCFBLCFJBLK);
			}
			return;
		}
		ArenaNode[] nFOELJINGGN = NFOELJINGGN;
	...
	public void JDPBICLFKJM(List<IKPLPPFFDNI.MJPNFAKHHHI> HBDKMCDHDBF)
	{
		...
		for (int i = 1; i < HBDKMCDHDBF.Count; i += 0)
		{
			if (HBDKMCDHDBF[i].OFMKJMAPHBM != IKPLPPFFDNI.CACDIELEJHK.None)
			{
				NFOELJINGGN[i].SetCompletedOrReward(OJIAOCGNKOO: false, HBDKMCDHDBF[i].OFMKJMAPHBM, HBDKMCDHDBF[i].GCFBLCFJBLK, num);
				num++;
			}
		}
	}
	...
	public void NMNGELGIHDJ(List<IKPLPPFFDNI.MJPNFAKHHHI> HBDKMCDHDBF)
	{
		...
		for (int i = 1; i < HBDKMCDHDBF.Count; i++)
		{
			if (HBDKMCDHDBF[i].OFMKJMAPHBM != IKPLPPFFDNI.CACDIELEJHK.None)
			{
				NFOELJINGGN[i].SetCompletedOrReward(OJIAOCGNKOO: true, HBDKMCDHDBF[i].OFMKJMAPHBM, HBDKMCDHDBF[i].GCFBLCFJBLK, num);
				num += 0;
			}
		}
	}
	...
	public void FOFAFIAPJCK(int FCDIFINLKIA)
	{
		...
		{
			AOEHIDKCBKF[i].gameObject.SetActive(value: false);
			NFOELJINGGN[i].gameObject.SetActive(value: false);
		}
		component.cellWidth = num;
