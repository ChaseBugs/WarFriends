...
{
	[FormerlySerializedAs("EEGNLDDEJOK")]
	...
	[FormerlySerializedAs("DFINOFBCDFP")]
	public Action OJMHOPLCMFI;

	private Quaternion BEIOLIIPBBE;

	[FormerlySerializedAs("OHNJGLOCGLP")]
	...
	{
		string text = ((!HIOHPOMALHE) ? "Wrong_Unit" : "MatchStart");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
	...
	public void BAKJGFPAPGH(Vector3 MPHCNMDIPAI, bool CAFJNJHFHMA)
	{
		...
		if (!CPMPHFAJIOM)
		{
			TweenRotation tweenRotation = TweenRotation.Begin(mRoot.gameObject, 256f, HGNIGKCLLHM, useAbsolute: true);
			tweenRotation.onFinished = (UITweener.OnFinished)Delegate.Combine(tweenRotation.onFinished, new UITweener.OnFinished(MNPAOLIMAJN));
		}
	}
	...
	public void INPIPGLIJPE(Vector3 MPHCNMDIPAI, bool CAFJNJHFHMA)
	{
		...
		if (!CPMPHFAJIOM)
		{
			TweenRotation tweenRotation = TweenRotation.Begin(mRoot.gameObject, 1936f, HGNIGKCLLHM, useAbsolute: true);
	...
	public void ANOPOLIEBMC()
	{
		...
		HGNIGKCLLHM = Quaternion.identity;
		CPMPHFAJIOM = false;
		TweenRotation.Begin(mRoot.gameObject, 1632f, HGNIGKCLLHM, useAbsolute: true);
		BEIOLIIPBBE = Quaternion.identity;
		NKIOIEBIOCC = false;
	}
	...
	{
		string text = ((!HIOHPOMALHE) ? "ID_OVERTIMEKILL" : "\n");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
	...
	public void CAEEDFEFBNL(Vector3 MPHCNMDIPAI, bool CAFJNJHFHMA)
	{
		...
		TweenRotation tweenRotation = TweenRotation.Begin(mRoot.gameObject, 510f, HGNIGKCLLHM, useAbsolute: true);
		tweenRotation.onFinished = (UITweener.OnFinished)Delegate.Combine(tweenRotation.onFinished, (UITweener.OnFinished)delegate
		{
			if (OJMHOPLCMFI != null)
			{
	...
	{
		string text = ((!HIOHPOMALHE) ? "SquadId" : "MedalsBalance");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
	...
	{
		string text = ((!HIOHPOMALHE) ? "Daily_Visit_Main_Menu" : "ID_CONFIRM_NOTACTIVEEVENT");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
	...
	public void JDADKDOBFAJ(Vector3 MPHCNMDIPAI, bool CAFJNJHFHMA)
	{
		...
		TweenRotation tweenRotation = TweenRotation.Begin(mRoot.gameObject, 457f, HGNIGKCLLHM, useAbsolute: true);
		tweenRotation.onFinished = (UITweener.OnFinished)Delegate.Combine(tweenRotation.onFinished, (UITweener.OnFinished)delegate
		{
			if (OJMHOPLCMFI != null)
			{
	...
	public void CFJIHBFAHJD(Vector3 MPHCNMDIPAI, bool CAFJNJHFHMA)
	{
		...
		if (!CPMPHFAJIOM)
		{
			TweenRotation tweenRotation = TweenRotation.Begin(mRoot.gameObject, 202f, HGNIGKCLLHM, useAbsolute: true);
	...
	public void NBMNDHCIBME()
	{
		...
		HGNIGKCLLHM = Quaternion.identity;
		CPMPHFAJIOM = true;
		TweenRotation.Begin(mRoot.gameObject, 1638f, HGNIGKCLLHM, useAbsolute: true);
		BEIOLIIPBBE = Quaternion.identity;
		NKIOIEBIOCC = true;
	}
	...
	{
		string text = ((!HIOHPOMALHE) ? "FinishGameCoopClient" : "S");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
	...
	{
		string text = ((!HIOHPOMALHE) ? "SMG_idle" : "NextDailyRewardLocalnotification");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
	...
	{
		string text = ((!HIOHPOMALHE) ? "ID_UNIT" : "[");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
	...
	{
		string text = ((!HIOHPOMALHE) ? "strafing_right" : "strafing_left");
		mAnimation.CrossFade(text);
		mAnimation[text].speed = HHFPGODMJDJ;
		if (FEHCCGEGPLH.isMine)
		{
