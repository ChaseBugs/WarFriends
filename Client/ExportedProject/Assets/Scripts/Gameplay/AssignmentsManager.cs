...
public class AssignmentsManager : DatabaseSerializedObjectGeneric<AssignmentsManager.AssignmentData>
{
	...
	[CompilerGenerated]
	private AssignmentsConstants _003CassignmentsConstants_003Ek__BackingField;

	public static AssignmentsManager instance
	{
		get
		{
			mInstance = mInstance ?? ((AssignmentsManager)SingletonSupport.FindOrCreate(typeof(AssignmentsManager)));
			return mInstance;
		}
	}

	public AssignmentsConstants assignmentsConstants
	{
		[CompilerGenerated]
		get
		{
			return _003CassignmentsConstants_003Ek__BackingField;
		}
		[CompilerGenerated]
		private set
		{
			_003CassignmentsConstants_003Ek__BackingField = value;
		}
	}
	...
	public int megaRewarPoints => (int)(float)assignmentsConstants.GetRow(AssignmentsConstants.rowIds.MegaRewardPoints).FLOATVALUE;

	public float megaRewardProgress => Mathf.Clamp01((float)currentMegaRewardPoints / (float)megaRewarPoints);
	...
	private void LoadAssignments()
	{
		...
		if (data != null && data.assignments != null && !StarterAssignmentsManager.instance.isActiveAndNotCompleted)
		{
			...
			{
				mPreparedAssignments.Add(Assignment.CreateAssignment(assignment, isSquadEvent: false, string.Empty));
			}
		}
		if (this.AssignmentsLoaded != null)
		{
