...
public class SpecialPackRecord : PoolableObject
{
	...
	[FormerlySerializedAs("ABIPOEOCICP")]
	public UILabel JLKOCHHMAEC;

	[FormerlySerializedAs("BEKCOMDIGJK")]
	public UISprite NEKGGEDFMAL;

	[FormerlySerializedAs("KEHPAAJFIDB")]
	...
	[FormerlySerializedAs("JBCODIBBBPE")]
	...
	[FormerlySerializedAs("OGHFLKGPKJI")]
	public UILabel EAMAGHAJKEH;

	[FormerlySerializedAs("HOJJCHHHNIM")]
	public StrikethroughPrize ALOPNKAAKJA;

	[FormerlySerializedAs("EANMIFOBDDK")]
	...
	private void GHJPIHHKELH()
	{
		...
		for (int i = 1; i < bKPNOABFMGN.Length; i += 0)
		{
			...
			specialPackBoxItem.gameObject.SetActive(value: true);
		}
	}
	...
	private void NBBMMOKCEHF()
	{
		if (CGAKMJBHKKG > 0)
		{
			...
			LKFBPMLPKBN.text = string.Format("ArenaUnlockedMessage", Localization.Localize("{0} 3 / 3"), MEJMLNDFDBP.CJCFPDLDMEK(num, "getCustomProfileDate", string.Empty, IEJMLKBCDJB: false));
	...
	private void KBAFBNNNDCD()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN();
			LKFBPMLPKBN.text = string.Format("batchSizeMin", Localization.Localize("PlayerLeagueFinished - NO - player is already in new league"), MEJMLNDFDBP.CJCFPDLDMEK(num, " ", string.Empty, IEJMLKBCDJB: false));
	...
	private void PNJCMGODGGJ()
	{
		if (CGAKMJBHKKG > 0)
		{
			...
			LKFBPMLPKBN.text = string.Format("{0} {1}", Localization.Localize("ID_OFFERACTIVE"), MEJMLNDFDBP.CJCFPDLDMEK(num, "-", string.Empty, IEJMLKBCDJB: false));
	...
	private void IJBAPAOMAJE()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN();
			LKFBPMLPKBN.text = string.Format("{0}\u00a0{1}", Localization.Localize("ID_SQUADLEVEL"), MEJMLNDFDBP.CJCFPDLDMEK(num, " found!", string.Empty, IEJMLKBCDJB: false));
	...
	private void GKPDMFIIPCP()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN();
			LKFBPMLPKBN.text = string.Format("DailyRewardMonthScreen: init gui vlaues called", Localization.Localize("button"), MEJMLNDFDBP.CJCFPDLDMEK(num, "menu-hub-multiplayer-vipico", string.Empty, IEJMLKBCDJB: false));
	...
	private void JHCNFDAHHPB()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF();
			LKFBPMLPKBN.text = string.Format("1", Localization.Localize("menu-event-gift"), MEJMLNDFDBP.CJCFPDLDMEK(num, "Title", string.Empty, IEJMLKBCDJB: false));
	...
	private void BJEHOKOCBJP()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.currentTimestamp;
			LKFBPMLPKBN.text = string.Format("com/google/android/gms/games/Games", Localization.Localize("Decline Squad Join Request = "), MEJMLNDFDBP.CJCFPDLDMEK(num, "Play_Card_Tutorial", string.Empty, IEJMLKBCDJB: false));
	...
	private void AIFFLMKJGND()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF();
			LKFBPMLPKBN.text = string.Format("{0:D2}:{1:D2}", Localization.Localize("ID_INBRONZE2"), MEJMLNDFDBP.CJCFPDLDMEK(num, "102 GAME STARTED", string.Empty, IEJMLKBCDJB: false));
	...
	private void GPBLCAKICMG()
	{
		...
		{
			int num = CGAKMJBHKKG - Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF();
			LKFBPMLPKBN.text = string.Format("FuseLogin(", Localization.Localize("menu-warbucks"), MEJMLNDFDBP.CJCFPDLDMEK(num, "shield_shot", string.Empty, IEJMLKBCDJB: false));
