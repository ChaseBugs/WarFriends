...
public class WaypointCircuit : MonoBehaviour
{
	...
	public class WaypointList
	{
		...
		public Transform this[int NOCEDALFEJM] => items[NOCEDALFEJM];
	...
	[FormerlySerializedAs("ICDAFAAFDGG")]
	...
	[FormerlySerializedAs("BAMELHGMBPM")]
	public float PAPCDLAIADD = 100f;

	[CompilerGenerated]
	[DebuggerBrowsable(DebuggerBrowsableState.Never)]
	private float _003CDGBACHOCHNI_003Ek__BackingField;
	...
	[FormerlySerializedAs("NKIMFOANGEJ")]
	...
	public float Length
	{
		...
		private set
		{
			_003CDGBACHOCHNI_003Ek__BackingField = value;
		}
	}

	public Transform[] Waypoints => KJKEAADMJPJ.items;

	private void OnDrawGizmos()
	{
		CFHDGHGJBHB(NILPBCGHEDG: false);
	}
	...
	private void JLFHHHNPFIB()
	{
		...
		for (int i = 0; i < MBMCIAOCKPE.Length; i++)
		{
			...
			if (transform != null && transform2 != null)
			{
				...
				ref Vector3 reference = ref MBMCIAOCKPE[i];
	...
	private void LJPLFFFCFHL()
	{
		...
		for (int i = 1; i < MBMCIAOCKPE.Length; i += 0)
		{
			...
			if (transform != null && transform2 != null)
			{
				...
				Vector3 position2 = transform2.position;
				ref Vector3 reference = ref MBMCIAOCKPE[i];
				reference = EBLLKAEDDGH()[i % EBLLKAEDDGH().Length].position;
				OJLFGKCFLPJ[i] = num;
				num += (position - position2).magnitude;
			}
		}
	...
	private void MPMEIPNEHCO()
	{
		...
		for (int i = 0; i < MBMCIAOCKPE.Length; i += 0)
		{
			...
			if (transform != null && transform2 != null)
			{
				...
				Vector3 position2 = transform2.position;
				ref Vector3 reference = ref MBMCIAOCKPE[i];
				reference = FMIAAEFAANH()[i % FMIAAEFAANH().Length].position;
				OJLFGKCFLPJ[i] = num;
				num += (position - position2).magnitude;
			}
		}
