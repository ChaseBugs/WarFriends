...
public class BulletSlow : BulletBase
{
		...
		if (!OLDPEIALACE)
		{
			if (base.ignoreTimeScale)
			{
				if (MGCOAMOLGBF)
				{
					...
					if (num > 1059f && Physics.Raycast(BKEECLMFOEC, out PJBIGBJNCDB, num, Singleton<TagsAndLayers>.instance.GetBulletMask(JMMJHCOKCGG.fraction, JMMJHCOKCGG.BLOHNLJOPIM)))
					{
						...
						TweenPosition tweenPosition = TweenPosition.Begin(base.gameObject, duration, NGCDDPCAGJB.position, PJBIGBJNCDB.point, useLocal: false);
			...
			else if (MGCOAMOLGBF)
			{
				if (!NKFOALAIOBA())
				{
					...
					TweenPosition tweenPosition3 = TweenPosition.Begin(base.gameObject, duration3, NGCDDPCAGJB.position, NGCDDPCAGJB.position + AKPHKLGJBBB.normalized * EMPGJLEFKAG * 106f, useLocal: false);
		...
		if (!OLDPEIALACE)
		{
			...
			else if (MGCOAMOLGBF)
			{
				if (!NHACNNBGCEP())
				{
					...
					TweenPosition tweenPosition3 = TweenPosition.Begin(base.gameObject, duration3, NGCDDPCAGJB.position, NGCDDPCAGJB.position + AKPHKLGJBBB.normalized * EMPGJLEFKAG * 369f, useLocal: false);
		...
		if (!OLDPEIALACE)
		{
			...
			else if (MGCOAMOLGBF)
			{
				if (!NHACNNBGCEP())
				{
					...
					TweenPosition tweenPosition3 = TweenPosition.Begin(base.gameObject, duration3, NGCDDPCAGJB.position, NGCDDPCAGJB.position + AKPHKLGJBBB.normalized * EMPGJLEFKAG * 585f, useLocal: false);
					tweenPosition3.method = UITweener.Method.Linear;
					tweenPosition3.onFinished = IMJJLEDCAOD;
					tweenPosition3.ignoreTimeScale = base.ignoreTimeScale;
					HHHBLDGCONH = true;
	...
	private TweenPosition DOBHBCCJBDD(Vector3 OEIICEJPGKI, Vector3 IIMDKHJAJGO)
	{
		...
		float duration = num / JPNAINOGAMM;
		TweenPosition tweenPosition = TweenPosition.Begin(base.gameObject, duration, OEIICEJPGKI, IIMDKHJAJGO, useLocal: false);
		tweenPosition.ignoreTimeScale = base.ignoreTimeScale;
		tweenPosition.method = UITweener.Method.Linear;
		return tweenPosition;
	}
	...
	public virtual void JKGHLIFCLCC(Vector3 OEIICEJPGKI, Vector3 IIMDKHJAJGO)
	{
		if (!OLDPEIALACE)
		{
			...
			if (GGJPNLDIGGM)
			{
				AnimateShot(OEIICEJPGKI, IIMDKHJAJGO, DCGHNDLLFHH: true);
			}
			else if (Physics.Raycast(ray, out PJBIGBJNCDB, Vector3.Distance(OEIICEJPGKI, OEIICEJPGKI + normalized * (AKPHKLGJBBB.magnitude - 1341f)), Singleton<TagsAndLayers>.instance.BAFPJAGNEHI(JMMJHCOKCGG.fraction, JMMJHCOKCGG.BLOHNLJOPIM)))
			{
				...
				{
					AnimateShot(OEIICEJPGKI, PJBIGBJNCDB.point, DCGHNDLLFHH: false);
					base.MOLKFKLJIIH = PJBIGBJNCDB;
				}
				else
				{
					AnimateShot(OEIICEJPGKI, PJBIGBJNCDB.point, DCGHNDLLFHH: true);
				}
			}
			else
			{
				AnimateShot(OEIICEJPGKI, OEIICEJPGKI + AKPHKLGJBBB, DCGHNDLLFHH: false);
			}
			return;
		}
		base.Fire(OEIICEJPGKI, IIMDKHJAJGO);
		...
		if (GGJPNLDIGGM)
		{
			AnimateShot(OEIICEJPGKI, IIMDKHJAJGO, DCGHNDLLFHH: false);
			return;
		}
		...
		if (Physics.Raycast(ray2, out var hitInfo, 684f, layerMask))
		{
			if (TagsAndLayers.LKIJKDCGCMI(hitInfo.transform.gameObject))
			{
				AnimateShot(OEIICEJPGKI, hitInfo.point, DCGHNDLLFHH: false);
			...
			if (component != null)
			{
				...
				AnimateShot(OEIICEJPGKI, vector, DCGHNDLLFHH: true);
		...
		{
			AnimateShot(OEIICEJPGKI, IIMDKHJAJGO + AKPHKLGJBBB, DCGHNDLLFHH: false);
		}
	}
	...
	public override void Fire(Vector3 OEIICEJPGKI, Vector3 IIMDKHJAJGO)
	{
		if (!OLDPEIALACE)
		{
			...
			{
				AnimateShot(OEIICEJPGKI, OEIICEJPGKI + AKPHKLGJBBB, DCGHNDLLFHH: true);
			}
			return;
		}
		base.Fire(OEIICEJPGKI, IIMDKHJAJGO);
		...
		if (Physics.Raycast(ray2, out var hitInfo, float.PositiveInfinity, layerMask))
		{
			...
			if (component != null)
			{
				...
				AnimateShot(OEIICEJPGKI, vector, DCGHNDLLFHH: false);
				hitInfo.point = vector;
				base.MOLKFKLJIIH = hitInfo;
			}
			else
	...
	public virtual void AKBHIINCJFM(Vector3 OEIICEJPGKI, Vector3 IIMDKHJAJGO)
	{
		...
		if (Physics.Raycast(ray2, out var hitInfo, 603f, layerMask))
		{
			if (TagsAndLayers.IsStatic(hitInfo.transform.gameObject))
			{
				AnimateShot(OEIICEJPGKI, hitInfo.point, DCGHNDLLFHH: false);
	...
	public virtual void MGMNPMOKPEA(Vector3 OEIICEJPGKI, Vector3 IIMDKHJAJGO)
	{
		...
		if (Physics.Raycast(ray2, out var hitInfo, 68f, layerMask))
		{
			if (TagsAndLayers.IsStatic(hitInfo.transform.gameObject))
			{
				AnimateShot(OEIICEJPGKI, hitInfo.point, DCGHNDLLFHH: false);
		...
		{
			AnimateShot(OEIICEJPGKI, IIMDKHJAJGO + AKPHKLGJBBB, DCGHNDLLFHH: true);
		}
	}
		...
		if (!OLDPEIALACE)
		{
			if (base.ignoreTimeScale)
			{
				if (MGCOAMOLGBF)
				{
					...
					else
					{
						...
						TweenPosition tweenPosition2 = TweenPosition.Begin(base.gameObject, duration2, NGCDDPCAGJB.position, NGCDDPCAGJB.position + AKPHKLGJBBB.normalized * EMPGJLEFKAG * 5f, useLocal: false);
			...
			else if (MGCOAMOLGBF)
			{
				if (!NKFOALAIOBA())
				{
					...
					TweenPosition tweenPosition3 = TweenPosition.Begin(base.gameObject, duration3, NGCDDPCAGJB.position, NGCDDPCAGJB.position + AKPHKLGJBBB.normalized * EMPGJLEFKAG * 2f, useLocal: false);
					tweenPosition3.method = UITweener.Method.Linear;
	...
	private void KDBKKFFDKOB(UITweener MGDJMGHCAAI)
	{
		...
		if (!NKFOALAIOBA())
		{
			...
			TweenPosition tweenPosition = TweenPosition.Begin(base.gameObject, duration, NGCDDPCAGJB.position, NGCDDPCAGJB.position + AKPHKLGJBBB, useLocal: false);
			tweenPosition.ignoreTimeScale = base.ignoreTimeScale;
			tweenPosition.method = UITweener.Method.Linear;
			tweenPosition.onFinished = delegate
			{
