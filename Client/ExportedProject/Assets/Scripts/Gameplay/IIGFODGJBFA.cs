...
public class IIGFODGJBFA
{
	...
	public string NCJDBDJJDNO
	{
		get
		{
			...
			text += $"Warbucks - League:{BMGOMCJOMLE} Battle:{LIBCLGEENBB} Extra:{CMKMMOLDDOM} Winstreak:{HGGAKLGNDIE} (offer mult {PMEEBBIMLOE})\n";
			text += $"Gold - League:{CGIHGALEAEM} Battle:{KFFMJGGCCGH} Extra:{HANAIHNOMCJ} Winstreak:{HDOEIEMJANC} (offer mult {IHDCGLIOEIF})\n";
			text += $"XP - Battle:{EPJJFIEPPEI} Extra:{NLEANDGDIGL} Winstreak:{DCDONOEMPAJ} Time:{HGFPBDCNGMP} (offer mult {DDGEBMMBCDH})\n";
			text += $"VIP - {GJGGMAEJLAP}\n";
			text += $"Medals - old:{ANLPOCBOKFA} new:{LEMFIOHBKCF} Balance old:{HGNIDLMHIFE} Balance new:{OKAGACLNLCJ} Remaining fights:{OMBMONINIGB}\n";
			text += $"Squad Points - {ILBOGEIEPMF}\n";
	...
	public string HPEFFOCJKEO
	{
		get
		{
			...
			text += $"lootboxes offer mult {AMKJPAEJEKM}\n";
	...
	public int EKJDLHMCPGL => (int)((float)BMGOMCJOMLE * PMEEBBIMLOE);

	public int MAHDEJFLPEL => (int)((float)BMGOMCJOMLE * Singleton<VipManager>.instance.vipWarbucksMultiplier * PMEEBBIMLOE);

	public int BJEALDGKMME => (int)((float)(LIBCLGEENBB + CMKMMOLDDOM) * PMEEBBIMLOE);

	public int BFBJGNAPEKO => (int)((float)(LIBCLGEENBB + CMKMMOLDDOM) * Singleton<VipManager>.instance.vipWarbucksMultiplier * PMEEBBIMLOE);

	public int HMPJBGNNBHE => (int)((float)HGGAKLGNDIE * PMEEBBIMLOE);

	public int OFBOPGDNCGM => (int)((float)HGGAKLGNDIE * Singleton<VipManager>.instance.vipWarbucksMultiplier * PMEEBBIMLOE);

	public int CNLNJKDJBBJ => EKJDLHMCPGL + BJEALDGKMME + HMPJBGNNBHE;

	public int CDCABGKPKPH => MAHDEJFLPEL + BFBJGNAPEKO + OFBOPGDNCGM;

	public int MHJFGOENBJI => CGIHGALEAEM;

	public int HDKDPJNDGAJ => (int)((float)CGIHGALEAEM * Singleton<VipManager>.instance.vipGoldMultiplier);

	public int KCCMCLDCJJN => KFFMJGGCCGH + HANAIHNOMCJ;

	public int NPGLGKHJODJ => (int)((float)(KFFMJGGCCGH + HANAIHNOMCJ) * Singleton<VipManager>.instance.vipGoldMultiplier);

	public int PONLJJCIKKF => HDOEIEMJANC;

	public int CENOLBBKONH => (int)((float)HDOEIEMJANC * Singleton<VipManager>.instance.vipGoldMultiplier);

	public int LLOFEOGFBAA => MHJFGOENBJI + KCCMCLDCJJN + PONLJJCIKKF;

	public int HKPAMPNIKJP => HDKDPJNDGAJ + NPGLGKHJODJ + CENOLBBKONH;

	public bool GKEOILLNCHG => OMBMONINIGB == 0;

	public int NPIGGODIKDB => ANLPOCBOKFA;

	public int FOEADOOIPGH => LEMFIOHBKCF;

	public int FCHPFJCBBAE => HGNIDLMHIFE;

	public int NKMEDCCIDCM => OKAGACLNLCJ;

	public int KPEKCHDBFIG => ILBOGEIEPMF;

	public int KHEICLEGJEM => OMBMONINIGB;

	public bool BOLNOFIFMEH => BMGOMCJOMLE > 0 || CGIHGALEAEM > 0;

	public List<CCNAEIGGMNK> GMOLDNDALEE => LDDKEHKEJBI;
	...
	public void KEPPKNMKMDK(int HOKCINCJPIJ, float JIEPJDLLBGP)
	{
		...
		Debug.Log($"#Game Rewards# - Squad Points: {HOKCINCJPIJ} (after offer mult x{JIEPJDLLBGP} on server)");
	}
	...
	public void MCCFNOMBBDB(int CJDHKJFKNAH, int GLODABHNMJO, int FGFFGKGLBII, float EDAFIGGJKEI)
	{
		...
		OKAGACLNLCJ = GLODABHNMJO;
		OMBMONINIGB = FGFFGKGLBII;
		Debug.Log($"#Game Rewards# - Medals old:{ANLPOCBOKFA} new:{LEMFIOHBKCF} Balance old:{HGNIDLMHIFE} new:{OKAGACLNLCJ} Remaining fights:{OMBMONINIGB} - (offer mult {KEJOAOJKKGH})");
	}
