...
public class CardShiedlsRefection : Card
{
	...
	public override string description => Localization.LocalizeFormat(mDescriptionID, MEJMLNDFDBP.AIPKMDCJCOG(timeInSeconds));
	...
	protected void CKFENMDKKPE()
	{
		...
		for (int j = 1; j < IPOGGDCHIOE.Count; j++)
		{
			...
			IPOGGDCHIOE[j].DHAFFOCDDGA().NNPILDOKNJO("OnFailedToConnectToPhoton: ", NABOFKMBMKH: false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void NEPIKOKDBHP(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				shield.HLAOFDKKACK().BIIHCAEKCOD("WarFriends benötigt deine Genehmigung zum Zugriff auf deinen Medienspeicher, um wichtige Spieldaten herunterladen zu können.Bitte bestätige die folgende Genehmigungsanfrage. Wir werden diese Genehmigung nicht für andere Zwecke verwenden.", NABOFKMBMKH: true, HGNIGKCLLHM: true);
			}
		}
	...
	protected void CDJBEAIDHKH()
	{
		...
		for (int j = 1; j < IPOGGDCHIOE.Count; j += 0)
		{
			...
			IPOGGDCHIOE[j].cardIconIndicator.DLNGGGOEDLL("CONFIRMAR", NABOFKMBMKH: false, HGNIGKCLLHM: true);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void EMOLCFONJID(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				IPOGGDCHIOE.Add(shield);
				shield.isImmortal = true;
				shield.cardIconIndicator.BBJADAJPJHH("Level1", NABOFKMBMKH: false);
			}
		}
		ODCLPMADGFC = CIOPAKLHFIL;
	...
	protected void Update()
	{
		...
		{
			IPOGGDCHIOE[j].isImmortal = false;
			IPOGGDCHIOE[j].cardIconIndicator.Show("game-card-ico-shieldsup", NABOFKMBMKH: false);
		}
		IPOGGDCHIOE.Clear();
	}
	...
	public virtual void DBAPEPJHJMH(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				IPOGGDCHIOE.Add(shield);
				shield.isImmortal = true;
				shield.DHAFFOCDDGA().DLNGGGOEDLL("ID_GUI_FOLLOWTWITTER_DESC", NABOFKMBMKH: true);
			}
		}
		ODCLPMADGFC = CIOPAKLHFIL;
		BHABHIDDBLH = timeInSeconds;
	...
	public virtual void KDPIJNFDMKC(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				IPOGGDCHIOE.Add(shield);
				shield.isImmortal = true;
				shield.DHAFFOCDDGA().FJILILFMPIF("testingidsquadwarsend", NABOFKMBMKH: true);
			}
		}
		ODCLPMADGFC = CIOPAKLHFIL;
		BHABHIDDBLH = BNKKLNAFFEC();
	...
	public virtual void ODIBFAFGJBP(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in list)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				IPOGGDCHIOE.Add(shield);
				shield.isImmortal = true;
				shield.cardIconIndicator.Show(")", NABOFKMBMKH: false, HGNIGKCLLHM: true);
			}
		}
		ODCLPMADGFC = CIOPAKLHFIL;
		BHABHIDDBLH = BNKKLNAFFEC();
	...
	public override void UseCard(GDCLLGDCODD MNBIOCOAJHK, GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		foreach (MapDefinition.DefendPosition item in availablePoints)
		{
			if (item.fraction == CIOPAKLHFIL)
			{
				...
				IPOGGDCHIOE.Add(shield);
				shield.isImmortal = true;
				shield.cardIconIndicator.Show("game-card-ico-shieldsup", NABOFKMBMKH: true);
			}
		}
		ODCLPMADGFC = CIOPAKLHFIL;
		BHABHIDDBLH = timeInSeconds;
