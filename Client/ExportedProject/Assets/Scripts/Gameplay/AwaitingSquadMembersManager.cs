...
public class AwaitingSquadMembersManager : Singleton<AwaitingSquadMembersManager>
{
	...
	public string RemovePlayer(string EJBEDLHIKDG, bool FGPIFPECHIC = true)
	{
		if (!NNIKNEECAMM.ContainsKey(EJBEDLHIKDG))
		{
			Debug.Log($"Awaiting players - Player id:\"{EJBEDLHIKDG}\" is not in list of awaiting members - cannot remove it");
			return null;
		}
	...
	public void OnPlayerAcceptedOrDeclined(string INFLHPGMEOB, bool IHJGCEHAAJN)
	{
		...
		if (IHJGCEHAAJN)
		{
			...
			UpdateListOfAwaitingMembers(LPGMLOCLBGP: false);
		}
	}
	...
	public void AddAwaitingMember(DatabasePlayer KHLGDCHJJPB)
	{
		if (NNIKNEECAMM.ContainsKey(KHLGDCHJJPB.id))
		{
			Debug.Log($"Awaiting players - Player \"{KHLGDCHJJPB.name}\" is already in list of awaiting members with state {NNIKNEECAMM[KHLGDCHJJPB.id].IGBEGIBGLKK.ToString().ToUpper()}");
			return;
		}
		HJHILHFDMKP hJHILHFDMKP = new HJHILHFDMKP();
		hJHILHFDMKP.KHLGDCHJJPB = KHLGDCHJJPB;
	...
	public void YesClick(string EJBEDLHIKDG)
	{
		...
		if (!NNIKNEECAMM.ContainsKey(EJBEDLHIKDG))
		{
			Debug.Log($"Awaiting players - Player id:\"{EJBEDLHIKDG}\" is not in list of awaiting members - cannot accept player");
			return;
		}
		NNIKNEECAMM[EJBEDLHIKDG].IGBEGIBGLKK = EFECGFGNLIE.Waiting;
		DatabasePlayer currentPlayer = GameLoginManager.currentPlayer;
	...
	public void HDCDHLECNNC()
	{
		...
		foreach (KeyValuePair<string, HJHILHFDMKP> item in NNIKNEECAMM)
		{
			...
			{
				string text = IMJMMDOEGGK(item.Key, FGPIFPECHIC: false);
				if (text != null)
				{
					list.Add(text);
				}
	...
	private void ONFMDLKHGPM(string EJBEDLHIKDG)
	{
		if (!NNIKNEECAMM.ContainsKey(EJBEDLHIKDG))
		{
			Debug.Log($"Awaiting players - Player id:\"{EJBEDLHIKDG}\" is not in list of awaiting members - cannot remove it from MC");
			return;
		}
		if (NNIKNEECAMM[EJBEDLHIKDG].NKDCPACFGGA == null)
		{
			Debug.Log($"Awaiting players - Player id:\"{EJBEDLHIKDG}\" was not added to message center - cannot remove it from MC");
			return;
		}
	...
	public void FFAGKCAPLIO(string INFLHPGMEOB, bool IHJGCEHAAJN)
	{
		if (NNIKNEECAMM.ContainsKey(INFLHPGMEOB))
		{
			GPNNPMOGGBP(INFLHPGMEOB, FGPIFPECHIC: false);
		}
		else
		{
			StringBuilder stringBuilder = new StringBuilder();
	...
	public void NoClick(string EJBEDLHIKDG)
	{
		...
		if (!NNIKNEECAMM.ContainsKey(EJBEDLHIKDG))
		{
			Debug.Log($"Awaiting players - Player id:\"{EJBEDLHIKDG}\" is not in list of awaiting members - cannot decline player");
			return;
		}
		NNIKNEECAMM[EJBEDLHIKDG].IGBEGIBGLKK = EFECGFGNLIE.Waiting;
		DatabasePlayer currentPlayer = GameLoginManager.currentPlayer;
	...
	public void JEONJCCHDMB(string INFLHPGMEOB, bool IHJGCEHAAJN)
	{
		if (NNIKNEECAMM.ContainsKey(INFLHPGMEOB))
		{
			RemovePlayer(INFLHPGMEOB, FGPIFPECHIC: false);
		}
		else
		{
			StringBuilder stringBuilder = new StringBuilder();
	...
	public void RemoveErrorPlayers()
	{
		...
		foreach (KeyValuePair<string, HJHILHFDMKP> item in NNIKNEECAMM)
		{
			...
			{
				string text = RemovePlayer(item.Key, FGPIFPECHIC: false);
				if (text != null)
				{
					list.Add(text);
				}
	...
	public void GFPELLMIKNA(string INFLHPGMEOB, bool IHJGCEHAAJN)
	{
		if (NNIKNEECAMM.ContainsKey(INFLHPGMEOB))
		{
			IMJMMDOEGGK(INFLHPGMEOB, FGPIFPECHIC: false);
		}
		else
		{
			StringBuilder stringBuilder = new StringBuilder();
