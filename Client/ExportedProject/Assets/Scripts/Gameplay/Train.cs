...
public class Train : Core_BaseScript
{
	...
	[FormerlySerializedAs("EDGIFAONGBA")]
	...
	[FormerlySerializedAs("CCEHDJPODNA")]
	public List<TrainLoad> NPCIOFPGMKO;

	[FormerlySerializedAs("INDOFHBIELK")]
	...
	private void NCKKBEJDJBG(byte[] CAGGPKLJPNE)
	{
		for (int i = 0; i < JBEIOADFJJE.Count; i++)
		{
			if (i < CAGGPKLJPNE.Length)
			{
				...
				JBEIOADFJJE[i].gameObject.transform.parent.gameObject.SetActive(value: true);
			...
			{
				JBEIOADFJJE[i].gameObject.transform.parent.gameObject.SetActive(value: false);
			}
		}
	...
	private void KIBMALEELIL()
	{
		...
		TweenPosition tweenPosition = TweenPosition.Begin(base.gameObject, num / JPNAINOGAMM, vector, vector2, useLocal: false);
