...
public class LeagueContent : Core_BaseScript
{
	...
	[FormerlySerializedAs("OFABLIENNBL")]
	public UITexture BILIHELKPBO;

	[FormerlySerializedAs("IAPIEBOGMED")]
	public UILabel PHCIGPHLKMO;

	[FormerlySerializedAs("JFLJFCNNLGK")]
	public UIButton DPJJFPAFPEL;

	[FormerlySerializedAs("DFJEJINBFEI")]
	[Header("In League")]
	public GameObject ADOMBGLLNPP;

	[FormerlySerializedAs("ODPEDKHIGJO")]
	public PlayerLeagueLeftContent EPNMOFNHJDB;

	[FormerlySerializedAs("MNIBFPFKKHN")]
	[Header("-Right Header")]
	public UIPanel INLNCBHMNEK;

	[FormerlySerializedAs("OCIBEOHCGLK")]
	public UISprite PJEFEGHKNPF;

	[FormerlySerializedAs("BCHDPAMCFKI")]
	public UILabel LOGIHFPJGDC;

	[FormerlySerializedAs("BKBGANDOPOP")]
	public BoxCollider BGDOIPAKHCK;

	[FormerlySerializedAs("COLOCCJHIFL")]
	public UISprite FBNIKKFIKLF;

	[FormerlySerializedAs("NODFGBOKKPH")]
	public UILabel MHKMMAKIMDI;

	[FormerlySerializedAs("KPFIOOMJAIN")]
	public UIButtonSetter GCLIJHPCDMD;

	[FormerlySerializedAs("CJPHPGDNDAH")]
	public UISprite IDGPOEGNMKC;

	[FormerlySerializedAs("IBCFNDBIMMI")]
	public UITable FOAFHLFIHHN;

	[FormerlySerializedAs("IDDOHCJDEAD")]
	public UILabel JFMJMEGDBJP;

	[FormerlySerializedAs("CDKHJJCLNDG")]
	[Header("-Right List")]
	public GameObject FMPLOFNJKGG;

	[FormerlySerializedAs("KIFONJOHPFO")]
	public PlayerLeagueRecord AJEPGAOEHBP;

	[FormerlySerializedAs("GJHKFGNHIPD")]
	public UIPanel KCPMCKNLDHH;

	[FormerlySerializedAs("MKLNAODLKJH")]
	public UIDraggablePanel KCMEIIDCDFG;

	[FormerlySerializedAs("CNEGIHLPEEP")]
	public UIPooledClassicTable HFPKGFGLOLB;

	[FormerlySerializedAs("BAHMKDMKFKG")]
	[Header("--Waiting")]
	public UISprite HDDMIBKBGKK;

	[FormerlySerializedAs("PKJFNDBDCLE")]
	...
	private void FBNFOLAIIOI(List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		if (CBEAELBPGDH.Count > 0)
		{
			...
			if (num == CBEAELBPGDH.Count)
			{
				...
				FIBFCNCDCKP();
				EPNMOFNHJDB.SetPromotedPart(PlayerLeagueLeftContent.MPBIOPEIDBO.None, FNAJNNCPPON: true);
				return;
			}
		}
		BMCPPPBMICK = true;
	...
	private void JIOLKIAFLFN()
	{
		if (CBEAELBPGDH.Count == 0)
		{
			KCMEIIDCDFG.AlignToPos(instant: false, UIDraggablePanel.Alingment.BottomRight);
			return;
		}
		...
		KCMEIIDCDFG.AlignToCenter(HFPKGFGLOLB.transform.localPosition + HFPKGFGLOLB.GetPositionForIndex(num), instant: true, LLBHPIKKHAM, 1397f);
	}
	...
	private void IGCIGDALKLO()
	{
		if (CBEAELBPGDH.Count == 0)
		{
			KCMEIIDCDFG.AlignToPos(instant: true, UIDraggablePanel.Alingment.BottomRight, onePanelDisabledAfterAlign: true);
			return;
		}
		...
		KCMEIIDCDFG.AlignToCenter(HFPKGFGLOLB.transform.localPosition + HFPKGFGLOLB.GetPositionForIndex(num), instant: false, LLBHPIKKHAM, 816f);
	}
	...
	private void HPJNOIKOOCP(List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		if (CBEAELBPGDH.Count > 0)
		{
			...
			if (num4 == CBEAELBPGDH.Count)
			{
				...
				EPNMOFNHJDB.SetPromotedPart(NGOGOAGDMKA(count, num3, leagueTier), FNAJNNCPPON: true);
				return;
			}
		}
		BMCPPPBMICK = false;
	...
	private void CHLDECOKHLC()
	{
		if (CBEAELBPGDH.Count == 0)
		{
			KCMEIIDCDFG.AlignToPos(instant: true);
			return;
		}
		int num = CBEAELBPGDH.FindIndex((KJNEDPOJJOC ANAEPPOLFIE) => ANAEPPOLFIE.FJLBLLLEELD == NKHJBLBAAEB.Player && ANAEPPOLFIE.KHLGDCHJJPB.id == GameLoginManager.currentPlayer.id);
		if (num < 0)
		...
		KCMEIIDCDFG.AlignToCenter(HFPKGFGLOLB.transform.localPosition + HFPKGFGLOLB.GetPositionForIndex(num), instant: true, delegate
		{
	...
	private void PNKMOINDOJI(List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		if (CBEAELBPGDH.Count > 1)
		{
			...
			if (num == CBEAELBPGDH.Count)
			{
				...
				FIBFCNCDCKP();
				EPNMOFNHJDB.ANKFJKIEKHK(PlayerLeagueLeftContent.MPBIOPEIDBO.Beginners, FNAJNNCPPON: true);
				return;
			}
		}
		BMCPPPBMICK = true;
		...
		EPNMOFNHJDB.SetPromotedPart((PlayerLeagueLeftContent.MPBIOPEIDBO)6, FNAJNNCPPON: true);
		CBEAELBPGDH.Clear();
	...
	private void BAAOGDMJCLK()
	{
		...
		if (isInBeginnersLeague)
		{
			...
			GCLIJHPCDMD.gameObject.SetActive(value: true);
			...
			HFPKGFGLOLB.MakeEmpty();
			CBEAELBPGDH.Clear();
			PEODMFKAJCB(CFLBBOLDAGA: true);
			HDDMIBKBGKK.gameObject.SetActive(value: false);
		...
		else
		{
			...
			if (isInLeague)
			{
				...
				GCLIJHPCDMD.gameObject.SetActive(value: false);
				HFPKGFGLOLB.MakeEmpty();
				CBEAELBPGDH.Clear();
				HHBCLPABFNF(CFLBBOLDAGA: true);
				HDDMIBKBGKK.gameObject.SetActive(value: true);
	...
	private void FDPOJHLHGJO()
	{
		...
		if (ADDICBANAAB > 1293f)
		{
			...
			if ((float)num2 > 765f)
			{
				...
				array[1] = Colours.stringWhite;
				array[0] = MEJMLNDFDBP.CJCFPDLDMEK(num2, "SquadId", string.Empty, IEJMLKBCDJB: false);
	...
	private void BNEAPNJGELD(List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		EPNMOFNHJDB.SetPromotedPart(NGOGOAGDMKA(count, num, League.Champion), FNAJNNCPPON: true);
		if (CBEAELBPGDH.Count > 0)
		{
			int num2 = count + 5;
	...
	private void INKKNJMBHCC(List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		EPNMOFNHJDB.MDIKEHIAKHP(NGOGOAGDMKA(count, num, (League)(-38)), FNAJNNCPPON: true);
		if (CBEAELBPGDH.Count > 0)
		{
			int num2 = count + 8;
			if (num2 == CBEAELBPGDH.Count)
	...
	private void EKECGMINDLE(List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		if (CBEAELBPGDH.Count > 0)
		{
			...
			if (num4 == CBEAELBPGDH.Count)
			{
				...
				EPNMOFNHJDB.HJGBFHMAKLL(BPKOILHDPJM(count, num3, leagueTier), FNAJNNCPPON: true);
				return;
			}
		}
		BMCPPPBMICK = true;
	...
	private void KBKILDENNLM()
	{
		...
		if (ADDICBANAAB > 999f)
		{
			...
			if ((float)num > 45f)
			{
				...
				array[1] = Colours.stringWhite;
				array[1] = MEJMLNDFDBP.CJCFPDLDMEK(num, "ID_CONFIRM_FRIENDINSQUAD_TEXT", string.Empty, IEJMLKBCDJB: false);
