...
{
	[FormerlySerializedAs("AEIAEOCLEHG")]
	...
	[FormerlySerializedAs("KDONJJFOAJN")]
	public string FLELEOEKFLI = string.Empty;

	[FormerlySerializedAs("AJBCNGPAGCJ")]
	...
	public int numberOfSpecialPacks => DGGJJHJBFHL.Count;

	public Dictionary<string, JGBBPCGNCPC>.ValueCollection specialPacks => DGGJJHJBFHL.Values;
		...
		if (PMOKDDENPLE.Count > 0)
		{
			...
			for (int i = 0; i < PMOKDDENPLE.Count; i++)
			{
				if (!PMOKDDENPLE[i].MJPOBONOBCD)
				{
					...
					RequestTexture(BFPPALFKOAF: false, PMOKDDENPLE[i].MODHHKHPHOJ, PMOKDDENPLE[i].GDIAEJILINE);
				}
	...
	public void CheckOffer()
	{
		if (LKELFGNDHDH)
		{
			...
			if (Singleton<BeanstalkServerManager>.instance.currentTimestamp > CGAKMJBHKKG && currentTimestamp - DBKFAPEIBEC > 300)
			{
				...
				fuseboxxService.KBIKJCKLJJE(NABOFKMBMKH: false);
	...
	public void AddSpecialPack(JGBBPCGNCPC HFJFLMIDNBJ)
	{
		...
		{
			UnityEngine.Debug.Log($"Special pack \"{HFJFLMIDNBJ.KJBPJOMHNKB}\" already in offer manager cache.");
		}
		else
		{
	...
	{
		base.Awake();
		UpdateFromServer(null, IPDFNBKLLLO: true);
		LKELFGNDHDH = false;
		DBKFAPEIBEC = 0;
