...
public class IKPLPPFFDNI
{
	...
	public class MJPNFAKHHHI
	{
		...
		public ArenaLootboxesRow GKNFEMNEEIM => (OFMKJMAPHBM != CACDIELEJHK.None) ? WarArena.instance.arenaLootboxes.GetRow(LADPKKACPIC[OFMKJMAPHBM]) : null;
	...
	public class CIFMKGIKJFM
	{
		...
		public int FMCHEIHFCFO => JGBFNKBFCKM + OBKOIHPHDEM;

		public int IHBCDJAHFMM => NGPMJNJPNMB + OBKOIHPHDEM;
		...
		public int LCGEMCALCCO => Mathf.Max(0, IHBCDJAHFMM - Singleton<BeanstalkServerManager>.instance.currentTimestamp);

		public int FBKJMMDCECF => Mathf.Max(0, FMCHEIHFCFO - Singleton<BeanstalkServerManager>.instance.currentTimestamp);
	...
	public bool CGJJJOEMPLK => Singleton<BeanstalkServerManager>.instance.currentTimestamp < JGBFNKBFCKM;

	public int MKOCGJKIJLB => Mathf.Max(0, JGBFNKBFCKM - Singleton<BeanstalkServerManager>.instance.currentTimestamp);

	public bool DIFPOOMLLEM => Singleton<BeanstalkServerManager>.instance.currentTimestamp > NGPMJNJPNMB;
	...
	public LevelBehaviour EJPOKHDDJBO
	{
		get
		{
			...
			{
				if (item is ONFFKNJJJGC oNFFKNJJJGC && oNFFKNJJJGC.NCHGGFNEPJC)
				{
	...
	public LevelBehaviour NEANBBHLPNB()
	{
		foreach (PHAMEDPLFLO item in EOIAFJNEOKL)
		{
			if (item is ONFFKNJJJGC oNFFKNJJJGC && oNFFKNJJJGC.HILMFDKACAO())
			{
				return oNFFKNJJJGC.ACLDFHJKBLI;
			}
		}
	...
	public LevelBehaviour DEGPONMEIIJ()
	{
		foreach (PHAMEDPLFLO item in EOIAFJNEOKL)
		{
			if (item is ONFFKNJJJGC oNFFKNJJJGC && oNFFKNJJJGC.PDMFFMJBFEC())
			{
				return oNFFKNJJJGC.ACLDFHJKBLI;
			}
		}
	...
	public LevelBehaviour MHHGOMNOBPB()
	{
		foreach (PHAMEDPLFLO item in EOIAFJNEOKL)
		{
			if (item is ONFFKNJJJGC oNFFKNJJJGC && oNFFKNJJJGC.LOIFOJBBBEG())
			{
				return oNFFKNJJJGC.ACLDFHJKBLI;
			}
		}
	...
	public LevelBehaviour HBNMHKMFJEL()
	{
		foreach (PHAMEDPLFLO item in EOIAFJNEOKL)
		{
			if (item is ONFFKNJJJGC oNFFKNJJJGC && oNFFKNJJJGC.ALJHDJKAOHE())
			{
				return oNFFKNJJJGC.ACLDFHJKBLI;
			}
		}
