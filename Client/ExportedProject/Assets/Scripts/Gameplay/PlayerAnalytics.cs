...
public class PlayerAnalytics : DatabaseSerializedObjectGeneric<PlayerAnalytics.PlayerAnalyticsData>
{
	public class PlayerAnalyticsData
	{
		...
		[FormerlySerializedAs("counterGDPR")]
	...
	public static PlayerAnalytics instance
	{
		get
		{
			mInstance = mInstance ?? ((PlayerAnalytics)SingletonSupport.FindOrCreate(typeof(PlayerAnalytics)));
			return mInstance;
		}
	}
	...
	public bool showMoneyPack => data != null && data.isMoneyPackAvailable && !data.IsPackBought(NGNPIOOAHEH.Money);

	public bool showStarterPack => data != null && data.isStarterPackAvailable && !data.IsPackBought(NGNPIOOAHEH.Starter);
