...
public class DestroyableObject : Core_BaseScript, KMHCMCBGFFC
{
	...
	private ObscuredFloat _003CNBKAGGBFMBE_003Ek__BackingField;
	...
	[FormerlySerializedAs("ELCKDHAAJEB")]
	public Transform HIPAPKDJBNL;

	[FormerlySerializedAs("NPHBCDIMINF")]
	public bool JEGFCLCJMLA;

	[FormerlySerializedAs("BGIOBKDLNLD")]
	public Vector3 MKOGHDKDFIN;

	[FormerlySerializedAs("LBMIOFEGECG")]
	public bool LMEKEPANAJI;

	[FormerlySerializedAs("IFBCJNAFOMI")]
	public bool NBNDIIENBLK = true;

	[FormerlySerializedAs("PIDMKHKLOEN")]
	public bool AEJBIHBEOCP;

	[FormerlySerializedAs("GODCPFOGPMA")]
	public float HHOCGCLBHKB = 1f;

	[FormerlySerializedAs("KPJCPMKFBAI")]
	...
	[FormerlySerializedAs("OLOMLIKGODG")]
	...
	private ObscuredFloat LEMMBHKKJEO
	{
		[CompilerGenerated]
		get
		{
		...
		set
		{
			_003CNBKAGGBFMBE_003Ek__BackingField = value;
		}
	}
	...
	public virtual KMHCMCBGFFC owner
	{
		[CompilerGenerated]
		get
		{
	...
	protected virtual bool HEFCGCLCANF => true;

	public Vector3 velocity => FDLBJLNOBLG / Time.deltaTime;
	...
	public bool isDead => IHIEMNMKCEE;

	private bool PFMKNMNBJKA => DebugSettings.debugEnabled;
	...
	{
		PGKPEONGOID(MNAJKODCACI, FJLBLLLEELD, AKGDPKJCILO, FMAOPCINBDA, 0, Vector3.zero, Vector3.zero, MBAGBOIGODP: false);
	}
	...
	{
		PGKPEONGOID(MNAJKODCACI, FJLBLLLEELD, AKGDPKJCILO, FMAOPCINBDA, MEFOHLAIBFA, MDOPFNNFLFB, LPGMLOCLBGP, MBAGBOIGODP: true);
	}
	...
	{
		PGKPEONGOID(MNAJKODCACI, FJLBLLLEELD, AKGDPKJCILO, FMAOPCINBDA, MEFOHLAIBFA, MDOPFNNFLFB, Vector3.zero, MBAGBOIGODP: false);
	}
	...
	protected void LLGOAKNKNJK(CJLNCMMNPBM KMJKKNFDFMM)
	{
		...
		if (health > 0f)
		{
			if (MHFIGDEMOCJ.FJLBLLLEELD == MANAGOLPMLH.Shot)
			{
				...
				if (PFMKNMNBJKA)
				{
					UnityEngine.Debug.Log($"DamageEventPlayerShoot damage: {KMJKKNFDFMM.EPDELOGPNAC} to {owner} new HP {health}");
				}
			}
			...
			else
			{
				...
				if (PFMKNMNBJKA)
				{
					UnityEngine.Debug.Log($"DamageEventPlayer damage: {KMJKKNFDFMM.EPDELOGPNAC} to {owner} new HP {health}");
				}
			}
		}
		else if (MHFIGDEMOCJ.FJLBLLLEELD == MANAGOLPMLH.Shot)
		{
			...
			if (PFMKNMNBJKA)
			{
				UnityEngine.Debug.Log($"DeathEventPlayerShoot damage: {KMJKKNFDFMM.EPDELOGPNAC} to {owner} new HP {health}");
			}
		}
		else if (GLLFFIHFCEF == byte.MaxValue)
		{
		...
		else
		{
			...
			if (PFMKNMNBJKA)
			{
				UnityEngine.Debug.Log($"DeathEventPlayer damage: {KMJKKNFDFMM.EPDELOGPNAC} to {owner} new HP {health}");
			}
		}
	}
	...
	{
		JOPGOFCDHIO(MNAJKODCACI, FJLBLLLEELD, FFLOJLDIHGI, 0, Vector3.zero, Vector3.zero, MBAGBOIGODP: false);
	}
	...
	{
		JOPGOFCDHIO(MNAJKODCACI, FJLBLLLEELD, FFLOJLDIHGI, MEFOHLAIBFA, MPHCNMDIPAI, LPGMLOCLBGP, MBAGBOIGODP: true);
	}
	...
	{
		PGKPEONGOID(MNAJKODCACI, FJLBLLLEELD, AKGDPKJCILO, FMAOPCINBDA, MEFOHLAIBFA, MDOPFNNFLFB, LPGMLOCLBGP, MBAGBOIGODP: false);
	}
	...
	{
		JOPGOFCDHIO(MNAJKODCACI, FJLBLLLEELD, FFLOJLDIHGI, 0, Vector3.zero, LPGMLOCLBGP, MBAGBOIGODP: true);
	}
	...
	{
		PGKPEONGOID(MNAJKODCACI, FJLBLLLEELD, AKGDPKJCILO, FMAOPCINBDA, 0, Vector3.zero, Vector3.zero, MBAGBOIGODP: true);
	}
	...
	{
		JOPGOFCDHIO(MNAJKODCACI, FJLBLLLEELD, FFLOJLDIHGI, MEFOHLAIBFA, MPHCNMDIPAI, Vector3.zero, MBAGBOIGODP: false);
	}
	...
	{
		PGKPEONGOID(MNAJKODCACI, FJLBLLLEELD, AKGDPKJCILO, FMAOPCINBDA, 0, Vector3.zero, LPGMLOCLBGP, MBAGBOIGODP: true);
	}
	...
	protected void KKOFHGIAJNE(CJLNCMMNPBM KMJKKNFDFMM, bool AEIKFAIKDGI)
	{
		...
		if (health <= 0f)
		{
			...
			if (!AEIKFAIKDGI)
			{
				UnityEngine.Debug.LogError($"NetworkDamage damage Error new HP is {health} and {owner} should NOT be DEAD, damage: {KMJKKNFDFMM.EPDELOGPNAC}, damage owner {KMJKKNFDFMM.HNFNINFCBEJ}");
			}
		}
		else if (AEIKFAIKDGI)
		{
			...
			if (health > 0f)
			{
				UnityEngine.Debug.LogError($"NetworkDamage damage Error new HP is {health} and {owner} should BE DEAD, damage: {KMJKKNFDFMM.EPDELOGPNAC}, damage owner {KMJKKNFDFMM.HNFNINFCBEJ}");
			}
		}
	...
	{
		JOPGOFCDHIO(MNAJKODCACI, FJLBLLLEELD, FFLOJLDIHGI, 0, Vector3.zero, Vector3.zero, MBAGBOIGODP: true);
	}
