...
public class DeathMatchUnitsGuiElement : GuiElement
{
	private sealed class BLGLIAGMEDE
	{
		...
		internal void LKKLABBEOAG(UITweener MGDJMGHCAAI)
		{
			...
			TweenPosition.Begin(GMPADNOGGNL.gameObject, 1168f * BJGCPDNMHDH.HAMKIKMJHDP, IMIJCJGPIAC, NDOMCAEINKF, useLocal: false).onFinished = BLEMBJFNOGA;
		}
		...
		internal void BDCAFHOHEGM(UITweener MGDJMGHCAAI)
		{
			...
			TweenPosition.Begin(GMPADNOGGNL.gameObject, 666f * BJGCPDNMHDH.HAMKIKMJHDP, IMIJCJGPIAC, NDOMCAEINKF, useLocal: false).onFinished = HONCMBHEKDP;
		}
		...
		internal void CFJHDIHLIML(UITweener MGDJMGHCAAI)
		{
			...
			TweenPosition.Begin(GMPADNOGGNL.gameObject, 1069f * BJGCPDNMHDH.HAMKIKMJHDP, IMIJCJGPIAC, NDOMCAEINKF, useLocal: false).onFinished = BLEMBJFNOGA;
		}
		...
		internal void INFLOOLPOBK(UITweener MGDJMGHCAAI)
		{
			...
			TweenPosition.Begin(GMPADNOGGNL.gameObject, 1961f * BJGCPDNMHDH.HAMKIKMJHDP, IMIJCJGPIAC, NDOMCAEINKF, useLocal: false).onFinished = BACBENPDILB;
		}
		...
		internal void NNFHNIMCBLH(UITweener MGDJMGHCAAI)
		{
			...
			TweenPosition.Begin(GMPADNOGGNL.gameObject, 1959f * BJGCPDNMHDH.HAMKIKMJHDP, IMIJCJGPIAC, NDOMCAEINKF, useLocal: false).onFinished = HJACFCPOPME;
		}
	...
	[CompilerGenerated]
	private static DeathMatchUnitsGuiElement _003CNLEFNAELOLN_003Ek__BackingField;
	...
	[FormerlySerializedAs("LGKPOBDIBPD")]
	public UILabel MPLFAOFAEDJ;

	[FormerlySerializedAs("BKBEKDHNJKK")]
	[Header("Auto Deploy")]
	public GameObject FAJBFGGJEFJ;

	[FormerlySerializedAs("HDEBIMPNBOP")]
	public GameObject EAIHOMFLPEJ;

	[FormerlySerializedAs("HGHGDLDFLCK")]
	public UISprite AFOIMHFICOA;

	[FormerlySerializedAs("JAEKJHOHNML")]
	...
	public static DeathMatchUnitsGuiElement instance
	{
		[CompilerGenerated]
		get
		{
		...
		protected set
		{
			_003CNLEFNAELOLN_003Ek__BackingField = value;
		}
	}

	protected virtual float PCPCPEFOOEJ => 0.25f;

	protected virtual bool BDKNNNCHILL => Singleton<GameController>.instance.mainController.spawningType == BNKNNCBAPIH.Classic;
	...
	{
		CNMDFMGELLH.SetActive(value: true);
	}
	...
	{
		CNMDFMGELLH.SetActive(value: false);
	}
	...
	{
		ShowHintText(NABOFKMBMKH: false);
		base.Awake();
		JDDIPOLFBDD = FAJBFGGJEFJ.transform.localPosition;
		BANNHIPBNDB = EAIHOMFLPEJ.transform.localPosition;
	}
	...
	{
		ShowHintText(NABOFKMBMKH: false, FMOIFJNOIKA: false);
		ACPHOCKEOII();
		JDDIPOLFBDD = FAJBFGGJEFJ.transform.localPosition;
		BANNHIPBNDB = EAIHOMFLPEJ.transform.localPosition;
	}
	...
	public void AnimateDeployEnergy(UISprite GMPADNOGGNL, Vector3 IMIJCJGPIAC, Vector3 NDOMCAEINKF, bool BEINCBHAGKC, float KALLNCAFIMP)
	{
		...
		bLGLIAGMEDE.GMPADNOGGNL.gameObject.SetActive(value: true);
	...
	public void ForceUpdatePower()
	{
		foreach (DeathMatchGuiCard item in INIKAAAIPCK)
		{
			item.PCKJMMKNDMJ.DoUpdate(FHKKPEIDDJN: true);
		}
	}
	...
	public void SendUnit(DeathMatchGuiCard CEKHIJECPNP, SpawningManagerDeathMatch.ArmyUnitDefinition OIJBDFANMCC, bool GCGOGMDOBED = false)
	{
		...
		Singleton<SpawningManagerDeathMatch>.instance.SendUnit(OIJBDFANMCC, PlayerController.OGMBJPKOPCB.fraction, EEAMBOACAGF: true, GCGOGMDOBED);
		PPKGFHAELEP();
		if (Singleton<SpawningManagerDeathMatch>.instance.autoDeployEnabled || Singleton<SpawningManagerDeathMatch>.instance.semiAutoDeployEnabled)
		{
			TweenPosition.Begin(EAIHOMFLPEJ, 0.5f, BANNHIPBNDB - Vector3.up * 450f).delay = 0f;
	...
	private void AAMJFPKPFEP()
	{
		...
		if (OCNICHHIGGH)
		{
			for (int i = 0; i < DGELKCABCEC.Count; i++)
			{
				...
				INIKAAAIPCK[i].Show(oIJBDFANMCC, PDPDGCNHCFN: true);
			}
			OCNICHHIGGH = false;
	...
	private void OJNCKCEHPBK()
	{
		...
		if (HKIIDANPPOP())
		{
			...
			OCNICHHIGGH = false;
			JADNIAFKNIF(NABOFKMBMKH: true, FMOIFJNOIKA: false);
			TweenPosition.Begin(FAJBFGGJEFJ, 1715f, JDDIPOLFBDD - Vector3.up * 1035f, useLocal: false).delay = 965f;
	...
	public void GJDOHINBPPA(DeathMatchGuiCard CEKHIJECPNP, SpawningManagerDeathMatch.ArmyUnitDefinition OIJBDFANMCC, bool GCGOGMDOBED = false)
	{
		...
		Singleton<SpawningManagerDeathMatch>.instance.KNFDNEMOOOC(OIJBDFANMCC, PlayerController.OGMBJPKOPCB.fraction, EEAMBOACAGF: false, GCGOGMDOBED);
		PPKGFHAELEP();
