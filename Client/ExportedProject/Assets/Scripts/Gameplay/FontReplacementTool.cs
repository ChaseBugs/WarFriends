...
public class FontReplacementTool : MonoBehaviour
{
	...
	[FormerlySerializedAs("MIDPANAHFCA")]
	public bool KEPHIOADKLN;

	[FormerlySerializedAs("FAPNDFOANOH")]
	public bool LFPLKPPPDBL;

	[FormerlySerializedAs("POFDFCACDDA")]
	...
	private void BLNBLGLCPKE()
	{
		...
		string text3 = " (#";
		UILabel[] componentsInChildren = base.gameObject.GetComponentsInChildren<UILabel>(includeInactive: false);
		foreach (UILabel uILabel in componentsInChildren)
		{
			UITable[] componentsInParent = uILabel.gameObject.GetComponentsInParent<UITable>(includeInactive: false);
			bool flag = componentsInParent.Length > 0;
			if (uILabel.font == JNOFHLBAAPP)
			{
				continue;
	...
	private string IIIHAFHMHGP(UILabel FKLKLCBOMNE)
	{
		...
		case UIWidget.Pivot.Right:
		{
			...
			return $"Label \"{FKLKLCBOMNE.gameObject.name}\" with pivot \"{FKLKLCBOMNE.pivot}\" and old font is under table. New size {num2} and yDown {num5}.";
		}
		...
		case UIWidget.Pivot.TopRight:
		{
			...
			return $"Label \"{FKLKLCBOMNE.gameObject.name}\" with pivot \"{FKLKLCBOMNE.pivot}\" and old font is under table. New size {num2} and yDown {num4}.";
		}
		...
		case UIWidget.Pivot.BottomRight:
		{
			...
			return $"Label \"{FKLKLCBOMNE.gameObject.name}\" with pivot \"{FKLKLCBOMNE.pivot}\" and old font is under table. New size {num2} and yDown {num3}.";
		}
		default:
			return null;
		}
	...
	private void FONHHKFDAAM()
	{
		...
		string[] array = new string[WarFriends.Legacy.DecompilerCompatibility.RuntimeArrayLength(-103)];
	...
	private string LMBOCLPPIBI(UILabel FKLKLCBOMNE)
	{
		...
		return $"Changing label \"{FKLKLCBOMNE.gameObject.name}\" with pivot \"{FKLKLCBOMNE.pivot}\". New size {num2} and yDown {num3}.";
	}
