...
public class SoldierAnimationController : Core_BaseScript, TimeScaleIgnorable
{
	...
	public float uncoverLength => LOFOBBMNNCD["player_look_right3"].length;

	public float swatShieldUncoverLength => LOFOBBMNNCD["shield_unhide"].length;
	...
	public bool IsShooting => LOFOBBMNNCD.IsPlaying(CECIFFDFMGB) || LOFOBBMNNCD.IsPlaying(ONKPFEDKFOJ);

	public bool isHiding => LOFOBBMNNCD.IsPlaying(DDPCENEDLPG) || LOFOBBMNNCD.IsPlaying(BPNFIAPPJCH);
	...
	public float mortarBuildTime => LOFOBBMNNCD["mortar_down"].length;

	public float mortarShotTime => LOFOBBMNNCD["mortar_hide"].length;

	public float mortarStandTime => LOFOBBMNNCD["mortar_up"].length;
	...
	public void ShootStandNew(Weapon.IGGFMJAKLGO FJLBLLLEELD, float IKPDANHGAPG, float GINCPOHPFBN)
	{
		...
		string text = KBDDHFLPJAB(FJLBLLLEELD, 0);
		string text2 = KBDDHFLPJAB(FJLBLLLEELD, 1);
		string text3 = KBDDHFLPJAB(FJLBLLLEELD, 2);
		...
		if (!LOFOBBMNNCD.IsPlaying(text))
		{
			...
			{
				LOFOBBMNNCD[text2].normalizedTime = 0f;
				LOFOBBMNNCD[text2].wrapMode = WrapMode.ClampForever;
				LOFOBBMNNCD.Play(text2);
				return;
			}
			float num = ((!LOFOBBMNNCD.IsPlaying(text3)) ? 1f : Mathf.Clamp01(LOFOBBMNNCD[text3].normalizedTime));
			LOFOBBMNNCD[text].normalizedTime = 1f - num;
			LOFOBBMNNCD.CrossFade(text, 0.05f);
			LOFOBBMNNCD[text2].wrapMode = WrapMode.ClampForever;
			AnimationState animationState = LOFOBBMNNCD.PlayQueued(text2);
	...
	public void ShootFromCrawl(float GINCPOHPFBN, float IKPDANHGAPG, Weapon.IGGFMJAKLGO FJLBLLLEELD)
	{
		...
		string text = OHEPCGBBELI(FJLBLLLEELD, 0);
		string text2 = OHEPCGBBELI(FJLBLLLEELD, 1);
		string text3 = OHEPCGBBELI(FJLBLLLEELD, 2);
	...
	public void Idle(Weapon.IGGFMJAKLGO FJLBLLLEELD, bool IAFJJMFIPCG = false)
	{
		...
		if (IAFJJMFIPCG)
		{
			LOFOBBMNNCD.CrossFadeQueued(text);
		}
		else
		{
			LOFOBBMNNCD.CrossFade(text);
		}
	}
	...
	private void LateUpdate()
	{
		...
		OGCAPEEHKIM = Mathf.Lerp(OGCAPEEHKIM, GEDIPEPLEEK, TimeManager.GetTimeScaledInterval(Time.deltaTime, DLMKHPAJKDJ: true) * 12f);
	...
	protected void Update()
	{
		...
		if (AHBOPHDJFNL == GBNNHKHOHMC.Right)
		{
			if (Time.time > OGAEOOMPJME && OGAEOOMPJME != 0f)
			{
				...
				Idle(BHHEOMELGCH, IAFJJMFIPCG: true);
			}
		}
		...
		if (IAFAEFABPIL && !PLCAOODHLCA)
		{
			...
			NGCDDPCAGJB.localRotation = Quaternion.Slerp(NGCDDPCAGJB.localRotation, NDJBLEOGFJN, TimeManager.GetTimeScaledInterval(Time.deltaTime, DLMKHPAJKDJ: true) * 10f);
	...
	public void Reload(Weapon.IGGFMJAKLGO FJLBLLLEELD)
	{
		...
		Idle(FJLBLLLEELD, IAFJJMFIPCG: true);
	}
	...
	private void FMCJJOFHEPJ(GBNNHKHOHMC MBMIBDKDGAD, Weapon.IGGFMJAKLGO BEACDIILBEG, float JPNAINOGAMM)
	{
		...
		string text = DIJHFEEBLGJ(BEACDIILBEG, MBMIBDKDGAD, 0);
		string text2 = DIJHFEEBLGJ(BEACDIILBEG, MBMIBDKDGAD, 1);
		string text3 = DIJHFEEBLGJ(BEACDIILBEG, MBMIBDKDGAD, 2);
		LOFOBBMNNCD[text].speed = JPNAINOGAMM;
		LOFOBBMNNCD[text2].speed = 1f;
		if (LOFOBBMNNCD.IsPlaying(text))
		{
			return;
		}
		...
		if (LOFOBBMNNCD.IsPlaying(text3))
		{
			LOFOBBMNNCD[text].normalizedTime = 1f - LOFOBBMNNCD[text3].normalizedTime;
			LOFOBBMNNCD.Play(text);
			LOFOBBMNNCD.PlayQueued(text2);
			return;
		}
		LOFOBBMNNCD.CrossFade(text, 0.02f);
		AnimationState animationState = LOFOBBMNNCD.PlayQueued(text2);
		if (AHCLADFDIMI)
		{
	...
	public void ShootStand(Weapon.IGGFMJAKLGO BEACDIILBEG)
	{
		float JPNAINOGAMM;
		string text = NPMNOCAJPFI(BEACDIILBEG, out JPNAINOGAMM);
		AnimationState animationState = LOFOBBMNNCD[text];
		...
		if (LOFOBBMNNCD.IsPlaying(text))
		{
			...
			LOFOBBMNNCD.Play(text);
			Idle(BEACDIILBEG, IAFJJMFIPCG: true);
		}
		else
		{
			LOFOBBMNNCD.CrossFade(text, 0.1f);
			Idle(BEACDIILBEG, IAFJJMFIPCG: true);
			KLLFLALOKOP = 0f;
		}
	}
