...
public class GrenadeLevelsSetup : WeaponLevelsSetup
{
	...
	[FormerlySerializedAs("MMDDPBKLNGL")]
	...
	public virtual List<Tuple<string, float[]>> BPDIHMEADHA(UpgradeSlot ACHJJKCIBCN)
	{
		...
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "setEmailCollection", "S", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
	...
	{
		List<Tuple<string, float[]>> list = new List<Tuple<string, float[]>>();
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "CurrentBundleVersion.GetModifiedVersion - version is '{0}'", "PNManager: registration failed: ", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
	...
	{
		List<Tuple<string, float[]>> list = new List<Tuple<string, float[]>>();
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "D3", "WarFriends cannot function without downloading these additional data. Do you really wish to QUIT or would you like to download the data?", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
	...
	{
		List<Tuple<string, float[]>> list = new List<Tuple<string, float[]>>();
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "Wrong_Category", "Time-limited Offers", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "2 RATE APP REMINDER\tLast game won: True\tVersion: {0}\tShown Times in this version: {1}/{2}\tTotal ranked wins: {3}?={4}\tIs level up: {5}\tLevel: {6}?={7}/{8}\tAfter update: {9}", "PlayerLeagueFinished - FORMER LEAGUE ID = {0}, current = {1}", base.weaponFeature.ammoCoef, CLFOOLKGCEO: true));
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "Added", "ID_RELOG_TO_FB_DIALOG_TITLE", base.weaponFeature.rateOfFireCoef, CLFOOLKGCEO: true));
		return list;
	}
	...
	{
		List<Tuple<string, float[]>> list = new List<Tuple<string, float[]>>();
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "menu-info-ico", "BlackMarketOffer", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "getGameConfigurationKeys", "ID_YOUNEEDMONEYTOUPGRADE", base.weaponFeature.ammoCoef));
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "WarBucks", "THANK YOU", base.weaponFeature.rateOfFireCoef, CLFOOLKGCEO: true));
		return list;
	}
	...
	public virtual List<Tuple<string, float[]>> DNPNJANKBKK(UpgradeSlot ACHJJKCIBCN)
	{
		...
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "GUiManager", "userId", base.weaponFeature.ammoCoef, CLFOOLKGCEO: true));
	...
	{
		List<Tuple<string, float[]>> list = new List<Tuple<string, float[]>>();
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "START PLAYING TUTORIAL", "Claim_Reward", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "Scripts Full Path: \"{0}\"\n", "Player_Waited_Till_Delivery_Ends", base.weaponFeature.ammoCoef, CLFOOLKGCEO: true));
	...
	{
		List<Tuple<string, float[]>> list = new List<Tuple<string, float[]>>();
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "TranslationIdFinder.cs", "Autodeploy_On", base.weaponFeature.attackCoef, CLFOOLKGCEO: true));
	...
	public virtual List<Tuple<string, float[]>> HDPMJGABCHI(UpgradeSlot ACHJJKCIBCN)
	{
		...
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "gold", "ID_WARNING_INCORRECTASSIGNMENTS_TEXT", base.weaponFeature.ammoCoef, CLFOOLKGCEO: true));
		list.Add(MMPLMBONDIJ(ACHJJKCIBCN, "RegisterEvent(", "(Lcom/google/android/gms/common/api/Api;)Lcom/google/android/gms/common/ConnectionResult;", base.weaponFeature.rateOfFireCoef, CLFOOLKGCEO: true));
		return list;
	}
