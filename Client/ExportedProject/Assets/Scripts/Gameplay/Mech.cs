...
public class Mech : MechanicalUnit<MechBehaviour>
{
	...
	private MechBehaviour NADNBCMGLBF => (MechBehaviour)ACLDFHJKBLI;
	...
	private void LAIGPKANOPC()
	{
		...
		mMechAnimator.LookAt(JDGADCDMFHM.transform.position, CAFJNJHFHMA: true);
	...
	private void EMPGAHCMPHP()
	{
		...
		{
			PickTarget(JJAEHBIOEHK: false, JJFKFKAEOOD: false);
		...
		{
			if (PickTarget(JJAEHBIOEHK: true, JJFKFKAEOOD: false))
			{
				OGGHKAKGMFE = (LLEHEFACLDA)6;
	...
	private void KPKKLHGBJLL()
	{
		...
		mMechAnimator.LookAt(base.transform.position + base.transform.forward, CAFJNJHFHMA: true);
	...
	private void AANLNCLMLGF()
	{
		...
		{
			PickTarget(JJAEHBIOEHK: true, JJFKFKAEOOD: false);
		...
		if (Time.time > HJEJNIJPFHF && base.isInField)
		{
			if (PickTarget(JJAEHBIOEHK: true, JJFKFKAEOOD: true))
			{
				OGGHKAKGMFE = LLEHEFACLDA.Strafe;
	...
	public bool PickTarget(bool JJAEHBIOEHK, bool JJFKFKAEOOD)
	{
		...
		if (JJAEHBIOEHK && flag)
		{
			mMechAnimator.LookAt(JDGADCDMFHM.transform.position, CAFJNJHFHMA: false);
		}
		return flag;
	}
	...
	{
		mMechAnimator.Walk();
		mMechAnimator.LookAt(base.transform.position + base.transform.forward, CAFJNJHFHMA: false);
		HJEJNIJPFHF = Time.time + UnityEngine.Random.Range(NADNBCMGLBF.mechDefinition.minShootTime, NADNBCMGLBF.mechDefinition.maxShootTime);
