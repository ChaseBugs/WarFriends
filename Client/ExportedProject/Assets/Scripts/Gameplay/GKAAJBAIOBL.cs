...
public class GKAAJBAIOBL
{
	...
	// ILSpy emitted the default member as a property while dropping its parameter.
	// Restore the recovered dictionary-backed indexer so every spawn-budget lookup uses
	// the requested unit category instead of an undefined decompiler placeholder.
	public int this[GHPGNELIDBM DFHAAIFFLOE] => EKONOCJNCJD[DFHAAIFFLOE] - LKJAMBIICFA[DFHAAIFFLOE];

	// Obfuscated callers reference the original getter metadata name directly.
	public int LGJKIBHFGAM(GHPGNELIDBM category)
	{
	...
	public virtual string NLAKJODBJOB()
	{
		...
		{
			string text2 = text;
			object[] array = new object[WarFriends.Legacy.DecompilerCompatibility.RuntimeArrayLength(-1)];
			array[0] = text2;
			array[1] = item.Key;
	...
	public virtual string MKCKMHOMECK()
	{
		...
		{
			string text2 = text;
			object[] array = new object[WarFriends.Legacy.DecompilerCompatibility.RuntimeArrayLength(-32)];
			array[1] = text2;
			array[1] = item.Key;
	...
	public virtual string MBHMDIIIIHA()
	{
		...
		{
			string text2 = text;
			object[] array = new object[WarFriends.Legacy.DecompilerCompatibility.RuntimeArrayLength(-95)];
			array[0] = text2;
			array[0] = item.Key;
	...
	public virtual string NJOBOGIBCFL()
	{
		...
		{
			string text2 = text;
			object[] array = new object[WarFriends.Legacy.DecompilerCompatibility.RuntimeArrayLength(-34)];
			array[0] = text2;
			array[1] = item.Key;
