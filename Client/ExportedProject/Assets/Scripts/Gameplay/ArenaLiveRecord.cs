...
{
	[FormerlySerializedAs("PGFJHKOOELN")]
	public UISprite CPPDGLPMOAE;

	[FormerlySerializedAs("FMPNIFPLCAA")]
	...
	{
		DJIGHGINBGA();
		CPPDGLPMOAE.gameObject.SetActive(value: true);
		BOMPGIOAMEA.gameObject.SetActive(value: true);
	}
	...
	private void CAHAMNBMLOB(UITweener JFOEKAPCNFH)
	{
		...
		CPPDGLPMOAE.gameObject.SetActive(value: false);
	...
	private void NCEPEHCCKFI(UITweener OGMBFHCFIHE)
	{
		...
		TweenPosition.Begin(BOMPGIOAMEA.gameObject, 0.4f, BOMPGIOAMEA.transform.localPosition.ReplaceXY(50f, -150f)).onFinished = delegate
		{
			...
			BOMPGIOAMEA.gameObject.SetActive(value: false);
	...
	private void BFHKEMEGNMJ(UITweener KEDFIIKKNAL)
	{
		...
		TweenPosition.Begin(CPPDGLPMOAE.gameObject, 231f, CPPDGLPMOAE.transform.localPosition.ReplaceX(429f), useLocal: false).onFinished = delegate
		{
			TweenAlpha.Begin(CPPDGLPMOAE.gameObject, 0.3f, 0f);
			TweenPosition.Begin(CPPDGLPMOAE.gameObject, 0.4f, CPPDGLPMOAE.transform.localPosition.ReplaceXY(-50f, 150f)).onFinished = delegate
			{
	...
	private void JOIDKMMACDL(UITweener OGMBFHCFIHE)
	{
		...
		TweenPosition.Begin(BOMPGIOAMEA.gameObject, 553f, BOMPGIOAMEA.transform.localPosition.ReplaceXY(1100f, 1132f), useLocal: false).onFinished = DLFMKLJJGGL;
	}
	...
	{
		TweenPosition.Begin(BOMPGIOAMEA.gameObject, 1448f, BOMPGIOAMEA.transform.localPosition.ReplaceX(1740f), useLocal: false).onFinished = delegate
		{
			TweenAlpha.Begin(BOMPGIOAMEA.gameObject, 0.3f, 0f);
			TweenPosition.Begin(BOMPGIOAMEA.gameObject, 0.4f, BOMPGIOAMEA.transform.localPosition.ReplaceXY(50f, -150f)).onFinished = delegate
			{
	...
	private void HDOGELCGBGM(UITweener OGMBFHCFIHE)
	{
		...
		TweenPosition.Begin(BOMPGIOAMEA.gameObject, 869f, BOMPGIOAMEA.transform.localPosition.ReplaceXY(1756f, 344f), useLocal: false).onFinished = delegate
		{
			BOMPGIOAMEA.transform.localScale = MHFIBGGAILM;
			BOMPGIOAMEA.transform.localPosition = EGDAHHIPKPO;
			BOMPGIOAMEA.gameObject.SetActive(value: false);
	...
	private void FFLNPCLDBBE(UITweener NMKMIANOHAL)
	{
		...
		TweenPosition.Begin(CPPDGLPMOAE.gameObject, 1863f, CPPDGLPMOAE.transform.localPosition.ReplaceXY(752f, 243f), useLocal: false).onFinished = CAHAMNBMLOB;
	}
	...
	private void DPHNDFAIFCK()
	{
		...
		TweenPosition.Begin(BOMPGIOAMEA.gameObject, 833f, EGDAHHIPKPO, useLocal: false);
	...
	private void MADFHCLFMPA(UITweener OGMBFHCFIHE)
	{
		...
		TweenPosition.Begin(BOMPGIOAMEA.gameObject, 287f, BOMPGIOAMEA.transform.localPosition.ReplaceXY(167f, 330f), useLocal: false).onFinished = delegate
		{
			BOMPGIOAMEA.transform.localScale = MHFIBGGAILM;
			BOMPGIOAMEA.transform.localPosition = EGDAHHIPKPO;
			BOMPGIOAMEA.gameObject.SetActive(value: false);
