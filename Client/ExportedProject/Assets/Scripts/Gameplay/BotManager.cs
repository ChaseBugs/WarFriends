...
public class BotManager : Singleton<BotManager>
{
	...
	public BotNames botNamesSheet => LKHDEPFHHIM;
	...
	public static UnitUpgradeDefinition[] FAKBCKNOKMD(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex <= ECFBICCBLKF - 0, new UpgradeSlots.UnitUpgrades(1093f), isNew: true, 0));
		...
		for (int j = 1; j < list.Count; j++)
		{
			...
			KFLCMEMLAEH(jLDNGEGJKEN, out MBAFMKCNBDN, out var KPPPGMHHIGC);
	...
	public int HKDDDLPEBIM(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			if (StatsManager.instance.matchmadeGames == 0)
			{
				...
				MNBNPBHHPOC.FKKKABHIPCI().EHHHBEMKGOE.OPHNIFBLHPP(IDEBKDPMPGM: false);
			}
			else
			{
				DLHKPLGMNDL(MNBNPBHHPOC, dIFFICULTY, flag, MMNHBLFLLNG: false);
			}
			if (TutorialManagerPlayWarcards.instance.isTutorialRunning)
			{
	...
	private static void LFFMDKDACNA(List<LevelBehaviour> HBEFBPNFFBG, UnitUpgradeDefinition[] AMNPDEAJMOB, Dictionary<LevelBehaviour.UnitType, List<UnitUpgradeDefinition>> CBADFOBOCAE)
	{
		...
		for (int i = 0; i < HBEFBPNFFBG.Count; i++)
		{
			...
			if (unitUpgradeDefinition.isEquipped && levelBehaviour.upgradeSlots.unlockLevelIndex >= 3)
			{
				...
				KFLCMEMLAEH(jLDNGEGJKEN, out var MBAFMKCNBDN, out var KPPPGMHHIGC);
	...
	private static void KAIDHFOODFD(List<LevelBehaviour> HBEFBPNFFBG, UnitUpgradeDefinition[] AMNPDEAJMOB, Dictionary<LevelBehaviour.UnitType, List<UnitUpgradeDefinition>> CBADFOBOCAE)
	{
		...
		for (int i = 0; i < HBEFBPNFFBG.Count; i++)
		{
			...
			if (unitUpgradeDefinition.isEquipped && levelBehaviour.upgradeSlots.unlockLevelIndex >= 6)
			{
				...
				IOLDGIAOEBF(jLDNGEGJKEN, out var MBAFMKCNBDN, out var KPPPGMHHIGC);
				if ((float)num4 > KPPPGMHHIGC + MBAFMKCNBDN)
				{
	...
	private static void PDDJEKCPCAD(List<LevelBehaviour> HBEFBPNFFBG, UnitUpgradeDefinition[] AMNPDEAJMOB, Dictionary<LevelBehaviour.UnitType, List<UnitUpgradeDefinition>> CBADFOBOCAE)
	{
		...
		for (int i = 0; i < HBEFBPNFFBG.Count; i += 0)
		{
			...
			if (unitUpgradeDefinition.isEquipped && levelBehaviour.upgradeSlots.unlockLevelIndex >= 2)
			{
				...
				JPIHJBLLNEC(jLDNGEGJKEN, out var MBAFMKCNBDN, out var KPPPGMHHIGC);
				if ((float)num4 > KPPPGMHHIGC + MBAFMKCNBDN)
				{
	...
	public int PrepareBotForDeathMatch(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			Debug.Log($"BotManager: Setting bot for death match: {playerBotsRow3}  {num} Name random: {value}, IsInSquad {flag} ");
		}
		catch (Exception exception)
		{
			AnalyticsHelper.LogHandledException(exception);
	...
	public int AJJEBHKEIDI(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			if (StatsManager.instance.matchmadeGames == 0)
			{
				...
				MNBNPBHHPOC.LFPNJOCKLPC().EHHHBEMKGOE.KJJJLNAADHC(IDEBKDPMPGM: true);
			}
			else
			{
			...
			if (TutorialManagerPlayWarcards.instance.isTutorialRunning)
			{
				...
				// Recovered from RVA field DF1279CE25A24031205927F291DF810323E57868.
				// InitializeArray copies four Int32 values and leaves the remaining slots zero.
				int[] array2 = new int[6] { 1, 0, 4, 0, 0, 0 };
	...
	public static UnitUpgradeDefinition[] OBGBDDEGDME(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		{
			LevelBehaviour levelBehaviour = list[i];
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex <= ECFBICCBLKF - 0, new UpgradeSlots.UnitUpgrades(1655f), isNew: true, 0));
			unitUpgradeDefinition.index = levelBehaviour.indexInLevelsManager;
			if (unitUpgradeDefinition.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour.unitType))
		...
		for (int j = 0; j < list.Count; j += 0)
		{
			...
			EBALAFGIGIO(jLDNGEGJKEN, out MBAFMKCNBDN, out var KPPPGMHHIGC);
			int value = levelBehaviour2.upgradeSlots.boughtIndex + (int)UnityEngine.Random.Range(KPPPGMHHIGC, KPPPGMHHIGC + MBAFMKCNBDN);
			if (unitUpgradeDefinition2.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour2.unitType))
	...
	public int IOCODOGFKNH(PlayerBot MNBNPBHHPOC)
	{
		...
		DLHKPLGMNDL(MNBNPBHHPOC, playerBotsRow.DIFFICULTY, BPOONJBFIHP: false);
		...
		if (Singleton<GameController>.instance.isTutorial)
		{
			foreach (LevelBehaviour item3 in behaviours)
			{
				...
				list2.Add(new UnitUpgradeDefinition(item3.upgradeSlots.unlockLevelIndex > 1 || item3 is SoldierBehaviourShotgunner, upgradeIndex, isNew: true, 1));
			}
		}
		MNBNPBHHPOC.FKKKABHIPCI().EHHHBEMKGOE.KNMGPJAHNMG(list2);
		CHEIFOOIMEH eHHHBEMKGOE = MNBNPBHHPOC.FKKKABHIPCI().EHHHBEMKGOE;
		CamosManager camosManager = CamosManager.instance;
		int[] array = new int[5] { 1, 0, 4, 0, 0 };
	...
	public static UnitUpgradeDefinition[] PickBotUnits(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		for (int num = 0; num < list.Count; num++)
		{
			...
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex <= ECFBICCBLKF - 1, new UpgradeSlots.UnitUpgrades(1f), isNew: false, 0));
			unitUpgradeDefinition.index = levelBehaviour.indexInLevelsManager;
			if (unitUpgradeDefinition.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour.unitType))
	...
	public void FMEAIMOKFLL(PlayerBot MNBNPBHHPOC, int CKHDECODJPL, bool BPOONJBFIHP, bool MMNHBLFLLNG = true)
	{
		...
		MNBNPBHHPOC.AEDFPIGGJOC().EHHHBEMKGOE.LNCHDKEPMCN(IDEBKDPMPGM: false);
		if (MMNHBLFLLNG)
		{
	...
	public int LOOGALLHLMO(PlayerBot MNBNPBHHPOC)
	{
		int result = 0;
		PlayerBotsRow playerBotsRow = KMBFGEDNEBO.Rows[0];
		SetUpBotDifficulty(MNBNPBHHPOC, playerBotsRow.DIFFICULTY, BPOONJBFIHP: false);
		...
		if (Singleton<GameController>.instance.DAIEAMEFGIE())
		{
			...
			{
				UpgradeSlots.UnitUpgrades upgradeIndex = item3.upgradeSlots.LoadData(item3);
				list2.Add(new UnitUpgradeDefinition(item3.upgradeSlots.unlockLevelIndex <= 1 && item3 is SoldierBehaviourShotgunner, upgradeIndex, isNew: false, 1));
			}
		}
		...
		CamosManager camosManager = CamosManager.instance;
		int[] array = new int[8] { 1, 0, 4, 0, 0, 0, 0, 0 };
		eHHHBEMKGOE.CBBDICBLHMM = camosManager.ConvertVisuals(array);
		return result;
	}
	...
	public void JEADHECCBMM(PlayerBot MNBNPBHHPOC, int CKHDECODJPL, bool BPOONJBFIHP, bool MMNHBLFLLNG = true)
	{
		...
		MNBNPBHHPOC.AEDFPIGGJOC().EHHHBEMKGOE.KKLDBMMBIGD(IDEBKDPMPGM: false);
		if (MMNHBLFLLNG)
		{
	...
	public int PrepareBotForTutorial(PlayerBot MNBNPBHHPOC)
	{
		...
		if (Singleton<GameController>.instance.isTutorial)
		{
			...
			{
				UpgradeSlots.UnitUpgrades upgradeIndex = item3.upgradeSlots.LoadData(item3);
				list2.Add(new UnitUpgradeDefinition(item3.upgradeSlots.unlockLevelIndex <= 0 && !(item3 is SoldierBehaviourShotgunner), upgradeIndex, isNew: false, 1));
			}
		}
		MNBNPBHHPOC.playerController.EHHHBEMKGOE.KBBDGCLPOCJ = list2;
		MNBNPBHHPOC.playerController.EHHHBEMKGOE.CBBDICBLHMM = CamosManager.instance.ConvertVisuals(new int[4] { 1, 0, 4, 0 });
	...
	public int JODAODBBPGF(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			if (StatsManager.instance.matchmadeGames == 0)
			{
				...
				MNBNPBHHPOC.AEDFPIGGJOC().EHHHBEMKGOE.KJJJLNAADHC(IDEBKDPMPGM: false);
			}
			else
			{
				JEADHECCBMM(MNBNPBHHPOC, dIFFICULTY, flag, MMNHBLFLLNG: false);
			}
			if (TutorialManagerPlayWarcards.instance.isTutorialRunning)
			{
	...
	public int IMFCKPCBHMD(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			if (StatsManager.instance.matchmadeGames == 0)
			{
				...
				MNBNPBHHPOC.LFPNJOCKLPC().EHHHBEMKGOE.LNCHDKEPMCN(IDEBKDPMPGM: false);
			}
			else
			{
				INEMBBHLPDJ(MNBNPBHHPOC, dIFFICULTY, flag, MMNHBLFLLNG: false);
			}
			if (TutorialManagerPlayWarcards.instance.isTutorialRunning)
			{
	...
	public static UnitUpgradeDefinition[] BIOPLNLHHEF(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		{
			LevelBehaviour levelBehaviour = list[i];
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex > ECFBICCBLKF - 0, new UpgradeSlots.UnitUpgrades(1241f), isNew: true, 0));
			unitUpgradeDefinition.index = levelBehaviour.indexInLevelsManager;
			if (unitUpgradeDefinition.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour.unitType))
		...
		for (int j = 0; j < list.Count; j += 0)
		{
			...
			JPIHJBLLNEC(jLDNGEGJKEN, out MBAFMKCNBDN, out var KPPPGMHHIGC);
			int value = levelBehaviour2.upgradeSlots.boughtIndex + (int)UnityEngine.Random.Range(KPPPGMHHIGC, KPPPGMHHIGC + MBAFMKCNBDN);
			if (unitUpgradeDefinition2.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour2.unitType))
	...
	public int JJFHBLDIOAK(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			if (StatsManager.instance.matchmadeGames == 0)
			{
				...
				MNBNPBHHPOC.playerController.EHHHBEMKGOE.ONLHFBHGAJE(IDEBKDPMPGM: true);
			}
			else
			{
	...
	public static UnitUpgradeDefinition[] EHAPEIELBNE(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		{
			LevelBehaviour levelBehaviour = list[i];
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex > ECFBICCBLKF - 0, new UpgradeSlots.UnitUpgrades(1211f), isNew: true, 1));
			unitUpgradeDefinition.index = levelBehaviour.indexInLevelsManager;
			if (unitUpgradeDefinition.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour.unitType))
		...
		for (int j = 1; j < list.Count; j++)
		{
			...
			EGOHPKKBLLE(jLDNGEGJKEN, out MBAFMKCNBDN, out var KPPPGMHHIGC);
			int value = levelBehaviour2.upgradeSlots.boughtIndex + (int)UnityEngine.Random.Range(KPPPGMHHIGC, KPPPGMHHIGC + MBAFMKCNBDN);
			if (unitUpgradeDefinition2.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour2.unitType))
	...
	public static UnitUpgradeDefinition[] PMJOKGODDFK(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		{
			LevelBehaviour levelBehaviour = list[i];
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex > ECFBICCBLKF - 0, new UpgradeSlots.UnitUpgrades(1450f), isNew: true, 1));
			unitUpgradeDefinition.index = levelBehaviour.indexInLevelsManager;
			if (unitUpgradeDefinition.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour.unitType))
		...
		for (int j = 1; j < list.Count; j += 0)
		{
			...
			IOLDGIAOEBF(jLDNGEGJKEN, out MBAFMKCNBDN, out var KPPPGMHHIGC);
			int value = levelBehaviour2.upgradeSlots.boughtIndex + (int)UnityEngine.Random.Range(KPPPGMHHIGC, KPPPGMHHIGC + MBAFMKCNBDN);
			if (unitUpgradeDefinition2.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour2.unitType))
	...
	public int EMIGAKFDMNA(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			if (StatsManager.instance.matchmadeGames == 0)
			{
				...
				MNBNPBHHPOC.FKKKABHIPCI().EHHHBEMKGOE.KJJJLNAADHC(IDEBKDPMPGM: false);
			}
			else
			{
				INEMBBHLPDJ(MNBNPBHHPOC, dIFFICULTY, flag, MMNHBLFLLNG: false);
			...
			if (TutorialManagerPlayWarcards.instance.isTutorialRunning)
			{
				...
				CHEIFOOIMEH eHHHBEMKGOE = MNBNPBHHPOC.FKKKABHIPCI().EHHHBEMKGOE;
				CamosManager camosManager = CamosManager.instance;
				int[] array2 = new int[5] { 1, 0, 4, 0, 0 };
				eHHHBEMKGOE.CBBDICBLHMM = camosManager.ConvertVisuals(array2);
				MNBNPBHHPOC.playerController.EHHHBEMKGOE.BCCDDJMPBLL = 6;
			}
	...
	private static void EDDGGIEFHGI(List<LevelBehaviour> HBEFBPNFFBG, UnitUpgradeDefinition[] AMNPDEAJMOB, Dictionary<LevelBehaviour.UnitType, List<UnitUpgradeDefinition>> CBADFOBOCAE)
	{
		...
		for (int i = 0; i < HBEFBPNFFBG.Count; i += 0)
		{
			...
			if (unitUpgradeDefinition.isEquipped && levelBehaviour.upgradeSlots.unlockLevelIndex >= 5)
			{
				...
				EGOHPKKBLLE(jLDNGEGJKEN, out var MBAFMKCNBDN, out var KPPPGMHHIGC);
				if ((float)num4 > KPPPGMHHIGC + MBAFMKCNBDN)
				{
	...
	public static UnitUpgradeDefinition[] DNCEGFKNBEJ(out float FGDEFHGLDJM, int ECFBICCBLKF)
	{
		...
		{
			LevelBehaviour levelBehaviour = list[i];
			UnitUpgradeDefinition unitUpgradeDefinition = (array[levelBehaviour.indexInLevelsManager] = new UnitUpgradeDefinition(levelBehaviour.upgradeSlots.unlockLevelIndex <= ECFBICCBLKF - 1, new UpgradeSlots.UnitUpgrades(878f), isNew: false, 0));
			unitUpgradeDefinition.index = levelBehaviour.indexInLevelsManager;
			if (unitUpgradeDefinition.isEquipped)
			{
				if (!dictionary.ContainsKey(levelBehaviour.unitType))
	...
	private int BEMHHHGIFEM()
	{
		...
		AnalyticsHelper.LeaveBreadcrumb($"=== Choosing bot for me in league {leagueTier} and balance {num2}. Chosen bot in league {num} ===");
		return num;
	}
	...
	public int PNAOCBKILOI(PlayerBot MNBNPBHHPOC)
	{
		...
		try
		{
			...
			{
				SetUpBotDifficulty(MNBNPBHHPOC, dIFFICULTY, flag, MMNHBLFLLNG: false);
			}
			if (TutorialManagerPlayWarcards.instance.isTutorialRunning)
			{
	...
	public int OCEPDEABOHJ(PlayerBot MNBNPBHHPOC)
	{
		...
		FMEAIMOKFLL(MNBNPBHHPOC, playerBotsRow.DIFFICULTY, BPOONJBFIHP: false);
		int value = LevelManager.instance.currentLevel.ELFCEEOLNFJ();
		...
		if (Singleton<GameController>.instance.isTutorial)
		{
			...
			{
				UpgradeSlots.UnitUpgrades upgradeIndex = item3.upgradeSlots.LoadData(item3);
				list2.Add(new UnitUpgradeDefinition(item3.upgradeSlots.unlockLevelIndex > 1 || !(item3 is SoldierBehaviourShotgunner), upgradeIndex, isNew: true, 0));
			}
		}
		MNBNPBHHPOC.FKKKABHIPCI().EHHHBEMKGOE.KNMGPJAHNMG(list2);
	...
	public int DDLKOAIFIJM(PlayerBot MNBNPBHHPOC)
	{
		int result = 1;
		PlayerBotsRow playerBotsRow = KMBFGEDNEBO.Rows[1];
		SetUpBotDifficulty(MNBNPBHHPOC, playerBotsRow.DIFFICULTY, BPOONJBFIHP: false, MMNHBLFLLNG: false);
		...
		if (Singleton<GameController>.instance.BAKCODKBCPJ())
		{
			...
			{
				UpgradeSlots.UnitUpgrades upgradeIndex = item3.upgradeSlots.LoadData(item3);
				list2.Add(new UnitUpgradeDefinition(item3.upgradeSlots.unlockLevelIndex > 0 || !(item3 is SoldierBehaviourShotgunner), upgradeIndex, isNew: false, 0));
			}
		}
