...
public class BattlePreparationDailyMissionButton : Core_BaseScript
{
	...
	public bool isColliderEnabled => GPABKIOEGKM != NKHJBLBAAEB.Locked;
	...
	public bool showNotification => isColliderEnabled && Singleton<NotificationManager>.instance.NotificationDailyMission();
	...
	public void InitGuiValues()
	{
		...
		if (nKHJBLBAAEB == NKHJBLBAAEB.HeroicLocked)
		{
			mHeroicLockedSoloProgression.text = $"{Colours.stringBlue}{DailyMissionsManager.instance.completedSoloMissions}[-] {Colours.stringGrayAbi2}/[-] 3";
			mHeroicPointsLabel.text = $"{Colours.stringGoldHeroic}{DailyMissionsManager.instance.data.heroicPoints}[-] {Colours.stringGrayAbi2}/[-] {DailyMissionsManager.instance.heroicPointsToUnlock}";
			float num = mHeroicPointsLabel.relativeSize.x * mHeroicPointsLabel.transform.localScale.x;
		...
		if (nKHJBLBAAEB == NKHJBLBAAEB.HeroicOpened)
		{
			mHeroicOpenedSoloProgression.text = $"{Colours.stringBlue}{DailyMissionsManager.instance.completedSoloMissions}[-] {Colours.stringGrayAbi2}/[-] 3";
			List<DailyMission> heroicMissions = DailyMissionsManager.instance.heroicMissions;
			for (int i = 0; i < mHeroicMissionsSrites.Length; i++)
			{
				mHeroicMissionsSrites[i].gameObject.SetActive(heroicMissions.Count > i && heroicMissions[i].completedHeroic);
