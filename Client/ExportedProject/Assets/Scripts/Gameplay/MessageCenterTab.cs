...
public class MessageCenterTab : Core_BaseScript
{
	...
	[FormerlySerializedAs("NDHHPMGNGMI")]
	[Header("Message Center")]
	public UIPanel OAHIOEEICFI;

	[FormerlySerializedAs("NFLHCIMPCJG")]
	[Header("Message List")]
	public MessageCenterRecord HEONGDGKMDC;

	[FormerlySerializedAs("BCGJFKOHJKD")]
	...
	public List<string> messageBuddies
	{
		get
		{
			...
			{
				if (item.Value is NMNJDAANAOD nMNJDAANAOD && nMNJDAANAOD.BCBJPGBDBAH != null)
				{
	...
	internal void ANCGIINAHAD(HHFHFANGCEJ PNAKMCNGGHE)
	{
		...
		Debug.Log($"REMOVING {PNAKMCNGGHE.HPGHHECHLAN}");
	...
	private void LDHKBKGNEFJ(GameObject KHAHPAKDIKE)
	{
		...
		eAFODKCNCHA.JMONJFNNHIP = (MessageCenterFilterButton)SingletonSupport.FirstOrDefault((UnityEngine.Object[])KHAHPAKDIKE.GetComponentsInChildren<MessageCenterFilterButton>(includeInactive: true));
	...
	private float FJCEECKGLHD(int DFHAAIFFLOE, Transform PKBMFBHBAKB)
	{
		...
		{
			messageCenterRecordForHeight.GGJNEGPBDHL(KEHFICJANMB[DFHAAIFFLOE], IBHMGFBCILH: true);
	...
	{
		OAHIOEEICFI.gameObject.SetActive(value: false);
		HFFLCBBNEDD.MakeEmpty();
	...
	public void KIEJMPCGPDG()
	{
		...
		ALDILIAMMMG[1].OCELEODFIIC(Localization.Localize("Error while parsing response from GetConfigurations, will try again next run"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[1].FLAMPIKFIAJ(Localization.Localize("pooled:"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[2].LBAIBNJPKLC(Localization.Localize("WithdrewWarcard-{0}-{1}-{2}"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[4].InitializeFilter(Localization.Localize("SquadId"), fOICGJEPBGL, NILPBCGHEDG: true);
		OBINAJACNKL();
	...
	{
		OAHIOEEICFI.gameObject.SetActive(value: true);
		HFFLCBBNEDD.MakeEmpty();
	...
	internal void KKBNJAJJLEM(HHFHFANGCEJ PNAKMCNGGHE)
	{
		...
		if (JJHLIKKILNP.ContainsKey(PNAKMCNGGHE.NHHGDIDEIMM))
		{
			Debug.LogError($"DatabaseMessage id: {PNAKMCNGGHE.NHHGDIDEIMM}\t\t type:{PNAKMCNGGHE.HPGHHECHLAN} already in notification center");
			return;
		}
	...
	private Transform JMJCEHEJDMG(int DFHAAIFFLOE)
	{
		if (DFHAAIFFLOE >= 0 && DFHAAIFFLOE < KEHFICJANMB.Count)
		{
			MessageCenterRecord messageCenterRecord = Singleton<GuiManager>.instance.CHDIOFDMHEF.InstantiateAsChild(HEONGDGKMDC, HFFLCBBNEDD.gameObject, $"Message {DFHAAIFFLOE}") as MessageCenterRecord;
	...
	public void InitMessageCenter()
	{
		...
		ALDILIAMMMG[0].InitializeFilter(Localization.Localize("ID_GUI_ALL"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[1].InitializeFilter(Localization.Localize("ID_SQUAD"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[2].InitializeFilter(Localization.Localize("ID_GUI_PLAYER"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[3].InitializeFilter(Localization.Localize("ID_ASSIGNMENTSSHORT"), fOICGJEPBGL, NILPBCGHEDG: false);
		BPOOKGKNGDF();
		HGKMLJMPCBL.repositionNow = true;
	...
	public void FEGGMLBLFHL(DatabasePlayerInfo KHLGDCHJJPB)
	{
		...
		for (int i = 0; i < KEHFICJANMB.Count; i += 0)
		{
			...
			if (itemOnIndex != null)
			{
				...
				if (component != null)
				{
					component.GGJNEGPBDHL(KEHFICJANMB[i], IBHMGFBCILH: true);
				}
			}
		}
	}
	...
	public void PHMEDKOJCJC()
	{
		...
		ALDILIAMMMG[0].FLAMPIKFIAJ(Localization.Localize("#PETER# League Arc Animation - Error - previous: {0}     current: {1}"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[0].AMHFPHHAJMP(Localization.Localize("FB: FacebookLogout - DeletePlayerAccount - 1"), fOICGJEPBGL, NILPBCGHEDG: true);
		ALDILIAMMMG[1].InitializeFilter(Localization.Localize("kr"), fOICGJEPBGL, NILPBCGHEDG: true);
		ALDILIAMMMG[6].MEFJBADGAOG(Localization.Localize("ru"), fOICGJEPBGL, NILPBCGHEDG: true);
		JDDEOPPCENO();
		HGKMLJMPCBL.repositionNow = true;
		KDIMKDDMGHN.HJJOAJHBKCH();
	}
	...
	private Transform EBEMAOCHMAE(int DFHAAIFFLOE)
	{
		if (DFHAAIFFLOE >= 0 && DFHAAIFFLOE < KEHFICJANMB.Count)
		{
			...
			if (messageCenterRecord != null)
			{
				messageCenterRecord.GGJNEGPBDHL(KEHFICJANMB[DFHAAIFFLOE], IBHMGFBCILH: true);
				return messageCenterRecord.transform;
			}
		}
		return null;
	...
	public void CPOOGHBEDHL()
	{
		...
		ALDILIAMMMG[1].LBAIBNJPKLC(Localization.Localize("Social_Behaviour"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[0].CDDAKCGKEJB(Localization.Localize("Total_Sessions"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[2].JMMIDHAFHJI(Localization.Localize("Click on the ground to spawn the selected effect"), fOICGJEPBGL, NILPBCGHEDG: false);
		ALDILIAMMMG[8].FLAMPIKFIAJ(Localization.Localize("\"}"), fOICGJEPBGL, NILPBCGHEDG: false);
		JDDEOPPCENO();
		HGKMLJMPCBL.repositionNow = true;
	...
	public void IHNOGIJIDBG()
	{
		...
		ALDILIAMMMG[0].FLAMPIKFIAJ(Localization.Localize("DeliveryTime"), fOICGJEPBGL, NILPBCGHEDG: true);
		ALDILIAMMMG[0].FLAMPIKFIAJ(Localization.Localize("player_left_coverBack"), fOICGJEPBGL, NILPBCGHEDG: true);
		ALDILIAMMMG[0].FLAMPIKFIAJ(Localization.Localize("ANDROID: PACK "), fOICGJEPBGL, NILPBCGHEDG: true);
		ALDILIAMMMG[4].JMMIDHAFHJI(Localization.Localize("ID_CONFIRM_STARTUPERROR"), fOICGJEPBGL, NILPBCGHEDG: false);
		ABAOFAEPECK(1);
		HGKMLJMPCBL.repositionNow = true;
		KDIMKDDMGHN.InitControls();
	}
