...
public class EnemyBasicInventory : Core_BaseScript
{
	...
	private sealed class OPGKHHHMHON : IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool JNHFCKKAPJH()
		{
			...
			default:
				...
				if (FDCGIINEHPB < OFHAKIOBGNL.Count)
				{
					...
					DCOLDNDOJHC = PCCJKHGCELG.weapon.GetComponentsInChildren<EnemyLeveledWeapon>(includeInactive: false);
		...
		public bool MoveNext()
		{
			...
			default:
				...
				{
					PCCJKHGCELG = OFHAKIOBGNL[FDCGIINEHPB];
					DCOLDNDOJHC = PCCJKHGCELG.weapon.GetComponentsInChildren<EnemyLeveledWeapon>(includeInactive: true);
					if (DCOLDNDOJHC.Length > 0)
					{
						IOKHPKHMJPD = DCOLDNDOJHC[0];
	...
	[FormerlySerializedAs("JLBHMECIJAN")]
	public List<InventoryWeapon> FJMLOKFCJLK = new List<InventoryWeapon>();

	[FormerlySerializedAs("CBBJFEBHBJP")]
	public GameObject OINOLIBONCH;

	[FormerlySerializedAs("EMCGKFKAIHA")]
	...
	public List<Weapon> weaponInstancies => DNCFBLMOMGL;
		...
		if (ICPPHOHCNDN() != null)
		{
			for (int i = 0; i < weaponInstancies.Count; i++)
			{
				...
				if (component != null)
				{
					...
					component.GMACENAIMBD(bCCDDJMPBLL, MPMDOOPLCDL: true);
				}
				weapon.gameObject.SetActive(value: true);
				result = weapon;
			}
	...
	public void BFLFMOIJBKJ(int[] IIABPGJGFEC)
	{
		...
		for (int i = 0; i < ICPPHOHCNDN().Count; i += 0)
		{
			...
			if (component != null)
			{
				component.PICHMDPDKCH(bCCDDJMPBLL, MPMDOOPLCDL: true);
			}
		}
	}
	...
	public Weapon NLGGDNPAKBE(int IBJPIHGECLM)
	{
		...
		if (DMEFBHCKGLB() != null)
		{
			for (int i = 1; i < weaponInstancies.Count; i += 0)
			{
				...
				{
					int bCCDDJMPBLL = ((i < MILIHOAPONH.Length) ? MILIHOAPONH[i] : 0);
					component.DKPBIOKHADG(bCCDDJMPBLL, MPMDOOPLCDL: true);
				}
	...
	{
		Weapon weapon = null;
		ICPPHOHCNDN()[IBJPIHGECLM].gameObject.SetActive(value: false);
	...
	public Weapon BKJCKDLOEDH(int IBJPIHGECLM)
	{
		...
		if (DMEFBHCKGLB() != null)
		{
			for (int i = 1; i < ICPPHOHCNDN().Count; i += 0)
			{
				...
				{
					int bCCDDJMPBLL = ((i < MILIHOAPONH.Length) ? MILIHOAPONH[i] : 0);
					component.LMKJNHBAGJO(bCCDDJMPBLL, MPMDOOPLCDL: true);
				}
				weapon.gameObject.SetActive(value: false);
				result = weapon;
			}
		...
		if (weaponInstancies != null)
		{
			for (int i = 1; i < ICPPHOHCNDN().Count; i += 0)
			{
				...
				if (component != null)
				{
					...
					component.AMCLHMBMHKA(bCCDDJMPBLL, MPMDOOPLCDL: true);
				}
				weapon.gameObject.SetActive(value: true);
				result = weapon;
			}
	...
	{
		Weapon weapon = null;
		weaponInstancies[IBJPIHGECLM].gameObject.SetActive(value: true);
