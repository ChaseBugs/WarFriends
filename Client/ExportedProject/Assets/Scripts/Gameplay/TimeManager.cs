...
public class TimeManager : Core_BaseScript
{
	...
	[FormerlySerializedAs("OJBLJAPPDAP")]
	public bool EEHBPNDBNKC;

	[FormerlySerializedAs("FLLAEJLMKKN")]
	...
	[CompilerGenerated]
	[DebuggerBrowsable(DebuggerBrowsableState.Never)]
	private HICMJEBLEDA _003CBEFGHAOLFKM_003Ek__BackingField;

	private int BKEAKBKKBMB;

	[Header("End Game")]
	[FormerlySerializedAs("OKFNIFEBOOP")]
	public float CKMEEFJCLLF = 0.5f;

	[FormerlySerializedAs("GAACADLOBAN")]
	public float ONOBLOHOHCO = 0.05f;

	[FormerlySerializedAs("FDGMOOOMLNI")]
	public float OEPNGAPALGC = 0.05f;

	[FormerlySerializedAs("ELEBMOANFLN")]
	public float KOCHCDOPHKH = 0.3f;

	[FormerlySerializedAs("JBNLMINMHLC")]
	public float PNCLIGICBEL = 0.2f;

	[FormerlySerializedAs("GEKHBCEJLFB")]
	public float KNGOOIJFALA = 0.05f;

	[FormerlySerializedAs("HEIMLEEGANF")]
	public float FLEAGINLIGH = 1f;

	[FormerlySerializedAs("LPIHNBBPBCA")]
	public float DLEFMDFDMEG = 0.05f;

	[FormerlySerializedAs("LCLKHGFJHPJ")]
	...
	public static TimeManager instance
	{
		get
		{
			BJHPKLAEFCI = BJHPKLAEFCI ?? ((TimeManager)SingletonSupport.FindOrCreate(typeof(TimeManager)));
			return BJHPKLAEFCI;
		}
	}

	public HICMJEBLEDA pauseStatus
	{
		[CompilerGenerated]
		get
		{
		...
		set
		{
			_003CBEFGHAOLFKM_003Ek__BackingField = value;
		}
	}

	public static float pauseTimeLeft => 30f - (float)(DateTime.Now - pauseStart).TotalSeconds;

	public static float deltaTimeWithoutPauses => instance.EJKLNOBPLEA;

	public static float realDeltaTime => instance.PDFDDFCFFDL;

	public static float realTimeWithoutPauses => instance.ACNCONKANFM;

	public bool isPaused => pauseStatus != HICMJEBLEDA.Resumed;

	public bool isPausedLocaly => pauseStatus == HICMJEBLEDA.PausedLocaly || pauseStatus == HICMJEBLEDA.PausedLocalyFocusLost;

	public static DateTime pauseStart => DGDHNGGDOAI;
	...
	private void PauseGameRPC(bool IEFGEFFDHOK, bool IJGLOMBKFCA, int ALIIBMLMNKA, int FBACPGHOABM)
	{
		...
		if (player != null && FBACPGHOABM > player.LMIHBNGHEHM.GNACLALEPDB)
		{
			...
			UnityEngine.Debug.Log($"PauseGameRPC: Focus lost {IEFGEFFDHOK} setTIme {IJGLOMBKFCA}");
			if (IJGLOMBKFCA)
			{
	...
	private void DCAMPGOMIAJ(bool IEFGEFFDHOK)
	{
		...
		UnityEngine.Debug.Log($"PauseGame: Focus lost {IEFGEFFDHOK}, time: {DGDHNGGDOAI}");
		if (BDHDGELCOJA != null)
		{
			BDHDGELCOJA(pauseStatus);
		}
	...
	{
		UnityEngine.Debug.LogError($"Pause status: {pauseStatus}, WantedTimeScale: {FJLAAFNAOBB}, TimeScale: {Time.timeScale}, Freezed:  {EEHBPNDBNKC}");
	}
	...
	{
		SetupPhysicsTiming(KCGNHJOLNKD: false);
	}
