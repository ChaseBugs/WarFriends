...
public class ArenaOpenedDialog : GuiElementSingle<ArenaOpenedDialog>, PAIIOKBBHBC
{
	...
	[Header("Middle")]
	[FormerlySerializedAs("GHOJPKDNGDG")]
	...
	[Header("Right")]
	[FormerlySerializedAs("PNIHJOCBOCL")]
	public WararenaRewardRecord AAPOHEGHHAC;

	[FormerlySerializedAs("IIAAMAMFFDK")]
	...
	private void DCKLBBDOHAE()
	{
		if (NPFFMLLLDAF())
		{
			Singleton<LootBoxCameraArena>.instance.MAPKMCFDDJE(IFFDIHCPKFE, LBCMGKGLFPI: false);
		}
	}
	...
	public override void InitGUIValues()
	{
		...
		OPJKGIHKOJE.AlignToPos(instant: true);
	...
	private Transform BKEGAEBBGPE(int DFHAAIFFLOE)
	{
		...
		{
			WararenaRewardRecord wararenaRewardRecord = Singleton<GuiManager>.instance.CHDIOFDMHEF.InstantiateAsChild(AAPOHEGHHAC, KCCOKFDFHGI.gameObject, $"Reward {DFHAAIFFLOE:D2}") as WararenaRewardRecord;
	...
	public virtual void OFCCACPEBHD()
	{
		...
		KCCOKFDFHGI.MakeEmpty();
		KCCOKFDFHGI.init(MLCJPKJBAHL.Count, BKEGAEBBGPE, MBHDDJLBEEM, OPJKGIHKOJE);
		OPJKGIHKOJE.AlignToPos(instant: true, UIDraggablePanel.Alingment.BottomRight, onePanelDisabledAfterAlign: true);
	...
	private void GAHAMMDHAOF()
	{
		if (NPFFMLLLDAF())
		{
			Singleton<LootBoxCameraArena>.instance.PEPEFFINIMM(IFFDIHCPKFE, LBCMGKGLFPI: false);
		}
	}
	...
	public void AONPDBAPDOL(LevelBehaviour IOIKKIIFOCB)
	{
		...
		if (isShowed)
		{
			Singleton<LootBoxCameraArena>.instance.MCJFOJJDAFC(IFFDIHCPKFE, LBCMGKGLFPI: false);
		}
	}
