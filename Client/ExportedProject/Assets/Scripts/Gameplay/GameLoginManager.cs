...
public class GameLoginManager : InGameSerializedObjectGeneric<GameLoginManager.LoginData>
{
	...
	public class PlayerAccount
	{
		...
		[FormerlySerializedAs("hashedKeySIWA")]
		public string hashKeyGuest;

		[FormerlySerializedAs("passwordSIWA")]
	...
	private sealed class HNIDECFHCGF : IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool PKOMECFNPIP()
		{
			...
			case 3u:
				...
				{
					BJGCPDNMHDH.ChangeAccountDialogCallback(null, b: false);
				}
				CLPPIAGLCJJ = -1;
				goto default;
			default:
		...
		public bool GAPIFMKNJBN()
		{
			...
			case 3u:
				...
				{
					BJGCPDNMHDH.ChangeAccountDialogCallback(null, b: true);
				}
				CLPPIAGLCJJ = -1;
				goto default;
			default:
	...
	public static GameLoginManager instance
	{
		get
		{
			mInstance = mInstance ?? ((GameLoginManager)SingletonSupport.FindOrCreate(typeof(GameLoginManager)));
			return mInstance;
		}
	}
	...
	public List<KFLKGGGJHIN.EBDBJAKJNLD> facebookFriends => mFacebookService.GBLOAJIGOHF;

	public Dictionary<long, KFLKGGGJHIN.EBDBJAKJNLD> facebookFriendsDict => mFacebookService.OLNAPDJGPEE;
	...
	public string squadIconName
	{
		get
		{
			...
			AANECPGDMGM aANECPGDMGM = Singleton<ServerResultsCache>.instance.MKKMNBIBCFM(data.currentPlayer.squadName, NICBOKPIBIA: true);
	...
	{
		UnityEngine.Debug.LogWarning("SetDepositedCards " + (mDepositedCards?.Count ?? (-1)));
	...
	public void RemoveCardFromDeposited(string cardId)
	{
		...
		UnityEngine.Debug.Log($"REMOVING WARCARD: \"{cardId}\" FROM DEPOSITED WARCARDS");
	...
	public void OnAccoutNotExistAfterLoginServices(AccountType type)
	{
		...
		}, Localization.Localize($"ID_TUTORIAL_NOACCOUNT_{arg}_TITLE"), Localization.Localize($"ID_TUTORIAL_NOACCOUNT_{arg}_TEXT"));
	...
	internal void RegisterOrLogin(SessionManager.MFGMKAJDCAI sessionUpdate = SessionManager.MFGMKAJDCAI.Full)
	{
		...
		if (data.currentPlayer != null && !data.currentPlayer.isFacebookConnected)
		{
			...
			{
				UnityEngine.Debug.LogError($"hasResponse: {Singleton<GameCenterProvider>.instance.HGKJLMMNDOE}; hashKeyGC : {data.playerAccount.hashKeyGC} ; gcID : {Singleton<GameCenterProvider>.instance.PIIIKJJHJPN}");
				if (!Singleton<GameCenterProvider>.instance.HGKJLMMNDOE)
				{
