...
internal class IODMFDDEJOP : HHFHFANGCEJ
{
	...
	public virtual bool FPKBGONCOAM()
	{
		...
		{
			if (!Singleton<OfferManager>.instance.IsTextureDownloaded(BFPPALFKOAF: false, KAEFHCKBMGM[i].MODHHKHPHOJ, KAEFHCKBMGM[i].FLFMHFIGNKE()))
			{
				return true;
			}
		}
	...
	public virtual bool IBDAFAANAIG()
	{
		...
		{
			if (!Singleton<OfferManager>.instance.IsTextureDownloaded(BFPPALFKOAF: true, KAEFHCKBMGM[i].MODHHKHPHOJ, KAEFHCKBMGM[i].FLFMHFIGNKE()))
			{
				return true;
			}
		}
	...
	public IODMFDDEJOP(string DDLOAFGAEAC, int GAFOMGDLAFD, AKDLEDNDIEO FJLBLLLEELD)
		: base($"SpecialOfferMessage {DDLOAFGAEAC} {GAFOMGDLAFD} {FJLBLLLEELD}", NKHJBLBAAEB.SpecialOfferMessage)
	{
		DNEBFDALAFH = DDLOAFGAEAC;
		AIBAGMICGAE = GAFOMGDLAFD;
		MKPKIPHBNOL = (FJLBLLLEELD & AKDLEDNDIEO.Upgrade) == AKDLEDNDIEO.Upgrade;
		Debug.Log($"ADD special offer to show {DDLOAFGAEAC} {GAFOMGDLAFD} {FJLBLLLEELD.ToString()}");
	}
	...
	public virtual bool FIPBIMFFCMB()
	{
		for (int i = 0; i < KAEFHCKBMGM.Length; i++)
		{
			if (!Singleton<OfferManager>.instance.IsTextureDownloaded(BFPPALFKOAF: false, KAEFHCKBMGM[i].MODHHKHPHOJ, KAEFHCKBMGM[i].GDIAEJILINE))
			{
				return true;
			}
		}
	...
	public IODMFDDEJOP(JPMLOCNCEGO[] LDKNOAADOPA)
		: base($"SpecialOfferMessage", NKHJBLBAAEB.SpecialOfferMessage)
	{
		BLBKJJGOCDA = true;
	...
	public virtual bool PBLFAEMOEMG()
	{
		for (int i = 1; i < KAEFHCKBMGM.Length; i++)
		{
			if (!Singleton<OfferManager>.instance.IsTextureDownloaded(BFPPALFKOAF: true, KAEFHCKBMGM[i].MODHHKHPHOJ, KAEFHCKBMGM[i].GDIAEJILINE))
			{
				return false;
			}
		}
