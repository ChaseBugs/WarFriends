...
public class GameVariables : Singleton<GameVariables>
{
	...
	private float PEEJAAPLHIA => 1.3333334f;

	private float KMDMJDAKCCG => 1.7777778f;

	private float KDFFNLONAFJ => Mathf.Clamp(UIHelper.activeWidthFull / UIHelper.activeHeight, PEEJAAPLHIA, KMDMJDAKCCG);
	...
	public float multiplierXfromRatio => (KDFFNLONAFJ - KMDMJDAKCCG) / (PEEJAAPLHIA - KMDMJDAKCCG);

	public bool isiPadResolution => KDFFNLONAFJ <= PEEJAAPLHIA + 0.001f;

	public CardPacks cardPacks => EFIPJDHGJAI ?? (EFIPJDHGJAI = GetComponent<CardPacks>());

	public VIP vip => AIAHJPDDBKK ?? (AIAHJPDDBKK = GetComponent<VIP>());

	public InApps inApps => LPHIJHINLPI ?? (LPHIJHINLPI = GetComponent<InApps>());

	public BalanceTable balancetable => HOCLHEONDDN ?? (HOCLHEONDDN = GetComponent<BalanceTable>());

	public CountryInfo countryInfo => CHFJOCBIICM ?? (CHFJOCBIICM = GetComponent<CountryInfo>());

	public Squads squads => AHBEHCBJDLK ?? (AHBEHCBJDLK = GetComponent<Squads>());

	public Constants constants => DCLFKNBHLGG ?? (DCLFKNBHLGG = GetComponent<Constants>());

	public UnitsContants unitsConstants => OKDBEFJDNLH ?? (OKDBEFJDNLH = GetComponent<UnitsContants>());

	public Versions versions => DGPOGHBJNEN ?? (DGPOGHBJNEN = GetComponent<Versions>());

	public HeroicRewards heroicRewards => NGHPCDLHGLH ?? (NGHPCDLHGLH = GetComponent<HeroicRewards>());

	public MatchMakingDeviation matchmakingDeviations => MNBOGBONEFH ?? (MNBOGBONEFH = GetComponent<MatchMakingDeviation>());

	public StringConstants stringConstants => EHEDJBICFKC ?? (EHEDJBICFKC = GetComponent<StringConstants>());

	public BonussesDefinitions bonussesDefinitions => HGKADKDMINC ?? (HGKADKDMINC = GetComponent<BonussesDefinitions>());

	public CardConstants cardConstants => ENKOOBNFPBC ?? (ENKOOBNFPBC = GetComponent<CardConstants>());

	public MatchMakingConstants matchMakingConstants => DJHPFJLOGKA ?? (DJHPFJLOGKA = GetComponent<MatchMakingConstants>());

	public PlayerLeaderboards playerLeaderboards => EFFOGEGIBCK ?? (EFFOGEGIBCK = Singleton<BeanstalkServerManager>.instance.GetComponent<PlayerLeaderboards>());

	public Packs packsDefinition => JCADGFMCMAP ?? (JCADGFMCMAP = Singleton<BeanstalkServerManager>.instance.GetComponent<Packs>());
	...
	public SquadEmblems squadEmblems => HJIGBGEJBIE ?? (HJIGBGEJBIE = GetComponent<SquadEmblems>());

	public SquadLeaderboards squadLeaderboards => IMFEGGKEFAN ?? (IMFEGGKEFAN = Singleton<BeanstalkServerManager>.instance.GetComponent<SquadLeaderboards>());

	public SquadWarsReward squadWarsReward => DHEMNDLHFJA ?? (DHEMNDLHFJA = Singleton<BeanstalkServerManager>.instance.GetComponent<SquadWarsReward>());

	public Lootboxes lootboxes => LDDKEHKEJBI ?? (LDDKEHKEJBI = GetComponent<Lootboxes>());

	public Warbucks warbucksForGold => BEBCNKHDJDC ?? (BEBCNKHDJDC = GetComponent<Warbucks>());
	...
	public int BeginnersLeagueMaxMedals(int OFGFLGHGEJF)
	{
		...
		int num = vALUE.IndexOf($"\"{OFGFLGHGEJF}\"");
		int result = 0;
		if (num > -1)
		{
			int num2 = vALUE.IndexOf(":", num);
