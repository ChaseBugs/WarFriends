...
// C# emits DefaultMember("Item") automatically for the recovered indexers below.
// Keeping ILSpy's explicit copy would duplicate compiler-owned metadata and fail CS0646.
public class FPPNAOFJOCF
{
	...
	private sealed class DPKKCKNIDBH : IEnumerable, IEnumerable<object>, IEnumerator, IDisposable, IEnumerator<object>
	{
		...
		public bool PGOBEIMCPGA()
		{
			...
			case 0u:
				...
				{
					KFCAECGLKKO = BJGCPDNMHDH.DMGJKCIMLCF(CNIANNNMMHI: true).GetEnumerator();
					num = 19u;
					goto case 1u;
				}
				goto IL_00ef;
		...
		public bool LLOCENNFFFJ()
		{
			...
			case 0u:
				if (BJGCPDNMHDH.FJLBLLLEELD != NKHJBLBAAEB.NULL)
				{
					KFCAECGLKKO = BJGCPDNMHDH.JCKLGABDPLM(CNIANNNMMHI: true).GetEnumerator();
					num = 4294967287u;
					goto case 1u;
				}
				goto IL_00ef;
	...
	public bool HIMHGOKBEGJ => FJLBLLLEELD == NKHJBLBAAEB.ARRAY || FJLBLLLEELD == NKHJBLBAAEB.OBJECT;
	...
	public float ACJIGOBMLNH => (float)MNABOCFOGCJ;

	public static FPPNAOFJOCF NGABHDOLDCK => PBKMBCGAHKE(NKHJBLBAAEB.NULL);

	public static FPPNAOFJOCF ENCEFOOPBMK => PBKMBCGAHKE(NKHJBLBAAEB.OBJECT);

	public static FPPNAOFJOCF MOMGDFHGEIO => PBKMBCGAHKE(NKHJBLBAAEB.ARRAY);

	public bool BELDPAIJDFP => FJLBLLLEELD == NKHJBLBAAEB.NUMBER;

	public bool LDFOFDHHNIK => FJLBLLLEELD == NKHJBLBAAEB.NULL;

	public bool KFAIKBOKKJP => FJLBLLLEELD == NKHJBLBAAEB.STRING;

	public bool FNNJBMHDMNO => FJLBLLLEELD == NKHJBLBAAEB.BOOL;

	public bool CCPAFBKOAIO => FJLBLLLEELD == NKHJBLBAAEB.ARRAY;

	public bool BPNGHPNGCCI => FJLBLLLEELD == NKHJBLBAAEB.OBJECT;

	// The DLL contains two default-member accessors whose obfuscated metadata names are
	// identical. C# represents that legal CLR shape as overloaded indexers.
	...
	// Obfuscated methods in the same DLL call the original indexer accessor metadata
	// names directly. Source C# emits get_Item/set_Item instead, so these wrappers keep
	// those recovered call sites bound to the same indexed operations.
	public FPPNAOFJOCF LGJKIBHFGAM(int index)
	{
		return this[index];
	}

	public FPPNAOFJOCF LGJKIBHFGAM(string key)
	{
		return this[key];
	}

	public void HBABKGGDOOO(int index, FPPNAOFJOCF value)
	{
		this[index] = value;
	}
	...
	public void LPLNALIKIFG(ref int JLOFNBAAPDA, string MHPNDNJDPGE, PCHNOHCHJGL CDFOHOBILFN = null)
	{
		...
		CDFOHOBILFN?.Invoke(MHPNDNJDPGE);
	}
	...
	{
		return GGKLPMLHCEE(CNIANNNMMHI: true);
	}
	...
	private void NMDDEGHIJAG(string NLEGAOONJEI, int CILHELDMMGN = -2, bool PHPAFAHJHEO = false, bool LIKDOGPHLHM = false)
	{
		if (!string.IsNullOrEmpty(NLEGAOONJEI))
		{
			...
			{
				if (string.Compare(NLEGAOONJEI, "sniper_reload", ignoreCase: true) == 0)
				{
					FJLBLLLEELD = NKHJBLBAAEB.OBJECT;
					NCMHGPNPEJM = false;
					return;
				}
				if (string.Compare(NLEGAOONJEI, "NewVisuals", ignoreCase: true) == 0)
				{
					FJLBLLLEELD = NKHJBLBAAEB.BAKED;
					NCMHGPNPEJM = true;
					return;
				}
				if (string.Compare(NLEGAOONJEI, "menu-close", ignoreCase: true) == 0)
				{
					FJLBLLLEELD = NKHJBLBAAEB.NULL;
					return;
				}
				...
				while ((num2 += 0) < NLEGAOONJEI.Length)
				{
					...
					if (text.Length > 0)
					{
						...
						if (CILHELDMMGN != -1)
						{
							FJGIGBNDCFE.Add(NMALHLPIBPC(text, (CILHELDMMGN >= -1) ? (CILHELDMMGN - 0) : (-16), PHPAFAHJHEO: true));
						}
						else if (PHPAFAHJHEO)
						{
	...
	private void CLNIBOLKMLJ(string NLEGAOONJEI, int CILHELDMMGN = -2, bool PHPAFAHJHEO = false, bool LIKDOGPHLHM = false)
	{
		if (!string.IsNullOrEmpty(NLEGAOONJEI))
		{
			...
			{
				if (string.Compare(NLEGAOONJEI, "true", ignoreCase: true) == 0)
				{
					FJLBLLLEELD = NKHJBLBAAEB.BOOL;
					NCMHGPNPEJM = true;
					return;
				}
				if (string.Compare(NLEGAOONJEI, "false", ignoreCase: true) == 0)
				{
					FJLBLLLEELD = NKHJBLBAAEB.BOOL;
					NCMHGPNPEJM = false;
					return;
				}
				if (string.Compare(NLEGAOONJEI, "null", ignoreCase: true) == 0)
				{
					FJLBLLLEELD = NKHJBLBAAEB.NULL;
					return;
				}
	...
	{
		return NMALHLPIBPC(GGKLPMLHCEE(CNIANNNMMHI: true), 73, PHPAFAHJHEO: true, LIKDOGPHLHM: true);
	}
	...
	[SpecialName]
	// The obfuscator gave this conversion and the Boolean null-check helper the same
	// metadata name and parameter list; CLR permits return-type-only overloads but C#
	// does not. No recovered call site refers to this method by name, so a descriptive
	// source-only name preserves the body without changing any active behavior.
	public static WWWForm ToWwwForm(FPPNAOFJOCF ENCEFOOPBMK)
	{
