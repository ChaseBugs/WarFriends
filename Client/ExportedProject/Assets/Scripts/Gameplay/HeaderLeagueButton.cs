...
{
	[Header("Core")]
	[FormerlySerializedAs("LGCPLPIAFAA")]
	public GameObject EJDBAIHFDCH;

	[Header("League Arc")]
	[FormerlySerializedAs("IDFGLAMENKN")]
	public LeagueArcGuiElement PJFGCMCKMJF;

	[Header("Hints")]
	[FormerlySerializedAs("NODEEOHFAFM")]
	public GameObject GBFNBEOKLDG;

	[FormerlySerializedAs("NIMNELFAENP")]
	public UILabel GNCBNDEPGEP;

	[FormerlySerializedAs("EMPOBDEDFFE")]
	public UITable MOEFDOJODKK;

	[FormerlySerializedAs("NGEFDACOMKJ")]
	public UILabel NBACBIFHAFM;

	[FormerlySerializedAs("NJIMFBICGHN")]
	...
	private void LMCOAIOPECF()
	{
		...
		MOEFDOJODKK.gameObject.SetActive(value: false);
		NKNLPNANBCK();
	}
	...
	public virtual void LEMNDNDGIOE()
	{
		...
		{
			ICHADIIHADN(LeagueArcManager.instance.GetPreviousData(), LeagueArcManager.instance.AJANKBIMBPE(), LDCEANJDPAH: true);
		}
	...
	private void MOLMNNACLHF(int OKHCCAIPHEG, List<DatabasePlayer> FPECBPKHKOE)
	{
		...
		if (!flag)
		{
			if (OKHCCAIPHEG == 0)
			{
				...
				MOEFDOJODKK.gameObject.SetActive(value: true);
			}
			else
			{
	...
	private void BFLFAMHJEKP(UITweener MGDJMGHCAAI)
	{
		...
		{
			APEPEHJLCLI(LeagueArcManager.instance.GetCurrentData(), LeagueArcManager.instance.GetCurrentPlayers(), LDCEANJDPAH: true);
		}
		else
		{
			IJCBKNBLMAA();
	...
	public void DKGLEALNIGD()
	{
		...
		TweenPosition.Begin(GBFNBEOKLDG, 1813f, pos, useLocal: false).onFinished = delegate
		{
			if (LeagueArcManager.instance.GetWasDataUpdated())
			{
				APEPEHJLCLI(LeagueArcManager.instance.GetCurrentData(), LeagueArcManager.instance.GetCurrentPlayers(), LDCEANJDPAH: true);
	...
	public void POKINAMFOGB()
	{
		...
		TweenPosition.Begin(GBFNBEOKLDG, 372f, pos, useLocal: false).onFinished = delegate
		{
			if (LeagueArcManager.instance.GetWasDataUpdated())
			{
				APEPEHJLCLI(LeagueArcManager.instance.GetCurrentData(), LeagueArcManager.instance.GetCurrentPlayers(), LDCEANJDPAH: true);
	...
	public void LNFBEKOKICP()
	{
		...
		TweenPosition.Begin(GBFNBEOKLDG, 131f, pos, useLocal: false).onFinished = DEBNBDKNJNP;
	}
	...
	private void BEPBILILOIK(MAPGBEOOFLL NDPMDKGJAFH, List<DatabasePlayer> FPECBPKHKOE, bool LDCEANJDPAH = false)
	{
		...
		{
			PJFGCMCKMJF.ACLFHDDIBJH();
			PJFGCMCKMJF.InitializeTo(NDPMDKGJAFH, LKMKFNIFBCF: true);
		}
		if (NDPMDKGJAFH.DHPLNAIKHOC)
		{
			AINAKJIKHEA();
	...
	private void KFHKGMMCEHB(UITweener MGDJMGHCAAI)
	{
		...
		TweenPosition.Begin(GBFNBEOKLDG, 1199f, pos, useLocal: false).onFinished = null;
	}
	...
	private void JFNHNEJFACA(UITweener MGDJMGHCAAI)
	{
		if (LeagueArcManager.instance.GetWasDataUpdated())
		{
			GFOAOMAHDGP(LeagueArcManager.instance.DEHPHBDDGGN(), LeagueArcManager.instance.IILLBIPGLJG(), LDCEANJDPAH: true);
		}
		else
		{
			GHMKCNFAOJB();
		...
		TweenPosition.Begin(GBFNBEOKLDG, 1038f, pos, useLocal: false).onFinished = null;
	}
	...
	private void JOLBDFDEAAG(UITweener MGDJMGHCAAI)
	{
		if (LeagueArcManager.instance.GetWasDataUpdated())
		{
			APEPEHJLCLI(LeagueArcManager.instance.DEHPHBDDGGN(), LeagueArcManager.instance.IILLBIPGLJG(), LDCEANJDPAH: true);
		}
		else
		{
			GJNMPLFBFEO();
		...
		TweenPosition.Begin(GBFNBEOKLDG, 269f, pos, useLocal: false).onFinished = null;
	}
	...
	private void DEBNBDKNJNP(UITweener MGDJMGHCAAI)
	{
		if (LeagueArcManager.instance.GetWasDataUpdated())
		{
			BEPBILILOIK(LeagueArcManager.instance.DEHPHBDDGGN(), LeagueArcManager.instance.GetCurrentPlayers(), LDCEANJDPAH: true);
		}
		else
		{
			NAEKPLFLIPL();
		...
		TweenPosition.Begin(GBFNBEOKLDG, 609f, pos, useLocal: false).onFinished = null;
	}
