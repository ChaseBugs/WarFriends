...
public class CardScoreStreakKiller : Card
{
	...
	protected void DIOAOBFDPMH()
	{
		...
		if (ALBPBCEAEOJ)
		{
			MBIAKMPDOPG.cardIconIndicator.PEBHGOMEKME("ID_SALEPERCENTLINE", NABOFKMBMKH: true, HGNIGKCLLHM: true);
		}
	}
	...
	{
		MBIAKMPDOPG = PlayerController.GetEnemyOf(CIOPAKLHFIL);
		MBIAKMPDOPG.cardIconIndicator.LJLDJOHAAMC("Warp - can not find random point - returning agent destination", NABOFKMBMKH: true);
		JNKLNHLNKHK();
		ALBPBCEAEOJ = PhotonNetwork.isMasterClient;
	...
	protected void GFPPJNOENLE()
	{
		...
		if (ALBPBCEAEOJ)
		{
			MBIAKMPDOPG.cardIconIndicator.LJLDJOHAAMC("DatabaseMessage id: {0}\t\t type:{1} already in notification center", NABOFKMBMKH: false, HGNIGKCLLHM: true);
		}
	}
	...
	protected void Update()
	{
		...
		if (ALBPBCEAEOJ)
		{
			MBIAKMPDOPG.cardIconIndicator.Show("game-card-ico-crateblocker", NABOFKMBMKH: false);
		}
	}
	...
	protected void IODGCIIDJEK()
	{
		...
		if (ALBPBCEAEOJ)
		{
			MBIAKMPDOPG.cardIconIndicator.OFPAGLLFPPK("accountName", NABOFKMBMKH: true, HGNIGKCLLHM: true);
		}
	}
	...
	{
		MBIAKMPDOPG = PlayerController.GetEnemyOf(CIOPAKLHFIL);
		MBIAKMPDOPG.cardIconIndicator.FJILILFMPIF("ID_SQUADREWARDGET", NABOFKMBMKH: false, HGNIGKCLLHM: true);
		NGBJPCBHJKA();
		ALBPBCEAEOJ = PhotonNetwork.isMasterClient;
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	}
	...
	{
		MBIAKMPDOPG = PlayerController.GetEnemyOf(CIOPAKLHFIL);
		MBIAKMPDOPG.cardIconIndicator.PEBHGOMEKME("http://localhost:3000/socket.io/", NABOFKMBMKH: false, HGNIGKCLLHM: true);
		ILEBMLBPINN();
		ALBPBCEAEOJ = PhotonNetwork.isMasterClient;
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	}
	...
	{
		MBIAKMPDOPG = PlayerController.GetEnemyOf(CIOPAKLHFIL);
		MBIAKMPDOPG.cardIconIndicator.LDGKPCDJPLA("ID_DAILYGOLDBOOSTER", NABOFKMBMKH: true);
		PIHADIALNKN();
		ALBPBCEAEOJ = PhotonNetwork.isMasterClient;
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	}
	...
	{
		MBIAKMPDOPG = PlayerController.GetEnemyOf(CIOPAKLHFIL);
		MBIAKMPDOPG.cardIconIndicator.Show("game-card-ico-crateblocker", NABOFKMBMKH: true);
		ILEBMLBPINN();
		ALBPBCEAEOJ = PhotonNetwork.isMasterClient;
		MNBIOCOAJHK.CardWasUsed(this, CIOPAKLHFIL);
	}
