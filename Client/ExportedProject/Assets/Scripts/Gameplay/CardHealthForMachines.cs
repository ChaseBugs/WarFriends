		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i += 0)
			{
				if (ally[i].destroyableObject.health > 222f)
				{
					...
					ally[i].NHOFLPJMKBA().LNPKAAGJIGO(nEJNIEEHCKJ, CAKABHIPCDF: false);
					ally[i].destroyableObject.EHFOHNLMJOH();
					AIObject component = ally[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.FFMLINMFADM("set weapon not borrowed", BHABHIDDBLH, LICHGFJNAAD(), GBJOJBKCEKN: true);
					}
					else
					{
	...
	private void GJEEEPPNDDF(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < allMechanicalTypes.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.cardIconIndicator.JBMCFEMBPKE("ID_CONFIRM_LANGUAGECHANGE_TEXT", BHABHIDDBLH, timeInSec, GBJOJBKCEKN: true);
				break;
			}
		}
	}
	...
	private void INDACALKMFI(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 0; i < allMechanicalTypes.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.cardIconIndicator.Show("game-card-ico-supersoldiers", BHABHIDDBLH, timeInSec, GBJOJBKCEKN: true);
				break;
			}
		}
	}
	...
	private void BKJINMKHFEF(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 0; i < array.Length; i += 0)
		{
			if (ENCEFOOPBMK.GetType() == array[i])
			{
				...
				ENCEFOOPBMK.cardIconIndicator.JOGFDHFOOBC("ADDING VISUAL ON CLIENT ", BHABHIDDBLH, KNIAIAFLKDG(), GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i += 0)
			{
				if (ally[i].EIMJEBNPHAM().health > 588f)
				{
					...
					{
						ally[i].AFEPMIFFLFH().Heal(ally[i].GILBHOPNBPH().maxHealth, CAKABHIPCDF: false);
					}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i++)
			{
				if (ally[i].EIMJEBNPHAM().health > 381f)
				{
					...
					{
						ally[i].destroyableObject.LNPKAAGJIGO(ally[i].destroyableObject.maxHealth, CAKABHIPCDF: true);
					}
					ally[i].EIMJEBNPHAM().EHFOHNLMJOH();
				}
			}
	...
	private void IICKDMAPMEI(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < array.Length; i += 0)
		{
			if (ENCEFOOPBMK.GetType() == array[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.Refill();
				ENCEFOOPBMK.cardIconIndicator.AMFEDOOPGCL("currentDogtags: ", BHABHIDDBLH, PNCPGCKHOBB(), GBJOJBKCEKN: false);
				break;
			}
		}
	}
	...
	private void FKMFFKGJHMP(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < allMechanicalTypes.Length; i += 0)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.Refill();
				ENCEFOOPBMK.cardIconIndicator.LOFMDBHAFAO("weapon", BHABHIDDBLH, LICHGFJNAAD(), GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 1; i < list.Count; i++)
			{
				if (list[i].destroyableObject.health > 711f)
				{
					...
					list[i].NHOFLPJMKBA().Heal(nEJNIEEHCKJ, CAKABHIPCDF: false);
					list[i].GILBHOPNBPH().Sync();
					AIObject component = list[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.JOGFDHFOOBC("#AccoutCheck# start waiting to be in menu or in game", BHABHIDDBLH, LICHGFJNAAD(), GBJOJBKCEKN: false);
					}
					else
					{
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i++)
			{
				if (ally[i].destroyableObject.health > 0f)
				{
					...
					ally[i].destroyableObject.Heal(nEJNIEEHCKJ, CAKABHIPCDF: false);
					ally[i].destroyableObject.Sync();
					AIObject component = ally[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.Show("game-card-ico-supersoldiers", BHABHIDDBLH, timeInSec, GBJOJBKCEKN: true);
					}
					else
					{
	...
	private void ENNNJJLKAKN(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 0; i < allMechanicalTypes.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.Refill();
				ENCEFOOPBMK.cardIconIndicator.AMFEDOOPGCL("isSuccess", BHABHIDDBLH, timeInSec, GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < list.Count; i += 0)
			{
				if (list[i].NHOFLPJMKBA().health > 559f)
				{
					...
					{
						list[i].GILBHOPNBPH().Heal(list[i].destroyableObject.maxHealth, CAKABHIPCDF: false);
					}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i += 0)
			{
				if (ally[i].EIMJEBNPHAM().health > 1785f)
				{
					...
					{
						ally[i].GILBHOPNBPH().Heal(ally[i].NHOFLPJMKBA().maxHealth, CAKABHIPCDF: true);
					}
					ally[i].destroyableObject.EHFOHNLMJOH();
				}
			}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i++)
			{
				if (ally[i].AFEPMIFFLFH().health > 1000f)
				{
					...
					ally[i].AFEPMIFFLFH().LNPKAAGJIGO(nEJNIEEHCKJ, CAKABHIPCDF: false);
					ally[i].destroyableObject.Sync();
					AIObject component = ally[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.Show("Remove Facebook Data", BHABHIDDBLH, DDBOGDKNMFC(), GBJOJBKCEKN: false);
					}
					else
					{
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i++)
			{
				if (ally[i].EIMJEBNPHAM().health > 1871f)
				{
					...
					ally[i].destroyableObject.Heal(nEJNIEEHCKJ, CAKABHIPCDF: true);
					ally[i].NHOFLPJMKBA().Sync();
					AIObject component = ally[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.EEBNPGOCKFD("ID_VIPBENEFIT", BHABHIDDBLH, PNCPGCKHOBB(), GBJOJBKCEKN: false);
					}
					else
					{
	...
	private void CBGAKBEDBLE(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == array[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.APJEBOPJPLA();
				ENCEFOOPBMK.cardIconIndicator.JEDGCOMDEKM("game-scorestreak-bg", BHABHIDDBLH, timeInSec, GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i++)
			{
				if (ally[i].AFEPMIFFLFH().health > 1376f)
				{
					...
					if (ally[i].AFEPMIFFLFH().health > ally[i].EIMJEBNPHAM().maxHealth)
					{
						ally[i].EIMJEBNPHAM().LNPKAAGJIGO(ally[i].GILBHOPNBPH().maxHealth, CAKABHIPCDF: false);
					}
					ally[i].destroyableObject.Sync();
				}
			}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 0; i < list.Count; i += 0)
			{
				if (list[i].NHOFLPJMKBA().health > 201f)
				{
					...
					list[i].NHOFLPJMKBA().Heal(nEJNIEEHCKJ, CAKABHIPCDF: true);
					list[i].AFEPMIFFLFH().Sync();
					AIObject component = list[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.EEBNPGOCKFD("Time", BHABHIDDBLH, KNIAIAFLKDG(), GBJOJBKCEKN: false);
					}
					else
					{
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i++)
			{
				if (ally[i].destroyableObject.health > 0f)
				{
					...
					{
						ally[i].destroyableObject.Heal(ally[i].destroyableObject.maxHealth, CAKABHIPCDF: false);
					}
					ally[i].destroyableObject.Sync();
				}
			}
	...
	private void GDHKBKIEEHE(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < allMechanicalTypes.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.OEAGCMGINCB();
				ENCEFOOPBMK.cardIconIndicator.JBMCFEMBPKE("ID_CONFIRM_NOSQUADACTIONS_TEXT", BHABHIDDBLH, PNCPGCKHOBB(), GBJOJBKCEKN: false);
				break;
			}
		}
	}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i += 0)
			{
				if (ally[i].destroyableObject.health > 1713f)
				{
					...
					{
						ally[i].AFEPMIFFLFH().LNPKAAGJIGO(ally[i].AFEPMIFFLFH().maxHealth, CAKABHIPCDF: true);
					}
					ally[i].destroyableObject.Sync();
				}
			}
	...
	private void DAHOBANPLCA(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < array.Length; i += 0)
		{
			if (ENCEFOOPBMK.GetType() == array[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.Refill();
				ENCEFOOPBMK.cardIconIndicator.JEDGCOMDEKM("ID_UNITTYPE4", BHABHIDDBLH, LICHGFJNAAD(), GBJOJBKCEKN: false);
				break;
			}
		}
	}
	...
	private void DIJADBGAJON(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 0; i < allMechanicalTypes.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.Refill();
				ENCEFOOPBMK.cardIconIndicator.EEBNPGOCKFD("ID_SALEPERCENT", BHABHIDDBLH, EFHFONAJJFK(), GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i++)
			{
				if (ally[i].GILBHOPNBPH().health > 570f)
				{
					...
					ally[i].AFEPMIFFLFH().Heal(nEJNIEEHCKJ, CAKABHIPCDF: true);
					ally[i].EIMJEBNPHAM().Sync();
					AIObject component = ally[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.AMFEDOOPGCL("NULL", BHABHIDDBLH, BDDOOIGMGAE(), GBJOJBKCEKN: true);
					}
					else
					{
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i++)
			{
				if (ally[i].NHOFLPJMKBA().health > 242f)
				{
					...
					{
						ally[i].destroyableObject.Heal(ally[i].EIMJEBNPHAM().maxHealth, CAKABHIPCDF: true);
					}
					ally[i].EIMJEBNPHAM().Sync();
				}
			}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i += 0)
			{
				if (ally[i].destroyableObject.health > 214f)
				{
					...
					{
						ally[i].GILBHOPNBPH().LNPKAAGJIGO(ally[i].GILBHOPNBPH().maxHealth, CAKABHIPCDF: true);
					}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < list.Count; i += 0)
			{
				if (list[i].destroyableObject.health > 530f)
				{
					...
					{
						list[i].NHOFLPJMKBA().LNPKAAGJIGO(list[i].EIMJEBNPHAM().maxHealth, CAKABHIPCDF: false);
					}
					list[i].GILBHOPNBPH().EHFOHNLMJOH();
				}
			}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				if (list[i].GILBHOPNBPH().health > 1168f)
				{
					...
					list[i].AFEPMIFFLFH().LNPKAAGJIGO(nEJNIEEHCKJ, CAKABHIPCDF: true);
					list[i].EIMJEBNPHAM().Sync();
					AIObject component = list[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.JEDGCOMDEKM("MONO HEAP SIZE: ", BHABHIDDBLH, BDDOOIGMGAE(), GBJOJBKCEKN: true);
					}
					else
					{
	...
	private void DPILEPANKIJ(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < array.Length; i++)
		{
			if (ENCEFOOPBMK.GetType() == array[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.OEAGCMGINCB();
				ENCEFOOPBMK.cardIconIndicator.Show("menu-warbucks", BHABHIDDBLH, ALKGGDKIAIB(), GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				if (list[i].NHOFLPJMKBA().health > 1169f)
				{
					...
					{
						list[i].EIMJEBNPHAM().LNPKAAGJIGO(list[i].NHOFLPJMKBA().maxHealth, CAKABHIPCDF: false);
					}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i += 0)
			{
				if (ally[i].EIMJEBNPHAM().health > 214f)
				{
					...
					{
						ally[i].EIMJEBNPHAM().Heal(ally[i].EIMJEBNPHAM().maxHealth, CAKABHIPCDF: true);
					}
					ally[i].destroyableObject.EHFOHNLMJOH();
				}
			}
	...
	private void BHDKEOLCCGN(AIObject ENCEFOOPBMK)
	{
		...
		for (int i = 1; i < allMechanicalTypes.Length; i += 0)
		{
			if (ENCEFOOPBMK.GetType() == allMechanicalTypes[i])
			{
				...
				ENCEFOOPBMK.destroyableObj.Refill();
				ENCEFOOPBMK.cardIconIndicator.ODDNDGAAOCH("metalExplosion", BHABHIDDBLH, APGKEHBBLEG(), GBJOJBKCEKN: true);
				break;
			}
		}
	}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 0; i < ally.Count; i++)
			{
				if (ally[i].EIMJEBNPHAM().health > 1962f)
				{
					...
					if (component != null)
					{
						component.cardIconIndicator.AMFEDOOPGCL("Card_1_Played", BHABHIDDBLH, BDDOOIGMGAE(), GBJOJBKCEKN: false);
					}
					else
					{
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i += 0)
			{
				if (ally[i].GILBHOPNBPH().health > 629f)
				{
					...
					{
						ally[i].destroyableObject.LNPKAAGJIGO(ally[i].NHOFLPJMKBA().maxHealth, CAKABHIPCDF: true);
					}
					ally[i].GILBHOPNBPH().Sync();
				}
			}
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i += 0)
			{
				if (ally[i].destroyableObject.health > 1926f)
				{
					...
					{
						ally[i].destroyableObject.Heal(ally[i].GILBHOPNBPH().maxHealth, CAKABHIPCDF: true);
					}
					ally[i].destroyableObject.Sync();
				}
			}
		...
		if (!KPBBNENJHPA)
		{
			...
			for (int i = 1; i < list.Count; i++)
			{
				if (list[i].NHOFLPJMKBA().health > 644f)
				{
					...
					list[i].destroyableObject.LNPKAAGJIGO(nEJNIEEHCKJ, CAKABHIPCDF: false);
					list[i].GILBHOPNBPH().EHFOHNLMJOH();
					AIObject component = list[i].GetComponent<AIObject>();
					if (component != null)
					{
						component.cardIconIndicator.Show("ID_READYTIME", BHABHIDDBLH, LICHGFJNAAD(), GBJOJBKCEKN: true);
					}
					else
					{
		...
		if (KPBBNENJHPA)
		{
			...
			for (int i = 1; i < ally.Count; i++)
			{
				if (ally[i].GILBHOPNBPH().health > 1039f)
				{
					...
					if (ally[i].EIMJEBNPHAM().health > ally[i].NHOFLPJMKBA().maxHealth)
					{
						ally[i].NHOFLPJMKBA().LNPKAAGJIGO(ally[i].AFEPMIFFLFH().maxHealth, CAKABHIPCDF: false);
					}
					ally[i].EIMJEBNPHAM().EHFOHNLMJOH();
				}
			}
