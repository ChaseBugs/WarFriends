...
{
	[FormerlySerializedAs("IDMFCIPAGKF")]
	[Header("Settings")]
	public float MBMBPGKJJLJ = 50f;

	[Header("Test")]
	[FormerlySerializedAs("EGFJJCCNHFK")]
	public bool NHNFGOBCFFA;

	[FormerlySerializedAs("HBDJGOBNAEI")]
	public bool PCPCJGNFDKB;

	[Header("Repair")]
	[FormerlySerializedAs("NAAFJBEOECP")]
	public bool CMLHBPLGEAD;

	[Header("Game Object Layers")]
	[FormerlySerializedAs("FFOCBNIIGHO")]
	public int CAOGHENGCOK = 11;

	[FormerlySerializedAs("DLBLCBIOLBI")]
	public bool AGLHIIBMLOA;

	private void HOIGLCCEKFH()
	{
		Transform[] componentsInChildren = base.gameObject.GetComponentsInChildren<Transform>(includeInactive: false);
	...
	private void JGIENPBJFPK()
	{
		...
		foreach (UILabel uILabel in componentsInChildren)
		{
			if (uILabel.font.isDynamic)
			{
				...
				if (dynamicFontSize > 0)
				{
					text += $"WRONG dynamic font size: {dynamicFontSize} for label: {DKMLPBNEAAB(uILabel.gameObject)}\n";
				}
			}
		}
		Debug.Log(text);
