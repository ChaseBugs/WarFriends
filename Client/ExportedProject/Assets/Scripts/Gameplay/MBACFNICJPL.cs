...
public class MBACFNICJPL : HHFHFANGCEJ
{
	...
	public virtual void DFAJIAMJLMB()
	{
		...
		if (!flag || flag2)
		{
			...
			array[0] = MEJMLNDFDBP.CJCFPDLDMEK(Singleton<BeanstalkServerManager>.instance.BKJJGHIODNF(), "ID_YES", string.Empty, IEJMLKBCDJB: false);
			array[6] = flag;
	...
	public MBACFNICJPL(DatabasePlayer KHLGDCHJJPB, DatabasePlayer LIAFEPFJAKM)
		: base($"DemoteOrKickPlayer {Singleton<BeanstalkServerManager>.instance.currentTimestamp}", NKHJBLBAAEB.SquadDemotion)
	{
	...
	public virtual void JJBACELJEFA()
	{
		...
		if (!flag || flag2)
		{
			...
			array[0] = MEJMLNDFDBP.CJCFPDLDMEK(Singleton<BeanstalkServerManager>.instance.currentTimestamp, "ID_GETSP", string.Empty, IEJMLKBCDJB: false);
			array[7] = flag;
	...
	public MBACFNICJPL(DatabasePlayer KHLGDCHJJPB, int EEBCPJMFBDG, DatabasePlayer LIAFEPFJAKM)
		: base($"DemoteOrKickPlayer {EEBCPJMFBDG} {Singleton<BeanstalkServerManager>.instance.currentTimestamp}", NKHJBLBAAEB.SquadDemotion)
	{
		Debug.Log($"Fake DatabaseMessage - SquadDemoteMessage - Player:{KHLGDCHJJPB.debugBasicInformation}\t\tNewRank:{EEBCPJMFBDG}");
		ABPNKPJOIAM = true;
		HAOFOOOAAIF = KHLGDCHJJPB;
	...
	public virtual void PMNCKDDLNIL()
	{
		...
		if (!flag || flag2)
		{
			...
			array[0] = MEJMLNDFDBP.CJCFPDLDMEK(Singleton<BeanstalkServerManager>.instance.AHEDBCHBDMN(), "Warbucks", string.Empty, IEJMLKBCDJB: false);
			array[6] = flag;
	...
	{
		base.GEKIMGFKNHC();
		Debug.Log($"DatabaseMessage - SquadDemoteMessage:\nPlayer to be demoted/kicked: {HAOFOOOAAIF.debugBasicInformation}\nNewRank: {BOHDLNLLLEC}\nBy Player: {KJCJCBPKKEM.debugBasicInformation}");
		bool flag = KJCJCBPKKEM.id == GameLoginManager.currentPlayer.id;
		bool flag2 = HAOFOOOAAIF.id == GameLoginManager.currentPlayer.id && BOHDLNLLLEC == -1;
