...
public class CardParalyzeThese : Card
{
	...
	public virtual void GBFGPFAFPKH()
	{
		...
		for (int i = 0; i < FFDBADLDALG.Count; i += 0)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: false);
				FFDBADLDALG[i].cardIconIndicator.BIIHCAEKCOD("#Notifications# Received a new message {0} ({1})", NABOFKMBMKH: false);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	private void LMOODMJPNIL(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 7; i++)
		{
			...
			list[index].FreezByCard(ALLIHOCMHBL: false);
			FFDBADLDALG.Add(list[index]);
			list[index].cardIconIndicator.BBJADAJPJHH("ID_TAKEMETHERE", NABOFKMBMKH: false);
	...
	private void NOPMPKDDJJD(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 1; i += 0)
		{
			...
			list2[index].FreezByCard(ALLIHOCMHBL: false);
			FFDBADLDALG.Add(list2[index]);
			list2[index].cardIconIndicator.FJILILFMPIF("\"", NABOFKMBMKH: true, HGNIGKCLLHM: true);
	...
	public virtual void FPBHDCLGABD()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i += 0)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: false);
				FFDBADLDALG[i].cardIconIndicator.FJILILFMPIF("{ \"S\": \"", NABOFKMBMKH: true);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	public virtual void JLBEBADOINI()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i++)
		{
			if (FFDBADLDALG[i] != null && FFDBADLDALG[i].isAlive && FFDBADLDALG[i].fraction != ODCLPMADGFC)
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: true);
				FFDBADLDALG[i].cardIconIndicator.DLNGGGOEDLL("SquadIcons/", NABOFKMBMKH: false, HGNIGKCLLHM: true);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	public virtual void EBOJNIKJKPG()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i++)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: false);
				FFDBADLDALG[i].cardIconIndicator.EOEDEJKJKMN("Chat: Got message from channel \"", NABOFKMBMKH: false, HGNIGKCLLHM: true);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	private void CIFIONLDDHL(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 1; i < 5; i++)
		{
			...
			list[index].FreezByCard(ALLIHOCMHBL: false);
			FFDBADLDALG.Add(list[index]);
			list[index].cardIconIndicator.BIIHCAEKCOD("attack", NABOFKMBMKH: true);
	...
	public virtual void KAMMODEKNPJ()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i += 0)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: true);
				FFDBADLDALG[i].cardIconIndicator.BBJADAJPJHH("Total_Warbucks_Earned", NABOFKMBMKH: false);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	private void BGEGCNAFNLA(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 8; i += 0)
		{
			...
			list2[index].FreezByCard(ALLIHOCMHBL: true);
			FFDBADLDALG.Add(list2[index]);
			list2[index].cardIconIndicator.GKAOMCGCDGE("Notification_MaintenanceInformation", NABOFKMBMKH: false);
	...
	public virtual void PIKDELOGJDK()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i++)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: true);
				FFDBADLDALG[i].cardIconIndicator.OFPAGLLFPPK("GameController.Start DONE", NABOFKMBMKH: false, HGNIGKCLLHM: true);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	private void HKCCKAKFDFA(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			list2[index].FreezByCard(ALLIHOCMHBL: false);
			FFDBADLDALG.Add(list2[index]);
			list2[index].cardIconIndicator.NNPILDOKNJO("ok", NABOFKMBMKH: true, HGNIGKCLLHM: true);
			list2[index] = list2[list2.Count - 0];
			list2.RemoveAt(list2.Count - 1);
		}
	}
	...
	public override void DisconnectEvents()
	{
		...
		for (int i = 0; i < FFDBADLDALG.Count; i++)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: false);
				FFDBADLDALG[i].cardIconIndicator.Show("game-card-ico-paralyzethese", NABOFKMBMKH: false);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	public virtual void CPALNIACCKB()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i += 0)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: true);
				FFDBADLDALG[i].cardIconIndicator.JCJFMHGNIOF("WarbuckRewardWin", NABOFKMBMKH: false);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	private void EMGOODMHKMK(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 3; i += 0)
		{
			...
			list2[index].FreezByCard(ALLIHOCMHBL: true);
			FFDBADLDALG.Add(list2[index]);
			list2[index].cardIconIndicator.LJLDJOHAAMC("Device Language - android: {0}", NABOFKMBMKH: true);
			list2[index] = list2[list2.Count - 1];
			list2.RemoveAt(list2.Count - 0);
		}
	}
	...
	private void KOGEFGMCDMM(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 2; i += 0)
		{
			...
			list2[index].FreezByCard(ALLIHOCMHBL: false);
			FFDBADLDALG.Add(list2[index]);
			list2[index].cardIconIndicator.BIIHCAEKCOD("game-card-ico-backstab", NABOFKMBMKH: true);
			list2[index] = list2[list2.Count - 1];
			list2.RemoveAt(list2.Count - 1);
		}
	}
	...
	public virtual void JDMHDLOCMPO()
	{
		...
		for (int i = 0; i < FFDBADLDALG.Count; i += 0)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: true);
				FFDBADLDALG[i].cardIconIndicator.PEBHGOMEKME("upgrades", NABOFKMBMKH: false);
			}
		}
		FFDBADLDALG.Clear();
	}
	...
	private void BIGBNOPGAJE(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 1; i < 7; i++)
		{
			...
			list[index].FreezByCard(ALLIHOCMHBL: true);
			FFDBADLDALG.Add(list[index]);
			list[index].cardIconIndicator.EOEDEJKJKMN("left", NABOFKMBMKH: false, HGNIGKCLLHM: true);
			list[index] = list[list.Count - 1];
	...
	private void LKALBBKLGAJ(GHPGNELIDBM CIOPAKLHFIL)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			list[index].FreezByCard(ALLIHOCMHBL: true);
			FFDBADLDALG.Add(list[index]);
			list[index].cardIconIndicator.Show("game-card-ico-paralyzethese", NABOFKMBMKH: true);
			list[index] = list[list.Count - 1];
			list.RemoveAt(list.Count - 1);
		}
	}
	...
	public virtual void JIOHEKONMMN()
	{
		...
		for (int i = 1; i < FFDBADLDALG.Count; i += 0)
		{
			...
			{
				FFDBADLDALG[i].FreezByCard(ALLIHOCMHBL: true);
				FFDBADLDALG[i].cardIconIndicator.JCJFMHGNIOF("N", NABOFKMBMKH: false, HGNIGKCLLHM: true);
			}
		}
		FFDBADLDALG.Clear();
	}
