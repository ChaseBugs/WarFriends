...
public class Chat : Singleton<Chat>, IChatClientListener
{
	...
	[FormerlySerializedAs("DEOMHFFMLKK")]
	public ChatClient JABOMKEMAOG;

	[HideInInspector]
	[FormerlySerializedAs("JEDDEHAMFMA")]
	...
	public bool connectingPhotonChat => JABOMKEMAOG.State == ChatState.ConnectedToNameServer || JABOMKEMAOG.State == ChatState.Authenticated;

	public bool connectedToPhotonChat => JABOMKEMAOG.State == ChatState.ConnectedToFrontEnd;
	...
	public void MNLGLBNIMLP(out List<PBGFEDGFOPG> CGCOHKMLCLL)
	{
		if (JABOMKEMAOG != null)
		{
			...
			{
				if (JABOMKEMAOG.TryGetChannel(NIOAEPLBKNM, isPrivate: true, out BHBPAGNIAIO))
				{
	...
	public void GetMessages(out List<PBGFEDGFOPG> CGCOHKMLCLL)
	{
		if (JABOMKEMAOG != null)
		{
			if (!string.IsNullOrEmpty(NIOAEPLBKNM))
			{
				if (JABOMKEMAOG.TryGetChannel(NIOAEPLBKNM, isPrivate: false, out BHBPAGNIAIO))
				{
					CGCOHKMLCLL = HDHELIAFENB(NIOAEPLBKNM, BHBPAGNIAIO.Senders, BHBPAGNIAIO.Messages);
					return;
				}
	...
	public string MNMBKOAHIAG(string HOLNKOFHDNK)
	{
		...
		string[] array = new string[WarFriends.Legacy.DecompilerCompatibility.RuntimeArrayLength(-39)];
	...
	{
		BHBPAGNIAIO?.Add("info", $"{CLGDIIODGCN} is {GDAELIBCLPP}. Msg:{PNAKMCNGGHE}");
	}
