...
public class DogTagManager : Singleton<DogTagManager>
{
	...
	[FormerlySerializedAs("GDAKGJLGDIJ")]
	...
	public int timeToGetDogtag => (!SubscriptionManager.instance.isSubscribed || ELLJNHLPDNF >= Singleton<BeanstalkServerManager>.instance.currentTimestamp) ? constDogtagSeconds : constDogtagSubscriptionSeconds;

	public int constDogtagSubscriptionSeconds => (int)(float)Singleton<GameVariables>.instance.constants.GetRow(Constants.rowIds.DogTagSubscriptionRefillTime).FLOATVALUE;

	public int constDogtagSeconds => (int)(float)Singleton<GameVariables>.instance.constants.GetRow(Constants.rowIds.DogTagRefillTime).FLOATVALUE;

	public int constDogtagCount => (int)(float)Singleton<GameVariables>.instance.constants.GetRow(Constants.rowIds.DogTagCap).FLOATVALUE;

	private int LNLAOANGHPN => HNDGIJBHMOD + (Singleton<BeanstalkServerManager>.instance.currentTimestamp - ADEOGAMOGDA);

	private int GOFEHOCFIMM => Singleton<VipManager>.instance.numberOfVIPDogtags * (timeToGetDogtag * KDJCCDGMEJL);

	private int GHFBGCNLEIO => (!Singleton<VipManager>.instance.NOGEIPHFNPK()) ? DMEHGIDJEBD : (DMEHGIDJEBD + GOFEHOCFIMM);

	private int INIHJJPBLNG => (!Singleton<VipManager>.instance.NOGEIPHFNPK()) ? LNLAOANGHPN : (LNLAOANGHPN + GOFEHOCFIMM);

	private int GEDPOCLNMNE => Mathf.Clamp(INIHJJPBLNG, 0, GHFBGCNLEIO);

	private int CHIPMPDAEJF => GEDPOCLNMNE - HBGABOKGAAM * timeToGetDogtag;

	private int HLLMEABKOIJ => Singleton<BeanstalkServerManager>.instance.currentTimestamp - NOJCDAOEKGG;

	private int KDJCCDGMEJL => (!SubscriptionManager.instance.isSubscribed || ELLJNHLPDNF >= Singleton<BeanstalkServerManager>.instance.currentTimestamp) ? 1 : (constDogtagSeconds / constDogtagSubscriptionSeconds);

	public int currentDogtags => CHIPMPDAEJF / (timeToGetDogtag * KDJCCDGMEJL);

	public int maximumDogtags => GHFBGCNLEIO / (timeToGetDogtag * KDJCCDGMEJL);

	public bool isEmpty => INIHJJPBLNG < timeToGetDogtag;
	...
	{
		PushNotificationManager.instance.CancelLocalNotification("warbucks", save: false);
		HNDGIJBHMOD = DMEHGIDJEBD;
		if (LELOCGKKJII != null)
		{
			LELOCGKKJII(PHMPHFFNMPD());
