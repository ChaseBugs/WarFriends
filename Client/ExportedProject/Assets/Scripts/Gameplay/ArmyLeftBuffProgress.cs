...
public class ArmyLeftBuffProgress : Core_BaseScript
{
	...
	public bool isAnimatingSpend => HOBNCOEIINC;
	...
	private void Update()
	{
		...
		if (LLGGKFFNBMH < IBECOAEMAHD)
		{
			...
			mProgressBackground.fillAmount = (float)num2 / (float)FJBKHPHNLFP;
			mProgressLabel.text = $"{Colours.stringGreenArena}{MEJMLNDFDBP.GMIPFLIEOHD(num2)}[-] {Colours.stringGray}/[-] {MEJMLNDFDBP.GMIPFLIEOHD(FJBKHPHNLFP)}";
		}
		else
		{
			HOBNCOEIINC = false;
	...
	public void Initialize(LevelBehaviour IOIKKIIFOCB)
	{
		...
		mProgressLabel.text = ((!flag) ? $"{Colours.stringGreenArena}{MEJMLNDFDBP.GMIPFLIEOHD(HFPAAGAIGHN)}[-] {Colours.stringGray}/[-] {MEJMLNDFDBP.GMIPFLIEOHD(FJBKHPHNLFP)}" : $"{Colours.stringGreenArena}{MEJMLNDFDBP.GMIPFLIEOHD(HFPAAGAIGHN)}[-]");
		bool aJNINHIKAFC = isUnlocked && !flag && HFPAAGAIGHN >= FJBKHPHNLFP;
		DJAEGONMPOO(aJNINHIKAFC);
		KIJINOGOOFO();
	}
