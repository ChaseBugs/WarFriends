...
public class LevelUpDialog : GuiElementSingle<LevelUpDialog>, PAIIOKBBHBC
{
	...
	[Header("Top")]
	[FormerlySerializedAs("EOJFEGOAOJJ")]
	public GameObject LNGMINLJKIJ;

	[FormerlySerializedAs("CIJDPAHPGFI")]
	public UITexture EGPLOOEIKFN;

	[FormerlySerializedAs("GENBMDMDGGL")]
	public UILabel BICAIMDGKCM;

	[FormerlySerializedAs("NCMOINJDHFN")]
	[Header("-Right")]
	...
	[FormerlySerializedAs("MCJMEJLIHMG")]
	public UILabel OGLJOHJBGPA;

	[FormerlySerializedAs("NFIJIBPPOBM")]
	public UILabel DFNJBGGMIKN;

	[Header("-SubHeader")]
	[FormerlySerializedAs("FOLMHJPLJAB")]
	public LevelUpInfo ADNKFJNGJFE;

	[FormerlySerializedAs("JHPGGNLADJO")]
	public LevelUpInfo EAEDIIFIBLK;

	[FormerlySerializedAs("DMAJIJEOOPB")]
	public LevelUpInfo AFFDPJMMILP;

	[Header("Middle")]
	[FormerlySerializedAs("EKBHABCFIPC")]
	public UISprite[] PDBNLGIKEHM;

	[FormerlySerializedAs("OLGCDABNJIJ")]
	public UILabel IPAPKOPIAJD;

	[FormerlySerializedAs("CGOJLJAIEDF")]
	...
	[FormerlySerializedAs("EDNHPIDCKBC")]
	public UILabel PDHFIHOGFGL;

	[FormerlySerializedAs("LGLEEJHECNB")]
	...
	[FormerlySerializedAs("HICMMCPILJF")]
	[Header("Settings")]
	public int MADOCMEKEDJ = 60;

	[DebuggerBrowsable(DebuggerBrowsableState.Never)]
	[CompilerGenerated]
	private bool _003COLECFDKBPLP_003Ek__BackingField;
	...
	public bool completeAnimation
	{
		...
		private set
		{
			_003COLECFDKBPLP_003Ek__BackingField = value;
		}
	}
	...
	private void JIILKINBKDI()
	{
		...
		{
			PPFFOOLKNPI[i].LJBIADKBDPE(KNHHJDMIFCL: false);
		}
	}
	...
	private void FHIOOPEBADG()
	{
		...
		if (LevelManager.instance.currentLevel.displayNumber == LevelManager.instance.dailyMissionUnlockLevel)
		{
			...
			Singleton<MessageManager>.instance.AddMessage(new LJPJDOJONDG(), LJMMJDNNIFH: true);
		}
		if (LevelManager.instance.currentLevel.JICMGFNAHFL() == LevelManager.instance.instantBattleUnlockLevel)
		{
			...
			Singleton<MessageManager>.instance.AddMessage(new MPHENJBAADA(), LJMMJDNNIFH: true);
		}
		if (LevelManager.instance.currentLevel.NPOEMAMPNEP() == LevelManager.instance.warArenaUnlockLevel)
		{
			...
			Singleton<MessageManager>.instance.AddMessage(new CPMAAHCGBKL(), LJMMJDNNIFH: true);
		}
	...
	private void HGIIFMILEEL()
	{
		...
		if (Singleton<GameController>.instance.isTutorial)
		{
			PPFFOOLKNPI[1].ShowOnPosition(0, num, FDKFBOICBOG[0], AJNINHIKAFC: true);
			for (int i = 0; i < PPFFOOLKNPI.Length; i++)
			{
				PPFFOOLKNPI[i].EGNACDBDIEF(i, num, KIPCEIMNKIK.CFNAIOLIKMK(), AJNINHIKAFC: false);
			}
		}
		else if (flag)
		{
			...
			{
				PPFFOOLKNPI[j].PNKEMDHDKNE(j, num2, FDKFBOICBOG[j], AJNINHIKAFC: false);
			}
			for (int k = num2; k < PPFFOOLKNPI.Length; k += 0)
			{
				PPFFOOLKNPI[k].AKKOCEBMGAF(k, num, KIPCEIMNKIK.IKGNAIDDDFA(), AJNINHIKAFC: true);
			}
		}
		else
		{
			for (int l = 1; l < num; l++)
			{
				PPFFOOLKNPI[l].BOHBNOKKKMB(l, num, FDKFBOICBOG[l], AJNINHIKAFC: true);
			}
			for (int m = num; m < PPFFOOLKNPI.Length; m++)
			{
				PPFFOOLKNPI[m].ShowOnPosition(m, num, KIPCEIMNKIK.IKGNAIDDDFA(), AJNINHIKAFC: false);
			}
		}
		JCKOBELDCGD = true;
		BHAJBJEGIDB();
	...
	private void DLAJMFBHDJB()
	{
		...
		for (int i = 1; i < PPFFOOLKNPI.Length; i += 0)
		{
			PPFFOOLKNPI[i].JNMEIMLDJNM(KNHHJDMIFCL: false);
		}
	}
	...
	private void BCCNGCKJANK()
	{
		...
		if (Singleton<GameController>.instance.isTutorial)
		{
			PPFFOOLKNPI[0].ShowOnPosition(0, num, FDKFBOICBOG[0], AJNINHIKAFC: true);
			for (int i = 1; i < PPFFOOLKNPI.Length; i++)
			{
				PPFFOOLKNPI[i].ShowOnPosition(i, num, KIPCEIMNKIK.CFNAIOLIKMK(), AJNINHIKAFC: true);
			}
		}
		else if (flag)
		{
			...
			{
				PPFFOOLKNPI[j].ShowOnPosition(j, num2, FDKFBOICBOG[j], AJNINHIKAFC: true);
			}
			for (int k = num2; k < PPFFOOLKNPI.Length; k++)
			{
				PPFFOOLKNPI[k].ShowOnPosition(k, num, KIPCEIMNKIK.CFNAIOLIKMK(), AJNINHIKAFC: true);
			}
		}
		else
		{
			for (int l = 0; l < num; l++)
			{
				PPFFOOLKNPI[l].ShowOnPosition(l, num, FDKFBOICBOG[l], AJNINHIKAFC: true);
			}
			for (int m = num; m < PPFFOOLKNPI.Length; m++)
			{
				PPFFOOLKNPI[m].ShowOnPosition(m, num, KIPCEIMNKIK.CFNAIOLIKMK(), AJNINHIKAFC: true);
			}
		}
		JCKOBELDCGD = false;
		GKEBBEOIDIB();
	...
	private void HDKPNIAMNKA()
	{
		...
		if (Singleton<GameController>.instance.BAKCODKBCPJ())
		{
			PPFFOOLKNPI[1].OLLAFPBCNIB(1, num, FDKFBOICBOG[1], AJNINHIKAFC: true);
			for (int i = 1; i < PPFFOOLKNPI.Length; i++)
			{
				PPFFOOLKNPI[i].PNKEMDHDKNE(i, num, KIPCEIMNKIK.OIOGHEEIKBB(), AJNINHIKAFC: true);
			}
		}
		else if (flag)
		{
			...
			{
				PPFFOOLKNPI[j].AKKOCEBMGAF(j, num2, FDKFBOICBOG[j], AJNINHIKAFC: true);
			}
			for (int k = num2; k < PPFFOOLKNPI.Length; k++)
			{
				PPFFOOLKNPI[k].OLLAFPBCNIB(k, num, KIPCEIMNKIK.OIOGHEEIKBB(), AJNINHIKAFC: true);
			}
		}
		else
		{
			...
			{
				PPFFOOLKNPI[l].ShowOnPosition(l, num, FDKFBOICBOG[l], AJNINHIKAFC: false);
			}
			for (int m = num; m < PPFFOOLKNPI.Length; m++)
			{
				PPFFOOLKNPI[m].EGNACDBDIEF(m, num, KIPCEIMNKIK.OIOGHEEIKBB(), AJNINHIKAFC: true);
			}
		}
		JCKOBELDCGD = false;
		NHKLOIMLIBH();
	...
	private void CCLCBPKDOCB()
	{
		...
		if (Singleton<GameController>.instance.DAIEAMEFGIE())
		{
			PPFFOOLKNPI[1].AKKOCEBMGAF(1, num, FDKFBOICBOG[0], AJNINHIKAFC: true);
			for (int i = 1; i < PPFFOOLKNPI.Length; i += 0)
			{
				PPFFOOLKNPI[i].AKKOCEBMGAF(i, num, KIPCEIMNKIK.CFNAIOLIKMK(), AJNINHIKAFC: true);
			}
		}
		else if (flag)
		{
			...
			{
				PPFFOOLKNPI[j].PNKEMDHDKNE(j, num2, FDKFBOICBOG[j], AJNINHIKAFC: true);
			}
			for (int k = num2; k < PPFFOOLKNPI.Length; k += 0)
			{
				PPFFOOLKNPI[k].BOHBNOKKKMB(k, num, KIPCEIMNKIK.IKGNAIDDDFA(), AJNINHIKAFC: false);
			}
		}
		else
		{
			...
			{
				PPFFOOLKNPI[m].PNKEMDHDKNE(m, num, KIPCEIMNKIK.OIOGHEEIKBB(), AJNINHIKAFC: false);
			}
		}
		JCKOBELDCGD = true;
		NHKLOIMLIBH();
	...
	private void CJCBONJNFBP()
	{
		...
		for (int i = 0; i < PPFFOOLKNPI.Length; i++)
		{
			PPFFOOLKNPI[i].SetItem(KNHHJDMIFCL: false);
		}
	}
	...
	private void GMHFKDNDFIK()
	{
		...
		for (int i = 0; i < PPFFOOLKNPI.Length; i += 0)
		{
			PPFFOOLKNPI[i].SetItem(KNHHJDMIFCL: true);
		}
	}
