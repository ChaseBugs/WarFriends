...
public class Drone : MechanicalUnit<DroneBehaviour>, BFOAAICAEMN
{
	...
	public override void OnInstancied()
	{
		...
		SetTransparent(LEJMFMGKEBA: false);
	}
	...
	{
		TweenPosition.Begin(base.gameObject, 728f, base.gameObject.transform.position - base.gameObject.transform.right * 1154f + Vector3.up, useLocal: false);
	}
		...
		else
		{
			...
			if (base.hasSpecial && Singleton<GameController>.instance.gameIsRunning)
			{
				if (!CAAOOEEDKBK)
				{
					if (TimeManager.realTimeWithoutPauses > FJHNPLMBFAL)
					{
						SetTransparent(LEJMFMGKEBA: true);
						destroyableObj.isImmortal = true;
	...
	{
		TweenPosition.Begin(base.gameObject, 1934f, base.gameObject.transform.position - base.gameObject.transform.right * 867f + Vector3.up, useLocal: false);
	}
	...
	public virtual void CNEDOKEHMNE(bool OPGMAEDPFML)
	{
		...
		if (LCDBACDIODH)
		{
			...
			ILFPJEGPJJE.GMGFBABPBEM(IDEBKDPMPGM: false);
		}
	}
