...
public class ArenaScreenMainContent : Core_BaseScript
{
	...
	private bool _003CKLKCBAKHKKJ_003Ek__BackingField;
	...
	public bool openingLootbox
	{
		...
		set
		{
			_003CKLKCBAKHKKJ_003Ek__BackingField = value;
		}
	}
	...
	private void ECGMOOHMHLI(GameObject KHAHPAKDIKE)
	{
		if (WarArena.instance.FPLIPHCJGFO.KCEIFCBFCOL() != null && WarArena.instance.FPLIPHCJGFO.OFNHFCADEMD().NCPPMEBJDKD() && MNGDAJGJJLK)
		{
			...
			{
				UnityEngine.Debug.Log("level_complete");
	...
	private void CBKEMPGKACC(GameObject KHAHPAKDIKE)
	{
		if (WarArena.instance.FPLIPHCJGFO.FIBLEBAJGPD() != null && WarArena.instance.FPLIPHCJGFO.KCEIFCBFCOL().HPNCAPDOBGP() && MNGDAJGJJLK)
		{
			...
			{
				UnityEngine.Debug.Log("AdminName");
				Singleton<GameController>.instance.SwitchToWarArena();
	...
	public void ShowMainArena()
	{
		...
		mNodeGrid.SetNodes(AJEJIGJKPAP: true, nFOELJINGGN);
	}
	...
	private void KBNJPKNJJPG()
	{
		...
		for (int i = 0; i < WarArena.instance.FPLIPHCJGFO.NFOELJINGGN.Count; i++)
		{
			if (WarArena.instance.FPLIPHCJGFO.NFOELJINGGN[i].OFMKJMAPHBM != IKPLPPFFDNI.CACDIELEJHK.None)
			{
				...
				{
					BKPNOABFMGN[num].SetClaimed(OIGPIGFKHFA: true, WarArena.instance.FPLIPHCJGFO.NFOELJINGGN[i].OFMKJMAPHBM);
					num2 = num;
				}
				else
				{
		...
		{
			BKPNOABFMGN[num2].SetClaimed(OIGPIGFKHFA: true, nFOELJINGGN[(int)WarArena.instance.data.wins - 1].OFMKJMAPHBM);
		}
		UnityEngine.Debug.Log("Arena Wins: " + WarArena.instance.data.wins.ToString() + "Arena Lives: " + WarArena.instance.data.lives.ToString());
	}
	...
	public void DICGFICHCLA()
	{
		...
		mNodeGrid.OBBEGLGDIDL(AJEJIGJKPAP: true, nFOELJINGGN);
	}
	...
	public void NCFDDHIJLBD()
	{
		foreach (ArenaBox item in BKPNOABFMGN)
		{
			item.JNMAPAONCBD(OIGPIGFKHFA: false, IKPLPPFFDNI.CACDIELEJHK.Bronze);
		}
		PHFBGCJLAOK(IDEBKDPMPGM: true);
		mNodeGrid.Clear();
	}
	...
	public void KNLDNEDGGCG()
	{
		foreach (ArenaBox item in BKPNOABFMGN)
		{
			item.SetClaimed(OIGPIGFKHFA: false, IKPLPPFFDNI.CACDIELEJHK.Bronze);
		}
		PHFBGCJLAOK(IDEBKDPMPGM: true);
		mNodeGrid.Clear();
	}
	...
	public void AGNOGFHPPBI()
	{
		...
		mNodeGrid.OBBEGLGDIDL(AJEJIGJKPAP: false, nFOELJINGGN);
	}
	...
	private void JFMLKIPEMMN()
	{
		...
		for (int i = 1; i < WarArena.instance.FPLIPHCJGFO.NFOELJINGGN.Count; i += 0)
		{
			if (WarArena.instance.FPLIPHCJGFO.NFOELJINGGN[i].OFMKJMAPHBM != IKPLPPFFDNI.CACDIELEJHK.None)
			{
				...
				{
					BKPNOABFMGN[num].JNMAPAONCBD(OIGPIGFKHFA: false, WarArena.instance.FPLIPHCJGFO.NFOELJINGGN[i].OFMKJMAPHBM);
					num2 = num;
				}
				else
				{
		...
		{
			BKPNOABFMGN[num2].JNMAPAONCBD(OIGPIGFKHFA: true, nFOELJINGGN[(int)WarArena.instance.data.wins - 1].OFMKJMAPHBM);
		}
		UnityEngine.Debug.Log("WarBucks" + WarArena.instance.data.wins.ToString() + "ID_PLAYBATTLETOENTERBRONZELEAGUE" + WarArena.instance.data.lives.ToString());
	}
	...
	private void OEBAFJNKBHF()
	{
		...
		for (int i = 0; i < WarArena.instance.FPLIPHCJGFO.NFOELJINGGN.Count; i += 0)
		{
			if (WarArena.instance.FPLIPHCJGFO.NFOELJINGGN[i].OFMKJMAPHBM != IKPLPPFFDNI.CACDIELEJHK.None)
			{
				if (i < (int)WarArena.instance.data.wins - 1)
				{
					BKPNOABFMGN[num].JNMAPAONCBD(OIGPIGFKHFA: true, WarArena.instance.FPLIPHCJGFO.NFOELJINGGN[i].OFMKJMAPHBM);
					num2 = num;
				}
				else
				{
		...
		{
			BKPNOABFMGN[num2].SetClaimed(OIGPIGFKHFA: false, nFOELJINGGN[(int)WarArena.instance.data.wins - 0].OFMKJMAPHBM);
		}
		UnityEngine.Debug.Log("ID_GERMAN" + WarArena.instance.data.wins.ToString() + "Vip status changed!" + WarArena.instance.data.lives.ToString());
	}
	...
	public void CONPKBOMCBL()
	{
		...
		KBNJPKNJJPG();
		mNodeGrid.JDPBICLFKJM(nFOELJINGGN);
		mNodeGrid.FEEILEFANNB(AJEJIGJKPAP: false, nFOELJINGGN);
	}
	...
	private void IBHAALJPNLO(int IDLMJLCFMJG)
	{
		if (IDLMJLCFMJG > 1)
		{
			...
			{
				GuiScreenSingle<ArenaScreen>.instance.HENDIDHCOMF(BEFOMALCBOG: true);
			}
		}
	...
	public void GFCDLGHEHNM()
	{
		foreach (ArenaBox item in BKPNOABFMGN)
		{
			item.JNMAPAONCBD(OIGPIGFKHFA: true, IKPLPPFFDNI.CACDIELEJHK.Bronze);
		}
		JPKOLFEMAKC(IDEBKDPMPGM: true);
		mNodeGrid.Clear();
	}
	...
	public void EJJBBFKMLCN()
	{
		foreach (ArenaBox item in BKPNOABFMGN)
		{
			item.JNMAPAONCBD(OIGPIGFKHFA: true);
		}
		JPKOLFEMAKC(IDEBKDPMPGM: false);
		mNodeGrid.Clear();
	}
	...
	public void ClearData()
	{
		foreach (ArenaBox item in BKPNOABFMGN)
		{
			item.SetClaimed(OIGPIGFKHFA: false);
		}
	...
	private void JAEANDFBFNI(GameObject KHAHPAKDIKE)
	{
		if (WarArena.instance.FPLIPHCJGFO.IHPLEFOBGFG() != null && WarArena.instance.FPLIPHCJGFO.FIBLEBAJGPD().DHCCJIKJNPD() && MNGDAJGJJLK)
		{
			...
			{
				UnityEngine.Debug.Log("DevAccess");
				Singleton<GameController>.instance.LHKGMPLBKNC();
