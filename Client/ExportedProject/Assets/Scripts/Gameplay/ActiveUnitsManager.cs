...
public class ActiveUnitsManager : Singleton<ActiveUnitsManager>
{
	...
	[FormerlySerializedAs("NDMFGMNEAGP")]
	...
	public int numberOfMenuEquippedUnits => BMINFBGENPF + ANPANBLKKDD;

	public bool menuMechanicalCanBeChosen => ELAGHFHEOPJ < 3;

	public bool mechanicalCanBeChosen => DKPMCIGJAJL < 3;

	public bool selectingUnit => INENFIICCOB != null;

	public bool nothingEquipped => HPCFNLHBOGJ.Count == 0 && NMJBNMAOJLK.Count == 0 && CGJNBPBEICP.Count == 0 && HPCFNLHBOGJ.Count == 0;
	...
	public List<LevelBehaviour> defenderUnits => AJCGBKLEKEF;

	public List<LevelBehaviour> rusherUnits => DKIDGKEPIHG;

	public List<LevelBehaviour> shooterUnits => HLPJEGALNID;

	public List<LevelBehaviour> explosiveUnits => MFLMCGHKGCI;

	public List<LevelBehaviour> equippedDefenders => HPCFNLHBOGJ;

	public List<LevelBehaviour> equippedRushers => NMJBNMAOJLK;

	public List<LevelBehaviour> equippedShooters => CGJNBPBEICP;

	public List<LevelBehaviour> equippedExplosives => MEAGIKAAMJA;

	public LevelBehaviour firstDefender => (GFLCDIFFCNG.Count != 0) ? GFLCDIFFCNG[0] : null;

	public LevelBehaviour secondDefender => (GFLCDIFFCNG.Count >= 2) ? GFLCDIFFCNG[1] : null;

	public LevelBehaviour firstRusher => (KFGDFHCDHPE.Count != 0) ? KFGDFHCDHPE[0] : null;

	public LevelBehaviour secondRusher => (KFGDFHCDHPE.Count >= 2) ? KFGDFHCDHPE[1] : null;

	public LevelBehaviour firstShooter => (JEHOOOPBMEC.Count != 0) ? JEHOOOPBMEC[0] : null;

	public LevelBehaviour secondShooter => (JEHOOOPBMEC.Count >= 2) ? JEHOOOPBMEC[1] : null;

	public LevelBehaviour firstExplosive => (GKNEGEBENEJ.Count != 0) ? GKNEGEBENEJ[0] : null;

	public LevelBehaviour secondExplosive => (GKNEGEBENEJ.Count >= 2) ? GKNEGEBENEJ[1] : null;
	...
	private void COFFGMNJONG()
	{
		foreach (LevelBehaviour item in AJCGBKLEKEF)
		{
			LDJGKDAGPLG state = GetState(item, EBLDCOKIEEG: true);
		...
		{
			LDJGKDAGPLG state2 = GetState(item2, EBLDCOKIEEG: true);
		...
		{
			if (GetState(item3, EBLDCOKIEEG: true) == LDJGKDAGPLG.Locked)
			{
				Equip(item3);
			}
		}
		...
		{
			LDJGKDAGPLG state3 = GetState(item4, EBLDCOKIEEG: false);
	...
	public bool MenuEquip(LevelBehaviour IOIKKIIFOCB)
	{
		...
		case LevelBehaviour.UnitType.Defender:
			...
			{
				FALOCEPDHOC(IOIKKIIFOCB, ILALBAIMABM: true);
				return true;
			}
			break;
		case LevelBehaviour.UnitType.AttackerRusher:
			...
			{
				BEPEBEKEIAJ(IOIKKIIFOCB, ILALBAIMABM: true);
				return true;
			}
			break;
		case LevelBehaviour.UnitType.AttackerShooter:
			...
			{
				OJNBOHPBACM(IOIKKIIFOCB, ILALBAIMABM: true);
				return true;
			}
			break;
		case LevelBehaviour.UnitType.AttackerExplosive:
			...
			{
				IBBDLODOAJA(IOIKKIIFOCB, ILALBAIMABM: true);
				return true;
			}
			break;
		}
	...
	{
		if (GetState(IOIKKIIFOCB, EBLDCOKIEEG: true) == LDJGKDAGPLG.Selecting)
		{
	...
	private void HMOPPAAPGOI()
	{
		foreach (LevelBehaviour item in AJCGBKLEKEF)
		{
			LDJGKDAGPLG state = GetState(item, EBLDCOKIEEG: false);
		...
		foreach (LevelBehaviour item2 in DKIDGKEPIHG)
		{
			LDJGKDAGPLG state2 = GetState(item2, EBLDCOKIEEG: false);
		...
		foreach (LevelBehaviour item3 in HLPJEGALNID)
		{
			LDJGKDAGPLG state3 = GetState(item3, EBLDCOKIEEG: false);
		...
		foreach (LevelBehaviour item4 in MFLMCGHKGCI)
		{
			LDJGKDAGPLG state4 = GetState(item4, EBLDCOKIEEG: false);
	...
	public bool Equip(LevelBehaviour IOIKKIIFOCB)
	{
		...
		case LevelBehaviour.UnitType.Defender:
			...
			{
				FALOCEPDHOC(IOIKKIIFOCB, ILALBAIMABM: false);
				return true;
			}
			break;
		case LevelBehaviour.UnitType.AttackerRusher:
			...
			{
				BEPEBEKEIAJ(IOIKKIIFOCB, ILALBAIMABM: false);
				return true;
			}
			break;
		case LevelBehaviour.UnitType.AttackerShooter:
			...
			{
				OJNBOHPBACM(IOIKKIIFOCB, ILALBAIMABM: false);
				return true;
			}
			break;
		case LevelBehaviour.UnitType.AttackerExplosive:
			...
			{
				IBBDLODOAJA(IOIKKIIFOCB, ILALBAIMABM: false);
				return true;
			}
			break;
		}
	...
	{
		if (GetState(IOIKKIIFOCB, EBLDCOKIEEG: false) == LDJGKDAGPLG.Selectable)
		{
			return Equip(IOIKKIIFOCB);
		}
		return false;
